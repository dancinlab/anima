...
Next BG would isolate engine_a layers (e.g., swap engine_a layers 0-11 vs 12-23 with random_init while keeping engine_g intact) to localize the mechanism within engine_a depth. That work is OUT-OF-SCOPE here (own 16 budget exhausted by 5-condition × 4-trajectory ablation).
...
5. **Single random_init seed (42) used as ablation source.** Multi-seed ablation source (seed-sweep on the swap-in random values) was skipped per own 16. Conservative read: results from a single seed; not repeated with seed=137/271 random_init weights.
