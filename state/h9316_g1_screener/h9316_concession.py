#!/usr/bin/env python3
"""H_9316 — G1 reopen screener, shot 1: does CONCESSION carry transferable non-additive information?

The instrument is H_9304's, UNCHANGED and therefore still certified -- only the B bit is swapped:
negation -> concession/adversative. That is the whole point of a screener: one certified instrument,
many (corpus x task class) points.

Why concession: the gamma census named the true non-additive class as "XOR-class only (negation /
adversative)", but only NEGATION was ever measured in this lane. Concession is the unmeasured
projection. Mechanism: "good BUT expensive" -- the clause after the adversative dominates the
document label, so the first clause's polarity interacts with the marker.

Fable's structural argument, which this shot tests from a second angle: NSMC's +0.002 was not "no
XOR" (delta_hat = -0.35, the direction was right) -- it was that the MAIN EFFECTS eat the label
variance. And the reason communicative language is like that: language is written TO BE UNDERSTOOD,
so a speaker lays down additive cues redundantly. Interaction-dominant structure is what
communicative language structurally AVOIDS.
...
CI = 98.3            # Bonferroni: 3 planned candidates -> alpha 0.05/3, so 95% -> 98.3%
XBIND_RULER = 5.29653  # the same certified instrument reads this on the synthetic XOR corpus

# pre-registered CLOSED SET of CONCESSION / ADVERSATIVE markers.
# researcher-DoF guard: this list is frozen in FREEZE.txt; no post-hoc additions.
NEG_PATTERNS = [
    (re.compile(r"지만"), "지만"),
    (re.compile(r"는데"), "는데"),
    (re.compile(r"ㄴ데"), "ㄴ데"),
    (re.compile(r"하지만"), "하지만"),
    (re.compile(r"그러나"), "그러나"),
    (re.compile(r"그런데"), "그런데"),
]
...
def build_cells(rows, min_occ):
    ...
    items = []
    for text, t in rows:
        b = 1 if has_concession(text) else 0
        for s in set(stems(text)):
            if s in keep:
...
def main():
    ...
    res = {"id": "H_9316", "delta_eq": DELTA_EQ, "k_perm": a.k_perm, "seed": a.seed}
    ...
    log(f"[corpus] rows={len(rows)}  concession-rate={np.mean([has_concession(t) for t,_ in rows]):.3f}")
    ...
    if int(ho.sum()) == 0:
        ...
        json.dump(res, open(os.path.join(a.out, "h9316_summary.json"), "w"), indent=2, ensure_ascii=False)
        sys.exit(0)
    ...
    log(f"  effect size vs the XBIND ruler: {e_n/XBIND_RULER:.5f}  ({100*e_n/XBIND_RULER:.3f}% of a planted operator)   [p-values are NOT the verdict: n is large enough that a speck is significant]")
