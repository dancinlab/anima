#!/usr/bin/env python3
"""H_6162 HE-AS-OBJECTIVE fair cheap-gate v2 (FROZEN 2026-07-02, FREEZE_v2.md).
v2 fixes v1's mis-specified sanity gate: replaces 'baseline must generalize held-out'
(which contradicts the G1 wall premise) with an ORACLE control (train WITH held-out
combos present -> must solve, proving the floor is compositional not noise). +0.15
delta bar UNCHANGED (frozen). +2 seeds for power. tune-to-green forbidden (p7)."""
import json
import numpy as np
import torch, torch.nn as nn, torch.nn.functional as F

NF, E, C = 6, 4, 9
D, H = 96, 192
STEPS, BS, LR = 4000, 256, 2e-3
LAMBDAS = [0.0, 0.3, 1.0, 3.0]
SEEDS = [7, 4302, 4303, 4304, 4305]
HELDOUT_FRAC = 0.22

def make_task(seed):
    g = np.random.default_rng(seed)
    T = g.integers(0, C, size=(NF, NF))
    combos = [(a, b) for a in range(NF) for b in range(NF)]
    g.shuffle(combos)
    nho = round(len(combos) * HELDOUT_FRAC)
    held = set(combos[:nho]); seen = [c for c in combos if c not in held]
    fa_seen = {a for a, _ in seen}; fb_seen = {b for _, b in seen}
    assert fa_seen == set(range(NF)) and fb_seen == set(range(NF))
    return T, seen, sorted(held)

def sample(combolist, T, rng, n):
    fa = np.array([c[0] for c in combolist]); fb = np.array([c[1] for c in combolist])
    idx = rng.integers(0, len(combolist), size=n)
    A_f, B_f = fa[idx], fb[idx]
    A_tok = A_f * E + rng.integers(0, E, size=n)
    B_tok = B_f * E + rng.integers(0, E, size=n)
    return (torch.tensor(A_tok), torch.tensor(B_tok), torch.tensor(T[A_f, B_f]))

class Net(nn.Module):
    def __init__(self):
        super().__init__()
        V = NF * E
        self.ea = nn.Embedding(V, D); self.eb = nn.Embedding(V, D)
        self.part = nn.Sequential(nn.Linear(D, H), nn.GELU(), nn.Linear(H, D))
        self.pair = nn.Sequential(nn.Linear(2*D, H), nn.GELU(), nn.Linear(H, D))
        self.readout = nn.Linear(D, C)
        self.g = nn.Sequential(nn.Linear(3*D, H), nn.GELU(), nn.Linear(H, D))
    def forward(self, at, bt):
        ra, rb = self.part(self.ea(at)), self.part(self.eb(bt))
        h = self.pair(torch.cat([ra, rb], -1))
        return self.readout(h), h, ra, rb
    def he(self, ra, rb):
        return self.g(torch.cat([ra, rb, ra*rb], -1))

def train(seed, lam, T, train_combos):
    torch.manual_seed(seed); np.random.seed(seed)
    rng = np.random.default_rng(1000+seed)
    net = Net(); opt = torch.optim.Adam(net.parameters(), lr=LR)
    for _ in range(STEPS):
        at, bt, y = sample(train_combos, T, rng, BS)
        logits, h, ra, rb = net(at, bt)
        loss = F.cross_entropy(logits, y)
        if lam > 0:
            loss = loss + lam * F.mse_loss(h, net.he(ra, rb))
        opt.zero_grad(); loss.backward(); opt.step()
    net.eval()
    def acc(cl, n=4096):
        at, bt, y = sample(cl, T, rng, n)
        with torch.no_grad():
            return (net(at, bt)[0].argmax(-1) == y).float().mean().item()
    return net, acc

def main():
    out = {"frozen": "FREEZE_v2.md", "chance": round(1.0/C, 4), "seeds": {}}
    for s in SEEDS:
        T, seen, held = make_task(s)
        rec = {"n_seen": len(seen), "n_held": len(held), "arms": {}}
        # ORACLE: train with held-out combos PRESENT -> proves task solvable
        _, oacc = train(s, 0.0, T, seen + list(held))
        rec["oracle_heldout_acc"] = round(oacc(held), 4)
        for lam in LAMBDAS:
            _, acc = train(s, lam, T, seen)
            rec["arms"][f"lam_{lam}"] = {"train_acc": round(acc(seen), 4),
                                          "heldout_acc": round(acc(held), 4)}
            print(f"seed={s} lam={lam}: train={rec['arms'][f'lam_{lam}']['train_acc']} "
                  f"held={rec['arms'][f'lam_{lam}']['heldout_acc']} (oracle={rec['oracle_heldout_acc']})", flush=True)
        out["seeds"][str(s)] = rec
    per_seed = []
    for s in SEEDS:
        arms = out["seeds"][str(s)]["arms"]
        off = arms["lam_0.0"]["heldout_acc"]
        on_best = max(arms[f"lam_{l}"]["heldout_acc"] for l in LAMBDAS if l > 0)
        per_seed.append({"seed": s, "off": off, "on_best": on_best, "delta": round(on_best-off, 4)})
    n_ge = sum(1 for p in per_seed if p["delta"] >= 0.15)
    no_regress = all(p["on_best"] >= p["off"] for p in per_seed)
    train_ok = all(out["seeds"][str(s)]["arms"][f"lam_{l}"]["train_acc"] >= 0.90
                   for s in SEEDS for l in LAMBDAS)
    oracle_ok = all(out["seeds"][str(s)]["oracle_heldout_acc"] >= 0.90 for s in SEEDS)  # FIXED sanity
    frac_ge = n_ge / len(SEEDS)
    if not (train_ok and oracle_ok):
        tier = "INCONCLUSIVE (sanity: oracle can't solve OR undertrained)"
    elif frac_ge >= 2/3 and no_regress:
        tier = "DIRECTIONAL-SUPPORT (>=+0.15 on >=2/3 seeds, no regress) — engine-native GPU authorized"
    else:
        tier = "🧱 DIRECTIONAL-FLOOR (NOT-SUPPORTED) — HE-objective does not reliably lift held-out composition"
    out["verdict"] = {"per_seed": per_seed, "n_delta_ge_0.15": n_ge, "frac_ge": round(frac_ge,3),
                      "no_regress": no_regress, "train_ok": train_ok, "oracle_ok": oracle_ok, "tier": tier}
    json.dump(out, open("result_v2.json", "w"), indent=2)
    print("\n=== VERDICT v2 ===")
    for p in per_seed: print(p)
    print("oracle_ok:", oracle_ok, "| n>=+0.15:", n_ge, "/", len(SEEDS), "| no_regress:", no_regress)
    print("TIER:", tier)

if __name__ == "__main__":
    main()
