#!/usr/bin/env python3
"""H_9300 — LINEAGE under COUNT-STARVATION (repair of the H_9299 vacuous sweep): is the mitosis division genealogy itself the statistical
...
def main():
    ...
    ap.add_argument("--starve-sweep", default="0.05:40,0.005:40,0.0:40,0.0:120,0.0:400")  # thresh:grow_max
    ...
    log(f"=== H_9300 — LINEAGE under COUNT-STARVATION (does shrinkage pay where cells ARE starved?) === device={dev}")
    ...
    sweep = {}
    for spec in a.starve_sweep.split(","):
        th, gm = spec.split(":"); th = float(th); gmax = int(gm)
        leaves, node_center, node_parent = grow_on_lineage(Xall, Yall, ntr, vj, dev, gmax, th, 2)
        K = len(leaves)
        ct = torch.tensor([node_center[i] for i in leaves], dtype=torch.float64, device=dev)
        ...
        sweep[spec] = dict(spec=spec, split_thresh=th, grow_max=gmax, cells=K, tree_depth=depth_max, A1=a1, FLAT=flat, LIN=lin,
                           SHUF_mean=shuf_mean, SHUF_per_seed=shuf,
                           paired=[lin - v for v in shuf])
        log(f"[{spec}] cells={K} depth={depth_max}  A1={a1:.5f}  FLAT={flat:.5f}  "
    ...
    # HEADLINE = the MOST STARVED point (max cells) -- pre-registered, not cherry-picked
    matched = max(sweep, key=lambda s: sweep[s]["cells"])
    M = sweep[matched]
    ...
    log("-" * 79)
    log(f"HEADLINE = MOST-STARVED point: {matched} (cells={M['cells']})")
    ...
    json.dump({"id": "H_9300", "device": str(dev), "ko_window_sha256": sha,
              ...
              open(os.path.join(a.out, "h9300_summary.json"), "w"), indent=2, ensure_ascii=False)
    log("[done]")
