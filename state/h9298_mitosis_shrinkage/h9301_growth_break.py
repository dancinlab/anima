...
def main():
    ...
    ap.add_argument("--starve-sweep", default="0.0:11,0.0:20,0.0:40,0.0:80,0.0:160,0.0:320")  # thresh:grow_max
    ...
    log(f"=== H_9301 — MITOSIS GROWTH-BREAK (repair: degenerate split blacklists the cell, not the loop) === device={dev}")
    ...
    for spec in a.starve_sweep.split(","):
        ...
        leaves, node_center, node_parent = grow_on_lineage(Xall, Yall, ntr, vj, dev, gmax, th, MIN_OWNED)
        K = len(leaves)
        ct = torch.tensor([node_center[i] for i in leaves], dtype=torch.float64, device=dev)
    ...
    json.dump({"id": "H_9301", "device": str(dev), "ko_window_sha256": sha,
              ...
              open(os.path.join(a.out, "h9301_summary.json"), "w"), indent=2, ensure_ascii=False)
    log("[done]")
