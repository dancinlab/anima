#!/bin/bash
# hexad d=768x12L PyTorch substrate fire dispatch cycle 2 — 2026-05-17
# Vast.ai A100. g_fire_dispatch_robust + orphan-watchdog (75min cap) + ckpt-pull
# Re-fire after cycle 1 (931dd68b0) ckpt-LOST evidence-only.
set -uo pipefail
...
LABEL="hexad-d768x12l-cycle2"
STEPS="${STEPS:-2500}"
WATCHDOG_MIN=75

cd "$LOCAL_DIR"
echo "=== hexad d=768x12L PyTorch substrate fire CYCLE 2 ==="; date -u
...
# (feedback_gpu_dispatch_orphan_watchdog_pattern carry).
( PARENT_PID=$$
...
$SSH "cd /workspace/anima && export PYTHONUNBUFFERED=1 && python3 train_d768x12l.py --mode sanity --corpus corpus_consciousness_v1.jsonl --out-dir out_sanity --steps 200 2>&1 | tee sanity.log" | tee "$LOCAL_DIR/sanity_remote.log" || true

echo "[7/9] MAIN fire: d=768 n_layer=12 from-scratch, $STEPS steps..."
$SSH "cd /workspace/anima && export PYTHONUNBUFFERED=1 && nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv -l 30 > gpu_util.log 2>&1 & SMI=\$!; python3 train_d768x12l.py --mode main --corpus corpus_consciousness_v1.jsonl --out-dir out_main --steps $STEPS 2>&1 | tee fire.log; kill \$SMI 2>/dev/null || true; echo DONE_MARKER rc=\$?" | tee "$LOCAL_DIR/fire.log" || true

echo "[8/9] Verify result.json then SAVE_POD auto-promote + pull retry >=3..."
...
if [ "$SAVED" = "SAVED" ]; then
  ...
  for i in 1 2 3 4 5; do
    ...
    # ckpt pull is the critical one — large file, retry-aware
