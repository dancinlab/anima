...
def build():
    ...
    for a in atoms:
        ...
        for j, l in enumerate(hits[:24]):
            # READ AT THE DECISION POINT, not at the atom. The model was taught to emit the
            # polarity AFTER the arrow — the hidden at the atom's last byte is a position where it
            # has had no reason to commit to anything yet. Every probe in this lane has been reading
            # a position where the answer has not been computed.
            frag = l.split("=>")[0] + "=> "
    ...
    json.dump({"items": items}, open(os.path.join(HERE, "griddec_prompts.json"), "w"),
              ensure_ascii=False)
    return meta, items
...
def main() -> int:
    meta, items = build()
    print(f"H_9302-FORK — the SAME probe, on the DECISION POINT (after the arrow)")
    ...
    for arm in ARMS:
        npz_p = os.path.join(HERE, "hid", f"griddec_{arm}.npz")
        if not os.path.exists(npz_p):
            subprocess.run([ANIMA, "evaluate", os.path.join(CKPT, f"natem_n2_{arm}.clm"),
                            "--dump-hidden", os.path.join(HERE, "griddec_prompts.json"),
    ...
    json.dump(out, open(os.path.join(HERE, "probe_liveness_griddec.json"), "w"),
