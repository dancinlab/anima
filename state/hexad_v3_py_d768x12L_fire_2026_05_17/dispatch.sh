#!/bin/bash
# hexad v3 d=768x12L PyTorch substrate fire CYCLE 4 — 2026-05-17
# Motivation-trigger corpus v3 (10.34 MB · sha256 1afcef43670e83bf… · γ pattern 37.5%).
# Vast.ai A100 SXM4. g_fire_dispatch_robust + 75-min orphan watchdog + SAVE_POD auto-promote.
# Carries forward cycle 3 lessons: remote-script-write pattern for `&` precedence isolation;
# 5-retry ckpt pull; result.json existence gate before SAVE_POD flip.
set -uo pipefail

VASTAI=/Users/ghost/Library/Python/3.14/bin/vastai
export PYTHONWARNINGS=ignore
LOCAL_DIR=/Users/ghost/core/anima/state/hexad_v3_py_d768x12L_fire_2026_05_17
...
LABEL="hexad-v3-d768x12l-cycle4-motiv"
STEPS="${STEPS:-2500}"
WATCHDOG_MIN=75

cd "$LOCAL_DIR"
echo "=== hexad v3 d=768x12L PyTorch substrate fire CYCLE 4 (motivation-trigger corpus v3 10× scale) ==="; date -u
...
# remote-script-write pattern (cycle 3 lesson): write a remote shell script with the
...
python3 train_d768x12l.py --mode sanity --corpus corpus_v3.jsonl --out-dir out_sanity --steps 200 2>&1 | tee sanity.log
SH_EOF
...
echo "[7/9] MAIN fire d=768 n_layer=12 from-scratch corpus v3 (10×), $STEPS steps..."
...
SMI=\$!
python3 train_d768x12l.py --mode main --corpus corpus_v3.jsonl --out-dir out_main --steps $STEPS 2>&1 | tee fire.log
