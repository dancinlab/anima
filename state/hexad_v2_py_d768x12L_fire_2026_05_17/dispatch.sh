#!/bin/bash
# hexad v2 d=768x12L PyTorch substrate fire dispatch CYCLE 3 — 2026-05-17
# Helper-free corpus v2 (1.10 MB · sha256 7359f0b9a3f0… · stimulus-stream).
# Vast.ai A100 SXM4. g_fire_dispatch_robust + 75-min orphan watchdog + SAVE_POD auto-promote.
# Carries forward cycle 2 lessons: remote-script-write pattern for `&` precedence isolation;
# 5-retry ckpt pull; result.json existence gate before SAVE_POD flip.
set -uo pipefail

VASTAI=/Users/ghost/Library/Python/3.14/bin/vastai
export PYTHONWARNINGS=ignore
LOCAL_DIR=/Users/ghost/core/anima/state/hexad_v2_py_d768x12L_fire_2026_05_17
CORPUS_LOCAL=/Users/ghost/core/anima/state/hexad_v2_corpus_spont_2026_05_17/corpus_consciousness_v2.jsonl
SSH_KEY=/Users/ghost/.ssh/id_vast_anima
LABEL="hexad-v2-d768x12l-cycle3-spont"
STEPS="${STEPS:-2500}"
WATCHDOG_MIN=75

cd "$LOCAL_DIR"
echo "=== hexad v2 d=768x12L PyTorch substrate fire CYCLE 3 (helper-free corpus v2) ==="; date -u
...
# Orphan watchdog: kill the instance after WATCHDOG_MIN min if parent dies
( PARENT_PID=$$
...
echo "[4/9] Upload arch + trainer + corpus v2..."
...
$SCP "$CORPUS_LOCAL" "root@$SH:/workspace/anima/corpus_v2.jsonl"

echo "[5/9] GPU + torch + corpus verify..."
$SSH 'cd /workspace/anima && python3 -c "import torch;print(torch.__version__, torch.cuda.is_available(), torch.cuda.get_device_name(0))" && wc -l corpus_v2.jsonl && sha256sum corpus_v2.jsonl'

# remote-script-write pattern (cycle 2 lesson): write a remote shell script with the
# entire fire-then-nvidia-smi pipe so `&` precedence is isolated.
...
python3 train_d768x12l.py --mode sanity --corpus corpus_v2.jsonl --out-dir out_sanity --steps 200 2>&1 | tee sanity.log
SH_EOF
...
echo "[7/9] MAIN fire d=768 n_layer=12 from-scratch corpus v2, $STEPS steps..."
...
SMI=\$!
python3 train_d768x12l.py --mode main --corpus corpus_v2.jsonl --out-dir out_main --steps $STEPS 2>&1 | tee fire.log
...
echo "[8/9] Verify result.json then SAVE_POD auto-promote + pull retry >=5..."
