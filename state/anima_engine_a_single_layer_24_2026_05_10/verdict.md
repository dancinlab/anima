...
2. **Single-seed trained run per condition.** Each L{i} condition uses ONE trained-model seed (42), as in §57. Mirror multi-seed (3) provides the V14 PASS denominator. A multi-seed trained per condition (e.g., 3 seeds × 25 conditions = 75 trained runs) would tighten the verdict polarity for borderline `V14_PARTIAL` cases; deferred per own 16 5h envelope.
