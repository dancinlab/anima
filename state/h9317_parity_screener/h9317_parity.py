#!/usr/bin/env python3
"""H_9317 — is the operator NEGATION-PRESENCE or NEGATION-PARITY?

This is not merely a third screening point. It is an audit of H_9304's own negative.

H_9304 set B = "a negation marker is present" (count >= 1). But if PARITY is the true operator,
a double negation ("not un-bad") must NOT flip the polarity -- and presence-B lumps those rows into
the SAME B=1 cell as the single negations. Two sub-populations pulling in opposite directions,
estimated by ONE coefficient delta => the effect is DILUTED. So H_9304's +0.00233 may in part be an
artifact of a MIS-SPECIFIED operator, which is a verdict-integrity question, not a new hunt.

Census (pre-fire, NSMC 149,995 rows): 0 markers 119,467 (79.6%) · 1 marker 25,741 (17.2%) ·
2 markers 3,873 (2.6%) · 3 markers 742 (0.5%) · 4+ 177 (0.1%)  =>  even(>=2) = 4,050 rows (2.7%)
were being mis-classified by presence-B.

The instrument is H_9304's, UNCHANGED (so its certification carries): only the definition of B is
repaired -- count>0 becomes count mod 2.
...
XBIND_RULER = 5.29653     # the same certified instrument reads this on the synthetic XOR corpus
H9304_PRESENCE = 0.00233  # H_9304's EARNED with presence-B -- the R1 comparison anchor
R1_MARGIN = 0.01
...
def neg_parity(text):
    """B = PARITY of the negation-marker count (count mod 2), not mere presence.

    H_9304 used presence (count >= 1), which puts double negations -- which must NOT flip -- into
    the same B=1 cell as single negations. Under parity, an even count (including zero) leaves the
    stem's polarity intact and an odd count flips it."""
    k = sum(len(p.findall(text)) for p, _ in NEG_PATTERNS)
    return k % 2
...
def main():
    ...
    res = {"id": "H_9317", "delta_eq": DELTA_EQ, "k_perm": a.k_perm, "seed": a.seed}
    ...
    log(f"[corpus] rows={len(rows)}  odd-parity rate={np.mean([neg_parity(t) for t,_ in rows]):.3f}  (H_9304 presence-rate was 0.204)")
    ...
    if int(ho.sum()) == 0:
        ...
        json.dump(res, open(os.path.join(a.out, "h9317_summary.json"), "w"), indent=2, ensure_ascii=False)
        sys.exit(0)
    ...
    r1_delta = e_n - H9304_PRESENCE
    r1 = r1_delta >= R1_MARGIN
    log(f"  effect size vs the XBIND ruler: {e_n/XBIND_RULER:.5f}  ({100*e_n/XBIND_RULER:.3f}% of a planted operator)")
    log(f"  R1 SPEC-REPAIR  (parity - presence >= +{R1_MARGIN}) : {r1}   "
        f"(parity {e_n:+.5f} - presence {H9304_PRESENCE:+.5f} = {r1_delta:+.5f})")
    ...
    if info_present:
    ...
    elif tost_pass and r1:
        v = ("SPEC-REPAIR - presence-B WAS diluting the effect (parity-B reads higher), but parity is "
             "STILL TOST-equivalent to zero => H_9304/H_9316's conclusion stands, with a scope caveat "
             "added to the B definition")
        ...
        v = ("DATA-ADDITIVE - presence-B is EXONERATED (parity buys nothing) AND parity is equivalent "
             "to zero => the XOR-class exhaustion gets STRONGER: the mis-specification alternative "
             "explanation for H_9304's negative is ruled out")
    elif False:
    ...
    res.update({"heldout_earned": e_n, "ci_lo": ci_lo, "ci_hi": ci_hi,
                "seen_synergy": seen_syn, "n_seen_cells": n_seen, "delta_hat_natural": d_n, "effect_vs_xbind_ruler": e_n/XBIND_RULER, "R1_spec_repair": bool(r1), "R1_delta_vs_presence": r1_delta, "h9304_presence_earned": H9304_PRESENCE, "CI_pct": CI, "delta_hat_xor_alive": d_x, "delta_hat_pedestal": d_a, "tost_pass": bool(tost_pass),
