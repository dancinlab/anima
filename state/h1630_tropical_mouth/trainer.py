...
Mechanism (per card):
  ...
  Each forward forms a role-filler SCORE matrix S[r,f] = role_proj + filler_proj
  (additive = multiplicative in log space), and the bound rep per role is a tropical
  matvec  b[r] = max_f ( S[r,f] + value[f] ), argmax routing exactly ONE filler to each
  role. Both legs combine in one pass via max-plus contraction = a hard, temperature-
  annealable (softmax T->0) winner-take-one assignment.

  Implemented as R roles x F fillers. A temperature T anneals from softmax (T=1,
  = ordinary attention) toward tropical (T->0, hard max-plus). The G1/G6 lift must
  VANISH monotonically as T->1, proving max-plus selectivity is load-bearing.

SERIALIZATION (.clm-safe, same pattern as H_1640/1641):
  The tropical head + its byte readout are TRAINING-ONLY (auxiliary loss L_bind shaping
  the shared trunk). The PRODUCTION additive readout Conv1d(d->V) is RETAINED for byte
  generation and serializes -> .clm engine-native loadable. The tropical binder is
  DROPPED before serialize. Binding is a trunk-objective pressure.

ARMS (frozen, PREREG · card ablation = single temperature knob):
  arm    : tropical    T=0.1  (near-hard max-plus, selective winner-take-one).
  soft   : ABLATION    T=1.0  (= log-sum-exp / softmax = ordinary attention) -> blend,
           crosstalk -> expect INERT (no selectivity, the lift should vanish).
  mid    : T=0.5       (intermediate, monotonicity check point).
All arms share IDENTICAL trunk init seed / data / steps / production readout; the ONLY
variable is the semiring temperature T.

USAGE:
  python3 trainer.py --arm {arm,soft,mid} --seed N --corpus <4 paths> \\
...
N_ROLES = 8                # role slots
N_FILLERS = 16             # filler slots
FILLER_DIM = 64            # value width per filler
ARM_TEMP = {"arm": 0.1, "soft": 1.0, "mid": 0.5}   # semiring temperature per arm


class TropicalBinder(nn.Module):
    """Tropical (max-plus) semiring binding aux head (TRAINING-ONLY).

    From the trunk feature, project per-position role scores (R) and filler scores (F)
    and filler VALUES (F x dim). The score matrix S[r,f] = role[r] + filler[f] lives in
    log space (additive = multiplicative). The bound rep per role is the tropical matvec
    b[r] = max_f (S[r,f] + value[f]) realized as a temperature-T softmax over f
    (T->0 = hard argmax = max-plus; T=1 = ordinary softmax-attention). Roles are then
    concatenated and a head predicts the next byte -> aux CE.
    """
    def __init__(self, d, V, R=N_ROLES, Fn=N_FILLERS, fdim=FILLER_DIM, temp=0.1):
        super().__init__()
        self.R, self.Fn, self.fdim, self.temp = R, Fn, fdim, temp
        self.role_proj = nn.Conv1d(d, R * Fn, 1)      # role-filler score S[r,f]
        self.value_proj = nn.Conv1d(d, Fn * fdim, 1)  # per-filler value vectors
        self.head = nn.Conv1d(R * fdim, V, 1)         # bound roles -> byte logits

    def forward(self, x, targets, V):
        B, _, Tt = x.shape
        S_rf = self.role_proj(x).view(B, self.R, self.Fn, Tt)        # (B,R,F,T)
        val = self.value_proj(x).view(B, self.Fn, self.fdim, Tt)     # (B,F,dim,T)
        # tropical / temperature-softened assignment weights over fillers
        w = F.softmax(S_rf / self.temp, dim=2)                       # (B,R,F,T)
        # b[r] = sum_f w[r,f] * value[f]  (T->0 -> picks the argmax filler value)
        bound = torch.einsum("brft,bfdt->brdt", w, val)             # (B,R,dim,T)
        bound = bound.reshape(B, self.R * self.fdim, Tt)            # (B,R*dim,T)
        logits = self.head(bound)                                   # (B,V,T)
        ce = F.cross_entropy(logits.transpose(1, 2).reshape(-1, V), targets.reshape(-1))
        return ce


class BindCLM(nn.Module):
    """CLMConvMoE (PRODUCTION additive readout retained) + tropical binding aux."""
    def __init__(self, cfg, arm):
        super().__init__()
        self.base = CLMConvMoE(cfg)
        self.cfg = cfg
        self.binder = TropicalBinder(cfg.d_model, cfg.vocab_size, temp=ARM_TEMP[arm])
...
def main():
    ...
    ap.add_argument("--arm", required=True, choices=["arm", "soft", "mid"])
    ...
    print(f"=== H_1630 TROPICAL-BIND 303M arm={a.arm} seed={a.seed} ===", flush=True)
    print(f"  device={device} d={d} L={L} E0={e0} Emax={emax} seq_len={seq_len} "
          f"steps={steps} bs={a.batch_size} sample={a.sample} "
          f"bind_lambda={BIND_LAMBDA} T={ARM_TEMP[a.arm]} R={N_ROLES} F={N_FILLERS} "
          f"fdim={FILLER_DIM}", flush=True)
    if device == "cuda":
