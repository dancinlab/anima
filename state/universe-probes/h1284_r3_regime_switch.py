...
def run_arm(facts, events, rng, LR0, TH0, abstain0, policy, fixed_writes=True,
    ...
    switch_polarity ∈ {+1,-1}: the LEARNED sign of the gate (a faithful regime-
    switcher learns its direction from outcome, basal-ganglia go/no-go H_1281 —
    it is NOT given the regime label; only WHICH sign of its own substrate signal
    pays off, calibrated on a DISJOINT tuning seed, no score-seed leak). The
    switch SIGNAL itself stays substrate-derived (write-pressure EMA)."""
    ...
    if policy in ('switch', 'switch_shuffle'):
        ...
        for t in range(len(ema_series)):
            above = ema_series[t] >= np.median(ema_series[:t+1])
            # polarity +1: writes-on when surprise HIGH; -1: writes-on when LOW
            sched[t] = above if switch_polarity > 0 else (not above)
...
def calibrate_polarity(n_facts, tune_seed, n_blocks, block_len, abstain0,
                       LR0, TH0):
    """Learn the gate SIGN from OUTCOME on the DISJOINT tune seed (no score-seed
    leak). The switcher is given neither the regime label nor the answer — only
    which sign of its OWN substrate write-pressure signal yields higher capability
    on held-out tuning data (basal-ganglia outcome-gating, H_1281)."""
    best = None
    for pol in (+1, -1):
        rng = np.random.default_rng(tune_seed)
        facts = make_facts(n_facts, rng)
        ev = gen_regime_stream(facts, rng, n_blocks, block_len)
        _, _, cap = run_arm(facts, ev, np.random.default_rng(tune_seed),
                            LR0, TH0, abstain0, policy='switch',
                            switch_polarity=pol)
        if best is None or cap > best[0]:
            best = (cap, pol)
...
def main():
    ...
    # learn the gate polarity from OUTCOME on the DISJOINT tune seed (no leak)
    pol_star, pol_cap = calibrate_polarity(
        N_FACTS, TUNE_SEED, N_BLOCKS, BLOCK_LEN, ABSTAIN0, LR0_star, TH0_star)
    ...
    summary = {
        ...
        'switch_polarity_learned': int(pol_star),
        'switch_polarity_tune_cap': round(pol_cap, 4),
