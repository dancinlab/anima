...
def main():
    ...
    # Dir-B INTUITOR (g_multidirectional_explore): the INTUITOR overlay trains
    # on the SAME E7 carving corpus/stream as UBM-E7 α (no path-specific
    # carving target — INTUITOR adds a reward-free self-certainty surrogate on
    # top of plain LM CE). For a FAIR head-to-head against the UBM-E7 α
    # baseline (JOINT 0.0155), Dir-B is probed with the IDENTICAL alpha
    # vacuum-form prefixes + identical 31-anchor set + identical joint metric.
    PATH_FORM = {"alpha": "alpha", "beta": "beta", "gamma": "gamma",
                 "weave": "weave", "intuitor": "alpha"}
    form = PATH_FORM.get(path, "alpha")
    ...
    result = {
        ...
        "ubm_e7_alpha_baseline": {
            "JOINT": 0.0155, "knowledge_access": 0.0323,
            "chat_uncontaminated": 0.6, "lane_separation": 0.8,
            "routing_accuracy": "1/31", "note": (
                "UBM-E7 α VACUUM-LANDSCAPE SCALE-UP baseline (carry from "
                "state/consciousness_carving_e7_alpha_scaleup_2026_05_17/"
                "eval_result_v2_e7.json) — identical 31 anchors / alpha "
                "vacuum-form prefixes / joint metric for a fair Dir-B vs α "
                "head-to-head.")},
        "path": path,
        "carving_form": form,
