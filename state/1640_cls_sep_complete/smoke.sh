#!/usr/bin/env bash
# H_1640 CLS-SEP-COMPLETE — $0 CPU smoke: tiny (d=64 L=2) train + serialize +
# clm_decodable + g_gates connection for all 3 arms. NO GPU, NO canon. Verifies
# the trainer wires end-to-end before the 303M GPU launch. Run on pool (summer)
# if mac lacks torch (heavy-anima-eval-pool-not-mini: mac swap risk; this is light
# so CPU ok, but torch must be importable).
set -e
HERE="$(cd "$(dirname "$0")" && pwd)"
REPO="$HERE"; while [ "$REPO" != "/" ] && [ ! -e "$REPO/cli/train.py" ]; do REPO="$(dirname "$REPO")"; done
cd "$REPO"
OUT="$HERE/ckpt"; mkdir -p "$OUT"; export SMOKE_OUT="$OUT"
echo "=== H_1640 smoke (repo=$REPO) ==="
...
# clean import: ONLY core/ on sys.path (exactly what `anima eval` / g_gates uses)
# — avoid the train/clm/model verify_clm_v2 mirror shadowing the core mouth.
cd "$REPO"
python3 - <<'PY'
import sys, os
...
ok_all = True
for obj in ("ce_marginal", "cls_sep", "cls_full"):
    ...
    print(f"  {obj:<12s} clm_decodable={dec} load_ok={loadok} "
