...
int main(int argc, char** argv) {
    ...
    /* ABI-immune device probe: cudaGetDeviceProperties is a macro that
     * maps to _v2 in CUDA 12.x; the struct can mis-resolve under some
     * header orders (prior fire's compile error). Use the attribute API
     * + cudaMemGetInfo instead — stable across all CUDA toolkit ABIs. */
    ...
    cudaError_t _ner = cudaDeviceGetPCIBusId(dev_name, sizeof(dev_name), 0);
    (void)_ner;
    size_t mem_free = 0, mem_total = 0;
    cudaMemGetInfo(&mem_free, &mem_total);
    fprintf(stderr, "[bench] device 0: pci=%s (cc=%d.%d, %ld MB total)\n",
            dev_name, cc_major, cc_minor, (long)(mem_total >> 20));
