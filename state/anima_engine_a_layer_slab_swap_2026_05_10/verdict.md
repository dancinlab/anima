...
2. **Single random_init seed pool for swap-source.** B (BG-LA pretrain) is one ckpt at one training step (step_12000). A multi-seed swap-source (e.g., averaging multiple BG-LA pretrain runs) would strengthen the inference; deferred per own 16.
...
10. **Sample size = 3 seeds, not 5.** Mission specified 3-seed budget for time. Strict V14 (own 14) used 5 seeds — this study trades seed coverage for slab coverage. Verdict polarity at 3 seeds is robust if separation magnitude is large; ambiguous within ±20% of the boundary.
