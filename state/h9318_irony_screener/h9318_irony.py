#!/usr/bin/env python3
"""H_9318 — the EXTREME LEVERAGE shot: does IRONY carry transferable non-additive information?

Fable's structural argument, which the three previous shots kept confirming: the EARNED effect in a
natural corpus is a speck NOT because there is no XOR (delta_hat pointed the right way every time)
but because THE MAIN EFFECTS EAT THE LABEL VARIANCE. And the reason communicative language is like
that: language is written TO BE UNDERSTOOD, so a speaker lays down additive cues redundantly.
Interaction-dominant structure is what communicative language structurally AVOIDS.

IRONY IS THE EXCEPTION. It is the one natural communicative construct where the surface lexical
marginal ACTIVELY MISLEADS (positive words -> negative meaning). Under negation and concession the
marginal is still informative (a negated sentence really is more negative); under irony it is
actively WRONG.

=> If even IRONY reads +epsilon, then no natural communicative task that is MORE marginal-friendly
   can clear the wall: "communicative natural language = DATA-ADDITIVE" is earned at the level of
   the CLASS, not just one corpus. If irony reads GREEN, we have found the first natural operator
   and the frontier reopens. Either way this shot ends the frontier.

Corpus: ArSarcasm (HF iabufarha/ar_sarcasm) -- Arabic tweets carrying BOTH a sarcasm flag AND a
sentiment label, as two INDEPENDENT human annotations. (H_9316 recorded the blocker: most public
irony corpora carry a sarcasm label ONLY, which degenerates B into T.)

The instrument is H_9304's, UNCHANGED (certification carries); only the stem regex moves from
Hangul to Arabic.
...
CI = 90                  # percent
CORPUS_SHA = "d23b4a22b793f0c8e03e4092f6806c1a1062f5de198682041017d9e1ffefba2f"
CI = 98.3                 # Bonferroni family (N=3), inherited from H_9316's FREEZE
XBIND_RULER = 5.29653     # what the same certified instrument reads on the synthetic XOR corpus
PRIORS = {"negation (H_9304)": 0.00233, "concession (H_9316)": 0.00908, "parity (H_9317)": 0.00318}

# B is NOT a regex here. It is the corpus's SARCASM flag -- a human annotation, outside the token
# stream, and annotated INDEPENDENTLY of the sentiment label T. That independence is what keeps the
# measurement from collapsing into a tautology.
...
def stems(text):
    """Content-word stems: Arabic letter runs. No lexicon, no polarity list.
    (The only change from H_9304: the script's alphabet. Everything else is verbatim.)"""
    out = []
    for w in re.findall(r"[\u0600-\u06FF]+", text):
        if 3 <= len(w) <= 12:
            out.append(w[:5] if len(w) > 5 else w)   # crude stem = first <=5 letters
    return out


def load_corpus(path):
    """(text, B=sarcasm flag, T=sentiment label). Both labels are human annotations."""
    rows = []
...
def build_cells(rows, min_occ):
    """items: (stem_id, B, T). B comes from the corpus's sarcasm flag, not from the tokens."""
...
def main():
    ...
    ap.add_argument("--min-occ", type=int, default=10)
    ap.add_argument("--occ-sweep", default="5,10,20")
    ap.add_argument("--seed", type=int, default=9318)
    ...
    res = {"id": "H_9318", "delta_eq": DELTA_EQ, "k_perm": a.k_perm, "seed": a.seed}

    sha = hashlib.sha256(open(a.corpus, "rb").read()).hexdigest()
    log(f"[corpus] ArSarcasm sha={sha[:16]}...  match={sha == CORPUS_SHA}")
    if sha != CORPUS_SHA:
        log("FATAL: corpus sha mismatch - provenance gate. STOP."); sys.exit(2)
    res["corpus_sha256"] = sha
    rows = load_corpus(a.corpus)
    log(f"[corpus] rows={len(rows)}  irony-rate={np.mean([b for _,b,_ in rows]):.3f}  "
        f"(B and T are two INDEPENDENT human annotations)")
    ...
    if int(ho.sum()) == 0:
        ...
        json.dump(res, open(os.path.join(a.out, "h9318_summary.json"), "w"), indent=2, ensure_ascii=False)
        sys.exit(0)
    ...
    log(f"  seen synergy    = {seen_syn:+.5f} nats  (n={n_seen})")
    log(f"  effect size vs the XBIND ruler: {e_n/XBIND_RULER:.5f}  ({100*e_n/XBIND_RULER:.3f}% of a planted operator)")
    log("  priors on the same ruler: " + " · ".join(f"{k} {100*v/XBIND_RULER:.3f}%" for k, v in PRIORS.items()))
    ...
    res.update({"heldout_earned": e_n, "ci_lo": ci_lo, "ci_hi": ci_hi,
                "seen_synergy": seen_syn, "n_seen_cells": n_seen, "delta_hat_natural": d_n, "effect_vs_xbind_ruler": e_n/XBIND_RULER, "CI_pct": CI, "delta_hat_xor_alive": d_x, "delta_hat_pedestal": d_a, "tost_pass": bool(tost_pass),
