# anima-voice A/B Baseline Sentences

Date: 2026-04-18
Purpose: objective baseline for anima-voice quality audit (sister session)
Voices compared: macOS `say` (Yuna/Reed), espeak, piper_ko, anima-voice
...
ab_test/
  ...
  anima_voice_latest/line_NN.wav        # CAVEAT: recycled from tts_say/persona_10/13
                                        # — anima_voice.hexa has no --text CLI,
                                        # text-to-wav regen path is a GAP
  metrics_20260419.tsv                  # previous objective metrics
```

## Regeneration Gap (anima_voice_latest)

`anima_voice.hexa main()` runs an internal **demo only** — there is no
`--text "..." --out file.wav` CLI. The files in `anima_voice_latest/` are
...
- (a) add a CLI text→wav mode to `anima_voice.hexa`, or
...
- [ ] Listen to anima_voice_latest (1-10) — identify failure modes
