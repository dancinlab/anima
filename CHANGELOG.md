...
## 2026-06-17 — fix(ENGINE+CLM): H_1393 — STREAMING/BOUNDED ConvMoE .clm 디코드: 🟢 GREEN (H_1392 메모리 폭발 벽을 anima-side 에서 FIX, byte-exact; GEN=110 해제 → M2-M5 FALS 재채점 측정가능)
...
**페이오프 — 막혔던 G6 측정 재실행:** H_1392 의 FROZEN G6 프로브(detector 10/10 VERBATIM, frame-guard 0 leaks, sampler det/diverse/in_topk, M1-M5 막대 불변)가 이제 실제 303M ConvMoE-RETRO 마운트(sha256==HF MANIFEST) 위에서 engine-native 로 실행됨 → M2-M5 FALS 점수 + 이제 측정가능한 용량-vs-아키텍처 verdict 를 verbatim 박제(막대 안 옮김, tune-to-green 없음, c9/p7) → `.verdicts/1393_convmoe_streaming_decode/g6_rescore.txt`.

**산출물:** CORE 편집 3파일 · `UNIVERSE/cards/H_1393_convmoe_streaming_decode.md` + jsonl 행 + H_1392 tier 패치(🧱→✅ resolved) · `.verdicts/1393_convmoe_streaming_decode/{FREEZE,byte_exact,rss_before,rss_after,rss_after_303m,g6_rescore,result}.txt` · 프로브 `state/1393_convmoe_streaming_decode/*.hexa`. **정직(c9):** 잔여 ~0.6MB/step(d768)/~10.7MB/step(303M) = 런타임 내부 forge_dispatch_matmul 출력 = UPSTREAM bump-allocator floor(into-output matmul 변형이면 0; 상류 핸드오프가 그 훨씬 작은 항목 유지). xref H_1392(이 벽을 고침)·H_1381·H_1362·a_break_the_wall·a_clm_gen_pipeline·a_core_engine_map·a_verified_must_wire·p7·p8·c9.
