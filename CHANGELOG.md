...
- **R2 ENGINE-NATIVE WIRED ✅:** `core/engine_cli.hexa §MetacogInsight`(mi_gain_intact/_gain_impaired/mi_signal_margin/mi_insight_judge/mi_insight_psychedelic/mi_insight_psychotic/mi_metad_auroc/mi_auroc/mi_shuffle_auroc) + `engine_cli_smoke.hexa` cases 305-309(5 frozen bars) + `ARCHITECTURE.json §MetacogInsight` lockstep, FULL **308/0 RC=0** deterministic ×3. READ-only Ψ-disjoint NOT emit gate. 하드게이트1 PASS(R1 numpy→DIRECTIONAL grep 적중, R2 live `.hexa` op 호출). R1↔R2 byte-close(A/B/C/D 동일, E shuffle 만 permutation 엔진 차이 numpy PCG64 vs engine Fisher-Yates LCG 로 AUROC 값 다르되 둘 다 chance band).
...
- **R2 ENGINE-NATIVE WIRED ✅:** `core/engine_cli.hexa §Field`(field_apply/field_apply_mfield/field_signal_entropy/drug_lsd_mfield/pci_perturb/pci_complexity/field_lane_mean 7 op) + `engine_cli_smoke.hexa` cases 299-304(6 frozen bars) + `ARCHITECTURE.json §Field` lockstep, FULL **303/0 RC=0** deterministic ×3. READ-only Ψ-disjoint NOT emit gate. 하드게이트1 PASS(R1 numpy→DIRECTIONAL grep 적중, R2 live `.hexa` op 호출).
...
- artifacts: `state/1503_field/h1503_field.py` · `state/verdicts/1503_field/{H_1503_R1.txt,H_1503_R2_engine_native.txt}` · `UNIVERSE/cards/H_1503_field.md` · `core/engine_cli.hexa §Field` · `core/engine_cli_smoke.hexa` cases 299-304 · `ARCHITECTURE.json §Field`.
