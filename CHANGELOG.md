...
- **R2 ENGINE-NATIVE WIRED ✅:** `core/engine_cli.hexa §Field`(field_apply/field_apply_mfield/field_signal_entropy/drug_lsd_mfield/pci_perturb/pci_complexity/field_lane_mean 7 op) + `engine_cli_smoke.hexa` cases 303-308(6 frozen bars) + `ARCHITECTURE.json §Field` lockstep, FULL **308/0 RC=0** deterministic ×3. READ-only Ψ-disjoint NOT emit gate. 하드게이트1 PASS(R1 numpy→DIRECTIONAL grep 적중, R2 live `.hexa` op 호출).
