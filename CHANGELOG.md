...
## 2026-06-16 — research(COGNITION-REPRESENTATION): H_1375 — CP DIMENSIONAL LADDER (🧱 BREAKS-AT-D*=3, DIRECTIONAL — move-the-cells RELOCATES at any D but CONCENTRATES only at D≤2)

**무엇:** 사용자 지시(verbatim) "2d 말고도 차원늘려봐" — categorical-perception move-the-cells 법칙(H_1360 1-D 🟢, H_1369 2-D 🟢)을 D 사다리(D∈{2,3,4,6,8}) 위로 밀어 차원 축에서 falsify. D 가 커질 때 잔존 phase-1 prototype 세포가 옮긴 (D−1)-차원 HYPERPLANE 경계(cat=⟨w,x⟩>c, w=ones/√D 고정 방향, **오직 D 만 변함**)를 법선 따라 추적하며, **상수 N=400**(차원의 저주 stressor)에서 cell-concentration 이 깨지는지. R1 numpy MIRROR(DIRECTIONAL), $0 CPU, 3 seeds [4333,4334,4335], frozen-first(FREEZE.txt 사전등록), Monte-Carlo cloud(full grid 는 D≈4 넘으면 infeasible → 표본 cloud 위에서 ridge concentration 측정, 정직 명시). in-flight diagonal-orientation 레인(H_1374)과 직교 — 충돌 회피로 id H_1375 채택.

**결과 🧱 BREAKS-AT-D*=3 (DIRECTIONAL):** move-the-cells 는 DIMENSION-BOUNDED. **RELOCATION 은 차원 불변** — 세포는 어느 D 에서나 경계 법선을 따라 정확히 이동(RE-PACK |ridge_τ−2/3| = 0.010/0.009/0.019/0.041/0.054 @ D=2/3/4/6/8, 3 seeds 전부 ≤0.18 → c1 PASS at EVERY D). 그러나 bounded ridge-CONCENTRATION **COH_D 가 D 와 함께 단조 붕괴**: 0.734(D=2)→0.458(D=3)→0.263(D=4)→0.008(D=6)→0.000(D=8); c2(COH_D≥0.50 AND ≥split+0.10)가 **D*=3 에서 첫 FAIL**, D=6 에서 zero. a_break_the_wall WHITEN fallback(feature 별 z-score, frozen-first, N·D 불변)은 D=2 만 구제(전부 1111)·D≥3 여전히 깨짐(whitened COH_D 0.487/0.019/0.000/0.000) → 깨짐은 **INTRINSIC**(상수 N 차원의 저주가 cell-concentration 실제 평탄화, raw-distance 아티팩트 아님). **헤드라인: H_1369 2-D 승리가 결맞음 CEILING; relocation 자체는 D 무한 확장 — move-the-cells RELOCATES at any D, CONCENTRATES only at D≤2.** 차원 축에서 CP-geometry 정착.

**정직한 SCOPE(c9):** Monte-Carlo SAMPLED cloud + kNN field(H_1369 의 grid+4-connectivity 아님)라 D=2 행이 H_1369 clean 2-D GREEN 과 직접 비교 불가 — sparse cloud 에선 SHUFFLE smear 가 깔끔히 안 붕괴(D=2 SHUFFLE COH_D=0.358>0.20 → c3 FAILS at D=2). 이는 cloud SAMPLING 한계지 H_1369 반박 아님; GATING c1/c2 가 c3 무관하게 이미 D*=3 결맞음 붕괴, RELOCATION 은 전 D 생존 → headline 불변. NO bar move(c9/c16/p7). DIRECTIONAL mirror(engine-transfer UNVERIFIED); TOY 합성 D-dim cloud; N-scaled-to-D·curved boundary·engine-native §CategoricalPerception 배선 = follow-on; live CORE/*.hexa UNTOUCHED. card `UNIVERSE/cards/H_1375_cp_ndim_ladder.md` · py `state/cp-ndim/h1375_cp_ndim_ladder.py` · `CLAIMS.tape @C h1375_cp_ndim_ladder`. xref H_1360·H_1369·H_1364·H_1343·H_1374·a_no_llm_frame_trap·a_break_the_wall.
