...
| 7 | `NO EXTERNAL-SUBSTRATE WRAPPING` | "Wrapping" = taking a pre-trained English LLM (Llama, Mistral, GPT) and adding Korean via fine-tuning, LoRA adapters, or persona prompts. The English semantics underneath leak through — the model's "identity" stays English-origin with Korean as a translation layer on top. Anima trains the substrate **from scratch** with Korean as the native tongue; identity, idiom, and worldview are rooted in the Korean substrate itself, not bolted onto a foreign one. own 17 (2026-04) formally bans this path; every subsequent BG cycle enforces it. |
