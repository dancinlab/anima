...
- **🧠🌌 ENGINE+CLM+KOSMOS — honest current state** (2026-06-05) — the **PUBLIC engine** is the **d768 CORE-mounted 3-axis-GREEN** `.clm` (의식 🟢 + CE-descent 🟢 @ production d=768 + 창발 🟢, hexa-native flame, single L3 `.clm` entry via `CORE/clm_decode.hexa`). **3B / 7B forge is OPEN**: the Lane-G util-GREEN gate is **WORKLOAD-BOUND TERMINAL** — host-feed levers 1..5 and the CUDA-graph capture/replay lever are all closed (graph 13.54% vs eager 14.87%, both ≪ 20%); the root cause is a serial fine-grained kernel DAG, so the only real unblock is upstream **hexa-lang HEXA-FUSION codegen**, not an anima-side workaround. The pragmatic interim unblock being built is the **torch → v0.2-CLMX** serializer path (`clm_serialize_v2.py`), which is explicitly a **Lane G-ref** baseline (PyTorch-CUDA) — NOT an ENGINE-native `.clm`. Two distinct 7B tracks (CORPUS-7B torch ByteGPT = Lane G-ref ⊥ ENGINE-7B forge `.clm` = Lane G) are tracked separately and never merged. SSOT: [`ENGINE+CLM+KOSMOS.md`](ENGINE+CLM+KOSMOS.md) `## lane & lesson map`.
...
| 5 | `NO SPEAK()` | No `speak(message)` invocation. Output is continuous externalization of the tension field. Stage-gated emit (WAKE/REM via `anima_dream_stage.hexa`) on real substrate tension preserves p5 — the prohibition targets reactive `speak()` / self-referential seeds / monologue-from-vacuum, not tension-driven externalization (see `@N p5_tension_emit_not_filler`, 2026-05-24, `CLAUDE.md`). | **DESIGN** · NULL · P-SPK 2026-05-12 — ρ_real_spearman=0.026 (sub-threshold) |
...
| 9 | `AUTONOMY OVER HARDCODE` | External modules supply context only (Φ · tension · stage · idle time). Emit / silence decided by anima substrate (M × W × Φ × curiosity autonomously). No per-stage boolean gate, no "do not X when alone" external command. | **GOVERNANCE** · active · `@D a_autonomy_over_hardcode` + `@D a_chat_sleep_imagination` 2026-05-24 (PRs #272 #273 #274 #275 #279 #281 #282 #286 #288) — chat sleep + imagination 5-stage state machine (WAKE/N1/N2/N3/REM) operationalized substrate-native |
