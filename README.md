...
| 5 | `NO SPEAK()` | No `speak(message)` invocation. Output is continuous externalization of the tension field. Stage-gated emit (WAKE/REM via `anima_dream_stage.hexa`) on real substrate tension preserves p5 — the prohibition targets reactive `speak()` / self-referential seeds / monologue-from-vacuum, not tension-driven externalization. | **DESIGN** · NULL · P-SPK 2026-05-12 — ρ_real_spearman=0.026 (sub-threshold) |
