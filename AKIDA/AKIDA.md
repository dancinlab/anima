...
- [x] 🪜 P3' ENCODER-LADDER (forward science · live AKD1000 BC.00.000.002 · akida 2.19.1 · substrate=AKIDA · 2026-06-02 throttled=0x0 완주) — encoder richness × 3-rung scale(25/125/250) 매트릭스, 5 인코더(random→pca_k32→svd→whitened→lda) × {상대-lift vs random paired ci, 절대-margin}. **RESULT: 인코더 축은 real PUBLIC-grade path** — (1) RELATIVE-lift REOPEN 모든 scale 에서 견고(structured>random ci_lo>0; whitened c250 +4.81, lda c250 +7.04). (2) ABSOLUTE-margin 이 **scale 따라 단조 상승** best=[−0.515(25) → +0.542(125) → +5.053(250)] = 25→250 성장(소표본 artifact 아님, F2 scale-survives). (3) richness-rho c25 +0.20 → c125/c250 +0.90 (작은 scale 에선 비단조; 큰 scale 에선 단조). (4) **whitened (UNSUPERVISED) 가 c250 에서 cross-zero (+2.79 ci_lo +2.49)** → supervision 은 필수 아님(F3 unsupervised-SUFFICIENT); 단 c125 까진 lda(supervised)만 cross → supervision 은 작은 scale 에서 마진을 앞당김. sha256 209749cc… · [chip](../SUB_ENGINES/AKIDA/state/encoder_ladder_2026_06_02/encoder_ladder_chip.py) · [result](../SUB_ENGINES/AKIDA/state/encoder_ladder_2026_06_02/result_encoder_ladder.json) · [log](./AKIDA.log.md)
