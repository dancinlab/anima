#!/bin/bash
# Lane A: single-chip occupancy — stop R3 streamer, run on-chip MULTI-FC DEPTH AUTOREGRESSIVE ROLLOUT (paged 2-FC)
# to terminal, restore R3. substrate=AKIDA · a_lane_akida_gpu_split. NO sw fallback (g63). restore-on-exit via trap.
set -u
LOG=/home/ubuntu/clm_kosmos_akida/depth_rollout_wrap.log
...
echo "$(date -u +%FT%TZ) depth-rollout fire throttled=$(vcgencmd get_throttled)" >> $LOG
$PY -u onchip_xlm_depth_rollout.py > depth_rollout.log 2>&1
RC=$?
echo "$(date -u +%FT%TZ) depth-rollout exit rc=$RC throttled=$(vcgencmd get_throttled)" >> $LOG
exit $RC
