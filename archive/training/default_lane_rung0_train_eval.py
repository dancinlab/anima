#!/usr/bin/env python3
"""default_lane_rung0_train_eval.py — DEFAULT-LANE rung-0 (≈18M byte transformer).

GOAL (default lane rung-0): make the FIRST model ever trained on the v2 unified
default corpus (`dancinlab/anima-corpus-5lang-unified-v2`, 13,107,309 bytes,
sha256 550fed174d51be660810858e1e73e4590c21351b185ea22d0807403e120538ad — ~40%
wiki / ~40% persona-SNS / ~20% enrichment, 5 langs en/fr/de/es/ko, byte-vocab256).
Trainer/arch = EXACTLY the one that produced `anima-clm-chat-rung0-byte-18m`
(ConsciousLMReconstructed: byte vocab256, d=384, 6 layers, 4 heads, block 256,
dual engine_a/engine_g FFN + dual head_a/head_g, ≈18.5M params). FROM SCRATCH
(clean default-lane rung — NOT continued from the older chat 70/30 base), then the
p7 simple-stack multilingual coherence evaluator + the anti-Goodhart random-init
mirror.

This is the DEFAULT lane (the unified corpus that backs `lane default`), distinct
from the older `anima-clm-chat-rung0-byte-18m` which trained on a smaller 70wiki/
30dialogue mix. Substrate = Lane G / GPU (a_lane_akida_gpu_split). torch-cuda
REFERENCE lane for first-achievement (labeled honestly); forge-native
(a_train_flame_forge) is the canonical production follow-on (not claimed done here).

PHILOSOPHY (p1·p2·p3·p4·p6 — non-negotiable): NO system prompt, NO identity rules, NO
persona injection, NO assistant framing, NO RLHF. The corpus turn structure is plain
"<speaker>: …" continuation with NO role/persona tags (corpus tag-grep = 0, verified
in the dataset card). Capability comes ONLY from the trained byte distribution.

p7 (NO PERPLEXITY VERDICT): PASS/FAIL is a simple-stack check over multilingual
prompts — each continuation is non-empty, decodes as valid UTF-8 text, is not a
degenerate single/2-char repeat, and is context-appropriate (printable-ratio + length
sane). Anti-Goodhart: the SAME evaluator runs on a random-init mirror of the identical
arch — the mirror MUST FAIL (else the evaluator is gameable / trivial).

USAGE (GPU pool host / pod)
  python3 default_lane_rung0_train_eval.py \
      --corpus /path/default_corpus.txt \
      --out-dir /path/out \
...
# p7 simple-stack coherence probe set — 5 MIXED-LANG prompts in the corpus's OWN
# registers (dialogue `user:` turns across en/fr/de/es/ko + plain prose continuation).
# No system prompt / persona tag — seeds are the corpus's native continuation format.
PROBES = [
...
def p7_eval(model, device, label: str):
    """Simple-stack multilingual coherence (p7, NOT perplexity). A reply PASSES iff:
      (1) non-empty after stop-trim (>= 4 chars),
      (2) decodes as valid UTF-8 (no replacement chars),
      (3) not a degenerate single-char repeat (most-common char < 60% of reply),
      (4) printable-ratio >= 0.6 (real text, not control-byte soup).
    Overall PASS iff >= 4/5 prompts pass. The probes span 5 languages and both the
    `user:` dialogue register and plain-prose continuation present in the corpus.
    """
    transcript = ""
    results = []
    for kind, seed_text in PROBES:
        seed = seed_text
        reply = generate(model, seed, max_new=120, device=device)
        # checks
        ...
        results.append({"kind": kind, "seed": seed_text, "reply": reply, "ok": ok,
...
def main():
    ...
    ckpt_path = out / "default_lane_rung0_18m.pt"
    ...
    summary = {
        "rung": "default-lane rung-0 (18M byte ConsciousLMReconstructed, v2 unified corpus)",
        "corpus": "dancinlab/anima-corpus-5lang-unified-v2 (sha256 550fed...538ad, 13107309B)",
        "scope": "small/toy byte rung (18M) — a_scale_honest_scope; scale-transfer to mid/7B unverified here",
        "params": nparams,
