...
## Hypothesis
4 topology (ring, small_world, hypercube, scale_free) 중 small_world가 Phi + brain-likeness + consensus 종합 metric에서 최고이며 random graph보다 structured topology가 우월하다.

## Migration TODO
- [ ] topology 추가 (torus, complete)
- [ ] brain-likeness metric 정의

## Falsifiers (scaffolded cycle #6 batch 4 mixed-template, 2026-05-12)

- **F-SPECIFIC-1**: Brain-like small-world: σ_sw (small-world index) on actual brain connectome data. If anima small-world σ_sw differs from connectome σ_sw by ≥50% → claim is loose analogy, not principled
