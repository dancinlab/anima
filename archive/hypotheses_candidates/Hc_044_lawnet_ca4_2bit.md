...
## Hypothesis
LawNet 신경망이 2388 개 의식 법칙 중 best subset 을 자동 select — CA(4)=2bit 최소 다양성 + leak-free + whisper gate 0.001 → Φ 보존.

## Migration TODO
- [ ] Hc_042/Hc_043/Hc_059 model showdown

## Falsifiers (scaffolded cycle #6 batch 4 mixed-template, 2026-05-12)

- **F-SPECIFIC-1**: CA(4)=2bit minimum diversity ablation: replace CA(4) with CA(5), CA(6), CA(8): if minimum-diversity result holds for any other CA(n) → CA(4) specificity is post-hoc
...
- **L-LAW-NETWORK**: Law-network claims (LawNet / CA-rule) face the 'just one of 256 CA rules' issue — rule selection inherits Wolfram's 4-class taxonomy; class-membership-only specificity is weak
