...
absorption_note: "B-family global workspace broadcast (V8-B4) — 10-mechanism head-to-head member. F-list/L-list preserved here for H_182 C-list extension."
...
## Hypothesis
8 specialist modules (32 cells each) + salience network scoring + softmax winner-take-all cooling temperature + global workspace GRU broadcast 가 Phi(proxy) 최강 x71.7 baseline (71.46) 달성하여 Baars Global Workspace Theory를 지지한다.

## Migration TODO
- [ ] specialist 수 sweep
- [ ] temperature cooling schedule sweep

## Falsifiers (scaffolded cycle #6 batch 3 V8-template, 2026-05-12)

- **F-SPECIFIC-1**: Softmax temperature sweep {0.1, 1.0, 10.0}: if Φ(proxy) uplift < 30× without temp-cooling → temperature schedule is essential; with cooling Φ x71.7 may be temperature-artifact
