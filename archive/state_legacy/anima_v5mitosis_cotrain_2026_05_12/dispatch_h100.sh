#!/bin/bash
# state/anima_v5mitosis_cotrain_2026_05_12/dispatch_h100.sh
#
# REBORN §88 cond.5 cotrain fire — v5-mitosis architectural lane.
# Vast.ai H100 (SXM or NVL), ~$2.30-2.70/hr, 8-10hr wall, $40 absolute max.
#
# user-verbatim: "fire" = OK CLM V5-MITOSIS H100 FIRE COST $40
#
# References:
#   tool/dispatch_vast_mac_template.sh (PSCC §28 canonical)
#   docs/anima_clm_v5_mitosis_engine_arch_spec_2026_05_12.md §7
#   memory: feedback_orchestrator_h100_gotchas (Ubuntu 24.04 PEP 668 fix,
#           scp timeout 3600, pod retain on pull fail)
#
# Cost guard: pre-fire estimate × per-hour > $44 ABORT.
# trap cleanup: pod auto-destroy unless SAVE_POD=1 set.
...
PHASE_LABEL="anima-v5mit-cotrain"
...
# Cost envelope (REBORN §88, user verbatim $40)
COST_CAP_USD="${COST_CAP_USD:-40.0}"
COST_PER_HR_MAX="${COST_PER_HR_MAX:-3.5}"   # if offer > this, abort
ESTIMATED_WALL_HR="${ESTIMATED_WALL_HR:-10.0}"

# Cost-hard-cap absolute (1.10× of cap)
...
echo "[2/9] Pre-fire cost gate ..."
echo "  est_cost = $OFFER_DPH × $ESTIMATED_WALL_HR hr = \$$EST_COST"
echo "  absolute_max = \$$ABSOLUTE_MAX_USD"
EXCEEDS=$(python3 -c "print('YES' if $EST_COST > $ABSOLUTE_MAX_USD else 'NO')")
if [ "$EXCEEDS" = "YES" ]; then
    echo "[ABORT] est_cost \$$EST_COST exceeds absolute_max \$$ABSOLUTE_MAX_USD"
    exit 1
fi
echo "  ✓ within budget"
...
echo "[5/9] Uploading files (model + script + corpus + probe) ..."
...
$SCP_CMD "$LOCAL_DIR/corpus_color_cosmology.txt" "root@$SSH_HOST:/workspace/anima/corpus/"
...
# ── 7) Train (cotrain) ───────────────────────────────────────────────
echo "[7/9] Cotrain ($STEPS steps, batch=$BATCH, ctx=$CTX, lr=$LR)..."
...
# ── 8) Pull artifacts vast→Mac (pod retain on pull fail) ─────────────
...
# Best-effort: intermediate ckpts (largest only, save space)
