...
5. **120 process calls is short-trajectory** — α super-linear regime (§43 archive) needs 1000–3000 turns. Sub-warmup-window (`adaptive_window=100`) means A2 per-cell threshold barely kicks in (warmup gate: `len(_global_tension_history) >= max(adaptive_window//2, 10)` = 50). For 120 steps × no warmup gate fixes, the dispersion trigger only opens after step 50, leaving 70 effective steps under the full A1 + A2 policy.
