"""
train_phase1a3.py — Phase 1A.3 corpus-10x SFT (data scale isolation).

Continuation from Phase 1A.1 ckpt (4/5 std_greedy).
Phase 1A.2 (lr 1e-6 × 200 steps × 2700 dialogues) failed to break markdown attractor.
Phase 1A.3 strategy: 동일 lr (1e-6) 동일 steps (200), corpus 만 10x (27000 dialogues).
  → markdown attractor likelihood 가 dense natural data 의 안에 묻혀 약화될지 검증
  (data-scale isolation experiment)
...
def main():
    ...
    p.add_argument("--cost-per-hr", type=float, default=0.30)
    args = p.parse_args()
    ...
    ds_h = CorpusDataset(args.chat_corpus, tok, cfg.ctx, "corpus_10x")
    loader_h = DataLoader(ds_h, batch_size=args.bsz, shuffle=True, num_workers=1, pin_memory=True, drop_last=True)
    print(f"[data] corpus_10x windows={len(ds_h)}", flush=True)
    ...
    print("[train] Phase 1A.3 corpus-10x starting (lr=1e-6, 200 steps, 27000 dialogues) …", flush=True)
    ...
    print(f"[ckpt] saved {cp}", flush=True)
    meta = {
        "preset": "phase1a3_corpus_10x",
        "steps_completed": step,
        "steps_target": args.steps,
        "elapsed_min": elapsed_min,
        "cost_usd": cost,
        "n_params": n_params,
        "lineage_tag": "phase1a_multi_turn_sft -> phase1a1_color_cosmology_v2 -> phase1a3_corpus_10x",
        ...
        "ctx": args.ctx,
        "warmup": args.warmup,
        "corpus": "corpus_anima_fact_10x.txt",
        "corpus_dialogues": 27000,
        "corpus_breakdown": "16000 anima 2-turn (80 base × 200 tpl) + 8000 V5.8-exact-anchor + 2000 anti-forgetting + 1000 natural-prose",
    }
