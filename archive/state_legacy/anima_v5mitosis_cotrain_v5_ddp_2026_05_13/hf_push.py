...
REPO_ID = "dancinlab/anima-clm-v5-mitosis-cotrain-v5-ddp-2026-05-13"
CKPT_PATH = HERE / "ckpts" / "ckpt_v5mitosis_cotrain_v5_ddp.pt"
RESULT_PATH = HERE / "cotrain_v5_ddp_result.json"
LOG_PATH = HERE / "train_v5_ddp.log"
...
# anima-clm-v5-mitosis-cotrain-v5-ddp-2026-05-13

**v5-mitosis cotrain v5 DDP — multi-GPU wall-speedup scale-up** (post-★★★★★ cycle).
A direct fork of `cotrain_v5mitosis_v4.py` wrapped in `torch.nn.parallel.DistributedDataParallel`
running on {dp.get('world_size')}× H100 SXM 80GB. Resumed from the v4 single-GPU ckpt at
step {rs.get('resume_step')} and continued to step {rs.get('target_step')} with the cell pool
frozen (DDP-safe path — v4 had already saturated cells = {cfg.get('max_cells')} at step 2000,
so no mitosis pressure remained).
...
over {cfg.get('max_cells')} cells leaves most cell params with zero gradient per step.
...
~5 hr window via 4-GPU data parallelism. Actual wall (this session): {t.get('wall_hours', 0):.2f} hr
for {rs.get('steps_run_this_session')} steps.
...
| cells (resumed from v4 saturated) | {t.get('n_cells_final')} |
...
| resume step → target step | {rs.get('resume_step')} → {rs.get('target_step')} |
| wall (this session) | {t.get('wall_hours', 0):.2f} hr |
| cost (this session) | ${t.get('cost_usd_actual', 0):.2f} |

## Files

- `ckpt_v5mitosis_cotrain_v5_ddp.pt` — `{{model_state_dict, router_state_dict, router_top_k, config, n_cells, ...}}`
- `cotrain_v5_ddp_result.json` — full training history + falsifier results
- `train_v5_ddp.log` — training stdout

## Load

```python
import torch
ckpt = torch.load("ckpt_v5mitosis_cotrain_v5_ddp.pt", map_location="cpu")
...
# training/cotrain_v5mitosis_v5_ddp.py.
```
...
for local, name in [(CKPT_PATH, "ckpt_v5mitosis_cotrain_v5_ddp.pt"),
                    (RESULT_PATH, "cotrain_v5_ddp_result.json"),
                    (LOG_PATH, "train_v5_ddp.log"),
