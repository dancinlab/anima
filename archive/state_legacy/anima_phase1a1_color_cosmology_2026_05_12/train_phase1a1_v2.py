"""
train_phase1a1_v2.py — chat-only continuation SFT from Phase 1A ckpt.

Simplified: no consciousness anchor (the model has already been trained on it),
just fine-tune on the boosted color/cosmology multi-turn corpus.

LR: 2e-6 (very gentle), warmup 30, 500 steps.
"""
import os
import sys
import json
...
def main():
    ...
    print(f"[data] chat windows={len(ds_h)}", flush=True)
    ...
    print("[train] Phase 1A.1 v2 chat-only starting …", flush=True)
    while step < args.steps:
        ...
        if step % 20 == 0 or step == 1:
            ...
            print(
                f"[step {step:>5}/{args.steps}] loss={avg_loss:.4f} lr={lr_now:.2e} mem={mem:.1f}GB elapsed={elapsed:.1f}min cost=${cost:.3f}",
                flush=True,
            )
            accum_loss = 0.0
            accum_count = 0
    ...
    print(f"[ckpt] saved {cp}", flush=True)
    meta = {
        "preset": "phase1a1_color_cosmology_v2_chat_only",
        "steps_completed": step,
        "steps_target": args.steps,
        "elapsed_min": elapsed_min,
        "cost_usd": cost,
        "n_params": n_params,
        "lineage_tag": "phase1a_multi_turn_sft -> phase1a1_color_cosmology_v2",
