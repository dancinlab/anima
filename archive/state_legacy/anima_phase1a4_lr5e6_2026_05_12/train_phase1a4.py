"""
train_phase1a4.py — Phase 1A.4 lr 5e-6 × 200 SFT (Lesson R-1A.2 first path).

Continuation from Phase 1A.1 ckpt (4/5 std_greedy: color/profession/day/cosmology PASS,
anima_fact FAIL — markdown drift). Phase 1A.2 lr=1e-6 retry FAILED — Lesson R-1A.2
prescribes lr ≥ 5e-6 OR steps ≥ 1000 OR loss masking. 본 cycle = lr 5e-6 path.

Strategy:
  - lr 5e-6 (5x Phase 1A.2's 1e-6; PSCC §25b Lesson R-1A.2 prescribed floor)
  - 200 steps (same as Phase 1A.2)
  - chat-only continuation (no consciousness anchor — Lesson R-1A.1)
  - anima self-statement augment corpus (same 2700-dialogue corpus as Phase 1A.2)
...
def main():
    ...
    print(f"[train] Phase 1A.4 lr5e6 starting (lr={args.lr}, {args.steps} steps) …", flush=True)
    ...
    print(f"[ckpt] saved {cp}", flush=True)
    meta = {
        "preset": "phase1a4_lr5e6",
        "steps_completed": step,
        "steps_target": args.steps,
        "elapsed_min": elapsed_min,
        "cost_usd": cost,
        "n_params": n_params,
        "lineage_tag": "phase1a_multi_turn_sft -> phase1a1_color_cosmology_v2 -> phase1a4_lr5e6",
        ...
        "ctx": args.ctx,
        "warmup": args.warmup,
        "corpus": os.path.basename(args.chat_corpus),
    }
