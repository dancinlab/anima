...
STATE_DIR='/Users/ghost/core/anima/state/anima_jk_llama_3b_lora_2026_05_07'
RESULTS_DIR='/Users/ghost/core/anima/state/anima_jk_llama_3b_lora_2026_05_07/results'
CORPUS_DIR='/Users/ghost/core/anima/state/anima_jk_llama_3b_lora_2026_05_07/corpus'
...
TRAIN_PY='/Users/ghost/core/anima/tool/transient_py/anima_jk_llama_lora_sft.py'
POD_INFO='/Users/ghost/core/anima/state/anima_jk_llama_3b_lora_2026_05_07/pod_info.json'
VERDICT='/Users/ghost/core/anima/state/anima_jk_llama_3b_lora_2026_05_07/verdict.json'
RUN_LOG='/Users/ghost/core/anima/state/anima_jk_llama_3b_lora_2026_05_07/run.log'
HEARTBEAT='/Users/ghost/core/anima/state/anima_jk_llama_3b_lora_2026_05_07/heartbeat.txt'
...
POD_NAME='anima-jk-llama-3b-lora-2026-05-07'
...
if [ $BOOT_RC -ne 0 ]; then
    ...
    jq -n --arg ts "$(date -u +%FT%TZ)" --arg reason "pod_boot_failed" '{cycle:"anima_jk_llama_3b_lora_2026_05_07",ts_utc:$ts,verdict:"FAIL",reason:$reason}' > "$VERDICT"
    exit 3
fi
...
_kill_pod() {
    ...
    if [ -n "${SSH_HOST:-}" ] && [ -n "${SSH_PORT:-}" ]; then
        timeout 60 scp -i "$SSH_KEY" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -P "$SSH_PORT" -r \
            "root@$SSH_HOST:/workspace/anima_jk_llama/results/*" "$RESULTS_DIR/" 2>&1 | tail -5 || true
        timeout 30 scp -i "$SSH_KEY" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -P "$SSH_PORT" \
            "root@$SSH_HOST:/workspace/anima_jk_llama/orchestrator.log" "$STATE_DIR/h100_orchestrator.log" 2>&1 | tail -1 || true
    fi
...
$SSH 'mkdir -p /workspace/anima_jk_llama/{corpus,results,ckpts,sentinels}'
$SCP "$CORPUS_LOCAL" "root@$SSH_HOST:/workspace/anima_jk_llama/corpus/corpus_persona_chat_template.txt" 2>&1 | tail -1
$SCP "$TRAIN_PY" "root@$SSH_HOST:/workspace/anima_jk_llama/anima_jk_llama_train.py" 2>&1 | tail -1
$SCP "/Users/ghost/core/anima/state/anima_jk_llama_3b_lora_2026_05_07/run_h100.bash" "root@$SSH_HOST:/workspace/anima_jk_llama/run_h100.bash" 2>&1 | tail -1
$SSH 'chmod +x /workspace/anima_jk_llama/run_h100.bash'

log "launching H100 pipeline (corpus + train + intermediate eval + final eval)"
$SSH 'cat /proc/1/environ | tr "\0" "\n" | grep ^HF_TOKEN= > /workspace/anima_jk_llama/hf_token.env && cd /workspace/anima_jk_llama && set -a && . hf_token.env && set +a && setsid nohup bash run_h100.bash < /dev/null > orchestrator.log 2>&1 & echo $! > run.pid; disown $! 2>/dev/null || true; sleep 2; cat run.pid' < /dev/null
...
while true; do
    ...
    PROBE=$($SSH 'ls /workspace/anima_jk_llama/results/COMPLETE.sentinel 2>/dev/null && echo SENTINEL_FOUND; ls /workspace/anima_jk_llama/results/EARLY_STOP.sentinel 2>/dev/null && echo EARLY_STOP_FOUND; ps -p $(cat /workspace/anima_jk_llama/run.pid 2>/dev/null) -o pid,etime,comm 2>/dev/null | tail -1; tail -1 /workspace/anima_jk_llama/orchestrator.log 2>/dev/null' 2>/dev/null)
    ...
    $SCP -r "root@$SSH_HOST:/workspace/anima_jk_llama/results/*.json" "$RESULTS_DIR/" 2>/dev/null || true
    $SCP "root@$SSH_HOST:/workspace/anima_jk_llama/orchestrator.log" "$STATE_DIR/h100_orchestrator.log" 2>/dev/null || true

    if echo "$PROBE" | grep -q SENTINEL_FOUND; then
        log "COMPLETE.sentinel detected — final sync FIRST then auto-kill"
        COMPLETE=1
        $SCP -r "root@$SSH_HOST:/workspace/anima_jk_llama/results/*" "$RESULTS_DIR/" 2>&1 | tail -5 | tee -a "$RUN_LOG" || true
        $SCP -r "root@$SSH_HOST:/workspace/anima_jk_llama/ckpts/final" "$RESULTS_DIR/adapter_final" 2>&1 | tail -3 || true
        break
    fi
    if echo "$PROBE" | grep -q EARLY_STOP_FOUND; then
        ...
        $SCP -r "root@$SSH_HOST:/workspace/anima_jk_llama/ckpts" "$RESULTS_DIR/ckpts_partial" 2>&1 | tail -3 || true
        break
    fi
...
# ── Stage 6: verdict (BG-JK polyglot LoRA SFT — simplified vs SFT parent) ──
log "computing BG-JK verdict"
...
# F-JK-1: train completed without crash + final adapter present
FJK1="FAIL"
if [ "$SENTINEL_OK" = "true" ] && [ -d "$RESULTS_DIR/adapter_final" ]; then
    FJK1="PASS"
fi

# F-JK-2: post-LoRA Korean coherence smoke (samples in samples_post_lora.json)
FJK2="PENDING_MANUAL"
...
" 2>/dev/null || echo "NO")
    if [ "$HAN_OK" = "YES" ]; then
        FJK2="PASS_CANDIDATE_MANUAL_REVIEW_NEEDED"
    else
        FJK2="LIKELY_FAIL_NO_KO_30PCT"
    fi
fi

# F-JK-3: cost discipline
FJK3="PASS"
if (( $(awk "BEGIN{print ($FINAL_COST > $BUDGET_HARD_CAP) ? 1 : 0}") )); then
    FJK3="FAIL_OVER_BUDGET"
fi

OVERALL="PARTIAL"
if [ "$FJK1" = "PASS" ] && [ "$FJK2" = "PASS_CANDIDATE_MANUAL_REVIEW_NEEDED" ]; then
    OVERALL="LORA_SFT_COMPLETED_KO_SAMPLES_PRESENT_AWAITING_V4_EVAL"
elif [ "$FJK1" = "PASS" ] && [ "$FJK2" = "LIKELY_FAIL_NO_KO_30PCT" ]; then
...
jq -n \
    --arg ts "$(date -u +%FT%TZ)" \
    --arg cycle "anima_jk_llama_3b_lora_2026_05_07" \
    --arg overall "$OVERALL" \
    --arg fja1 "$FJK1" --arg fja2 "$FJK2" --arg fja3 "$FJK3" \
    --arg wall "$FINAL_ELAPSED_MIN" --arg cost "$FINAL_COST" \
    --argjson complete "$COMPLETE" --argjson early_stop "$EARLY_STOP" \
    '{
        schema: "anima/bg_jk/verdict/1",
        bg_id: "BG-JK",
        cycle: $cycle,
        ts_utc: $ts,
        verdict: $overall,
        ...
        falsifiers: {
            "F-JK-1_train_completed": $fja1,
            "F-JK-2_ko_coherence_smoke": $fja2,
            "F-JK-3_cost_discipline": $fja3
        },
        runtime: {
...
log "BG-JK verdict=$OVERALL F-JK-1=$FJK1 F-JK-2=$FJK2 F-JK-3=$FJK3 wall=${FINAL_ELAPSED_MIN}min cost=\$${FINAL_COST}"
hb "verdict=$OVERALL"
