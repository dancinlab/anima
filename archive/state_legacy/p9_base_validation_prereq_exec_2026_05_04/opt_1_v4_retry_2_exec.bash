#!/bin/bash
# v4 RETRY-2 (BG-γ'' 2026-05-04) — HF token rotated, sentencepiece fix carried over.
# Derived from opt_1_v4_retry_exec.bash. Adds pre-flight whoami-v2 (lesson L9).
...
RESULTS_DIR='/Users/ghost/core/anima/state/p9_base_validation_prereq_exec_2026_05_04/opt_1_v4_retry_2_results'
POD_INFO='/Users/ghost/core/anima/state/p9_base_validation_prereq_exec_2026_05_04/opt_1_v4_retry_2_pod_info.json'
VERDICT='/Users/ghost/core/anima/state/p9_base_validation_prereq_exec_2026_05_04/opt_1_v4_retry_2_verdict.json'
RUN_LOG='/Users/ghost/core/anima/state/p9_base_validation_prereq_exec_2026_05_04/opt_1_v4_retry_2_run.log'
HEARTBEAT='/Users/ghost/core/anima/state/p9_base_validation_prereq_exec_2026_05_04/opt_1_v4_retry_2_heartbeat.txt'
BOOT_LOG='/Users/ghost/core/anima/state/p9_base_validation_prereq_exec_2026_05_04/opt_1_v4_retry_2_boot.log'
...
TRAIN_AVG_DST='/Users/ghost/core/anima/state/p9_base_validation_prereq_exec_2026_05_04/opt_1_v4_retry_2_train_avg_fixture.json'
...
RUN_H100_PATH='/Users/ghost/core/anima/state/p9_base_validation_prereq_exec_2026_05_04/opt_1_v4_retry_2_run_h100.bash'
...
# ── Stage 0b: pre-flight HF whoami-v2 (Lesson L9 — fail at $0 cost) ──
log "stage0b: pre-flight HF whoami-v2 (fail-fast on token validity)"
WHOAMI_RAW=$(curl -sH "Authorization: Bearer $HF_TOKEN_LOCAL" https://huggingface.co/api/whoami-v2 2>&1)
WHOAMI_NAME=$(echo "$WHOAMI_RAW" | jq -r '.name // empty' 2>/dev/null)
if [ -z "$WHOAMI_NAME" ]; then
    log "FATAL: HF whoami-v2 pre-flight failed — token rejected"
    log "raw response (first 200 chars, redacted): $(echo "$WHOAMI_RAW" | redact_hf | head -c 200)"
    jq -n --arg ts "$(date -u +%FT%TZ)" '{cycle:"opt_1_v4_retry_2_2026_05_04",ts_utc:$ts,verdict:"FAIL_PRE_FLIGHT",reason:"HF whoami-v2 rejected token at $0 cost",pre_flight_hf_auth:"FAIL"}' > "$VERDICT"
    exit 2
fi
log "stage0b: HF auth OK — whoami=$WHOAMI_NAME"
hb "stage0b_hf_auth_ok user=$WHOAMI_NAME"

# ── Stage 1: boot H100 pod ──
POD_NAME='anima-p9-opt-1-v4-retry-2-2026-05-04'
log "booting pod=$POD_NAME (H100 80GB HBM3 secure on-demand @ \$$OD_RATE/hr)"
BOOT_RAW_OUT="$STATE_DIR/opt_1_v4_retry_2_boot.raw.tmp"
...
if [ $BOOT_RC -ne 0 ]; then
    ...
    jq -n --arg ts "$(date -u +%FT%TZ)" --arg reason "pod_boot_failed" '{cycle:"opt_1_v4_retry_2_2026_05_04",ts_utc:$ts,verdict:"FAIL",reason:$reason}' > "$VERDICT"
    exit 3
fi
...
while true; do
    ...
    $SCP "root@$SSH_HOST:/workspace/p9_v4_exec/orchestrator.log" "$STATE_DIR/opt_1_v4_retry_2_h100_orchestrator.log" 2>/dev/null || true
    ...
    if echo "$PROBE" | grep -q SENTINEL_FOUND; then
        ...
        $SCP "root@$SSH_HOST:/workspace/p9_v4_exec/orchestrator.log" "$STATE_DIR/opt_1_v4_retry_2_h100_orchestrator.log" 2>&1 | tail -1 || true
        break
    fi
...
jq -n \
    --arg cycle "opt_1_v4_retry_2_2026_05_04" \
    --arg ts "$(date -u +%FT%TZ)" \
    --arg pod "$POD_ID" \
    ...
    --arg whoami "$WHOAMI_NAME" \
    '{
        cycle: "opt_1_v4_retry_2_2026_05_04",
        bg_lane: "BG-γ'\'' RETRY-2",
        ts_utc: $ts, pod_id: $pod,
        pod_terminated: true,
        pod_kill_verified_404: $pod404,
        pre_flight_hf_auth: "PASS",
        pre_flight_whoami_user: $whoami,
        scope: "RETRY-2: HF token rotated; sentencepiece+tiktoken fix carried over; F-SHIM-V4-3 canonical_zero finite forward + train_avg harvest + limit=100 hellaswag sanity eval",
        ...
        train_avg_fixture_path: "state/p9_base_validation_prereq_exec_2026_05_04/opt_1_v4_retry_2_train_avg_fixture.json",
        ...
        sentencepiece_fix_status: "VERIFIED_WORKING (BG-γ'\'' carry-over — pre-install + import assertion both PASS)",
        honest_c3: [
