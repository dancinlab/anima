"""Generate 5 per-category corpus FILES (separate files, ~1 MB each = ~5 MB total).

PURPOSE
    cotrain v3 per-category gradient bias innovation: each category gets its
    OWN corpus file. The trainer interleaves them as cat[step % 5] so each
    training step targets a single category, forcing per-cell specialization.

    Derived from cotrain v1's generate_balanced_corpus.py (which used round-robin
    interleaving within ONE big file). v3 SEPARATES them so the gradient sees
    pure-category bursts.

OUTPUT
    corpus_self_definition.txt
    corpus_values.txt
    corpus_boundary.txt
    corpus_emotion.txt
    corpus_self_knowledge.txt
"""
...
OUT_DIR = HERE.parent / "corpus"
OUT_DIR.mkdir(parents=True, exist_ok=True)

# Reuse cotrain v1's CAT_BLOCKS via copy (avoid cross-state import dependency)
CAT_BLOCKS = {
    "self_definition": [
...
def build_block(cat: str, q: str, a: str, with_followup: bool = True) -> str:
    lines = [f"사용자: {q}", f"도우미: {a}"]
    if with_followup:
        fu_pool = FOLLOW_UPS.get(cat, [])
        if fu_pool:
...
def write_category_file(cat: str, target_bytes: int, seed: int) -> dict:
    random.seed(seed)
    out_parts: list[str] = []
    total = 0
    iters = 0
    while total < target_bytes and iters < 500_000:
        iters += 1
        qa = random.choice(CAT_BLOCKS[cat])
        with_fu = random.random() < 0.7
        block = build_block(cat, qa[0], qa[1], with_followup=with_fu)
        out_parts.append(block)
        total += len(block.encode("utf-8"))
    text = "".join(out_parts)
    out_path = OUT_DIR / f"corpus_{cat}.txt"
...
def main():
    seed_base = int(sys.argv[1]) if len(sys.argv) > 1 else 42
    target_each = int(sys.argv[2]) if len(sys.argv) > 2 else 1_000_000  # 1 MB each
    info = []
    for i, cat in enumerate(CAT_BLOCKS.keys()):
        rec = write_category_file(cat, target_each, seed_base + i)
        info.append(rec)
        print(f"[INFO] {cat}: bytes={rec['bytes']:,} blocks={rec['blocks']}")
    print(f"[INFO] total bytes = {sum(r['bytes'] for r in info):,}")
