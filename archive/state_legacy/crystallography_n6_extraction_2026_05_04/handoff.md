...
## Status: LANDED (CANON pushed; nexus delete staged, NOT committed)

## What was done

1. **Pre-extraction grep** (nexus/anima/hive/canon, .hexa/.py/.md/.json/.toml, excluding worktrees + __pycache__): **0 functional consumers**. Only audit-doc references.
2. **Module audit**:
...
3. **Copied** to `/Users/ghost/core/canon/domains/physics/crystallography/`:
...
6. **CANON additive**: new `reports/changelogs/2026-05-04-crystallography-n6-extraction.md` (no README mutation — main README is auto-managed and 175KB).
7. **Git**:
   - CANON commit `38d66066` pushed to `main` (PUBLIC GitHub).
...
in CANON. Used `domains/physics/crystallography/` (already houses the canonical
...
git commit -m "chore(extract): remove crystallography_n6 — absorbed by CANON@38d66066"
```
...
CANON working tree shows untracked `domains/compute/chip-isa-n6/chip_isa_n6.{hexa,README.md}`
