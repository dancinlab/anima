"""stride_hw.py -- AKD1000 conv stride/VALID-padding frontier probe (round-5).

Round-4 conv only did stride 1 SAME. This parameterizes the InputConv->Conv
cascade with --stride and --padding {same,valid} and emits the FULL end-to-end
output tensor + sha per probe input so stride_sw.py can be byte-diffed against
silicon. Same two-layer-type structure (InputConv SW xcorr front-end -> on-chip
Conv HW true-conv) recovered in round 4.

Usage: python stride_hw.py --wseed 7 --act-bits 4 --f1 8 --f2 8 --k 3 --hw 8 \
...
def main():
    ...
    a = ap.parse_args()
    H = W = a.hwsz; C = 1
    pad = akida.Padding.Same if a.padding == "same" else akida.Padding.Valid
    dev = akida.devices()[0]
    m = akida.Model()
    ...
    for idx, x in enumerate(probe_imgs(H, W, C)):
        ...
        rec = {"side": "HW", "axis": "stride", "input_idx": idx, "input_sha": sh(x),
