...
pre-training (large general corpus, knowledge) + SFT (chat-format only, behavior alignment) two-stage sequential 학습이, mixed corpus pre-training only (BG-HA 패러다임)보다 simple-stack C2.4 strict PASS rate가 높다. pre-train 단계는 일반 지식 + 한국어 fluency, SFT 단계는 prompt-response mapping 학습 — 단계 분리가 학습 효율과 신호를 명확히 한다.
...
- **H94.3 (stage2 chat alignment)**: stage2 후 C2.2-C2.4 strict 모두 monotonic 상승 (stage2 epoch 0/1/2/3에서 PASS rate trajectory)
...
- 본 cycle 18M scope; 100M+은 별도 cycle (watchdog)
...
- **C5 (정합)**: stage1 corpus_hangul_ratio ≥60% AND stage2 chat_format_ratio ≥80%
...
- **L6**: 100M+ scale은 H100 cost discipline scope OUT
