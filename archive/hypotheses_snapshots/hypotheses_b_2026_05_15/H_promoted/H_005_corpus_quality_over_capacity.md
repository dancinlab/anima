...
chat-cap (simple stack PASS) 도달은 model capacity (param size) 보다 corpus quality (Hangul ratio + chat-template format 비율 + 의미 풍부) 우선이다. capacity 7.7× 차이 무관 corpus가 unlock하는 evidence 본 cycle 발견.
...
- **H5.2 (corpus-controlled)**: corpus KO-heavy + chat-template ≥30% N param model이 corpus KO-heavy + chat-template <10% N param model보다 simple stack C2.4 PASS rate 높다 (chat-template ratio가 C2.4 결정)
...
- cost discipline 적용 — 100M+ cycle은 watchdog mandate
...
- **C5 (정합)**: simple stack PASS 모델 모두 (corpus_hangul_ratio ≥60% AND chat_template_ratio ≥30%) 정합 — corpus QA gate spec 정합
...
- **F2**: chat-template ratio 0% N param model이 C2.4 PASS → H5.2 FALSIFIED
...
- **F5**: simple stack PASS 모델 중 corpus_hangul_ratio <60% 또는 chat_template_ratio <30% — H5.5 (spec) FALSIFIED
...
- **L5**: 100M+ capacity scaling 별도 cycle (cost discipline 적용 — H100 budget watchdog mandate)
- **L6**: H5.5 (corpus QA gate spec)는 본 H의 corollary — 정합 lane

## Cross-Links

- **sister roadmaps**: `.roadmap.law` R1 + `.roadmap.philosophy` D4 corpus priority + `.roadmap.clm_native_chat`
- **own**: (anima-native identity) + (simple stack 4-condition) + (corpus priority) + (chat-template format)
- **evidence_paths**:
...
evidence_summary:
  ...
  - (corpus priority) PASS (BG-FK FAIL evidence)
  - (chat-template format mandate) PARTIAL — named-speaker-leak prevention만 verified, prompt domain match는 NOT
...
criteria_met:
  ...
  - C5 (정합): PASS — BG-HA both ≥60% Hangul + chat-template ≥30% gate confirmed
next_cycle:
  - HF private upload BG-HA model + verification gate
  - HF public promote 후 첫 SIMPLE_STACK_PASS HF release
