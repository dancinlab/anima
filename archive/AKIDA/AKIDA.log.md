...
## 2026-06-03T20:30Z — Lane-A ON-CHIP RECURRENCE 7-후보 알고리즘 탐사 🔴 — 5 CLOSED-NEGATIVE + 2 INFEASIBLE-ON-CHIP, 1-hop wall HOLDS (substrate=AKIDA · spike-streamer 정지→칩 독점→7 순차→복원)

multi-step wall 이 ALGORITHM-bound (1-bit Hebbian = no recurrence; scale/width/code/paged-depth 전부 closed-negative; off-chip HYBRID head 가 proven fix) 라는 frontier 위에서, **ON-CHIP recurrence/composition 메커니즘이 1-hop wall 을 깰 수 있나** 를 7개 사전등록 falsifier 로 탐사. 1 칩 SEQUENTIAL (절대 동시 실행 안 함), spike-streamer (`--port 9512 --duration 86400 --regime R3`) systemctl --user stop → `akida.devices()`=1 (BC.00.000.002 IpVersion.v1) 확인 → 7개 ONE-AT-A-TIME → systemctl --user start 복원 (active+enabled+exact argv, throttled=0xe0000 sticky-occurred 변화 없음, temp 67→76°C 전구간 82°C guard 아래). akida SDK 2.19.1 그대로 (anima-akida venv core 미변경, 별도 venv 불필요 — 필요 SDK 클래스 전부 이미 설치됨). 사전등록 falsifier (모두 run 전, script docstring): hop-2 위 shuffle-NULL p≤0.01 AND mean material >0.038 (>0.01 over pure-on-chip wall 0.028).

- [x] **A1 reservoir/echo-state** 🔴 CLOSED-NEGATIVE — 고정(미학습) reservoir FC(echo recurrence state=R(bind(code,prev_state))) + 학습 readout FC. decay [0.1904,0.0282,0.0138]; hop-1 p=0.0050 통과하나 **hop-2 acc=0.0282 ci_lo=0.0212 shufNULL_hi=0.0357 p=0.1393 (NULL 내부)**, hop-3 p=0.7463. F-A1-1 NOT-REFUTED. readout_learned_all=True(live silicon). verbatim `.verdicts/lane-a-microexp-reservoir/F-RESERVOIR.txt`.
- [x] **A2 state-accumulator feedback** 🔴 CLOSED-NEGATIVE — 진짜 on-chip running SPIKE-COUNT trace(trace=trace*0.6+spike_count→ctx, x=bind(g_bin,ctx)) re-fed; vs no-trace stateless. decay TRACE [0.3920,0.0282,0.0122] vs NOTRACE [0.3920,0.0319,0.0133]; **hop-2 p=0.0697(NULL), hop-3 p=0.8706**; trace delta NEGATIVE(-0.0037/-0.0011) = trace 가 오히려 약간 害. F-A2-1/F-A2-2 NOT-REFUTED. 8/8 learn=True. verbatim `.verdicts/lane-a-microexp-state-accum/F-STATE-ACCUM.txt`.
- [x] **A3 native recurrent-layer** ⚪ INFEASIBLE-ON-CHIP (HONEST hardware-cannot, NOT a wall verdict) — `akida.StatefulRecurrent` construct+add OK 이나 **map() RuntimeError 'The IP version of the model and device must be identical'**. SDK 클래스 akida 2.19.1 에 존재(≠ sdk-not-installed) 하나 AKD1000=IpVersion.v1 이 layer(AKD1500/v2 타깃)를 mesh 에 못 올림 = HARDWARE IP-version mismatch, pip 으로 못 고침. NO SIM(g63). 네이티브 on-chip recurrence 는 AKD1500-class 실리콘 필요. verbatim `.verdicts/lane-a-microexp-native-recurrent/F-NATIVE-RECURRENT.txt`.
- [x] **A4 STDP / temporal-order spike code** ⚪ INFEASIBLE-ON-CHIP (HONEST hardware-cannot) — akida 2.19.1 에 STDP 클래스 NONE; 유일 temporal primitive `akida.BufferTempConv`(FIFO spatiotemporal conv) construct+add OK 이나 **동일 map() RuntimeError 'IP version must be identical'** (BufferTempConv = AKD1500/v2 TENNs 타깃). AKD1000 IP-v1 mesh 에 temporal-FIFO/spike-timing op 없음. NO SIM. temporal-order spike coding 은 AKD1500-class 필요. verbatim `.verdicts/lane-a-microexp-temporal-code/F-TEMPORAL-CODE.txt`.
- [x] **A5 VSA hypervector binding** 🔴 CLOSED-NEGATIVE — 1-bit VSA 대수(x=bind(permute(g_bin,hop), bundle_history); permute=roll, bundle=OR-superpose, bind=XOR-roll) + on-chip FC cleanup; vs no-VSA stateless. decay VSA [0.4319,0.0218,0.0101] vs NOVSA [0.4319,0.0266,0.0144]; **hop-2 p=0.2886(NULL), hop-3 p=0.8060**; VSA delta NEGATIVE(-0.0048/-0.0043). F-A5-1/F-A5-2 NOT-REFUTED. 8/8 learn=True. binding 대수가 compositional multi-hop 을 안 줌. verbatim `.verdicts/lane-a-microexp-vsa-binding/F-VSA-BINDING.txt`.
- [x] **A6 attractor/Hopfield** 🔴 CLOSED-NEGATIVE — on-chip recurrent settling(y=binarize(chip(bind(y,anchor))) S=4, mean_settle=3.91); vs no-settle S=1. decay ATTRACTOR [0.0298,0.0160,0.0218] — **settling 이 단일-step 신호까지 파괴**(hop-1 delta -0.3798). F-A6-1/F-A6-2 NOT-REFUTED. ⚠ HONEST 부수관찰(등록 메커니즘 아님): S=1 no-settle 대조군 hop-2=0.1059 (~3.2x NULL) — 원인은 no-settle 입력 bind(y,anchor) 가 FIXED seed anchor 를 매 hop 영속 bind(강한 input-side persistent-context). 전용 사전등록 follow-up(persistent-anchor 입력) flag, 여기 verdict 아님. verbatim `.verdicts/lane-a-microexp-attractor/F-ATTRACTOR.txt`.
- [x] **A7 forward-forward** 🔴 CLOSED-NEGATIVE — goodness-based layerwise FF(FF1+FF2 각자 AkidaUnsupervised 1-bit FC, layerwise no-BP); #1689 paged-BP 와 SAME 2-FC depth, 다른(local-goodness) 학습규칙. decay DEPTH(FF1→FF2) [0.1665,0.0654,0.0426] vs FF1-ONLY [0.4085,0.1761,0.0926]; FF-depth hop-2=0.0654 는 NULL 통과(p=0.0050) 이나 **hop-3 p=0.0100 미통과 → F-A7-1(hop-2 AND hop-3) NOT met**; FF-depth delta vs FF1-only 매 hop NEGATIVE(-0.2420/-0.1106/-0.0500) = 2번째 1-bit FC 가 신호 파괴, #1689 paged-BP depth-2(hop-2 0.0298) 와 동일. **DEPTH ceiling 은 학습규칙-독립**(BP-paged AND FF-local 둘 다 closed). ff1+ff2 8/8 learn=True. verbatim `.verdicts/lane-a-microexp-forward-forward/F-FORWARD-FORWARD.txt`.
- [x] **종합 — 7/7 어느 메커니즘도 wall 안 깸. 1-hop wall HOLDS (HARDER).** 등록 falsifier 기준 GREEN-WALL-BROKEN = 0건. on-chip recurrence 후보 5개(A1·A2·A5·A6·A7) 전부 closed-negative; A3·A4 는 honest infeasible-on-chip(AKD1000 IP-v1 = StatefulRecurrent/BufferTempConv 모두 map 불가, AKD1500/v2 필요). 이것은 직전 frontier(off-chip HYBRID head = proven fix)를 **강화**: AKD1000 1-bit/256-unit Hebbian 의 1-hop ceiling 은 입력(state/trace/VSA/anchor)도, 깊이(paged-BP·FF-local 둘 다)도, 고정 recurrence(echo-state·attractor)도 못 들어올림. 추가 사전등록 follow-up 2건 명명: (i) **persistent-anchor 입력** (A6/A7 대조군 hop-2 ~0.1-0.18 above-NULL 의 원인을 단독 메커니즘으로 등록), (ii) **AKD1500-class 하드웨어** rung (A3·A4 가 거기선 feasible). 둘 다 toy 250앵커/256-unit (a_scale_honest_scope). substrate=AKIDA, Lane-G/GPU 수치와 NEVER 병합 (a_lane_akida_gpu_split). 전원 — spike-streamer 정상 정지→복원(active+enabled), 전구간 throttled active-bit 없음, temp ≤76°C.
...
- [x] **문서 SSOT + 감사 H 2건** (PR-F) — `AKIDA/HW_FIRST_INTEGRATION_2026_05_30.md` (전체 구조 + 2-lane 표 + provenance + 크로스포인터) · `HYPOTHESES/cards/H_679_plasticity_hw_first.md` (🔴 CLOSED-NEGATIVE 4/4) · `HYPOTHESES/cards/H_680_decoder_hw_first.md` (🟢 SUPPORTED-NUMERICAL verify 5/5).
...
- [ ] 환류 — 측정 결과는 HYPOTHESES/CANDIDATES.md 에 기록 (bench SSOT)
...
## 2026-06-02 — Lane A HYBRID BRANCHING-CORPUS 홀드아웃 🟢 GENERALIZES (substrate=HYBRID on-chip⊕off-chip) — PUBLIC RE-UPGRADE

PR#1694 CHAIN-FITTING 의 ROOT CAUSE(결정론 i→i+1 단일체인 = per-concept lookup BY CONSTRUCTION, TEST-block Wo row gradient 0 → 구조적 held-out 0.0000)를 a_completeness_over_cheap 로 재설계. 코퍼스를 concept-identity-independent 분기 연산자 succ(i)={(i+d) mod NC : d∈{1,7,19}} (branching B=3, ring wrap 으로 TEST→TRAIN successor 가능 = held-out 구조적 0 해소)로 교체. off-chip Elman head(D_H=64, numpy BPTT, byte-match PR#1692/#1694)를 랜덤 분기 walk(매 step target = B-set 중 RANDOM valid successor, KEEP-only-if-TRAIN-target)로 학습 → 단일 결정론 target 없음 = lookup 불가능, transition OPERATOR 강제 학습. on-chip 1-bit FC encoder 는 full 분기 전이 비지도 fit. live AKD1000 (BC.00.000.002, akida 2.19.1, throttled=0x0, streamer stop→fire→restore rc=0) 전 trial encoder_learned=True (g63 no sw fallback). 분기-aware metric = set-membership(pred ∈ succ(현재 concept), 다중 successor 정답).

HEADLINE NC=50 (chance 0.0612, B=200 shuffle-NULL): **decay TRAIN(in-dist) [0.6929, 0.9357, 0.9721] / decay HELD-OUT [0.0183, 0.8967, 0.9600]** (PR#1694 결정론 holdout [0,0,0] 대비). **F-BRANCH-1 REFUTED** (held-out hop-2 0.8967 ci_lo=0.879 ≫ NULL hi=0.147 p=0.005, hop-3 0.9600 ci_lo=0.937 ≫ NULL hi=0.173 p=0.005 = transferable OPERATOR 강제) · **F-BRANCH-2 REFUTED** (held-out hop-2 0.8967 이 in-dist 0.9357 의 2× 이내, ratio 0.958). ≥3-rung codebook ladder (a_scale_honest_scope) 일관: NC=30 held-out [0.114, 0.922, 0.944] · NC=40 [0.05, 0.906, 0.946] · NC=50 [0.018, 0.897, 0.960] (held/in-dist ratio hop-2/3 전 rung ~0.95-0.99). RULING: 분기 코퍼스는 transferable transition OPERATOR 를 강제 — off-chip head 가 학습 중 한 번도 target 으로 안 본 TEST concept 의 hop-2/3 successor 를 valid set 안에 디코드 = GENUINE multi-step composition. PR#1694 exact-0.0000 은 결정론 단일체인 ARTEFACT, ROOT CAUSE 에서 REPAIRED. 🌱 EMERGENCE axis 🟢 RE-LIFTED. 정직 CAVEAT: held-out hop-1=0.0183 (NULL 아래) — 첫 emit 은 TEST start succ 가 TEST-block 출력 row gradient-coverage gated; falsifier 는 multi-step(hop-2/3)에 사전등록되어 둘 다 decisively pass, hop-1 은 falsifier 아님. Lane A HYBRID PUBLIC RE-UPGRADE [~]→[x] (hybrid-scoped/branching-validated). 인코더+single-step+순수 on-chip closed-negatives UNAFFECTED. a_lane_akida_gpu_split · a_scale_honest_scope toy 250앵커 3-rung ladder · a_paper_negative_ok. 순수-AKIDA 아님, Lane G 아님. next=3B. result_onchip_xlm_branching.json (sha256 5a585326…) · AKIDA/state/branching_run_verbatim.log · `.verdicts/lane-a-branch/F-BRANCH.txt` (hexa verify CLI broken → live-chip stdout verbatim).

## 2026-06-02 · Lane A 3B — chip-fit/페이징 capacity ladder (F-3B) — substrate=HYBRID(on-chip AKD1000 encoder ⊕ off-chip host-CPU decode head)
Lane A 3B 마일스톤(AKIDA 3B chip-fit/페이징 ladder ≥3 rung, a_scale_honest_scope) 발사. 분기-검증 multi-step composition(PR#1697 F-BRANCH GENERALIZES 🟢; 256-unit single FC + D_H=64 off-chip head, held-out hop-2/3 0.90/0.96)을 byte-match 유지한 채, on-chip 인코더 capacity 를 layerpage single-residency primitive(8MB SRAM 메시에 1 FC 만 상주; depth-D paged FC, 각 FC fit→host page OFF→del→다음 FC map)로 3B-class 향해 scale. live AKD1000 (BC.00.000.002, akida 2.19.1, throttled=0x0, streamer stop→fire→restore rc=0, N=8 칩 trial/rung, g63 no sw fallback). per_fc_params=U×INC(256)×NW(8), paged_params=D×per_fc.

**4-rung ladder (전 rung map_all=learn_all=True on live silicon)**: D=1 U=256 524K chip_fit=True comp_survives=**True** held hop-2/3 [0.835,0.938] ci_lo 0.783/0.912 ≫ NULL hi 0.208/0.216 p=0.005 (분기 baseline 재현) · D=2 U=512 2.1M chip_fit=True comp_survives=**False** (held hop-2=0.0 NULL hi=0.364 p=1.0) · D=3 U=1024 6.3M comp_survives=False (hop-2/3 ci_lo<NULL hi p=0.06/0.06) · D=4 U=2048 16.8M (frontier probe) comp_survives=False.

**F-3B-1 = False** (composition D=1 baseline 에서만 above-NULL, D≥2 붕괴) · **F-3B-2 = False** (3B-class 미도달 max paged 1.678e7 ≪ 3e9 AND map/learn SRAM ceiling 도 안 침). **VERDICT = COMPOSITION DEGRADES UNDER CAPACITY SCALING (honest closed-negative, a_paper_negative_ok) — 3B 마일스톤 OPEN 유지.**

**KEY HARDWARE FINDING**: AKD1000 8MB SRAM 가 binding limit 아님 — layerpage 가 U=2048/4 paged layer/16.8M trainable paged params 까지 전 rung map+learn 성공(overflow 無, saturation 無). binding constraint = 더 깊은/넓은 1-bit Hebbian AkidaUnsupervised FC stacking 이 off-chip head 가 의존하는 단일-step transition signal 파괴 (각 추가 1-bit FC 가 fold-to-INC+frozen-median 재이진화 → depth≥2 head BPTT CE 수렴 안 함 ~3.5 vs depth-1 ~0.30). PR#1690 MULTI-FC DEPTH 와 동일 physics. paged 1-bit Hebbian depth = composition 보존에 대해 CLOSED axis. NAMED next bridge = OFF-CHIP head scale (proven D=1 인코더 위) OR 1-bit Hebbian depth 없는 richer 인코더. 순수-AKIDA 아님, Lane G 아님 (a_lane_akida_gpu_split). result sha256 5a1bc3e7… · AKIDA/state_3b_chipfit_verbatim.log · `.verdicts/lane-a-3b/F-3B.txt`.

## 2026-06-02 · Lane A 3B — OFF-CHIP HEAD SCALE-UP (F-3B-HYBRID) — substrate=HYBRID(on-chip AKD1000 D=1 인코더 ⊕ off-chip host-CPU multi-layer head)
PR#1705/F-3B 가 명명한 NAMED BRIDGE 실행: 칩은 proven D=1 256-unit FC 인코더(524K, byte-match PR#1697 고정), OFF-CHIP host decode head(multi-layer Elman, numpy BPTT)를 3B-class 향해 scale, 분기 held-out split(byte-match PR#1697) 재측정. live AKD1000 BC.00.000.002, N=8 칩 trial/rung, 인코더 enc_learned=True 전 trial (g63 no sw fallback). ladder: **NL=1 D_H=64 total 548K chip_frac=0.957 comp_survives=True (held hop-2/3 [0.893,0.938] p=0.005)** · **NL=2 D_H=512 total 1.47M chip_frac=0.357 comp_survives=True (held hop-2/3 [0.892,0.928] p=0.005)** — head 40× scale 에도 composition PRESERVED. NL=3 D_H=2048 22M chip_frac=0.024 = 칩 인코더 map+learn 성공·off-chip BPTT 진행 중 host THERMAL THROTTLE(0x80000, ~84°C, 7GB Pi5) 로 미완. NL=4/6(~4e8/~6e9, 3B-class)은 ~50GB RAM 필요 → Pi5 host-RAM-INFEASIBLE (AKIDA ceiling 아님). **F-3B-HYBRID-1=TRUE** (composition 측정 전 rung above-NULL = bridge 작동, on-chip depth axis #1705/#1690 와 정반대) · **F-3B-HYBRID-2=NOT reached**. **DEFINITIONAL-HONESTY GATE: chip_fraction 0.957→0.357→0.024→(3B)1.75e-4 ≪ 1e-3 = TRIVIAL.** **HONEST TERMINAL (a_scale_honest_scope·a_paper_negative_ok): 3B-class HYBRID 는 on-chip AKIDA 기여 trivial(~0.017%) → 정직한 pure-AKIDA 3B 아님. Lane A ON-CHIP 은 PUBLIC(~524K composition-preserving D=1 인코더)에서 cap; 3B·7B 는 AKIDA substrate 위에서 도달 불가 (scale 축 quantified closed-negative).** Lane A 3B [ ] 유지 + on-chip ceiling 문서화, ladder PUBLIC 종료 제안. NO fabricated AKIDA 3B. result sha256 0da3516e2dcd1aa33000113efc4606f562ca96c8f9a37a2f70a764735e63133c · AKIDA/state_3b_offchip_head_verbatim.log · `.verdicts/lane-a-3b-hybrid/F-3B-HYBRID.txt`.
