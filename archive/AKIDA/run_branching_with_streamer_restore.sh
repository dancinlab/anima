#!/bin/bash
# Lane A HYBRID BRANCHING-CORPUS held-out rung — single-chip occupancy. Stop the R3 spike-streamer (it holds the
# AKD1000 file lock), run the branching held-out experiment to terminal, RESTORE R3 on exit (trap).
# substrate=HYBRID(on-chip encoder ⊕ off-chip host-CPU decode) · a_lane_akida_gpu_split. NO sw fallback (g63).
set -u
LOG=/home/ubuntu/clm_kosmos_akida/branching_wrap.log
...
echo "$(date -u +%FT%TZ) branching fire throttled=$(vcgencmd get_throttled)" >> $LOG
$PY -u onchip_xlm_branching.py > branching.log 2>&1
RC=$?
echo "$(date -u +%FT%TZ) branching exit rc=$RC throttled=$(vcgencmd get_throttled)" >> $LOG
exit $RC
