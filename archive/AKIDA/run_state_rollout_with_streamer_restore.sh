#!/bin/bash
# Lane A: single-chip occupancy — stop R3 streamer, run on-chip STATE-CARRYING AUTOREGRESSIVE ROLLOUT to
# terminal, restore R3. substrate=AKIDA · a_lane_akida_gpu_split. NO sw fallback (g63). restore-on-exit via trap.
set -u
LOG=/home/ubuntu/clm_kosmos_akida/state_rollout_wrap.log
...
echo "$(date -u +%FT%TZ) state-rollout fire throttled=$(vcgencmd get_throttled)" >> $LOG
$PY -u onchip_xlm_state_rollout.py > state_rollout.log 2>&1
RC=$?
echo "$(date -u +%FT%TZ) state-rollout exit rc=$RC throttled=$(vcgencmd get_throttled)" >> $LOG
exit $RC
