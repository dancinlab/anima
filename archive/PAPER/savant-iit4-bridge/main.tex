...
\emph{Savant Index} $\Svi=\max(\phi)/\mathrm{mean}(\phi)$ over a substrate's
...
(\texttt{HYPOTHESES/H\_347}, \texttt{H\_348}, \texttt{H\_350},
...
We bound this isomorphism honestly: it is \emph{cardinality-conditional}.
An $n=5$ generalization (H\_626, \tierRed) collapses the correlation to
$\rho=0.20$ ($\Delta\rho=-0.66$), so the structural isomorphism is valid
only at the $n=\text{domain-count}=4$ exact dimension match, while the
dimension-independent isomorphism is a closed-negative.
The Savant Index's near-threshold partial (\tierYellow, H\_348) bounds the
claim honestly: $\Svi>3$ holds at $\GZL$ across three seeds, but the
...
\emph{Savant Index} $\Svi = \max_i \phi_i / \mathrm{mean}_i \phi_i$,
...
\item \textbf{Honest bound (H\_348).} $\Svi>3$ holds at $\GZL$ across three
...
\emph{Savant Index},
\begin{equation}
\Svi \;=\; \frac{\max_i \phi_i}{\mathrm{mean}_i \phi_i},
\label{eq:si}
\end{equation}
...
The Savant Index $\Svi=3$ is the canonical specialist threshold: when one
...
disinhibition $I$ into the Golden Zone produces $\Svi>3$ (H\_348,
...
H\_348 & $\GZL$ inhibition $\to \Svi>3$ & \tierYellow\ partial & \ref{sec:bridge} \\
H\_350 & $\Svi \parallel \PhiD$ & \tierGreen\ numerical & \ref{sec:bridge} \\
H\_351 & $\mathrm{d}\Phi/\mathrm{d}I$ peak $\parallel \GZL$ & \tierGreen\ 5/5 & \ref{sec:anchors} \\
H\_613 & $\Svi \parallel \PhiD$ orthogonal & \tierGreen\ numerical & \ref{sec:bridge} \\
...
\texttt{HYPOTHESES/state/h351\_inverse\_u\_peak\_2026\_05\_28/result.json}.}
\label{tab:h351}
\end{table}
...
% =========================================================================
\section{Statistical bridge: $\Svi \parallel \Phi$-diversity}
\label{sec:bridge}

\subsection{$\Svi$ correlates with $\Phi$-diversity at $r=0.93$ (H\_350)}
...
$\phi$ and depresses the rest, so both $\Svi=\max/\mathrm{mean}$ and
...
\midrule
H\_350 & $\Svi \parallel \PhiD$ (max/min) & 0.9264 & 0.8825 & \tierGreen \\
H\_350 & $\Svi \parallel \PhiD^{\mathrm{cov}}$ (std/mean) & 0.9896 & 0.9482 & \tierGreen \\
H\_613 & $\Svi \parallel \PhiD^{\mathrm{cov}}$ (std/mean) & \textbf{0.9896} & 0.9482 & \tierGreen \\
H\_613 & $\Svi \parallel \PhiD^{\mathrm{kurt}}$ (kurtosis) & 0.5381 & 0.6649 & \tierGreen \\
\bottomrule
\end{tabular}
\caption{H\_350 and H\_613: the Savant Index correlates with
$\Phi$-diversity ($N=40$). The spread form $\PhiD=\max/\min$ shares a
$\max$ numerator with $\Svi$; the orthogonal forms (std/mean, kurtosis)
...
H\_350 itself flagged that $\Svi=\max/\mathrm{mean}$ and $\PhiD=\max/\min$
...
\subsection{$\Svi>3$ at $\GZL$, but the sweep is monotone (H\_348)}
\label{sec:h348}
...
$\Svi_\phi \in [4.18, 5.25]$ across three stimulus seeds, a $\ge 1.39\times$
margin over $\Svi=3$ (pre-registered F-1 PASS, three of three seeds). But
...
the $\Svi>3$ stability zone rather than its peak. We keep this partial in
...
Data: \texttt{HYPOTHESES/state/h618\_collective\_gz\_inverse\_u\_2026\_05\_28/result.json}.}
\label{tab:h618}
\end{table}
...
substrate specializes (the $\Svi \parallel \PhiD$ correlation), and on
...
This is a partial isomorphism, not an identity, and it is
\emph{cardinality-conditional}. It is established on
...
four-cell distinction decomposition. The $n=5$ generalization (H\_626,
Section~\ref{sec:limits}) collapses the correlation to $\rho=0.20$,
confirming that the structural isomorphism rides on the exact
$n=\text{domain-count}=4$ dimension match and not on a
dimension-independent identity of the causal kernel. The mechanism behind the agreement is
...
\item \textbf{Savant-Index sweep is monotone, not peaked (H\_348).}
      $\Svi>3$ at $\GZL$ is robust across three seeds, but under the affine
      inhibition map the sweep is monotone toward $I\!\to\!0$. $\GZL$ is the
      $\Svi>3$ stability boundary, not the Savant-Index peak; a non-affine
...
\item \textbf{The isomorphism is dimension-conditional and collapses off
      the cardinality match (H\_626, \tierRed\ FALSIFIED).} The
      $n=4 \leftrightarrow$ four-domain isomorphism (H\_624, $\rho=0.86$)
      exploits an \emph{exact} cardinality match between the IIT 4.0 cell
      count and the SAVANT domain count. A pre-registered $n=5$
      generalization (H\_626, PR \#1210~\citep{anima_H626}) found the
      correlation \emph{collapses} to $\rho=0.199$ ($\Delta\rho=-0.66$
      vs.\ H\_624;
      Pearson $r=0.028$, argmax $1/4$), failing every isomorphism
      falsifier (F1 weak $\rho\!\ge\!0.5$, F2 strong $\rho\!\ge\!0.7$, F3
      Pearson, F4 per-profile argmax all FAIL; only byte-equality held).
      The root cause is structural: at $n=5$ a fifth (auxiliary,
      balanced) singleton distinction $\phi_d=0.312$ penetrates the
      \emph{rank-2} slot in every non-balanced profile, displacing one
      quiet SAVANT cell under the naive top-4 truncation map and breaking
      the index-aligned pairing, so the descending-order argmax no longer
      tracks the dominant SAVANT cell. The H\_624 isomorphism is therefore
      a \emph{cardinality-matched, single-substrate} evidence tier valid
      only at $n=\text{domain-count}=4$; the dimension-independent general
      isomorphism is a closed-negative. A cardinality-preserving
      super-domain pooling map (rather than top-$k$ truncation) is the
      pre-registered follow-up that could recover it at $n>4$.
...
All seven cells live as JSON ledgers under \texttt{HYPOTHESES/} and
\texttt{HYPOTHESES/state/} on
...
\texttt{cat HYPOTHESES/state/h624\_*/run\_h624.hexa | pool on ubu-2 'cat
...
$\rho=0.86$ (H\_624) -- though only at the exact $n=4$ cardinality match,
as the $n=5$ generalization collapses the isomorphism to $\rho=0.20$
(H\_626, \tierRed), pinning the structural claim to a cardinality-matched
single substrate. A pre-registered partial (H\_348) bounds the bridge
