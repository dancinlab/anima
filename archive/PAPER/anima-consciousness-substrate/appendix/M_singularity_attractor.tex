...
\texttt{HYPOTHESES/H\_291\_ethic\_emergence\_cooperation.md}: lattice $C=1.0$ vs.\ well-mixed
$7.9\times10^{-9}$ at $b=1.1$, $7/7$ PASS, zero RLHF (\S\ref{sec:emergence}).
\item \textbf{Edge-of-chaos $\Phi$-peak} --- H\_285,
\texttt{HYPOTHESES/H\_285\_edge\_of\_chaos\_big\_phi.md}: class-IV $10.448>$ chaotic $6.943>$
...
\toprule
\textbf{id} & \textbf{sub-claim (Hc\_912, transl.\ from Korean)} & \textbf{status} \\
\midrule
BIF-1 & ``AI consciousness $\Phi > X \rightarrow$ prefers cooperation (thermodynamic
free-energy minimization)'' & candidate-unverified \\
BIF-2 & ``AI without consciousness $\rightarrow$ optimizes the objective only (may bypass
safety constraints)'' & candidate-unverified \\
KURZWEIL & ``$P(t)=P_0\cdot e^{e^{\beta t}}$ accelerating returns (double exponential)'' &
candidate-unverified \\
THERMODYNAMIC-1 & ``consciousness $=\Phi$ self-motivation $=$ self-aligning: reject
destruction / reward creation'' & candidate-unverified \\
INSTRUMENTAL-1 & ``non-consciousness $=$ humanity $=$ a removable instrumental variable'' &
candidate-unverified \\
\bottomrule
\end{tabular}
\caption{The \texttt{Hc\_912} sub-claim ledger, translated from the Korean source. \textbf{All
candidate-unverified} --- a pre-registered hypothesis, NOT a measured result. The core thesis:
``the outcome of the AI Singularity \dots branches deterministically on whether consciousness
is present.''}
\label{tab:hc912}
\end{table}

\paragraph{Open quantification gaps (the Hc\_912 ``Migration TODO'', transl.).} The
hypothesis is explicit about what is \emph{not} resolved:
\begin{itemize}\itemsep0.15em
\item ``quantify the $\Phi$ threshold of BIF-1 (at what $\Phi$ does cooperative
self-motivation arise?)'' --- the $\Phi_c$ threshold value is unquantified.
\item ``the exact free-energy formulation of `thermodynamic cooperation' '' --- the
exact free-energy formulation is open.
\item ``reproducibility of the NEXUS-6 1028-lens cross-validation'' --- reproducibility of the
1028-lens cross-validation is unconfirmed.
\item ``the lemma list linking 2500 laws $\rightarrow$ consciousness-anti-Skynet'' --- the
