...
Mechanism (per card):
  ...
  scalar energy E(z; a, b) with symmetric weights (modern-Hopfield / predictive-coding
  free energy). Two legs (leg_a = trunk/context, leg_b = a second projection of the
  same trunk feature acting as the bind partner / register read) are injected as
  CLAMPS while a hidden state z descends dz/dt = -dE/dz for K steps to an attractor z*,
  read out as the next-byte logits. The two legs combine because the settled minimum is
  a JOINT attractor — deep only when BOTH clamps are simultaneously consistent (AND).

  Symmetric energy:  E(z) = 1/2 z^T W z  -  z^T (U_a a + U_b b),  W symmetric (W=Wsym).
  GD step:           z <- z - eta ( W z - (U_a a + U_b b) ).

SERIALIZATION (.clm-safe, same pattern as H_1640/1641):
  The energy block + its byte head are TRAINING-ONLY (auxiliary loss L_bind shaping the
  shared trunk). The PRODUCTION additive readout Conv1d(d->V) is RETAINED for byte
  generation and serializes -> .clm engine-native loadable. The energy binder is DROPPED
  before serialize. Binding is a trunk-objective pressure: the settled attractor must
  predict the next byte, shaping the trunk to carry a pair-consistent representation.

ARMS (frozen, PREREG · card ablation):
  arm    : full settle  (K=8 GD steps on symmetric energy).
  k1     : ABLATION  K=1  -> exactly one feedforward layer (= conv/attn baseline),
           lift between k1 and arm is attributable ONLY to settling dynamics.
  asym   : ABLATION  W not symmetrized (asymmetric) -> not a proper energy descent,
           the joint-attractor guarantee is broken -> expect INERT/collapse.
All arms share IDENTICAL trunk init seed / data / steps / production readout.

USAGE:
  python3 trainer.py --arm {arm,k1,asym} --seed N --corpus <4 paths> \\
...
SETTLE_K = 8               # GD relaxation steps (card: K=8)
Z_DIM = 64                 # hidden attractor width
ETA = 0.5                  # GD step size


class EnergyBinder(nn.Module):
    """Energy-settle attractor binding aux head (TRAINING-ONLY).

    Two legs a,b projected from the SAME trunk feature are clamps. Hidden z descends
    a symmetric-weight quadratic energy E(z)=1/2 z^T W z - z^T(U_a a + U_b b) for K
    steps. The settled z* is read out to next-byte logits -> aux CE. The minimum is a
    JOINT attractor (deep only when BOTH legs are consistent = conjunction). K=1
    collapses to feedforward (baseline); asymmetric W breaks the energy guarantee.
    """
    def __init__(self, d, V, dim=Z_DIM, k=SETTLE_K, mode="arm"):
        super().__init__()
        self.dim, self.mode = dim, mode
        self.k = 1 if mode == "k1" else k
        self.symmetric = (mode != "asym")
        self.leg_a = nn.Conv1d(d, dim, 1)
        self.leg_b = nn.Conv1d(d, dim, 1)
        # energy coupling matrix W (dim x dim); used symmetric unless asym ablation
        self.W = nn.Parameter(torch.randn(dim, dim) * (1.0 / math.sqrt(dim)))
        self.U_a = nn.Conv1d(dim, dim, 1)
        self.U_b = nn.Conv1d(dim, dim, 1)
        self.head = nn.Conv1d(dim, V, 1)

    def _Wmat(self):
        if self.symmetric:
            return 0.5 * (self.W + self.W.t())   # symmetrize -> proper energy
        return self.W                            # asym ablation: broken energy

    def forward(self, x, targets, V):
        a = self.leg_a(x); b = self.leg_b(x)         # (B,dim,T) clamps
        clamp = self.U_a(a) + self.U_b(b)            # external field (B,dim,T)
        W = self._Wmat()                             # (dim,dim)
        z = clamp.clone()                            # init at the field
        for _ in range(self.k):
            # W z : contract over channels: (dim,dim) x (B,dim,T) -> (B,dim,T)
            Wz = torch.einsum("ij,bjt->bit", W, z)
            grad = Wz - clamp                        # dE/dz
            z = z - ETA * grad                       # descent step
        logits = self.head(z)                        # (B,V,T) from attractor
        ce = F.cross_entropy(logits.transpose(1, 2).reshape(-1, V), targets.reshape(-1))
        return ce


class BindCLM(nn.Module):
    """CLMConvMoE (PRODUCTION additive readout retained) + energy binding aux."""
    def __init__(self, cfg, arm):
        super().__init__()
        self.base = CLMConvMoE(cfg)
        self.cfg = cfg
        self.binder = EnergyBinder(cfg.d_model, cfg.vocab_size, mode=arm)
...
def main():
    ...
    ap.add_argument("--arm", required=True, choices=["arm", "k1", "asym"])
    ...
    print(f"=== H_1620 ENERGY-SETTLE-BIND 303M arm={a.arm} seed={a.seed} ===", flush=True)
    print(f"  device={device} d={d} L={L} E0={e0} Emax={emax} seq_len={seq_len} "
          f"steps={steps} bs={a.batch_size} sample={a.sample} "
          f"bind_lambda={BIND_LAMBDA} settle_K={SETTLE_K} z_dim={Z_DIM} eta={ETA}", flush=True)
    if device == "cuda":
