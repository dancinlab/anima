"""
anima_km_qwen7b_h100.py — raw#37 transient (BG-KM-QWEN-7B)
...
Architecture:
    base: meta-llama/Qwen2.5-7B-Instruct (3.2B params, instruction-tuned, multilingual incl. Korean)
...
def _is_on_pod():
    return os.path.exists("/workspace") and os.path.exists("/workspace/anima_km_qwen7b/_pod_marker")
...
# ── Paths ──────────────────────────────────────────────────────────────
POD_ROOT = "/workspace/anima_km_qwen7b"
...
POD_BASE_DIR = os.path.join(POD_ROOT, "qwen7b_base")

MAC_ANIMA_ROOT = "/Users/ghost/core/anima"
MAC_STATE_DIR = os.path.join(MAC_ANIMA_ROOT, "state/anima_km_qwen7b_h100_2026_05_08")
...
# Cost discipline
COST_HARD_CAP_USD = 15.0  # 7B is ~2.3x larger
COST_EARLY_KILL_USD = 12.0  # early kill at 80% of cap
COST_PER_HOUR = 2.99
...
def orch_pod_create():
    ...
    args = [
        "pod", "create",
        "--name", f"anima-km-qwen7b-{int(time.time())}",
...
def orch_main():
    ...
    try:
        ...
        rc, out, err = orch_scp_put(ssh, MAC_THIS_SCRIPT, f"{POD_ROOT}/anima_km_qwen7b.py", timeout=120)
        ...
        rc, out, err = orch_ssh_run(ssh,
            f"cd {POD_ROOT} && export HF_TOKEN=$(cat hf_token.txt 2>/dev/null) && nohup python anima_km_qwen7b.py > {POD_ROOT}/state/train_stdout.log 2>&1 & echo started",
            timeout=30)
        ...
        if verdict:
            ledger_entry = {
                ...
                "artifacts": [
                    "tool/transient_py/anima_km_qwen7b_h100.py",
