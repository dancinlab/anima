...
`/tmp/eye_tracker_helper.py` (raw#37 transient) — OpenCV + dlib (68-pt) or
...
## Calibration protocol (raw#12 frozen)
...
## Output schema (raw#77 — JSONL, one row per 1-min window)
```json
{
...
(rotates daily, append-only, idempotent, `chflags uchg` after rotation per raw#22).

## Privacy invariants (raw#13 reinforced — falsifiable, F2/F4)
...
- I6 `chflags uchg` on JSONL after first daily rotation (raw#22)
...
- F6 idempotent (raw#65): same (mode, seed) ⇒ identical JSONL row.
...
- `/tmp/eye_tracker_helper.py` — raw#37 transient. Per-tick polls webcam
...
## RAW compliance
raw#9 hexa-only · raw#10 honest C3 (lighting-dependent pupil proxy clearly
marked) · raw#12 frozen pre-registration (5 metrics, 30 Hz, 9-pt cal, EAR
< 0.20, r ≥ 0.30) · raw#13 ai-config-ban + privacy mandate (NO image, NO
encoding) · raw#22 uchg ledger · raw#37 transient /tmp helper · raw#65
idempotent (mode, seed) · raw#71 ≥ 5 falsifiers (here 7, incl. privacy F4
+ calibration F5 + sentinel F2/F3) · raw#77 schema-stable JSONL · raw#82
darwin-native (Mac webcam + Camera permission prompt) · raw#91 honesty triad
