...
raw#10 honest C3 is built into the design (§5).
...
silent inversion (raw#12 silent-error-ban).

---

## 5. Frozen criteria (raw#12, no post-hoc tuning)
...
## 7. raw#10 honest C3 (limitations)
...
- `anima-eeg/tool/resting_state_network_analyzer.hexa` (~150 LoC) — pure hexa, raw#9
- `/tmp/rsn_helper.py` — emitted at runtime (raw#37 transient), uses scipy.signal.welch / coherence + numpy
...
- `anima-eeg/tool/eeg_daily_life_verifier.hexa` — daily-life axis (raw#48 orthogonal)
- `anima-clm-eeg/tool/clm_eeg_lz76_real.hexa` — resting eyes-closed LZ76 (raw#12 SSOT)
