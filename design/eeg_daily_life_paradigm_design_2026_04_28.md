...
**Status**: NEW_PARADIGM_DISCOVERY (orthogonal axis to raw#12 frozen LZ76; raw#12 untouched)
**Trigger**: User does NOT meditate. Daily-life (eyes-open, task-engaged, motion-rich) EEG analysis required. Direct re-use of resting eyes-closed criteria (Schartner 2017 b ≥ 0.65 / raw#12 frozen) is paradigm-mismatched.
**Companion**: `design/eeg_consciousness_paradigms_omega_cycle_2026_04_28.md` (resting-axis Top-5)
**Compliance**: raw#12 (separate axis, no edit), raw#46 (6 metrics not 1), raw#48 (orthogonal), raw#52 (negative oracle), raw#70 (K≥4 grid), raw#71 (≥5 falsifiers), raw#91 (high-variance honest), raw#106 (genus naming), raw#117 (5-check)
...
### Problem
- raw#12 frozen LZ76 criteria (b ≥ 0.65) are **resting-eyes-closed** specific (Schartner 2017 cohort).
...
- Single metric is insufficient (raw#46 reverse-engineering, raw#91 high-variance).

### Solution
A **6-axis multi-metric verifier** with daily-life-specific pre-registered criteria, supermajority verdict (≥4 of 6 pass), and explicit differential against resting-axis (raw#52 negative oracle: sleep / unconscious / anesthesia produce opposite signature on every axis).
...
Verdict: `DAILY_LIFE.PASS = (≥ 4 of 6 criteria pass)` (raw#91 honest C3 — daily-life is high-variance; supermajority not unanimous).

---

## 1. Resting vs Daily-Life Differential (raw#52 negative-oracle-contrastive)

| axis | Resting eyes-closed (raw#12 anchor) | Daily life eyes-open | Sleep / Anesthesia (negative) |
|---|---|---|---|
...
**Differential clarity**: each of the 6 daily-life axes has a sleep/anesthesia counter-signature that is opposite-direction (raw#52 satisfied — daily-life ≠ resting paradigm AND daily-life ≠ unconscious paradigm).

---

## 2. 6-Axis Metric Matrix (Detailed)

### Axis 1 — `spectral-entropy-broadband` (genus, raw#106)
...
- **Frameworks (raw#117 ≥2)**: Inouye-Shannon, Rezek-medical.
- **Channels (raw#117 ≥3)**: 16ch × {Welch, multitaper, wavelet} = 48 sub-axes.
...
- **Wraps** `anima-clm-eeg/tool/clm_eeg_lz76_real.hexa` — does NOT modify raw#12 frozen criteria; calls it N times with `--start/--end`.
...
## 3. Orthogonality Check (raw#48 multi-axis-orthogonal)
...
**Verdict**: 6 axes, 3 pairs in BORDERLINE 0.4-0.7 (acceptable per raw#48 < 0.7 threshold), 3 pairs orthogonal < 0.5. **Aggregate orthogonality: PASS** (no pair ≥ 0.7).
...
- Frozen criteria as `let` constants (raw#12 SSOT discipline).
...
- Audit JSONL append (raw#82-friendly).
...
## 5. Pre-Registered Falsifiers (raw#71 ≥ 5)
...
Falsifier count: **7** (raw#71 ≥ 5 satisfied; ≥ 3 per metric covered by F1-F7 + counter-examples).
...
- **Counter-example**: sleep/anesthesia produces opposite signature on every axis (raw#52).
- **Falsifiers (≥ 3)**: 7 listed above.

---

## 7. raw#47 Cross-Repo Angle
...
| **anima-clm-eeg** (TERTIARY) | re-uses LZ76 implementation; raw#12 frozen criteria preserved | wrapper around `clm_eeg_lz76_real.hexa` for sliding window; NOT modified |
...
|---|---|---|
| paradigm v11 G0..G7 (8-axis stack) | G3 (time-binding) ↔ change-points; G5 (mode-shifting) ↔ engagement; G7 (vigilance) ↔ drowsiness | MEDIUM (raw#91 speculative) |
| Mk.XI v10 4-backbone ensemble | per-backbone EEG signature: each backbone monitors 1-2 of the 6 axes (e.g., backbone-1 → spec-ent + alpha-atten) | LOW (no integration spec yet — raw#91 speculative) |
...
| σ/τ=3 (own 3) → daily-life β/α ratio | β/α target 0.80-1.20 ≈ τ=3 anchor zone | LOW (numerology bridge — raw#91 speculative) |

**Note (raw#91)**: rows marked LOW are flagged speculative; require empirical correlation before claiming integration.

---

## 9. raw#91 Honest C3 Disclosure (speculative axes)

- **Axis-3 / Axis-4 sliding LZ76**: re-uses raw#12 LZ76 algo — but window-size, stride, change-point threshold are NEW free parameters; **not pre-validated on real daily-life data**.
...
- **Verdict rule "≥4 of 6"**: chosen for daily-life high-variance (raw#91 honesty); not derived from a population study. May need re-tuning after first-N daily-life sessions.
...
- Wire real-data path: sha256 manifest, `.npy` loader (via `/tmp` `.py` helper, raw#9 OK).
...
Confirms differential signature ≥ 3 axes between awake and each negative oracle (raw#52).

---

## 12. raw#106 Multi-Realizability
...
- **UNTOUCHED**: `anima-clm-eeg/tool/clm_eeg_lz76_real.hexa` (raw#12 frozen)
- **UNTOUCHED**: `anima-clm-eeg/tool/clm_eeg_p1_lz_pre_register.hexa` (raw#12 frozen)
