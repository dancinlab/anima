...
**Track**: anima B10 / β Learning-Free orthogonal axis (raw#12 frozen criteria untouched)
**Compliance**: raw#9 (pure-hexa, .py /tmp-only), raw#10 (3-tier honest), raw#12 (frozen criteria pre-registered),
raw#37 (transient .py auto-regenerated), raw#65 (no API), raw#71 (≥5 falsifiers), raw#73
(structural-admissibility — deterministic seeded training), raw#82 (darwin-native venv-eeg),
raw#91 (synthetic≠real classification), own 5 (genus + algo + hyperparams + ablation + falsifiers).
...
- AE captures non-linear manifold (8-d bottleneck, raw#12 hyperparameter).
...
80-d feature vector  (16 ch × 5 bands)
        ...
        ▼
Autoencoder  (numpy, deterministic seeded; raw#73 structural-admissibility)
...
**Frozen hyperparameters** (raw#12, this commit 2026-04-28):
...
| seed | 17 | deterministic init (raw#73) |
| min_baseline_n | 50 | frozen lower bound |

---

## 2. Pre-registered criteria (raw#12)
...
- **No torch dependency** — pure numpy autoencoder (forward + manual back-prop). The user's
  ...
  for an 80→8 AE on N=50–500 segments. raw#82 darwin-native compatible.
...
- **L2 weight decay = 0.01** on weight matrices (not biases) to regularize the over-parameterized
  AE on small N (raw#71 F3 mitigation).
- Helper writes a deterministic JSON dump (model weights NOT serialized — raw#73 demands
...
- raw#73 structural-admissibility: with seed=17, hyperparams frozen, identical input → identical
  reconstruction error to floating-point precision. The verdict is deterministic.
- raw#91: synthetic-only selftest never claims real-hardware verification. The user must
  collect ≥50 real eyes-closed baseline segments before `--mode real` produces a `REAL_HW_*`
  classification.
- raw#46: the autoencoder is *one* anomaly axis. It does not replace LZ76/PE/Hjorth/GCG/TLR
  per-segment hypothesis tests; it complements them with a per-segment unsupervised score.
- raw#10: PARTIAL exists. If the model trains but C3/C4/C5 fail individually, we report it
