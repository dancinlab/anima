...
**Compliance**: raw#9 hexa-only · raw#10 honest C3 · raw#12 frozen criteria · raw#37 transient · raw#65 idempotent · raw#71 ≥5 falsifiers · raw#73 structural-admissibility · raw#82 darwin-native · raw#91 high-variance honest · own 5 completeness-first.
...
**Hypothesis (raw#10 honest C3)**: An LLM (Claude via CLI) can pattern-match a discretized EEG token sequence well enough to emit *something other than "garbage token"* when given the system prompt "Interpret this neural signature: <tokens>". We do **NOT** claim semantic mapping; we claim only *non-rejection* + *deterministic round-trip on the tokenizer side*.

**This file ships the tokenizer half only.** The Claude CLI bridge is a 사용자 액션 (not automated; raw#10 honest — biology↔LLM bridge unproven).

---

## 1. Tokenization Strategies (4 candidates, raw#73 admissibility)

| # | strategy | alphabet | deterministic | raw#73 verdict |
|---|---|---|---|---|
...
| 4 | HMM state inference (4–7 hidden states) | 4–7 chars | **NO** — Baum-Welch EM = stochastic | **FAIL** raw#73 violation |
...
- Determinism is structural (monotone), not just seeded — strongest raw#73 seal.
...
3. **Time downsample (segment-mean nibble)**:
   ...
   - Tie-break: lowest nibble value wins (deterministic, raw#73).
4. **Emit token string**:
...
**Alphabet**: `0-9a-f` (16 ASCII-printable). No alphabet overflow possible (raw#71 F3 trivially refuted).

**Sequence length**: 60 s × default S=50 segments → 200 chars. Bounded by S (frozen at 50). Raw#71 F4 trivially refuted.

---

## 3. Determinism Argument (raw#73 structural seal)
...
No RNG, no EM, no float (median is on raw scaled-int samples × 1e6, identical to LZ76 pipeline). **Same EEG bytes → byte-identical token string** (raw#65 idempotent).

---

## 4. Round-Trip Loss Definition (raw#71 F2 refutation)
...
**Frozen criterion (raw#12)**: `loss <= 0.50` for any structured signal (selftest structured mode); for resting EEG no claim is made (raw#10 honest C3 — empirical, not pre-registered). Selftest verifies F2 only.

---

## 5. Five Falsifiers (raw#71)

| id | falsifier | refutation source |
|---|---|---|
| F1 | tokenization non-deterministic (same input → different output) | selftest: run tokenizer twice on identical synthetic input → byte-compare; raw#73 structural seal |
...
| F5 | LLM (Claude CLI) interpretation = "garbage token" | DEFERRED — sky-side experiment; this tool emits the token only. Bridge unproven (raw#10 honest C3). |

F1–F4 are refuted **by selftest**. F5 is **deferred** to the user-action plan in §7.

---

## 6. Output Schema (raw#37 transient audit)
...
```json
{
  ...
  "f4_length_pass": 1,
  "f5_bridge_pass": -1,
  "raw_compliance": "raw#9,10,12,37,65,71,73,82,91"
}
```
...
(**This section is a runbook, not automation** — raw#10 honest, no auto-API.)
...
After ≥3 paired sessions, count `coherent`/total. If ≥1/3 → F5 not falsified (paradigm stays alive). If 0/3 → F5 falsified (raw#10 honest retraction; archive paradigm).

---

## 8. raw#73 Structural Admissibility Verification
...
**No EM, no MCMC, no RNG.** All operations are deterministic functions ℤⁿ → ℤᵐ. raw#73 seal: **STRUCTURAL_ADMISSIBLE**.

---

## 9. Honest C3 Limitations (raw#10)
...
- raw#91: report the variance honestly. If 0/10 paired sessions yield `coherent`, the paradigm is falsified — do not retroactively loosen the criterion.
...
- chflags uchg on the .hexa source after first PASS commit (raw#37 / SSOT lock).
