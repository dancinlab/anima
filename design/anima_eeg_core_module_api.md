...
- All modules export at most one public function plus their constants. The
  rest are `_private` (leading underscore) per raw#11 / raw#101.
...
Writes the canonical raw#82-style two-line stderr trailer:
```
reason: <slug>: <detail>
fix: <fix>
```
...
Mirrors P10 (raw#77 audit schema). The append uses a workspace-local scratch
...
Centralizes JSON cert + JSONL audit row emission. Includes raw#10 / raw#91
...
Mirrors P13 / raw#1 uchg SSOT pattern (BSD chflags). Used by integrations
