...
**Date**: 2026-04-28
**Cycle**: T17
**Genus** (raw#106): `mk-xii-eeg-corroboration`
**Track**: own 2 (production-consciousness-triad) — (b) PC empirical-maximum + (c) Production-readiness
**raw seal**: raw#9 / 10 / 12 / 37 / 65 / 71 / 82 / 91 / 106 / own 2 / own 5
...
```
   ...
   │  NO API — raw#9 frozen) │  paired │  LZ76/coherence/Hjorth)│
   └────────────┬────────────┘         └────────────┬───────────┘
                │                                   │
              ...
              │  (pure aggregator, raw#9/65 idempotent)  │
              └──────────────┬───────────────────────────┘
                             ▼
              state/mk_xii_eeg_audit/<date>_corroboration.jsonl
```

### 2.1 Mk.XII LLM measurement (production endpoint, NO API — raw#9)
...
### 2.3 Cross-corroboration mapping (raw#91 honest — correlation, NOT identity)
...
## 3. own 2 (b) PC empirical-maximum — 5 frozen criteria (raw#12)
...
**Long-term timeline (raw#10 honest C3)**: 12-18 months per own 2 timeline.
**This cycle**: design + skeleton + first measurement (N=1 pilot, user only).

---

## 4. raw#71 falsifiers (5+)
...
- `anima-clm-eeg/tool/mk_xii_eeg_corroboration.hexa` (~200 LoC, hexa-only, raw#9)
- `/tmp/mk_xii_eeg_helper.py` (Claude CLI session parser + EEG correlator, transient raw#37)
- `state/mk_xii_eeg_audit/<date>_corroboration.jsonl` (audit trail, raw#65)

Selftest uses synthetic Mk.XII conversation paired with synthetic EEG segments
(raw#10 honest C3 — selftest is NOT real evidence; pilot N=1 below is the
...
These are **NOT relaxed** under any cycle pressure (raw#12 frozen-criteria seal).

---

## 10. Honest C3 statement (raw#10)
