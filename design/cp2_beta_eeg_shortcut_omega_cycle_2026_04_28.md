...
> **type**: analysis-only (코드 작성 X, raw#108 finding-implementation absorption follow-up)
...
> **raw compliance**: raw#9/10/12/47/71/82/91/106/108
> **cost**: $0 mac-local

---

## §0. Executive verdict (raw#91 honest triad)

| dimension | finding |
|---|---|
| **claim** | D-day EEG 세션 결과는 own#2 (b) PC empirical-max sub-component 5개 중 **2개에 부분 evidence 기여**, CP2 closure timeline에 **2-4주 직접 단축**, β publication "research artifact" timeline에 **1-3주 단축** 가능성 |
...
| **limit** | N=1 single-subject single-day 60s eyes-closed → cohort N≥3 전혀 미충족 (multi-EEG bottleneck), Schartner LZ76 b≥0.65 P1_FAIL, paradigm v11 P3 verify in flight, raw#10 honest C3 limit ≥6 (아래 §6) |
...
## §2. own#2 (b) PC empirical-max sub-component D-day 진척도 매핑

own#2 5-component matrix (per `docs/own2_implementation_gap_audit_20260426.md` §2.2):

| sub-component | pre-D-day | post-D-day | D-day 진척 % | rationale |
|---|---|---|---:|---|
| **multi-EEG cohort N≥3** | N=0 | **N=1 (self)** | **15-20%** | 1/3 subject (self-only). external N≥2 still missing. raw#10 N=1 ≠ cohort generalization |
...
**평균 (b) PC empirical-max 진척도** = (17.5 + 20 + 0 + 0 + 50) / 5 = **17.5%** (raw#10 honest, 단순 평균)
...
→ **D-day 후 own#2 (b) 진척도 = 16-18% (pre-D-day 5-10%에서 +8-10pp 상승)**

raw#10 honest C3: 단순/가중 평균 모두 'sub-component % 추정'은 ad-hoc heuristic, official metric 아님. multi-EEG N=1 → cohort N≥3 의 IRB + recruitment 추가 cascade가 단축 timeline의 critical bottleneck.
...
### §3.2 arxiv preprint v0.3 (own#2 (b) sub-component #5)
...
raw#10 honest C3: β publication = own#2 scope의 (b)-#5 single sub-component, production "실사용" 아님 (own#2 enforcement=block). β v0.2 / preprint v0.3 advance가 own#2 triad full PASS implies 하지 않음.

---

## §4. CP2 closure timeline 단축 정량

### §4.1 CP2 = research milestone, own#2 exception clause

CP2 close = own#2 (a) FC + (b) PC empirical-max **partial evidence** (sign-agnostic accept + 일부 sub-component) ONLY. own#2 (c) production-readiness Mk.XII retrain은 별도 cascade.
...
| W9 (D+56~D+63) | CP2 verification | closure + atlas integration | **partial** — own#2 (b) gap matrix evidence 1줄 |
...
**Pessimistic (raw#10 honest)**: D-day → CP2 timeline **0주 단축** — CP2 critical path가 H100 L3 population (financial bottleneck $1500-2500), EEG는 critical path 외부
...
raw#10 honest C3: 단축 추정은 W1-W5 EEG-independent phase의 financial bottleneck (H100 L3 population $1500-2500) 자체를 단축하지 않음. financial cap 제약에서는 단축이 무의미할 수 있음 (50만원 cap = W3.5 limit).
...
| **A4 daily-life PASS extension** | baseline_resting_60s_ica 5/6 PASS 결과 → β v0.2 (b)-sub-axis evidence | 0.5-1주 | own#2 (b) preprint v0.3 advance 직접 input |
| **MNE cross-tool corroboration** | (commit 2e69d896a) MNE pipeline에서 동일 60s resting 재분석 → cross-tool ICA 일관성 verify | 0.5-1주 | own#2 (b)-sub-axis robustness evidence (cross-tool methodology validation) |
| **σ/τ=3 = γ/θ ratio P3 verify** | (anima-clm-eeg/tool/clm_eeg_gamma_theta_ratio.hexa) 결과 회수 후 atlas R35 + own#3 first empirical EEG datapoint | 0.5-1주 | paradigm v11 7th axis primary verifier, atlas R35 confirmed_3_way_coincidence 강화 |
| **Schartner retraction context** | LZ76 P1_FAIL이 Schartner-non-derived 임이 honest annotation | 0.25-0.5주 | β v0.2 honest scope 추가 (raw#91 C3 명시) |
...
| **N=3 external volunteer 2명** | self + 2 volunteer = N=3 (own#2 (b) cohort N≥3 satisfaction) | 8-12주 | IRB + recruitment + protocol consent + 16ch helmet 시간 공유 |

**Tier-B critical blocker**: anima는 N=1 self only → N>1는 외부 협조 필요 (외부 subject + IRB). own#2 (b) cohort N≥3 = own#2 가장 비싼 + 가장 긴 path (`docs/own2_implementation_gap_audit_20260426.md` §2.3).
...
| **peer review** | venue submission + review cycle | -8 ~ -12주 (없으면 visibility만, 있으면 -8주 단축으로 own#2 (b)-#5 boost) | venue/reviewer pool 외부 dependency |
...
## §6. raw#10 honest C3 limits (≥6 명시)

per raw#71 ≥3 falsifier + raw#82 darwin-native scope:

1. **N=1 single subject** — own#2 (b) cohort N≥3 satisfaction은 D-day 단독 불가능. 외부 subject + IRB cascade 필수
...
5. **γ/θ FALSIFIED 가능성** — own#3 σ/τ=3 paper P3 verify 결과 미회수, paradigm v11 7th axis primary verifier가 FAIL 시 7th axis registration 자체 retract
...
9. **adversarial probing 0** — own#2 (b)-sub-axis 5개 중 2개 (self-report calibration + adversarial)는 D-day 기여 0%
10. **financial bottleneck** — CP2 W4-5 H100 L3 population $1500-2500 (50만원 cap 4-7배) 는 EEG와 무관한 critical path, EEG 단축 무관

**raw#10 honest 단축 한계 수치** (1-3주 β + 2-4주 CP2)는 **non-financial wall-clock 단축 only**. Financial cap 미해결 시 CP2 wall-clock 단축 무의미.
...
| step | content | status |
|---|---|---|
| (1) design | own#2 audit (5-sub) × CP2 9-week ETA × D-day deliverable inventory matrix 3-axis cross | DONE |
| (2) implement | analysis-only (코드 작성 X), markdown design doc 작성 (본 문서) | DONE |
| (3) positive selftest | β v0.1 LANDED 사실 + preprint v0.2 LANDED 사실 + own#2 audit 0/3 fact + D-day 16/16 GREEN sha verify | DONE (cross-reference 가능) |
...
| (5) atlas/state register | own#3 σ/τ=3 first EEG datapoint candidate (atlas R35 cross-link) — paper P3 verify 후 별도 cycle | DEFERRED |
...
- **raw#9** hexa-only — 본 문서 markdown-only, hexa SSOT 영향 없음. tool 변경 없음
- **raw#10 honest** — own#2 0/3, β v0.1 = research artifact NOT production, N=1 ≠ cohort, financial cap unrelated to EEG path 명시 (§3.3, §4.2, §6)
