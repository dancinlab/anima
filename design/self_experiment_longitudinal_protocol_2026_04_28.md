...
**Owner**: search5599@proton.me (single-subject longitudinal, raw#91 honest C3)
**Linked SSOT files**:
...
## 1. Rationale (raw#12 frozen experimental design)

Multi-subject N≥10 cross-sectional baseline collection requires recruitment, IRB,
budget. **N=1 longitudinal** trades external generalisability (raw#91 honest C3)
...
in `state/clm_eeg_p1_lz_pre_register.json` (raw#12 transition condition) with
...
Spread over **~14 days** at ~12 sessions/day (achievable, raw#82 protocol-load).
...
3. **60s eyes-closed resting** (raw#12 anima-clm-eeg paradigm primary)
4. **5min eyes-open daily-life** (typing / reading / conversation — raw#48 axis)
...
## 4. Hypotheses (pre-registered, raw#12 — see card §6)
...
## 6. Threshold replacement workflow (raw#12 transition)
...
5. Old `0.85` placeholder is **retracted with provenance** (raw#82 retraction protocol).
...
## 8. Falsifiers (raw#71 ≥5, see card §7)

F1-F5 in pre-registration card.

## 9. RAW compliance map

- raw#9 (pure-hexa, .py /tmp-only): runner is hexa, no .py
- raw#10 (deterministic): FNV seed from session timestamp
- raw#12 (pre-registered, no post-hoc tuning): card frozen + uchg-locked
- raw#37 (single-source-truth): ledger = SSOT
- raw#65 (idempotent re-run): session_id = sha256(timestamp+axis+level)
- raw#82 (retraction protocol): old 0.85 placeholder retracted explicitly
- raw#91 (honest C3): §7 limits enumerated
