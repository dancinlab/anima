...
- **Date**: 2026-05-01
- **Author**: anima sub-agent (raw#9 hexa-only, raw#10 honest, raw#12 frozen-spec contract)
...
- **Trigger**: F3 raw#91 honest 후속 권고 (P2 PLV-preserving 4 stage real .npy ALL falsifier triggered, P3 GCG real FALSIFIED, P1 LZ P1_FAIL b=351 < 650 frozen lo, composite ≥2/3 NOT met)
- **Cap**: spec only, $0, mac local, raw#9 strict, hexa-only, LLM=none

---

## §0. Executive Summary

F3 raw#91 honest verify 결과 P2 PLV-preserving 4 stage real .npy 가 **전수 falsifier triggered**:
...
본 spec 은 P2 C2 threshold 재정식 (reformulation) 을 위한 **3 후보 비교 + falsifier preregister + 결정 protocol** 만 정의한다. **frozen criteria 자체는 변경하지 않는다** (raw#12 frozen-spec contract 보존). 변경은 별도 v2 bump cycle 에서, 본 spec 의 결정 결과에 의거.
...
## §1. 현재 frozen criteria 보존 (raw#12 frozen-spec contract)
...
- **rationale**: F3 raw#91 ref-short stage 0.372 가 0.30 위로 올라가 P2_PASS 가능. 단순 threshold relax.
...
- **장점**: post-hoc relax 아님 (metric 자체 reformulation), volume conduction 의심 제거, F3 raw#91 두 falsifier 모두 직접 대응.
...
## §3. 각 후보의 raw#71 falsifier preregister + raw#10 honest C3
...
raw#12 contract: **silent in-place edit 절대 금지**, **byte-identical lock**, **별도 file 발행**, **chain_sha256 재계산**.
...
2. **Volume conduction 의심 해소**: F3 raw#91 F_PLVP_02 (0.993/0.999 collapse) 가 후보 metric 에서 *해소되거나 식별 가능* 해야. (후보 A 는 미해소 → 자동 fail.)
...
**post-hoc threshold change 회피 원칙**: F3 raw#91 결과 본 후 threshold 를 *측정값에 맞춰* 조정하는 행위는 falsifiability 약화. 따라서 후보 A 는 *공식 v2 bump 절차* + *외부 reference rationale* + *new falsifier set* 패키지 안에서만, 그리고 후보 B/C 가 모두 부적합하다고 §3 falsifier 로 입증된 경우에만 fallback.

---

## §6. raw#71 Falsifier 명세 (3 specified)
...
- **F_SPEC_02 — frozen contract 위반**: 본 spec 작성 결과 `state/clm_eeg_pre_register_v1.json` 또는 `v1_1.json` 의 sha 가 변경되면 → spec 자체가 raw#12 위반 → revoke.
...
1. **Honest reformulation, not post-hoc relax**: F3 raw#91 falsifier 결과를 받아 *threshold 를 내리는* (후보 A) 대신, *측정 metric 자체* 를 바꾼다. Popperian falsifiability 원칙 보존.
...
- 본 spec doc 자체는 *decision input* 으로 보존 (raw#12 contract: doc 은 frozen).
- D+1 P1 LZ verify (별도 cycle) 와 조정 — composite ≥2/3 PASS 재 시도 시점.

### 8.5 raw#10 honest closure
...
## §9. raw#10 Honest C3 — 본 spec 미입증 의심
...
- F3 raw#91 honest verify result: F3 P2 PLV-preserving 4 stage real .npy report
...
- raw#9 hexa-only contract, raw#10 honest, raw#12 frozen-spec contract, raw#71 preregister falsifier, raw#91 honest verify — anima governance ledger.
