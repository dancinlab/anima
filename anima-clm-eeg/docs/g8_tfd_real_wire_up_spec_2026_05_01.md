...
raw#9 hexa-only · raw#10 honest · pre-register-frozen · spec-only (no code this cycle)
...
raw#10 honest verdict for the D-day-current empirical state: real measurement is **NOT YET POSSIBLE** in any meaningful information-theoretic sense, because three of the five components are deterministic single-trial verdicts on this subject/session (F_A1=FAIL across 4 runs, F_A2=INSUFFICIENT pending C2 V_sync, F_A3=FAIL across both real .npy). With every trial yielding an identical bit per stream, marginal entropy H(X)=0 and pairwise MI(X,Y) is **trivially 0** — G8 PASS would be vacuously satisfied without testing the assumption it is supposed to test. Real wire-up therefore requires a multi-trial sample-generating protocol (§3 below) before the existing MI measurement loop can produce non-vacuous evidence.
...
| ledger I/O | none — pure FNV in-process | 5 ledger paths read at startup, hash-pinned for raw#9 reproducibility |
...
The frozen criterion (10 pairwise MI ≤ 100 ×1000-fp) and the JSON cert schema are unchanged. The real wire-up extends the dispatcher in main(), it does not edit the measurement primitives (entropy_x1000_from_hist / Miller-Madow correction / log2_x1000 table / FNV fingerprint emission), preserving raw#9 byte-identical determinism for the surrogate path.
...
- F_A1: bit = 1 if b_n_x1000 ≥ 650 else 0 (frozen P1 lo, raw#12 operational pre-commitment)
...
Three options, ordered by raw#10 honesty:
...
raw#10 honest: window axis works for F_A1/F_A2/F_A3 but breaks F_B/F_C identity. Spec §6 raw#71 falsifier 3 flags this.

### §3.4 Hash-pinned reproducibility

Each ledger path is read once and its sha256 emitted in the cert under inputs[].sha256. Re-run with a different ledger sha256 ⇒ raw#9 violation flag, separate cert path.
...
## §5 raw#10 honest C3 — D-day empirical joint MI feasibility
...
| ω2 | g8_tfd_real_wire_up_v1.hexa — real-mode dispatcher + drop-trial + 5 ledger reader stubs (session-axis only) + 3 raw#71 falsifier guards + cert schema extension. preserve --selftest / --selftest-negative byte-identical | ~1 cycle, ~250 LoC delta on top of #175 frozen tool (or sister file with shared primitives) |
| ω3 | g8_tfd_real_wire_up_v1_landing.md — live run with 2026-05-01 ledger sha pins → expected verdict=VACUOUS_PASS_INSUFFICIENT_VARIANCE (raw#10 honest) | ~0.5 cycle |
...
raw_strict: raw#9 hexa-only · raw#10 honest C3 — vacuous-pass surfaced explicitly · raw#12 frozen criteria preserved · raw#65 idempotent (spec is read-only) · raw#71 3 falsifier listed · raw#82 darwin-native · raw#91 honest triad — single-trial single-subject D-day state explicitly labelled non-informative
