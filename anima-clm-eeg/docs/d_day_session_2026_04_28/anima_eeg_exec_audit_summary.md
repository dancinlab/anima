...
- VIOLATION (raw#12 silent-error-ban, no annotation): 1
...
- For intentional discard (electrode_adjustment_helper:807 `clear`): add `@allow-silent-exit` annotation per raw#12, OR migrate to `let _ = exec_with_status("clear")` with explicit comment.

## raw compliance notes

- raw#9 hexa-only: audit grep is build-tool, audit emit is `/tmp/*.jsonl` (not committed) — OK.
- raw#10 honest: every classification grounded in code line context, not hypothesis. Downstream propagation traced where present.
- raw#12 silent-error-ban: the 9 BUGs and 1 VIOLATION are all instances of this principle.
- raw#77 schema: JSONL rows include {ts, file, line, exec_call, pattern, classification, recommendation, severity, downstream}.
- raw#91 honest C3: `exec_status()` from the task hypothesis was NOT found in hexa runtime — only `exec_with_status()` exists. RFC-006 paired task should reference the real binding.
