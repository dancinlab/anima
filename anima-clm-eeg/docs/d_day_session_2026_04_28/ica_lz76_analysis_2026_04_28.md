...
## 6. raw#91 honest C3 — ICA limitations
...
6. **C1 still NOT met** — neither input pipeline (raw / filt / ICA) cleared `b ≥ 0.65`. The pre-registered criteria (raw#12) **are not modified post-hoc**. The honest read is that this 60-s OpenBCI Cyton+Daisy resting recording does not pass Schartner-floor LZ76, irrespective of how aggressively we pre-process — the underlying recording quality (electrode contact, environmental EMI, motion) is the rate-limiter, not the analysis.
