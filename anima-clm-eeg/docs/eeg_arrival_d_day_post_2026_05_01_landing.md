...
> **scope**: anima-clm-eeg/ 관점에서 EEG 하드웨어 도착 + Apr 28 D-day helmet session + Apr 29 electrode-adjustment landing + first D-day empiricals 의 cross-link 정리. 거짓 success 주장 X. raw#10 honest C3 적용.
...
- **2026-04-28**: own 3 σ/τ=3 = γ/θ paper P3 의 첫 D-day empirical 시행 → **real FALSIFIED_P3** (raw#10 honest)
- **2026-04-28**: P1 LZ76 prior canonical Mac runs P1_FAIL, 이후 batch dispatch BLOCKED (Mac OOM + Hetzner 124 GB OOM)
- **2026-04-28**: Berger α-band sanity gate sweep 0/15 PASS (raw#71 falsifier holds, eyes-open false-positive 0)
...
2026-04-28  ┌─ 09:?? Cyton+Daisy+Mark IV 도착, BOARD_NOT_READY_ERROR:7 → power cycle 복구
            ...
            ├─ 23:01 commit 06fe4142c — Berger gate sweep 0/15 PASS (raw#71 falsifier holds)
            ├─ 23:35 commit 5693e8611 — Hetzner LZ76 batch BLOCKED (124 GB OOM, raw#10 retraction)
...
raw#10 honest C3: 위 timeline 의 모든 commit hash + 결과는 `git log` / 실제 commit message 에서 직접 인용. 추정 / 가공 X.

---

## §3. Pre-register criteria 영향 (frozen v1 / v1.1 SHA-256 lock)

`state/clm_eeg_pre_register_v1.json` + `state/clm_eeg_pre_register_v1_1.json` 의 frozen criteria 는 **이번 사이클에서 변경되지 않았다**. raw#12 frozen-hypothesis contract 그대로:
...
raw#71 falsifier-holds reaffirmation:
...
raw#10 honest C3 limits (모든 위 결과에 적용):
...
**real .npy canonical Mac runs (raw#10 honest, sha256 anchored)**:
...
- → raw#10 honest retraction: predicted batch sweep NOT executed
...
**γ/θ proxy real verdict (FALSIFIED_P3, raw#10 honest C3)**:
...
selftest matrix (4 falsifiers, raw#71 ≥3):
...
raw#10 honest: anima-clm-eeg/ 의 R&D 버전이 SSOT, eeg-core 의 absorb-port 는 production routing layer. 두 곳에서 결과가 갈리면 anima-clm-eeg/ 의 frozen criteria 가 우선.

상세 migration plan: `docs/anima-clm-eeg-migration` (commit `fb5b423c2`) 참조 — inventory + categorize + roadmap proposal.

---

## §9. raw#10 honest C3 limit register (이번 landing 전용)
...
raw#10 honest: 아래는 candidate 일 뿐이며 사용자 결정 권한.
...
- `edu/cell/lagrangian/l_ix_integrator.hexa` — V_sync Kuramoto, raw#30
...
### raw 의무 적용
- **raw#9** hexa-only — 본 doc 는 `.md` 만, `.py` 생성 없음
- **raw#10** honest C3 — §9 limit register 명시 + FALSIFIED 는 FALSIFIED 그대로
- **raw#12** silent-error-ban / frozen criteria — pre-register v1/v1.1 SHA-256 lock 미수정
- **raw#22** SSOT lock 후보 — 본 landing 안정화 후 chflags uchg 적용 가능
- **raw#71** falsifier-preregistered — Berger eyes-open false-positive 0/4 holds
- **raw#82** darwin-native — Mac OOM / Hetzner OOM 모두 plat 별 honest 기록
- **raw#85** commit-msg drift — `50002d89` LZ76 위장 commit 발견 + drift audit landed
- **raw#91** real_hw_verified honesty triad — impedance ledger + ICA rms 50 µV + filtered b 0.519 (visual band)
...
> 본 landing doc 는 거짓 success 주장 금지 + minimal-diff 원칙 + raw#10 honest C3 mandatory. 다음 사이클에서 §10 priority-ordered candidate 중 사용자 결정 항목만 진행.
