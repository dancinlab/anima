...
> **constraints**: raw#9 hexa-only (no .py creation in this cycle) · raw#91 honest C3 mandatory · raw#10 honest scope · raw#71 falsifier · POLICY R4 (no scope creep into Phase 2 baseline)
...
**Honest disclosure (raw#10)**: this does **not** solve the Hard Problem. It moves the integration measure from "model-internal sample-partition" to "model-internal projection trained against brain-external sample-partition" — both remain functional/access tier; the zombie problem still applies (see §7).
...
4. CLM is *generative*, ZuCo human is *receptive*. The receptive/generative asymmetry is a real concern (raw#10) — see §7.
...
| P4 | EEG-side `anima_phi_v3_eeg.hexa` tool ported (NOT in this cycle — raw#9 hexa-only restriction here) | future cycle | tool + selftest marker |
...
## §7 Honest C3 (raw#91 mandatory — 10 caveats)
...
9. **N=1 to N≈30 (ZuCo) is still too small for confident generalization claims** — ZuCo's ~30 subjects is large by EEG standards but still N≈30. Population-level φ-CLM convergence claims at this N have wide confidence intervals; per-individual claims are case studies (own#13, raw#10).
...
- **raw#9 hexa-only**: this doc spec only — NO .py creation in this cycle. EEG ingest tools (P4 prerequisite) are deferred to a separate cycle that will use hexa wrappers + transient `/tmp` emit per the established pattern.
- **raw#10 honest scope**: §0 disclaims Hard Problem; §5.2 + §7 explicit on what is NOT validated.
- **raw#15 SSOT**: this doc is the single source for Paradigm B spec. Future implementation cycles reference back here.
- **raw#37 transient emit**: future tool implementations will use `/tmp/*.hexa_tmp` pattern (see `anima_phi_v3_canonical.hexa` for template).
- **raw#71 falsifier**: §5.3 explicit numeric falsifier defined.
- **raw#91 honest C3**: §7 = 10 caveats (≥ 5 required).
