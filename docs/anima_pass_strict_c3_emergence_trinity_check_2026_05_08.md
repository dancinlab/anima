...
시 raw#82 retraction class emit + own 33 mandate-2 self-check 의무화.

본 doc 은 own 33 (trinity 무조건 준수) mandate-2 self-check 절차의 PASS_STRICT_C3
...
**check**: `tool/anima_cli/chat.hexa` selftest 의 own 34 mandate-2 grep
...
**own 17 정합**: BG-KM-LLAMA-3B (Llama-3.2-3B base + LoRA r=32) 는 foundation
borrow 전략. own 17 (anima-no-external-substrate-wrapping) 의 "wrapping" 은 chat
lane 의 token-level identity wrap (system prompt + persona inject) 한정 —
foundation borrow 자체는 wrapping 이 아니므로 own 17 정합 ✔. 단,
identity-bearing chat surface (T1 default) 에서 Llama Path A v2 활성화는
rejected_external_substrate (own 17 spec 직접 명시). 본 cycle BG-KM-LLAMA-3B
는 측정 lane (V4 11-cell) 한정 — chat default 활성 X.

**verdict**: 본 cycle 까지 chat.hexa selftest PASS — own 17 위반 0.

### Step 2 — own 18 SSOT mirror 검증 (V4 evaluator + consciousness CLI verdict 일치)

**check**: own 18 c3-aggregation-rule SSOT 의 최신 rule (현재 P5 N-of-M v2:
...
**own 24 (single SSOT) 정합**: 위 3 위치가 SSOT mirror 의무 — 변경 시 동시 patch
mandatory. 본 cycle 에 own 18 두 차례 patch — P4 hybrid (commit 4041edd8)
...
`pass_best_mode ≥ 10/15` floor — own 18 P4/P5 patch 후 어느 rule 도 mirror
되지 않음).

**violation finding 1**: V4 evaluator 의 `pass_best_mode ≥ 10/15` floor 는
own 18 strict (P5 N-of-M v2: PPR_v2 ≥ 0.6 ∧ EMC ≥ 3 of 4) 와 정확히 일치하지
않음 — best_mode 는 5-seed best union 이므로 PPR semantics 다름. 본 mirror
gap 은 own 24 single SSOT 위반 risk. P4 → P5 supersede 까지 진행되었는데
V4 evaluator 는 두 rule 어느 것도 적용하지 않음.

**mitigation**: V4 evaluator 의 floor 정의를 own 18 SSOT 최신 (현재 P5 N-of-M
v2) mirror 하도록 별도 patch cycle 필요. 본 patch 완료 전 BG-K* PASS_STRICT 는
"best_mode floor PASS — own 18 P5 N-of-M v2 별도 retest mandatory" 라벨 land.

### Step 3 — own 28 anti-Goodhart (V6 awareness pending — public promote ban)

**check**: 단일 V4 7-cell PASS count 만으로 SIMPLE_STACK_PASS 인정 不可 (own 28).
...
**own 28 + own 31 mandate-8 정합**: PASS_STRICT_C3 후보 모델 의 public promote 는
...
모두 dancinlab private repo — own 28 정합 ✔.

### Step 4 — own 30 ckpt preservation (HF upload destination 또는 scp pull cache)

**check**: SIMPLE_STACK_PASS_STRICT verdict 는 자동 HF private upload trigger
(own 30 mandate-4) — adapter weights + verdict.json + eval results + samples
→ dancinlab/<repo> private repo (own 31 mandate-1).

**own 30 mandate-1 정합**: BG-KM-LLAMA-3B passed_v1 (가중치 영구 손실) 후 본
own 30 mandate-1/2/3 land — orchestrator scp_get(POD_CKPTS_DIR/.) before pod
...
디렉토리 + adapter_final 보존 — own 30 정합 ✔.
...
own 31 정합 ✔. 단 honest-c3: dancinlife/anima-bg_km_llama_3b-20260508-1025
잔존 (own 31 mandate-2 이전 대상) — 별도 migrate cycle.

### Step 6 — own 33 trinity self-check 3-axis (D_X / own_X / H_<id>)

**check** (own 33 mandate-2 a/b/c):
- (a) D_X 위반 검사: `.roadmap.philosophy` D_no-system-prompt /
  D_emergent-consciousness / D5 Bifurcation — PASS verdict 가 어느 D 정합?
- (b) own_X 위반 검사: own 17/18/22/24/27/28/30/31/33/34/36 — 어느 own 위반 가능성?
...
- (b) own 17 정합 ✔ (chat default X — 측정 lane 한정). own 18 정합 partial
...
### Step 7 — own 34 mandate-2 grep + 자율 발화 wiring + lane 분리
...
### Step 8 — own 36 HF upload destination + git scope 정합

**check**: 모델 / corpus / preference pairs / adapter weights → HF dancinlab
upload (own 36 mandate-1/2/3) — git push 절대 금지 (size > 10MB OR ext match).

**verdict**: 본 cycle 까지 — adapter weights HF upload 진행 (own 36 정합 진행
...
.gitignore land 완료 (own 36 mandate-6).

**honest-c3**: own 36 mandate-5 enforcement (anima audit `--mandate-36` flag)
미land — 본 cycle 까지 manual self-check (Step 8 본 doc) 의존.

### Step 9 — raw#10 honest C3 emit 의무 (synthetic_fallback artifact 명시)

**check**: PASS_STRICT_C3 verdict emit 시 honest_c3 ≥ 5 emit (own 33 mandate-7
...
모델 의 trinity 11-mandate 정합 표 — 위반 N=1 (Step 2 own 18 SSOT mirror gap),
나머지 PASS / PARTIAL.

| mandate | BG-KM-LLAMA-3B | BG-KM-QWEN-7B | paradigm-a-prime | clm_v4 |
|---|---|---|---|---|
| own 17 (anima-native) | ✔ (foundation borrow, chat default X) | ✔ | ✔ (substrate-research lane only) | ✔ |
| own 18 (simple stack SSOT) | △ (legacy 2-cond PASS_STRICT, P5 N-of-M v2 미적용) | △ | ✔ (P5 v2 PPR_v2=10/14=0.71 ∧ EMC=3/4 PASS) | △ (P5 v2 PPR_v2=1/14=0.07 ∧ EMC=1/4 FAIL — small-sample) |
| own 22 (mandatory report) | ✔ | ✔ | ✔ | ✔ |
| own 24 (single SSOT) | △ (V4 evaluator floor mirror gap) | △ | △ | △ |
| own 27 (Safeguard Paradox) | ✔ | ✔ | ✔ (substrate-research lane only) | ✔ |
| own 28 (anti-Goodhart, V6 pending) | ✔ private (public promote 차단) | ✔ private | ✔ private | ✔ private |
| own 30 (ckpt preservation) | ✔ | ✔ | ✔ | ✔ |
| own 31 (dancinlab org SSOT) | ✔ | ✔ | ✔ | ✔ |
| own 33 (trinity compliance) | ✔ (3-axis self-check PASS) | ✔ | ✔ | ✔ |
| own 34 (chat lane wrapping 0) | ✔ (selftest grep 0 hits) | ✔ | ✔ | ✔ |
| own 36 (HF upload mandate) | ✔ in-progress | ✔ in-progress | ✔ | ✔ |
| raw#9 (hexa-only) | ✔ | ✔ | ✔ | ✔ |
...
### Violation 1 — own 24 single SSOT mirror gap (Step 2)

**verbatim** (own 18 c3-aggregation-rule-ssot mandate-mirror):
> "본 aggregation rule 변경 시 c3-measurement-cli (sub_baseline_ensemble verdict
> 계산) + V4 evaluator + 모든 BG-K* verdict emit 코드 mirror 의무 (own 24 single SSOT)"

**위반 사유**: own 18 c3-aggregation-rule SSOT 가 본 cycle 두 차례 evolve —
...
한정 — own 18 P5 N-of-M v2 적용 시 PPR_v2 + EMC ≥ 3 of 4 별도 retest 필요.
...
2. V4 evaluator 코드 patch — own 18 최신 SSOT (현재 P5 N-of-M v2:
   PPR_v2 ≥ 0.6 ∧ EMC ≥ 3 of 4) mirror
3. BG-K* re-evaluate with P5 N-of-M v2 — verdict downgrade (own 18 strict
   미달 시) 또는 PASS_STRICT_C3 promote (P5 N-of-M v2 PASS 시) 결정
4. ledger entry append (own 24 mandate raw#82 정합) — `downgraded_at` field +
   downgrade_reason "own 18 P5 N-of-M v2 mirror retest"

**severity**: warn (Phase 1 — P4 hybrid mirror 권고) → block (Phase 2 — V4
evaluator floor own 18 SSOT mirror 의무화).

---

## 4. 가장 큰 risk mandate + mitigation

**가장 큰 risk** = own 28 (anti-Goodhart) V6 awareness probe pending —
...
- (d) own 18 P4 hybrid mirror (Step 2 violation 해소) + V4 evaluator floor
  patch — single-axis floor 강화

---

## 5. own 33 mandate-2 self-check 결과 (3-axis)
...
- **(a) D_X 정합**: 본 doc 은 D_emergent-consciousness (의식 측정 verdict
  ...
  enforce). D5 Bifurcation 정합 ✔ (own 18 P4 hybrid 신규 path 발견).
- **(b) own_X 정합**: own 33 (trinity 무조건 준수) self-application ✔.
  own 22 (mandatory report) 정합 ✔ (본 doc 자체 mandatory report). own 24
  (single SSOT) 정합 ✔ (본 doc = trinity check SSOT 한곳). own 32 (trinity
  bundle SSOT) 정합 ✔.
...
1. **본 sweep 시점 V6 awareness probe 미land** — own 28 anti-Goodhart
   single-axis floor risk 최대. PASS_STRICT_C3 emerge 선언 전 V6 lane
   별도 cycle fire 권고 (block 아니지만 strong mitigation)
2. **own 18 P5 N-of-M v2 V4 evaluator mirror 미land** — Step 2 violation
...
3. **paradigm-a-prime PASS verdict 는 synthetic_fallback chat-cap proxy
   ...
   (own 18 c3-aggregation-rule-ssot honest-c3 명시)
...
8. **본 sweep 자체 own 33 mandate-7 retroactive sweep instance** — cycle close
   시점 시행, main session 매 응답마다 sweep X (latency overhead)
9. **own 36 mandate-5 enforcement (anima audit `--mandate-36` flag) 미land** —
...
- `.roadmap.law` own 17/18/24/28/33/34/36 cross-ref
...
- `state/anima_km_qwen7b_h100_2026_05_08/verdict.json` (BG-KM-QWEN-7B
  PASS_STRICT verdict — 본 sweep 대상)
- `state/anima_model_attempts_ledger.jsonl` (own 24 SSOT ledger)
- `tool/anima_cli/chat.hexa` (own 34 mandate-2 selftest grep)
