...
```
gh api repos/dancinlab/qmirror/actions/secrets         → {"total_count":0,"secrets":[]}
gh api repos/dancinlab/sim-universe/actions/secrets    → {"total_count":0,"secrets":[]}
gh api repos/dancinlab/hexa-bio/actions/secrets        → {"total_count":0,"secrets":[]}
gh api repos/dancinlab/honesty-monitor/actions/secrets → {"total_count":0,"secrets":[]}
```
...
1. **Set HF_TOKEN secret on each of 4 repos** (write-scope token from https://huggingface.co/settings/tokens):
   ```
   gh secret set HF_TOKEN --repo dancinlab/qmirror         --body "hf_xxx..."
   gh secret set HF_TOKEN --repo dancinlab/sim-universe    --body "hf_xxx..."
   gh secret set HF_TOKEN --repo dancinlab/hexa-bio        --body "hf_xxx..."
   gh secret set HF_TOKEN --repo dancinlab/honesty-monitor --body "hf_xxx..."
   ```
...
3. **Re-trigger** all 4: `gh workflow run sync-to-hf.yml --repo dancinlab/<repo>` and verify SUCCESS.
