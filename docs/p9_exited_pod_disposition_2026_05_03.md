...
**Substrate**: local `docs/`, `state/`, `git log`, sister-repo `anima-tribev2-pilot/`. RunPod inventory taken from earlier-this-session GraphQL snapshot. HF mirror check (per user): no `need-singularity/*` repo for any of these 6 pods; `huggingface-cli` not installed locally so no independent re-confirmation.
...
2. **HF mirror check not independently re-run.** `huggingface-cli` not installed in this audit environment; relied on user-stated finding ("`need-singularity` org has only 7 `clm-v4-sft-*` repos — none of the 6 EXITED pods have an HF mirror"). If user wants belt-and-suspenders, run `huggingface-cli repo list need-singularity` + `… dancinlife` after waking.
