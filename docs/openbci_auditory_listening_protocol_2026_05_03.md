...
> raw#9 NO .py on Mac (script lives as .hexa wrapper; ubu1 .py allowed under @resolver-bypass)
> raw#10 honest C3 — synthetic harness mode, no actual capture executed
...
Two delivery options (raw#9: prefer .hexa on macOS, .py only on ubu1):
...
let DEFAULT_PORT          = ""         // raw#15: must be passed
...
forbidden under raw#9 — .hexa wrapper invokes via ssh ubu1 'python3 ...'")
...
2. **Near-real-time** — `lsl_capture.hexa` writes `.npy` chunks every 1 s;
   ...
   `phi_stream.jsonl` with ~1 s lag. raw#10 honest C3 — **not phenomenal-tier**.
...
```
   ...
   QC3 blink         | Fp1/Fp2 amplitude > 100 µV peaks /min    | < 30 / min
                     | (high-pass > 1 Hz first; raw#10 honest:   |
...
- raw#10 honest — overshooting the floor invites cycle FAIL despite real signal
  being present.
...
## §8 Three honest C3 caveats (raw#10)
...
2. **OpenBCI GUI ↔ LSL transport ≠ phenomenal-tier closed-loop.**
   `lsl_capture.hexa` raw#10 caveats (lines 57-66 of that file) carry through
