...
**Scope**: curation only — no new generation (own 16 cost discipline), no corpus commit (own 34 mandate-2 wrapping 0)
**Trinity**: D-axis (D1 SCOPE_CLAMP probe) / own-axis (own 36 mandate-2 + own 37 lifecycle) / H-axis (avoid prior 102MB git track regression)
...
|---|---|---|---|---|
| `state/anima_persona_tier_a_2026_05_08.txt` | 104 MB | 1,478,588 | persona chat-template (own 20 format) | NO (gitignore L317-318, 326) |
...
**Total active curation surface**: ~122 MB. **All large artifacts confirmed gitignored** (own 36 mandate-2 wrapping = 0 holds).
...
- anima persona chat-template (30 MB, own 20 format)
...
Some prompts use bare strings (`안녕하세요`, `한국어 가능?`) while others use chat-template (`사용자: ... | 도우미:`). own 20 chat-template format mandate says everything should be chat-template. Bare strings should be wrapped or split into a separate "raw greeting" sub-corpus.
...
3. **Diversify preference prompts**: regenerate iter2 with ≥ 100 unique prompt stems (sample from `[anima 우주뇌지도]` opener pool) instead of the current ~10. Resolves Q3 — but defer to next cycle (own 16: requires LLM gen, costed pass needed).
...
- own 16 cost discipline: held — 0 LLM API calls in this iter, only file inspection.
- own 34 mandate-2 wrapping 0: held — corpus files NOT staged, NOT committed; only this doc (under 1 MB) is committed.
- own 36 mandate-2: held — all 122 MB of corpus surface is gitignore-covered (`state/anima_*_persona_*.txt`, `state/anima_*_pairs_*.jsonl`, `state/anima_*_tier_*.txt`).
- own 37 visibility lifecycle: not relevant to this iter (no HF promote action).
