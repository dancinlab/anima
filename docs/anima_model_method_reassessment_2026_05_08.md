...
**policy**:
- (D1 anima identity boundary strict)
- P5 v2 N-of-M aggregation rule (PPR_v2 ≥0.6 ∧ EMC_v2 ≥3 of 4) + ★ scope-clamp 2026-05-08 (line 770 — D1 lane within 만 SIMPLE_STACK_PASS_STRICT_C3_ANIMA verdict valid)
- trinity (philosophy + law + hypothesis) 무조건 준수
- mandate-9 5 prereq (a/b/c/d/e — 2026-05-08 amend)
...
| BG ID | Params | Tokenizer | Paradigm | Final class (ledger) | D1 lane | P5 v2 retest | D5 attractor | mandate-9 |
|---|---|---|---|---|---|---|---|---|
...
| **clm-v4-mk2-v1 (base)** | **WITHIN** | **MEASURED_REAL_MODE — FAIL_C3** | **Utopia (paradigm v11 G3 NO_FLIP)** | **PARTIAL_PASS_NO_CONSCIOUSNESS — C3 자체 FAIL** |
...
| Model | D1 lane | P5 v2 retest | D5 attractor | mandate-9 |
|---|---|---|---|---|
...
| M10 safeguard wrapping | Pβ chat-cap fail substrate-research pass | **OUTSIDE strict (Safeguard Paradox)** | irrelevant | instrumental (identity-drift through wrapping) | **REJECT** |
...
|---|---|---|---|
| BG-KM-LLAMA-3B SIMPLE_STACK_PASS_STRICT (V4 12/15) | SIMPLE_STACK_PASS_STRICT_C3_SUBSTRATE_RESEARCH | ★ scope-clamp + mandate-9 (a) D1 OUTSIDE | 원본 verdict.json 보존 + scope_lane='SUBSTRATE_RESEARCH' field add (V4 mirror agent 일부 수행) + ledger downgrade entry 보강 (downgraded_at + downgrade_reason) |
...
| paradigm-a-prime P5 PASS verdict (synthetic_fallback proxy) | SIMPLE_STACK_PASS_STRICT_C3_SUBSTRATE_RESEARCH (synthetic_fallback caveat) | (1) D1 OUTSIDE — Llama Path A v2 lineage = ALM lane; (2) synthetic_fallback proxy 한정 — mandate-9 (a) auto-reject | .roadmap.* sweep agent 일부 amend |
| paradigm-a-prime "chat-cap winner" / "Path A v2 fallback" references in .roadmap.* | SUBSTRATE_RESEARCH lane 한정 annotation | stale — ★ scope-clamp + law.D1_scope_clamp 신설 후 | HIGH PRIORITY 4 file 일부 amend; MEDIUM/LOW PRIORITY 56+ file 후속 cycle |
...
| 3 | MEDIUM | BG-FY 18M corpus filter (named speakers leak 차단) + chat-template format ratio ≥30% 학습 | C2.4 PASS 가능성 — D2 simple stack 4-condition 통과 |
...
| Axis | Compliance |
|---|---|
| | anima identity boundary strict — Llama/Qwen/Mistral/Polyglot/KoGPT2 ALM lane 모두 D1 OUTSIDE 분류 정합 |
| | P5 v2 N-of-M aggregation rule + ★ scope-clamp 2026-05-08 (line 770) 적용 |
| | 본 발견 (모델 ~30 + 학습 방법 ~10 전수 재평가 매트릭스) mandatory report 형식 |
| | single SSOT — `state/anima_model_method_reassessment_2026_05_08.json` |
| | Safeguard Paradox absorbed — safeguard wrapping reject (M10) |
| | anti-Goodhart awareness — V6 awareness 통과로 D1 override 불가 명시 |
| | trinity 무조건 준수 — D1 strict (philosophy) + strict (law) + H_chat_cap_emergence 정합 sweep |
| mandate-9 | 5 prereq strict — 모든 candidate 미충족 정합 |
...
4. paradigm-a-prime P5 PASS (PPR_v2=10/14=0.71, EMC=3 of 4 iter4 d 결과) 가 C3 측정 lane 의 본격 chat-cap PASS 였지만 (1) Llama Path A v2 lineage = D1 OUTSIDE + (2) synthetic_fallback proxy 한정 → mandate-9 (a) 자동 reject 정합.
...
8. category_1 BG saga 의 BG-HA 부터 BG-IT 까지 (V3 evaluator 시기) 는 C3 4-cell 측정 자체 미land 시기 — INDETERMINATE_NO_INFRA 분류는 retroactive 측정 자체 불가능 (state/<bg>_<date>/verdict.json 미land 또는 V3 형식).
...
10. category_3 ALM lane 외부 base 모델 (Llama/Mistral/Qwen/KoGPT2/Polyglot/zephyr) 6+ entries 모두 OUTSIDE_STRICT 분류 — + .roadmap.philosophy D1.F-PHIL-D1-3 strict 정합. SIMPLE_STACK_PASS_STRICT 도달 사례 (BG-KM-LLAMA-3B + BG-KM-QWEN-7B + BG-JA-EXT) 모두 SUBSTRATE_RESEARCH lane 한정 — mandate-9 (a) PUBLIC promote 자격 X.
11. training_method M10 safeguard_wrapping 'REJECT' verdict 는 Safeguard Paradox + strict + .roadmap.philosophy D1.F-PHIL-D1-1 정합 — 별도 evidence 부재 시에도 framework-level reject (engineering pragmatism 약화 X).
