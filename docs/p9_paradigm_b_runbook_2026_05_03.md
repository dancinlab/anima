...
> **constraints**: raw#9 hexa-only after Phase 3 entry; raw#10 honest scope; raw#37 transient `/tmp` emit pattern; POLICY R4 (no scope creep into current Phase 2 baseline)
...
**No tools (.hexa) created** — raw#9 hexa-only restriction holds for this cycle (doc-only scaffolding).
...
| **P4** | EEG-side `anima_phi_v3_eeg.hexa` tool ported | **DEFERRED** — raw#9 hexa-only restriction in current cycle; spec is stable (this doc §3.3 has reference Python, can wrap to hexa in next cycle) | spec §3.3 in this doc |
...
7. **No git-commit of EEG data**: `.mat` files are not to be committed (would balloon repo by 37 GB+); they live on ubu1 SSD only. State-summary JSONs (manifest, phi histograms, train/val/test splits) ARE committed per raw#15 SSOT.
8. **Hexa wrapping (raw#9)**: when implementing P4 (`tool/anima_phi_v3_eeg.hexa`), follow the established `tool/anima_phi_v3_canonical.hexa` template — shebang to bash, transient .py emit to `/tmp/*.hexa_tmp`, $0 cost, hexa is the SSOT.

---

## §8 raw compliance summary

- **raw#9 hexa-only**: this doc + sample data only — NO .py created in this cycle. ETL spec includes reference Python (§3.3) but it lives in this doc only, not in `tool/`. Implementation deferred to a separate cycle that will use hexa wrappers.
- **raw#10 honest scope**: §6 explicit on what is gated and pending; §5.2 honest about under-determined Cov risk + shrinkage mitigation; defers to spec §7 for Hard Problem caveats (10 listed there, not duplicated).
- **raw#15 SSOT**: this runbook is the operational SSOT for Paradigm B; spec doc remains the conceptual SSOT. Future cycles reference both.
- **raw#37 transient emit**: download script emits to `/tmp/`; future tools follow `*.hexa_tmp` pattern.
- **raw#71 falsifier**: §5.3 references spec §5.3 falsifier (numeric, |Δφ|/φ ≤ 0.30 on ≥80% of held-out prompts).
- **raw#91 honest C3**: spec doc has 10-caveat C3; this runbook does not duplicate, references it.
