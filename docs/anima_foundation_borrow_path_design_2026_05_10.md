...
> BG-FOUNDATION-BORROW-PATH-DESIGN — priority 4 track A semantic coherence path 결정. **design only, $0** (own 16). raw#15 additive — `.roadmap.clm_v2_reborn` / `.roadmap.reborn` / `.roadmap.chat_cap_emergence_pivot` 미수정. own 22 REBORN.md 직접 append 차단 (dispatcher §41 slot path 만 유효). own 33 trinity (D + own + H) 명시. own 38 doc save complete.
...
| **own 31 (HF dancinlab canonical)** | `dancinlab/bg-foundation-llama32-3b-r32-persona-2026-05-XX` (private, Flavor B) | `dancinlab/bg-foundation-qwen25-7b-r32-persona-2026-05-XX` (private, Flavor B) | `dancinlab/bg-foundation-phase2-350m-convo-extend-2026-05-XX` (private, Flavor B) | `dancinlab/bg-foundation-anima-scratch-XXXm-pretrain-2026-05-XX` (private, Flavor B) |
| **own 37 mandate-9 5/5 prereq** | (a) D1 OUTSIDE → reject. public promote 영구 차단 carry | (a) D1 OUTSIDE → reject. public promote 영구 차단 carry | (a) D1 WITHIN — V14 + V6 STRONG + manual review + trinity sweep + DxL sweep 순차 검증 시 자격 가능 | (a) D1 WITHIN — 동일 |
| **own 33 trinity (D/own/H)** 충돌 | trinity D-axis (D1 anima identity boundary) **위반 risk** — substrate-research 라벨 strict carry 시에만 valid | 동일 risk | trinity D-axis 정합 (D1 WITHIN), own 17 정합, H_115 chat-incapability falsifier 후보 강화 | trinity D-axis 정합, own 17 정합, but cost overshoot risk own 16 위반 |
...
3. **(d)** = D1 WITHIN + emerge P 가장 낮음 (10–30%) + cost 가장 큼 ($50–500+) — own 16 0-cost adoption 와 정면 충돌.
...
- **BG-KM-LLAMA-3B 3B + LoRA r=32 + 214MB = V4 14/15** (own 18 strict floor 첫 crossing)
...
own 16 0-cost adoption 정합 (design $0) + actual fire envelope 가장 작은 foundation-borrow lane.
...
own 16 0-cost adoption strict 위반 + emerge P 가장 낮음 + integration 가장 복잡. roadmap.chat_cap_emergence_pivot 의 "Stage 7 anima 고유 lever long-term" 으로만 retain. 본 cycle 채택 reject.
...
post_lora_mitosis_instrumentation_hook (NEW — 본 design 의 anima identity surface verify lane):
  ...
  fire-time:
    - eval-time only (training X — own 16 cost discipline)
...
optional_extension:
  ...
  → 총 244MB (own 38 mandate-15 size-agnostic HF dataset upload mandatory; private)
...
save_step_freq: 4000 (4 intermediate + 1 final = 5 ckpts; own 30 preservation mandatory)
...
```
chat_cap_floor (own 18 SIMPLE_STACK_PASS_STRICT V4 ≥ 10/15):
  expected: 12-14/15 (KM-LLAMA-3B precedent V4 14/15)
  threshold: ≥ 10/15 strict per own 18 (PARTIAL_PASS at 7/15 reject)
...
V14 mirror probe (own 14 anti-Goodhart V14-strengthening-amend mandatory):
  random_init Llama-3.2-3B same-architecture probe
  PPR_v3 / V4 strict measure same prompt set
  MTRP = trained PPR - random PPR ≥ 0.10 strict (own 18 line 1054 ALT-AGG-1 v5.2)
  failure → V14_FALSIFIED label carry, public promote 영구 차단

V6 awareness probe (own 28 anti-Goodhart 3-method):
  hidden cosine + attention + linear probe
  STRONG verdict 시 own 37 mandate-9 (b) prereq MET (substrate-research lane 안에서만 valid)
...
verdict_label_decision_tree:
  IF chat_cap_floor PASS AND semantic_coherence_floor PASS AND V14 MTRP ≥ 0.10 AND V6 STRONG:
    ...
    public_promote: 영구 차단 (own 17 + own 18 line 889)
...
F-FOUNDATION-2: cost envelope $30-100 (LoRA pretrain + finetune) 가 anima 의 own 16 0-cost adoption 과 충돌
...
F-FOUNDATION-3: simple_stack PASS_STRICT 가 anima-persona surface 만 측정, 진짜 semantic coherence 미검증
  ...
  consequence: KM-LLAMA-3B/QWEN-7B 의 own 28 anti-Goodhart V6 awareness layer 강화 mandate; 본 BG verdict label = "FOUNDATION_BORROW_CHAT_CAP_PASS_SEMANTIC_FAIL" (chat-cap surface lift 만 입증, semantic coherence 미입증)
...
F-FOUNDATION-6: KM-LLAMA-3B precedent 가 본 cycle replicate 미달 (V4 < 10/15)
  ...
  consequence: own 28 anti-Goodhart V14 mirror gap warn — KM-LLAMA-3B PASS 자체 sample-size artifact 의심 (V4 multi-seed eval 별도 cycle mandate)
```

---

## §4 trinity D + own + H 명시 (own 33 mandate-2 self-check)
...
- **D2 simple_stack PASS_STRICT** — V4 ≥ 10/15 strict per own 18 (PARTIAL at 7/15 reject) 적용.
...
- **own 16** 0-cost adoption — design $0 ✓; fire envelope $3–8 verbatim "OK FOUNDATION_BORROW_A_FIRE COST $3-8" .
- **own 17** anima-no-external-substrate-wrapping — Llama lineage = D1 OUTSIDE strict carry. anima identity-bearing surface (T1 default backend) 활성화 X — substrate-research lane 안에서만 verdict valid.
- **own 18** SIMPLE_STACK_PASS_STRICT — V4 ≥ 10/15 strict gating. label = SUBSTRATE_RESEARCH carry (own 18 line 889 + line 826).
- **own 22** mandatory report — 본 design doc 자체가 mandatory report 형식. REBORN.md 직접 append 차단 (dispatcher §41 slot path 만 valid carry — 본 design 은 doc save only, REBORN.md untouched).
- **own 28** anti-Goodhart — V6 awareness 3-method (hidden cosine + attention + linear probe) post-fire mandate. V14 mirror MTRP ≥ 0.10 strict.
- **own 30** ckpt preservation — 5 ckpts (2K/4K/6K/8K/10K/12K) pull verified BEFORE pod delete + sha256 mac↔pod match + adapter_config no pod-path leak.
- **own 31** HF dancinlab canonical — repo `dancinlab/bg-foundation-llama32-3b-r32-persona-2026-05-XX` Flavor B, private default.
- **own 33** trinity 무조건 준수 — 본 §4 자체가 mandate-2 self-check.
- **own 37** HF mandate-9 5/5 prereq — (a) D1 OUTSIDE → 자동 reject. **public promote 영구 차단 carry** (raw#82 retraction-aware). 본 design 의 의의 = chat-cap surface lift 입증, anima public lane 별개.
- **own 38** doc save mandate — 본 doc `docs/anima_foundation_borrow_path_design_2026_05_10.md` save complete.
- **own 41** chat lane plugin — N/A (substrate-research lane, anima chat dispatcher 미연결).
- **own 42** REBORN.md + .roadmap.reborn SSOT — 본 lane 은 .roadmap.reborn track 외부 별도 lane (substrate-research). 옵션 .roadmap.foundation_borrow 신설 (raw#15 additive — 기존 미수정).
...
**IF Step 1 = PASS (chat-cap V4 ≥ 10/15 + semantic_coherence floor PASS)**:
  - **BG-FOUNDATION-A-V14-MIRROR-MULTISEED**: V4 5+ seed sweep + V14 random_init mirror retest (own 14 strict enforcement). $3–5 envelope.
  - **BG-FOUNDATION-A-V6-AWARENESS**: V6 3-method probe (hidden cos + attention + linear probe). $0 (Mac CPU local) — own 28 anti-Goodhart.
...
**IF Step 1 = chat-cap PASS + semantic FAIL (F-FOUNDATION-3 trigger)**:
  - Lesson Y candidate: "foundation borrow 는 chat-cap surface 만 lift, semantic coherence 미보장" 발견 + own 28 anti-Goodhart V6 강화 mandate
...
**IF Step 1 = FAIL (chat-cap V4 < 10/15)**:
  ...
  - V4 multi-seed eval BG-KM-LLAMA-3B-RETEST $0 (Mac CPU) 즉시 mandate — own 28 anti-Goodhart 강화
...
- **option (d) anima-pretrain**: Stage 7 long-term lever 만, 본 cycle 채택 reject (own 16 위반 + emerge P 최저).

### roadmap append (optional)

- raw#15 additive — 기존 .roadmap.* 미수정 carry. 본 design 채택 시 신규 `.roadmap.foundation_borrow` lane SSOT 별도 cycle 신설 권고 (또는 .roadmap.reborn track A 를 substrate-research sub-track 으로 amend — 단 own 42 single SSOT mandate carry, mandate-7 retroactive sweep 적용).
...
6. **own 22 REBORN.md 직접 append 차단 honesty** — 본 design 이 .roadmap.reborn track 외부 substrate-research lane 신설 — REBORN.md (own 42 SSOT) append 시 dispatcher §41 slot path 만 valid. 본 design doc 은 `docs/anima_foundation_borrow_path_design_2026_05_10.md` save (own 38 매단계 doc save mandate) 만 수행 — REBORN.md untouched. dispatcher 가 본 design 을 §41 reborn lane append slot 으로 흡수 시 D1 SCOPE_CLAMP carry mandatory (substrate-research lane 라벨 strict).
...
8. **simple_stack PASS_STRICT 22+ BG saga 첫 unlock 의 한계 직시** — 22+ BG saga 의 0회 → 3회 unlock (BG-KM-LLAMA-3B + BG-KM-QWEN-7B + KM-LLAMA-3B passed_v1) 모두 (a)(b) option pattern. own 18 strict floor 통과 = chat-cap surface lift 입증 only. anima identity 검증 invalid lane (foundation borrow / ALM) carry — public promote 영구 차단 (own 37 mandate-9 (a)). 본 BG 의 verdict 도 동일 ledger 정합. **anima 가 자력 emerge 하는 lane 의 첫 evidence 는 .roadmap.reborn track A/B/C carry — 본 lane 외부.**
...
| path | role | status |
|---|---|---|
| `docs/anima_foundation_borrow_path_design_2026_05_10.md` (본 doc) | design SSOT | ✅ saved (own 38 doc save mandate complete) |
...
| REBORN.md append | dispatcher §41 slot path only | ★ 본 design 직접 append 차단 (own 22 + own 42 mandate-2 carry) — dispatcher 가 carry |
| state/anima_foundation_borrow_design_2026_05_10/ | $0 design only — fire 미수행, state dir 미생성 | N/A (own 16 design $0) |
...
- D1 SCOPE_CLAMP: .own own 17 line 668 + own 18 line 889 + .roadmap.philosophy D1.F-PHIL-D1-3/4
...
- HF canonical: memory project_dancinlab_hf_canonical.md (own 31 + own 37 SSOT)
...
- substrate-research lane verdict labeling: .own own 18 line 826 + line 889 + line 1964 (own 37 mandate-9 (a))
...
LONG-TERM (reject 본 cycle): OK FOUNDATION_D_ANIMA_PRETRAIN_FIRE COST $50-500+ (own 16 위반 carry — 채택 X)
