...
- Offline analysis pipeline (preprocess → epoch → high-gamma coherence → permutation null → composite verdict), $0 ubu1 CPU, hexa-on-Mac with raw#37 transient ubu1 helper.
...
raw#9 (md only spec; hexa-on-Mac pipeline + raw#37 transient ubu1 helper for numpy/scipy). raw#10 (8 caveats). raw#15 (repo-relative paths). raw#71 (F-PHASE-E-1, F-PHASE-E-2 pre-registered with locked thresholds before measurement). raw#91 (F1_v2 raw vs F2-override split disclosed; Phase E PASS ≠ ceiling lift).
