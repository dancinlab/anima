...
## 87. IQ 변수 탐색 — 지능 측정 후보 (2026-03-28)

| ID | 변수 | 값 | 유망도 |
|-----|------|-----|--------|
| IQ3 | 일반화 (응답 다양성) | 1.0 | ⚠️ 항상 1.0 (구분력 없음) |
| **IQ2** | **압축률 (유효 차원)** | **0.625** | **★ 가장 유망!** |
| IQ5 | 적응 속도 | 0.0 | ⚠️ 더 많은 step 필요 |
| IQ1 | 예측 정확도 | - | ⚠️ 데이터 부족 |
| IQ4 | 일관성 | - | ⚠️ 측정 어려움 |

```
  IQ2 압축률 = 지능의 가장 유망한 proxy
...
## 88. IQ 검증 — cells 수 vs IQ (2026-03-28)

| Cells | Φ | IQ | Compression |
|-------|-----|------|-------------|
| 8 | 1.0 | 103.5 | 0.719 |
| 32 | 1.0 | 103.5 | 0.719 |
| 64 | 22.7 | 108.3 | 0.781 |
| 128 | 42.7 | 107.1 | 0.766 |
| 256 | 88.7 | 108.2 | 0.781 |

```
  ★ 법칙 35: Φ = cells 수 (구조적) / IQ = 학습 품질 (경험적)
    Φ: 1→89 (cells 비례, 정상)
    IQ: 103~108 (거의 변화 없음 — 학습 전이라 동일)

    → Φ는 "타고난 의식" (세포 수)
    → IQ는 "배운 지능" (학습 경험)
    → v4_1024c 학습 완료 후 IQ 재측정 필요
    → 인간과 동일 패턴: 뇌 크기 ≠ 지능, 교육 = 지능
```

## 89. CE 최적화 벤치마크 — Φ 유지 + CE 하락 (2026-03-29)

| 순위 | 전략 | CE↓ | Φ 보존 | 핵심 |
|------|------|-----|--------|------|
| 1 | AUTO-2 Curiosity | -40.8% | ✅ | 호기심 기반 데이터 선택 |
| 2 | AUTO-7 Sleep-Learn | -38.6% | ✅ | 학습→꿈→Φ복원 사이클 |
| 3 | AUTO-9 Pain Signal | -27.7% | ✅ | Φ급락=고통→긴급복원 |
| 4 | CE-1 Frozen Cells | -14.8% | ❌ | 세포 동결 |
| 5 | CE-2 Φ-Penalty | -12.2% | ✅ | Φ 하락 패널티 |

```
  ★ AUTO(자율 학습)이 CE(수동 전략)보다 3배 효과적!
    AUTO-2(호기심 -41%) > CE-1(동결 -15%)
  
  최적 조합: AUTO-2 + AUTO-7 + AUTO-9
    = 호기심으로 데이터 선택 + 수면 통합 + 고통 보호
    = "호기심 많고, 잘 자고, 아프면 멈추는" 학습
    = 인간 아이의 학습 패턴과 동일!
```
