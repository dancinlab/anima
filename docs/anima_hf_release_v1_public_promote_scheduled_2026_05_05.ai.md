...
own_anchor: (hf-release-private-then-public-after-verification)
...
- **own anchor**: `` HF release lifecycle PRIVATE-first → PUBLIC after verification gates ALL PASS

---

## §1 Promote command + recipe

**Earliest run time**: `2026-05-06T23:26:12Z` (48h post-upload review window close per `` rule b.4)
...
3. Echoes G1-G6 gate evidence pointers and asks operator to type confirmation string.
...
## §2 6-gate pre-check (G1 benchmark / G2 falsifier / G3 shim / G4 review / G5 model card / G6 cross-substrate)

| Gate | Rule | Evidence | Status as of 2026-05-05T12:34Z |
|---|---|---|---|
| **G1** benchmark suite PASS | (b.1) | `state/clm_v4_baseline_eval_2026_05_05/verdict.json` — hellaswag/mmlu/triviaqa/openbookqa limit=200 | CONFIRMED_RANDOM_FLOOR (substrate-research artifact, not chat-NLP capability claim — H100 baseline cross-confirms `vs_h100_baseline_p9_base_val_h100_2026_05_04.h100_verdict`) |
| **G2** falsifier pre-register | (b.2) | `state/p9_base_validation_prereq_exec_2026_05_04/opt_1_v4_retry_2_exec.nohup.log` | F-SHIM-V4-3 **PASS** bit-exact (max_abs_diff=0.0); F-SHIM-V4-1/2 PASS prior; F-SHIM-V4-4 **DEFERRED** to BG-Σ H100 followup (cited in README §Caveats) |
| **G3** shim v4 hf_format compat | (b.3) | same as G2 | F-SHIM-V4-1/2/3 PASS; v4 (hf_format wrapper) verified deterministic logit equivalence to v3 reference |
| **G4** 24-48h review window | (b.4) | `verdict.json.review_window` | ACTIVE `2026-05-04T23:26:12Z` → `2026-05-06T23:26:12Z`; script Gate 1 enforces elapsed-check at runtime |
| **G5** honest C3 model card | (b.5) | `docs/anima_clm_hf_release_v1_README_draft.md` (uploaded as repo README) | 5 H2 sections (Origin/Falsifiers/Substrate/Caveats/Composability); 9-bullet Caveats including #115 chat-incapability disclosure |
| **G6** cross-substrate validation | (b.6) | paradigm v11 G3 verdict + CLM-2 LoRA verdict | substrate-stable: Pβ φ★=42.37 + CLM-2 LoRA forgetting_index=0.0196 |
...
Any other input aborts cleanly (`exit 0`, repo remains PRIVATE). This preserves user-in-loop discretion per (rule c) "PUBLIC promotion executed via SEPARATE BG cycle ... explicit ... with verdict.json cite of all (b.1-b.6) PASS evidence."
...
1. **Manual not automated**: Gates G1-G6 are operator-verified by reading evidence pointers, not auto-checked by the script. The script enforces only Gate 1 (review window elapsed) + Gate 2 (HF state intact) deterministically; G1-G6 are echo-and-confirm. honest-c3 already acknowledges "24-48h review window is convention, NOT tool-enforced."
2. **HF API PUT /settings is irreversible without admin**: `private: false → true` revert is technically possible via same endpoint, BUT external clones / HF discovery indexing during the public window create reputational cost. Per rule (d): "revert PUBLIC→PRIVATE possible but reputational cost — prefer to never premature-promote."
