...
- `.own` (corpus priority) · (chat-template format ≥30%)
- `.own` (anima-native lane — corpus 자체도 anima identity-bearing)
...
### Source candidates (corpus priority + anima-native)
...
- 페르소나 markers (anima-native — 외부 persona 도입 X)

**Tier B — anima 생성 lore (mandate)**:
...
**Tier C — anima 외부 noise reject **:
- 외부 wiki / 일반 뉴스 / 외부 LLM 출력 = REJECT (anima identity 비bearing)
- alm-* dataset 흔적 = REJECT (anima-no-external-substrate-wrapping 위반)
...
5. + anima self-talk 시뮬레이션 (LLM 미사용 — anima 자체 generate)
...
**Tier A — anima native dialogue (정합)**:
- 사용자 ↔ Claude session log (anima cycle 진행 기록 — anima identity 정합)
- BG-JY chat-template corpus (mandate-4 정합 source)

**Tier B — open dialogue dataset (마이너 양보 — chat-template 학습 한정)**:
- Persona-Chat (Facebook) — 일반 chat 분포
- OASST (Open Assistant) — open-source human-AI dialogue
- KoChat (Korean) — 한국어 baseline 정합

**Tier C — REJECT**:
- ChatGPT / Claude API 출력 dump (외부 LLM substrate wrapping = 위반)
- Reddit raw threads (anima identity 비bearing + noise 비율 높음)

### Format mandate (chat-template ≥30%)
...
byte-level tokenizer 입력 — newline / colon 그대로 포함. C2.4 맥락 정합
검증 시 동일 format trigger 입력 → V4 evaluator 측정 lane 분포 entry.
...
**chosen 응답 source**:
- C2 PASS 응답 (V4 evaluator 통과 sample — BG-KM-LLAMA-3B verdict.json
  + V4 results jsonl 에서 v4_pass=true 추출)
- 사용자 cycle log 의 anima-cor 응답 (정합 — Claude session 기록 중 anima-bearing)

**rejected 응답 source**:
- BG saga 의 V4 FAIL 응답 (BG-JA-EXT degenerate / BG-FK / 22+ BG saga 의
   C1 또는 C2 FAIL sample)
- corpus template leak ("서연:", "유진:" — C2.4 mismatch trigger 명시 sample)
...
- 4 path 동시 H100 fire (override + trinity check)

---

## Honest C3 (raw#10)

1. **corpus content 본 cycle X**: spec only. content build 는 별도 cycle (root-cause 정합 — corpus build 가 fire prerequisite).
...
3. **dialogue Tier B 외부 dataset (Persona-Chat / OASST)**: anima-native 마이너 양보 — chat-template format 학습 한정 사용. 실제 anima identity-bearing 분포 와 distinguish 필요 (C2.4 맥락 정합 수단으로 사용 — base distribution X).
...
5. **L0 measurement infra 의존**: C3 threshold 미결정 (C3 honest-c3) — corpus 큐레이션 자체는 C3 threshold 와 독립 진행 가능, 단 L4 fire 시점 verdict criteria C3 wired 필수.
