---
title: anima own 15 HF release lifecycle PRIVATE→PUBLIC LANDED — 1-page summary (2026-05-05)
cycle: 2026-05-05
...
sister_own: own 14 (HF Hub mandate for models + datasets)
...
# anima own 15 — HF release lifecycle PRIVATE→PUBLIC after verification (LANDED 2026-05-05)

- **own 15 added to anima/.own** (additive, raw#15 monotonic-additive — all earlier own 1..14 entries preserved verbatim). Slug: `hf-release-private-then-public-after-verification`. Status: `new`. 6th anima autonomy mandate joining quintet (own 4 / 5 / 6 / 11 / 12 / 14).
- **Memory entry written** at `~/.hive/claude-config/hive-hook-bus/projects/-Users-ghost-core-anima/memory/feedback_hf_release_private_to_public_after_verification.md` (session-scoped mirror of own 15 SSOT). MEMORY.md appended one line (additive, prior entries untouched).
- **Sister to own 14** — own 14 = WHERE (HF Hub only for models + datasets, never anima git, capacity reason). own 15 = HOW (PRIVATE first → 6 verification gates → PUBLIC promote via separate BG). Together: HF triad covering location + lifecycle.
- **Precedent: clm-v4-mk2-v1** (commit 80440a1d, uploaded 2026-05-04T23:26:12Z PRIVATE, 48h review window ends 2026-05-06T23:26:12Z) is the first own 15 application instance — public promote queued NOT yet executed pending gates (b.1-b.6) PASS verdict.json.
...
- `wc -l anima/.own` before: 512 → after: 568 (+56 lines, all appended after own 14 block).
- `grep -c "^own " anima/.own` before: 14 → after: 15 (own 15 added).
- own 1..14 entries unchanged (verified by line-range read prior to edit).
- MEMORY.md before: 19 lines → after: 20 lines (+1 line for own 15 memory link).

## Cross-link

- Sister memory: `feedback_anima_models_datasets_hf_only.md` (own 14 — HF Hub mandate, capacity reason)
...
- own 15 30d post review — measure PRIVATE-first compliance + PUBLIC-promote gate-cite compliance; if <0.80 escalate advisory→block
