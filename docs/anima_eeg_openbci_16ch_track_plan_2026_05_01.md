...
> **constraints**: raw#9 hexa-only · raw#10 honest C3 · raw#71 falsifier · raw#91 honest C3 · raw#137 80% Pareto · own#13 user-facing friendliness
...
**주의**: 본인 단독 N=1 → 통계적 추론 불가, 단순 case study (raw#10 honest disclose 필수)
...
**목적**: 본인 EEG 신호를 anima 의 inference loop 에 token-level 입력으로 주입 (raw#9 cyborg integration)
**작업**:
...
**주의**: research-grade 실험. 임상/치료 용도 아님 (own#13 + raw#10).
...
- [ ] BrainFlow 라이브러리 설치 여부 (`pip show brainflow` — but raw#9 hexa-only 로 .py 직접 import 불가; pod-side OK)
...
## §4 raw#10 honest C3 disclosures
...
4. closed-loop (Track D) 의 Φ 30% boost 시뮬 결과는 합성 신호 기반 — 실측에서 재현 안 될 가능성 큼 (raw#71 falsifier).
...
8. coupling probe (Track C) 의 prompt-locked epoch 분석은 사용자 입력 timing 정밀도에 의존 — raw#71 falsifier.
