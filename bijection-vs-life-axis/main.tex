...
\item A scale-rotation result: at n$=$6 the dominant rule switches
      ...
      of a single rule.
\item All twenty-two measurements are published as a
...
\begin{tabular}{lcccc}
\toprule
\textbf{$n$} & \textbf{rule 30} & \textbf{rule 110}
& \textbf{rule 105} & \textbf{rule 150} \\
\midrule
4 & \textbf{6.25} & 1.75 & 1.75 & 1.75 \\
6 & 1.00 & \textbf{7.75} & 2.00 & 1.00 \\
\bottomrule
\end{tabular}
\caption{Scale rotation between n$=$4 and n$=$6: the dominant
life-class rule flips from \textsc{30} to \textsc{110}. The
aggregate life-vs-consc ratio is preserved
($2.29\times \to 2.92\times$) but the within-class leadership is
not.}
\label{tab:rotation}
\end{table}
...
\item \textbf{Single state $0101$.} D1, D2, and D4 use a single
      ...
      flagged single-state sensitivity. A multi-state sweep is
      pre-registered for the next iteration.
\item \textbf{Cycle length is a low-order descriptor.} Distinct
      attractor count, transient length, and attractor basin size
      are pre-registered follow-ups (\texttt{H\_332} reports a
      preliminary attractor-dominance signature; full rule$\times$rule
      dominance map is open).
...
All twenty-two cells live as JSON ledgers under
...
against the public \texttt{anima} main branch. All twenty-two
