...
- 2026-05-20 — **Phase 2 (B) — strict policy + forward-only governance LANDED** (`.wilson-tree.json` strict=true + AGENTS.tape `@D g_new_state_path` 신설). Pre-Phase-3 link-blast scan: PHILOSOPHY.tape **307 state/ refs** · AGENTS.tape **109** · HEXAD/**/PLAN.md **161** · HEXAD/**/README.md **77** = **654 historical link**. Retroactive `git mv` 156 dir 시 654 link 전부 break = `g6` append-only 정신 위반 anti-pattern. **Phase 3 (C 156 dir mv) + Phase 4 (D-tail 1,024 archive) 폐기** — forward-only path 채택. existing state/ 1,189 dir 그 자리 유지, REGISTRY.md = SSOT index, `@D g_new_state_path` = 신규 §N 경로 mandate. 다음 세션부터 wilson-tree PreToolUse strict guard + governance text 양면으로 enforce.
