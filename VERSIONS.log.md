...
| 0.13.1 | 2026-06-19 | 🗂️ PHASE 2 멀티엔진 ARCHIVE — core/engines/ 전체 → archive/engines-multiengine/ 보관(이력보존) + engine_cli resolve 단일 conv 상수 축약. 내부 리팩터(공개 무변경, engine_cli_smoke 169/0·h1205 byte-id). |
| 0.13.0 | 2026-06-19 | 🔧 GPU DECODE + CANONICAL core/ + 단일엔진 — 추론 decode GEMM→flame_mm.mm GPU 경로(#2386, CUDA cuBLAS·Mac byte-id) · canonical core/ 재구성(#2384)+pod 패키징 · 단일 production 엔진(conv) 노출정리(#2396, 멀티엔진 research-legacy) · thalamus PhaseField WIRED(H_1448) · hexa-lang cloud rent --gpu N fix(#3671). |
