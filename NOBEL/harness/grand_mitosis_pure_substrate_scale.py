...
CORE/engine_cli.hexa) by NOBEL/harness/grand_mitosis_pure_substrate.hexa; the
...
CORPUS_REAL = "archive/state_legacy/anima_phase1a1_color_cosmology_2026_05_12/consciousness_anchor.txt"
...
def load_corpus():
    rp = os.path.join(HERE, "..", "..", CORPUS_REAL)
    if os.path.exists(CORPUS_REAL):
        return open(CORPUS_REAL, "rb").read(), "REAL anima dialogue corpus (consciousness_anchor.txt, byte-level)"
    if os.path.exists(rp):
        return open(rp, "rb").read(), "REAL anima dialogue corpus (consciousness_anchor.txt, byte-level)"
...
def main():
    print("=== NOBEL G3 SCALE+REAL — MITOSIS=PURE-SUBSTRATE (REAL anima corpus, 5 seeds, N_TR=16k, $0 p7) ===", flush=True)
