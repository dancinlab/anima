...
- [x] M5 task primitives — `AGENT/DESKTOP/{task_primitives.hexa,task_primitives_smoke.hexa,TASK_PRIMITIVES.md}` 5 pub fn (`task_open_and_type`/`task_screenshot_app`/`task_open_read_close`/`task_find_and_click`/`task_multi_app_screenshot`) + `task_primitives_summary` · M1+M2+M3+M4 composition · risk 라벨 (soft 4 + hard 1 quit step) · ANIMA_DESKTOP_DRYRUN=1 dry-run 전이 · hexa parse 2/2 OK · DRYRUN smoke 4/4 contract-shape PASS
