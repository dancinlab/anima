"""AURA-DEEP app — 심부핵(보상·각성·기분) read 도달성 — 비침습 read는 깊이서 붕괴 확인(침습 필요) (runnable toy, numpy).
honest: toy. See verify/dbs_reach.txt."""
import numpy as np
G=10; N=G*G
...
if __name__=="__main__":
    for lbl,dep in [("M1 cortex 1.0",1.0),("thalamus 5.5",5.5),("VTA/LC 7.0",7.0)]:
        L=LF(256,7,dep,"meg"); x=patt(np.random.RandomState(1)); y=L@x+np.random.RandomState(2).randn(256)*np.sqrt(((L@x)**2).mean())/30
        print("%-16s read R2=%.3f"%(lbl,r2(x,inv(L,y,256))))
