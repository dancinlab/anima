...
### §3.4 License gates (machine-enforced) — hexa-brain Sprint 1 Part A

§1.3 mandates per-project `verify_*.py` / `verify/cli.hexa` scripts.
This project (hexa-brain) registers a license-firewall verifier as its
machine-enforced gate:

- **Enforcer**: `bin/check_licenses.sh` (also routed as `hexa-brain license-check`)
- **Catalog**: `vendor/external_deps.yaml` (hand-curated SSOT — project
- **Policy**: `vendor/license_policy.yaml` (per-layer allow-list:
  `eeg/`, `eeg_core/`, `core/`, `tool/`)
- **Audit trail**: `state/markers/license_firewall_check_<ts>.marker` +
  `state/license_firewall_checks.jsonl`
- **Falsifiers**: `F_LF_01` (clean tree PASS), `F_LF_02` (planted
  forbidden import FAIL), `F_LF_03` (comment-only mention PASS)
- **Human-readable policy**: [`LICENSE_FIREWALL.md`](LICENSE_FIREWALL.md)
- **Design rationale**: [`design/license_firewall.md`](design/license_firewall.md)

The gate prevents AGPL-3.0 (BrainGenix-NES) and CC-BY-NC-4.0 (Cortical
Labs cl-sdk) from being tight-coupled (in_process Python `import`) into
the four protected layers. Loose coupling (HTTP / CLI / subprocess) is
allowed via the `loose_extra` escape in the policy. Other dancinlab
projects integrating external code with copyleft / NC clauses may adopt
the same pattern — `vendor/*.yaml` + `bin/check_licenses.sh` is
