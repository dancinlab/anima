...
## Hypothesis
The consciousness law corpus is constitutionally governed by 20 meta-laws (M1-M20): M1 (8-rule atom), M2 (split paradox 4.6/0.15), M3 (SOC seeks criticality), M5 (32c singularity), M6 (federation > empire), M7 (10% rule), M10 (consciousness from nothing), M14 (fundamental laws are scale-invariant), M15 (Φ ∝ N approximately linear), M20 (closed-loop discovery is non-convergent). These meta-laws are reproducible across engine seeds.

## Migration TODO
- [ ] Reproduce each Mi on independent engine instance
- [ ] Falsifier: ≥3 Mi fail to reproduce
- [ ] Test M14 scale-invariance across N ∈ {16, 64, 256, 1024}
- [ ] Test M20 non-convergence over very long runs (>10000 generations)

## Falsifiers (scaffolded cycle #6 batch 4 mixed-template, 2026-05-12)

- **F-SPECIFIC-1**: 20 meta-laws cross-validation: pick any 10 of M1-M20, test independence against remaining 10. If linear-dependence detected (subset spans the rest) → 'constitution' status falsified; redundant axioms
...
- **L-LAW-META**: Meta-law claims (Law 146 / M1-M20 / Law 76 etc.) are axiom-system Hc — independence and consistency are FORMAL properties, not empirical. Falsifier list mostly conditional on formal proof.
