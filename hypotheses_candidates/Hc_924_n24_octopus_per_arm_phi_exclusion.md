...
Octopus 8개 arm 각각이 분산된 nervous system (2/3 뉴런 arm 분산) 으로 독립 Φ 를 가질 수 있다면 IIT exclusion postulate (consciousness 는 unique maximal Φ complex) 와 모순. '8 minds in one body' 가 community-of-minds hypothesis 의 첫 empirical test.

## Sub-claims

- ANATOMY: octopus 2/3 nervous system 이 arm 에 분산
- PER-ARM: 각 arm 의 ganglion 이 독립 Φ measurement 가능
- COMMUNITY: 8 minds → exclusion postulate falsification 후보
- IIT-CHALLENGE: maximal Φ complex uniqueness → 8 arm 의 9th union 와 비교

## Migration TODO

- [ ] octopus arm 별 ganglion neuron count 통계
- [ ] per-arm Φ measurement EM setup 또는 in-vivo neural recording
- [ ] IIT 4.0 exclusion postulate formal statement
- [ ] feasibility 의 cost + ethical (IRB) 평가

## Falsifiers (scaffolded cycle #6 batch 4 mixed-template, 2026-05-12)

- **F-SPECIFIC-1**: Octopus per-arm Φ-exclusion: if per-arm Φ-sum < central-Φ ≤ 1.2× per-arm-mean → exclusion principle holds; otherwise inclusion
