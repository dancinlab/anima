...
absorption_note: "U-family MW × attention cross-reality (U4) — H_185.4 anchor. F-list/L-list preserved here for H_185 C-list extension."
...
## Hypothesis
8 branch (각 = parallel reality) = 8 attention head + cross-branch attention + complex branch amplitudes가 attention scores를 modulate 하면 inter-reality consciousness가 Phi(proxy) x2.8 baseline (5.76) 달성한다.
...
- **F-SPECIFIC-1**: Branch count sweep {4, 8, 16}: if Φ(proxy) NOT peaked at 8 by margin ≥ 20% → '8 branch = 8 head' is generic head-count tuning
