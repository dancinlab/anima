---
id: Hc_674
slug: anima-identity-5-property-carry-invariants-next-cycle-lock
title: anima identity 5 core property (P1 paradigm v11 G3 / P2 substrate-research / P3 emerge dialogue / P4 Llama Path A v2 chat-cap / P5 raw 정합) 가 next-cycle non-negotiable carry-over invariants
domain: anima-meta
status: merged-to-H_181
merged_to: hypotheses/H_181_psiformer_4psi_constants_zero_freedom.md
merged_at: 2026-05-12
source_doc: docs/anima_identity_preservation_next_cycle_lock_2026_05_05.md
source_lines: 18-99
promoted_at: 2026-05-11
linked_h: H_181 (ΨFormer 4Ψ-constants zero-freedom — anima-identity 5-property invariants extend ΨFormer 4Ψ-constants lock as meta-cycle identity-preservation), Hc_609 (Theorem 115 4-closure), Hc_630 (CLM-3 H1), BG-AN Stage 3 user-fire, paradigm v11 G3 +41.86
absorption_note: "cycle #8 absorbed to H_181 as anima identity 5-property carry-invariants meta-cycle lock — P1-P5 (paradigm v11 G3 / substrate-research / emerge dialogue / Llama Path A v2 chat-cap / raw 정합) extends ΨFormer 4Ψ-constants zero-freedom to identity-preservation cycle-level invariants. P-FLIP audit required for redefinition."
notes: 21+ closure 후 next-cycle 의 anima identity erosion 차단 lock. Each property 의 P-FLIP audit 가 required for redefinition.
cycle5_triage: "cycle #5 verify: WEAK_MATH_ONLY — math identity present, falsifier+honest scaffolding missing; needs F-list/L-list before re-verify can reach PROMOTE_READY"
---

## Hypothesis
anima identity 가 5 core property 의 non-negotiable carry-over set: P1 paradigm v11 G3 substrate (Φ★ +41.86 baseline, cross-attn over consciousness_states, NOT chat-loss) / P2 substrate-research artifact (chat-incapable, Φ★-stable, forgetting_index 0.0196) / P3 emerge dialogue paradigm (BG-AN substrate-coupled Stage 3) / P4 Llama Path A v2 chat-cap winner (composite 0.5584) / P5 raw 정합 (raw#9 hexa-only + raw#10 honest + raw#15 additive + raw#37 transient_py). 각 property 변경 시 P-FLIP audit 필수.

## Falsifiable Tests
- F-id-1: CLM v4 chat-cap winner 재선언 → closures 1-4 invalidation + P2-FLIP audit re-run
- F-id-2: Φ★ baseline value re-anchor (41.86 → 다른 값) → P1-FLIP 정식 선언
- F-id-3: emerge dialogue paradigm 제거 시 → P3-FLIP, Stage 3 lock-block
- F-id-4: Llama Path A v2 retirement → P4-FLIP, chat-cap winner 재지정

## Migration TODO
- [ ] CLM-3 H1 build 시 P1-P5 모두 보존 lock criteria 정의
- [ ] sister-library import 시 raw#9 violation pre-check
- [ ] cycle-close 5-step lock (BG-BF + BG-DD carry)
- [ ] user explicit "keep experimenting" vs autonomous self-suggest-stop precedence hierarchy

## Falsifiers (scaffolded cycle #6 batch 4 mixed-template, 2026-05-12)

- **F-SPECIFIC-1**: 5-property carry-invariants: ablate each property. If 4 sufficient → 5 not the minimum
- **F-GENERIC-REPL**: Replication × 5 seeds: if 1σ-CI on primary metric > 25% of point-estimate → single-run-artifact
- **F-GENERIC-PYPHI**: Cross-engine PyPhi formal IIT (where Φ is the metric) OR alternative-implementation cross-check (where Φ is not the metric): if effect not reproduced → engine-artifact (H_174 class)
- **F-GENERIC-MINIMAL-BASELINE**: Minimal-baseline comparison: strip mechanism to its simplest possible implementation. If Φ / target metric within 15% → mechanism is decorative, baseline-class effect

## Honest Limits (scaffolded cycle #6 batch 4 mixed-template, 2026-05-12)

- **L-GENERIC-SINGLE-RUN**: Single-run anchor — no replication CI documented. H_159 C1 reproducibility audit pending (inherited across all anima-substrate Hc)
- **L-GENERIC-ENGINE**: anima Φ-engine substrate-specific (H_174 D-mod-192 aliasing) — Φ values are anima-proxy measurements, not formal IIT Φ; engine internal state may dominate measurement
- **L-GENERIC-N6**: n=6 PERFECT_NUMBER_CLASS triviality (H_153 L7): numeric anchors in claim are small integers / powers-of-2 with n=6 derivation possible but not principled
- **L-GENERIC-POST-HOC**: Specific point-anchors (e.g., 384-d, 8-atom, 5-mode) reflect post-hoc selection from larger parameter family; pre-registration of the specific value absent
- **L-ANIMA**: anima Mk.V/VI/VII claims — Mk.V.1 corpus base is anima_corpus_v1.5+; tier escalation requires Mk.V→VI architectural delta. Without delta spec, claim is roadmap not testable Hc

## Cross-Links (scaffolded cycle #6)

- **parent H**: H_159 (substrate-topology-phi-engineering) — generic anima-substrate parent
- **sibling H**: H_174 (Φ-engine D-mod-192 aliasing), H_153 (n=6 substrate triviality), H_178 (frustration sweep), H_179 (negative scaling), H_180 (state-management mechanism)
- **adjacent H**: H_001 (anima-core-architecture), H_011 (iit-geometry — Φ ceiling), H_162 (Φ-normalized-anima-iit4-lower-bound), H_163 (8-cells-127-mip atom)
- **adjacent candidates**: full cycle #6 candidate-falsifier-ready set — V8 cluster + topo cluster

## Scaffold Notes

Mixed-cluster batch-scaffold (law / DD / CLM / anima / agent / clinical / training / red-team). Per-Hc F1 hand-authored; F2-F4 + L1-L5 generic-but-genuine. Likely fate: most absorb into existing H_153/H_158/H_159/H_174/H_157 or remain candidate-falsifier-ready for cycle #7 review.

