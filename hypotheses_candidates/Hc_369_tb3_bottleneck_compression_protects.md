...
## Hypothesis
C→8dim→D 정보 압축 병목 (TB-3)이 CE gradient의 C 역류를 물리적으로 차단하여 Phi(IIT) 1.27 최대 달성 — 8차원만 통과 = C 본질만 전달 + 잡음 제거.

## Migration TODO
- [ ] bottleneck dim sweep (2/4/8/16/32)
- [ ] gradient flow 측정

## Falsifiers (scaffolded cycle #6 batch 3 V8-template, 2026-05-12)

- **F-SPECIFIC-1**: Bottleneck-compression depth sweep: if Φ peak NOT at specific compression ratio → bottleneck decorative
