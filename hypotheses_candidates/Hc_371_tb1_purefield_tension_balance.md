...
## Hypothesis
PureField tension = sqrt(|A-G|^2) * direction (TB-1) bridge가 Phi(IIT) 1.19 + CE 0.0855 양쪽 모두 개선하는 유일한 다리 — 강도(magnitude) + 방향(direction) 동시 전달이 핵심.

## Migration TODO
- [ ] direction encoding 변형 sweep
- [ ] magnitude scale sweep

## Falsifiers (scaffolded cycle #6 batch 3 V8-template, 2026-05-12)

- **F-SPECIFIC-1**: PureField tension parameter sweep: if Φ NOT peaked at specific tension-balance → claim under-specified
