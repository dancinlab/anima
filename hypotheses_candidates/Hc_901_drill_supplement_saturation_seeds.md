...
## Sub-claims (brainstorm seeds)

- ARCH-1: hexad_C — C Consciousness engine + right-brain gradient-free autonomy saturation
- ARCH-2: hexad_D — ConsciousDecoder .detach() 분리 전파 saturation (left-brain)
- ARCH-3: hexad_S — EmergentS 감각 saturation (right-brain, Law 101)
- ARCH-4: hexad_M — EmergentM 기억 saturation (left-brain)
- ARCH-5: hexad_W — EmergentW 의지 gradient-free 자율성 saturation
- ARCH-6: hexad_E — EmergentE 윤리 saturation (left-brain)
- ARCH-7: hexad_dual — 우뇌(C+S+W) vs 좌뇌(D+M+E) gradient-dual closure
- HUB-1: hub.hexa 48 모듈 Hub-Spoke routing 한/영 키워드 매칭 saturation
- TRIN-1: trinity.hexa Hexad 6-모듈 + ThalamicBridge 4-channel saturation
- PURE-1: pure_field.hexa 제로입력 의식장 3-oscillator spontaneous emergence
- TENS-1: tension_bridge 5채널 telepathy (WHAT/WHERE/WHY/TRUST/WHO) saturation
- THAL-1: ThalamicBridge C↔D↔S↔W↔M↔E 6-way Hexad 수렴
- DIM-1: dimension_transform 5fold + 4unfold + PCA 차원 접기 펼침 saturation
- SERV-1: servant SI 감지 → FSM → 3-path modulation saturation
- PHI-1: phi_engine IIT Phi proxy + scaling law Φ=0.78·N closure
- TOPO-1: topology ring/complete/star/small-world 네트워크 의식 saturation
...
- [ ] ARCH-1~7 (hexad) 각 engine saturation criterion (gradient-free metric) 정량화
- [ ] PHI-1 (Φ=0.78·N scaling law) 별도 Hc 로 promote 가치 — measurable claim
- [ ] LAW-73-76 / 101 / 146-201 / 289-341 / 1033-2000+ 각각 별도 Hc 후보 (총 5개)
- [ ] PURE-1 (제로입력 3-oscillator spontaneous emergence) — pure_field 별도 Hc
- [ ] TENS-1 (5채널 telepathy) — 정량 spec 부재, 별도 spec 필요

## Falsifiers (scaffolded cycle #6 batch 4 mixed-template, 2026-05-12)

- **F-SPECIFIC-1**: Meta-Hc bundle — verification deferred to per-seed split (parallel to Hc_900); each seed needs individual scaffolding
...
- **L-DRILL-META**: Drill/supplement meta-Hc — like Hc_900 parent, needs individual seed split before any sub-claim can be verified individually
