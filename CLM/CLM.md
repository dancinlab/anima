...
- [x] **P-ARRAY MITOSIS-ARRAY 돌파엔진 (DISSOLVE + BRIDGE)** ✅ **양 arm 완료 2026-05-30 · 둘 다 🔴 CLOSED-NEGATIVE** — H_847 🔴(routing-z tiny~small 한정·측정타당성 ⊥ AKIDA 온칩 충돌)의 돌파엔진. 설계 [P0_ARCHITECTURE.md §11](./P0_ARCHITECTURE.md#11-mitosis-array) · 출처 [CLM.breakthrough.mining.md](./CLM.breakthrough.mining.md) DISSOLVE(depleted-both). **DISSOLVE(H_852 F-CLM-MONO-ARRAY 🔴)**: scale 축을 model-dim → **expert-COUNT** 로 이동(big=Σ chip-fit expert ≤1.2M), routing-diversity 재정의 = **expert-count sweep inter-expert dispatch entropy**(chip-native). 측정(E∈{4,8,16,32,64}×seed{42,43,44}, Dirichlet(1) null z-score, $0 Mac): raw H 는 E 로 상승(1.19→3.17 nats) **하나 uniform-null 대비 z 급락(+0.53→−7.61)** — uniform ceiling 이 ln(E)로 커져 큰 array 가 더 sub-uniform → scale=expert-count 가 z-척도 상 escape 를 scale 못시킴. 하니스 [model/{array_moe,run_array_sweep}.py](./model/) + [array_moe.hexa](./model/array_moe.hexa). **BRIDGE(H_853 F-CLM-BRIDGE-XFER 🔴)**: teacher(E32/d128 유효 scale) escape 측정 → Hinton KD distill(α=0.7 T=3.0) → chip-fit student(E8/d64) transfer Δ. fire(ubu-1 RTX5070 dedicated $0): teacher z 음수(−3.74) · student distill 후 near-uniform(+0.60) · **transfer Δ +4.34 > 3.0 임계 · 2/3 seed sign-flip** → monopoly-escape 가 chip-fit student 로 생존 못함(균형 회귀). student chip-fit ✅. 파이프라인 [distill/](./distill/). HF `dancinlab/anima-clm-bridge`(PRIVATE, negative-result · teacher+student .pt + sha256). expert=mitosis cell=AKD1000 chip(@L3) · 배포 N-chip array + 1-chip time-mux fallback(@L5) · 정직 caveat: 물리 다중-AKD1000 = 현재 pi5 1칩 → SW-sim+GPU sparse-MoE 먼저 측정(@L6). verdict [.verdicts/clm-mitosis-array/](../.verdicts/clm-mitosis-array/) + H_852/H_853. **a_scale_honest_scope cross-check**: 두 🔴 모두 toy expert-count/distill axis 한정 — 3B 일반 주장 금지. 후속 lever = routing-distribution 직접 정합(dispatch KL distill) · H_847 AXIS_MAP 합류.
