...
- [ ] **P2 학습 (from-scratch pretraining)** — ✅ **hexa-native 학습 해결**(2026-05-30, 사용자·hexa-lang측 — 기존 DECODER M5 0.28 step/s 🔴 INFEASIBLE 전제 소멸). 3-arm × scale-ladder full-fire 를 **hexa-native(g1-pure)**로, 학습 job 은 **`/dojo`로 생성**(job.hexa+train+run.sh 빵틀) · H100 fire(a_fire_autonomous). step-rate 재측정으로 최종 확인(p7). AKIDA는 추론칩 → 추론만 AKIDA-int4, 학습은 GPU(hexa-native).
