...
- [x] **B0 trainer 2-track (d5)** ✅ scaffold — PyTorch fp16 conv-MoE 트레이너(`CORE/DECODER/clm_b0_pytorch_trainer.py`) smoke-runs **feasible 실측**(CPU 22 step/s, CE 5.66→0.26, 4/4 experts) ∥ hexa-native 🔴 INFEASIBLE root-cause → hexa-lang handoff `5cd0e4c8`. 본문 [B0_TRAINER.md](./B0_TRAINER.md). 추론 AKIDA-only 불변(g1).
- [ ] **P2 학습 (GPU pretraining)** — from-scratch pretraining (H100 fire · a_fire_autonomous). **B0 Track 1 PyTorch fp16 트레이너로** 진행 (런치 unblock). hexa-native trainer 는 production scale 🔴 INFEASIBLE 실측(DECODER M5: 0.23~0.50 step/s ≈ 44~122 GPU-days + RSS 328MB/step leak-OOM) → Track 2 로 분리(hexa-lang handoff `5cd0e4c8`), 런치 비차단. AKIDA는 학습칩 아님 → pretraining만 GPU, 추론은 AKIDA.
