...
- [ ] **P-ARRAY MITOSIS-ARRAY 돌파엔진 (DISSOLVE + BRIDGE)** — H_847 🔴(routing-z tiny~small 한정·측정타당성 ⊥ AKIDA 온칩 충돌)의 돌파엔진. 설계 [P0_ARCHITECTURE.md §11](./P0_ARCHITECTURE.md#11-mitosis-array) — **DISSOLVE**: scale 축을 model-dim → **expert-COUNT** 로 이동(big=Σ chip-fit expert ≤1.2M), routing-diversity 재정의 = **expert-count sweep inter-expert dispatch entropy**(chip-native), expert=mitosis cell=AKD1000 chip 매핑, 배포 N-chip array + 1-chip time-mux fallback. **BRIDGE**: GPU teacher(유효 scale) monopoly-escape 측정 → chip-fit student distill → transfer Δ 검증. 정직 caveat: 물리 다중-AKD1000 = 현재 pi5 1칩 → SW-sim+GPU sparse-MoE 로 먼저 측정. 출처 [CLM.breakthrough.mining.md](./CLM.breakthrough.mining.md) DISSOLVE(depleted-both). 신규 falsifier F-CLM-MONO-ARRAY(dispatch entropy E-단조) + F-CLM-BRIDGE-XFER(transfer 생존).
