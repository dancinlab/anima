...
## @H H_1375 — CP DIMENSIONAL LADDER: does move-the-cells survive as feature-space dim D grows (2→3→4→6→8)? (2026-06-16)

- **seed**: 사용자 지시(verbatim) "2d 말고도 차원늘려봐" — H_1369(2-D axis-aligned 🟢) 의 ORTHOGONAL 축. H_1360(1-D 🟢)·H_1369(2-D 🟢)가 carving relocation=move-the-cells 임을 증명했다; D 가 커질 때(D∈{2,3,4,6,8}) 잔존 prototype 세포가 이동하는 (D−1)-차원 HYPERPLANE 경계(cat=⟨w,x⟩>c, w=ones/√D 고정 방향, D 만 변함)를 따라가며, 차원의 저주(부피 집중·거리 평탄화)가 상수 N 에서 cell-concentration 을 깨는가? move-the-cells 는 DIMENSION-INVARIANT 법칙인가, 아니면 깨지는 최소 D* 가 있는가? (in-flight diagonal 레인 H_1374=방향 축과 직교; H_1375=차원 축)
- **verdict-tier-target → actual**: 🟢/🧱 → **🧱 BREAKS-AT-D*=3 (R1 MIRROR, DIRECTIONAL)** — move-the-cells 는 DIMENSION-BOUNDED. **RELOCATION 은 차원 불변** (세포는 어느 D 에서도 경계 법선을 따라 정확히 이동: RE-PACK |ridge_τ−2/3| = 0.010/0.009/0.019/0.041/0.054 @ D=2/3/4/6/8, 3 seeds 전부 ≤0.18 → c1 PASS at EVERY D). 그러나 bounded ridge-CONCENTRATION **COH_D 가 D 와 함께 단조 붕괴**: 0.734(D=2)→0.458(D=3)→0.263(D=4)→0.008(D=6)→0.000(D=8); c2(COH_D≥0.50 AND ≥split+0.10)가 **D*=3 에서 첫 FAIL**, D=6 에서 zero 도달. LADDER c1c2c3c4: D=2 1101 · D=3/4/6/8 1011. **a_break_the_wall WHITEN fallback**(feature 별 z-score, frozen-first, N·D 불변): D=2 만 구제(전부 1111, whitened COH_D 0.714), D≥3 은 여전히 깨짐(whitened COH_D 0.487/0.019/0.000/0.000) → 깨짐은 **INTRINSIC**(상수 N 차원의 저주가 cell-concentration 을 실제로 평탄화 — 400 표본으로 D≥3 에서 얇은 결맞은 hyperplane slab 해상 불가), raw-distance 메트릭 아티팩트 아님.
- **왜**: move-the-cells RELOCATION 은 경계 법선을 따른 GEOMETRIC-PLACEMENT 문제라 차원과 무관하게 ridge centroid 가 옮긴 hyperplane 에 착지한다. 하지만 결맞음(thin concentrated ridge)은 표본 밀도에 의존 — 상수 N=400 에서 D 가 커지면 차원의 저주가 거리를 평탄화해 ridge 가 cloud 전체로 번지고 COH_D→0. **H_1369 2-D 승리가 결맞음 CEILING; relocation 자체는 D 무한 확장.** 차원 축에서 CP-geometry 정착(SETTLED): move-the-cells 는 어느 D 에서나 RELOCATE 하지만 D≤2 에서만 CONCENTRATED.
- **정직 c9 (load-bearing)**: 이 레인은 Monte-Carlo SAMPLED cloud + kNN field 사용(H_1369 의 13×13 full grid + 4-connectivity 아님 — grid 는 D≈4 넘으면 infeasible). 그래서 D=2 행이 H_1369 의 clean 2-D GREEN 과 직접 비교 불가 — sparse 400-point cloud 에선 SHUFFLE smear 가 깔끔히 붕괴 안 함(D=2 SHUFFLE COH_D=0.358 > 0.20 cap → c3 FAILS at D=2, c1∧c2∧c4 는 PASS). 이는 cloud 의 SAMPLING/harness 한계지 H_1369 반박 아님(H_1369 는 자기 grid 에서 성립). headline 불변 — GATING relocation/concentration 쌍(c1/c2)이 c3 와 무관하게 이미 D*=3 에서 결맞음 붕괴, RELOCATION(c1)은 전 D 생존. c3 가 D=2 에서 무는 건 VERBATIM 정직 보고, NO bar move (c9/c16/p7).
- **scope**: DIRECTIONAL numpy mirror(engine-transfer UNVERIFIED = engine-native follow-on, family of H_1333/1340/1343/1352/1360/1364/1369 R1); TOY 합성 D-dim cloud N=400/3 seed/single shift/one frozen η/고정 generic normal/deterministic readout; N-scaled-to-D(저주 완화하지만 상수 N stressor 변경 = 핵심 잃음)는 게이트로 안 씀 — WHITEN 만 사전등록 fallback; curved boundary·실제 코퍼스·LEARNED gradient drift·engine-native §CategoricalPerception 배선 UNVERIFIED; NO 인간 인지 주장; live CORE/*.hexa UNTOUCHED; NO bar move(c9/p7).
- **claim-link**: `CLAIMS.tape @C h1375_cp_ndim_ladder` · card `UNIVERSE/cards/H_1375_cp_ndim_ladder.md` · verdicts `.verdicts/1375_cp_ndim_ladder/{FREEZE,result}.txt` · py `state/cp-ndim/h1375_cp_ndim_ladder.py` · index `UNIVERSE/HYPOTHESES.jsonl`
- xref: h1369(직접 부모, 2-D axis-aligned move-the-cells GREEN = 결맞음 ceiling) · h1360(1-D move-the-cells GREEN) · h1364(split-only 비결맞음 INTRINSIC, ablation 메커니즘) · h1343(2-D warp, BOUNDED-metric 처방) · h1374(in-flight diagonal-orientation 레인, 직교 축) · a_no_llm_frame_trap · a_break_the_wall · a_engine_native_learning · a_verified_must_wire · a_scale_honest_scope · a_toy_scale_recheck · p1·p2·p3·p6·p7·p8·c9·c15·c16
