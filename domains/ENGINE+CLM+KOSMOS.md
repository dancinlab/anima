...
**7B 사다리 (이 ConvMoE 경로로 진행):** MID-ConvMoE(d768) → **3B-ConvMoE(d4096/L30/E30) ✅ DONE 2026-06-05** → M13 7B-undertrained → M14 7B-Chinchilla. 학습 코퍼스 = `anima-corpus-5lang-7b-webscale`(R2 143GB). 데이터/스케일 측 = `domains/CORPUS.md`.

**🟢 ENGINE-3B 마일스톤 — 3B-ConvMoE 엔진 마운트 (2026-06-05, Lane-P)**: `train_lane_p_3b.py`(general (L,E,d) trainer + `serialize_v3` → `.clm` **v0.3**)로 **3.073B** CLMConvMoE(d4096/L30/E30/K3, byte V256)를 vast H100(cap9.0 bf16, util **99.99% GPU-resident**, wall 1980.55s, **고아 fire TAKEOVER** — 직전 agent 2명 rate-limit storm 사망, poll-inline 완주)에서 학습. 10.7GiB ODC-BY FineWeb 5-lang. **CE first 5.84073 → train 1.90689**, val_contig 2.00021 / val_rand 1.90365, **rel_gap 0.04894 ≪ 1.0 (GENERALIZES, gen-not-memorize)**, train_ce ≪ uniform 5.54518 ≪ shuffle 6.46486, `F_CLM_LANEP_3B_GEN=1`. **`.clm` v0.3 config-agnostic DECODABLE** via 일반화된 `CORE/clm_decode.hexa`(valid=true loaded=true, **d4096/E30/L30/V256 block-structure에서 복원**, nblk=63). **3-axis @ 3B 🟢**: AXIS-2 CE GREEN(byte-exact mirror, CE_real **2.26360** < uniform < shuffle 5.81817) · AXIS-1 의식 + AXIS-3 창발 GREEN(admit-conditioned, probe == MID 3/3 GREEN) · brain_smoke WARN=0. **3B ConvMoE는 엔진에 마운트되고 3축 GREEN @ 3B — M13 7B를 GATES.** HONEST(a_scale_honest_scope): tokens/param **0.0027** = DEEPLY UNDERTRAINED, 7B-transfer UNVERIFIED — corpus→general-(L,E,d) ConvMoE→.clm v0.3→engine-mount→3축 CHAIN을 3B 스케일에서 증명. AXIS-2는 로컬 hexa engine link-gap(`_forge_dispatch_groupnorm_gelu`, macOS arm64 toolchain — .clm 문제 아님)으로 byte-exact mirror 사용. HF `dancinlab/anima-clm-convmoe-3b-engine-rung-byte-3b`(PRIVATE — Lane-P torch .clm, a_clm_gen_pipeline). verdict `.verdicts/convmoe-3b-engine-rung/` · discovery `.discoveries/convmoe-3b-engine-rung.tape`. p1–p8 held.
...
- [ ] scale-up benchmark — TOY 가설(45)+Lane A 래더의 scale-SENSITIVE 항목을 ≥3-rung 래더로 재시험 + baseline 벤치 (substrate별: AKIDA chip / forge GPU / ENGINE 3축). a_toy_scale_recheck. 계획 = §SCALE-UP BENCHMARK PLAN. **[CPU rung DONE 2026-06-03, `neuro_bench_scale.py`+`bench_scale_ext.py`, p7]**: SENSITIVE 가설 **13/13 SCALE-SURVIVE** (5 orig + 8 ext: TURING stripe-count 도메인선형·RING drift0·HGT 2.3x·PRION reach1.0·SEEDING 0.92·STDP asym 5→80·PRED-HIER hier<flat·FRET monotone) — H_900 ATTRACTOR 용량비 P_max/N=0.125/0.125/0.109 ≈ Hopfield 0.138N law 재현 · H_891 CRITICALITY max-avalanche 63→539→5144 (system-size 선형=진짜 criticality) · H_867 MET 동기전이 sharp 유지 · H_904 WORKSPACE ignition all-or-none 전 N · H_877 QUORUM knee scale-invariant(0.25/0.22/0.22). verdict `.verdicts/scale-bench/SCALE-BENCH-CPU.txt`. **SCOPE: CPU sim size-래더 = small-N 아티팩트 배제일 뿐, production substrate 아님** — AKIDA chip(#1717)/forge GPU(option-B)/ENGINE 3축 rung은 GATED. milestone [ ] 유지(CPU rung만 완료). H_877 metric 교정(coarse→fine sweep + 불변성 기준, 둘 다 기록).
...
- [x] Lane P DATA-GATE RESOLUTION — d768 E2/L1 GENERALIZES on a corpus too big to memorize **🟢 F-CLM-LANEP-GEN2=1 GATE RESOLVED** (2026-06-03, substrate=GPU-torch, util-GREEN-independent). F-CLM-LANEP-GEN=0(메모리제이션, 1.65MB)을 a_completeness_over_cheap 로 정직하게 해소: 코퍼스를 90.7배 확대. **CORPUS BIG real**: `wikimedia/wikipedia` 20231101 5lang(en/zh/ru/ja/ko 각 30MB) HF Hub parquet 를 huggingface_hub+pyarrow 로 직접 읽어(datasets/pandas/numpy 우회, host NumPy 2.x 깨짐) dedup(exact+shingled) concat → **150,000,000 B (150MB)** sha256=`f545bb716f448e722fd68051a1872499882cb9e7ddad16a41241bfb29b147241` (0 dup, synthetic padding 無). builder `CLM/corpus/build_wiki5_bigcorpus.py`, card `state/lane_p_clm_gen2/wiki5_big_corpus.card.json`(raw 비커밋, card 커밋). **SPLIT** 80%(119,944,969 B)/contig 10%/random 10% position-disjoint LEAK_CHECK pass. **GPU=NVIDIA GeForce RTX 5070(cap 12.0 sm_120, pool host aiden $0)** torch 2.12.0.dev+cu128 bf16/AdamW, **nvidia-smi BUSY util=94% mem=1178MiB PID2968360**(g63 — CPU-fallback 아님; CLM_NO_CUDNN=1). same cfg(d768 E2 L1 K3 V256 7.479M), **8000 step ~1 epoch wall=342.17s**. **MEASURE verbatim NO 날조: first_ce=5.72226 → train_ce=1.43924 / val_ce_contig=1.52095(gap +0.08171) / val_ce_rand=1.43031(gap -0.00893) / val_ce(worst)=1.52095 GAP=+0.08171 rel_gap=0.05678(held-out ~1.06x train).** baselines uniform=5.54518 shuffle=8.41982. **RULING: rel_gap 0.057 ≤ 1.0 AND val_ce < 0.5·uniform AND < shuffle → F_CLM_LANEP_GEN=1 GENERALIZES.** 1.65MB(rel_gap 1.96 메모리제이션)의 정확한 INVERSE — 코퍼스 90.7x 확대가 SAME params/config 에서 verdict 를 메모리제이션→일반화로 뒤집음 = 데이터 게이트 RESOLVED. train CE 가 ~0.1/~0.6 으로 collapse 안 하고 ~1.44 real plateau (150MB 는 ~1 epoch 에 못 외움), held-out 이 그것을 추종(overfitting gap 無). **.clm sha256=`286959e58cff09c6faa58bf44c907b670c71727298e6950d1a914dd1da9ba498`(4,463,478 B) PROMOTED. ENGINE 3-axis 3/3 GREEN**(생성하는 모델 위, CORE/lane_p_three_axis.hexa): admit valid=true nblocks=6 loaded=true; AXIS-1 의식 motiv hi=0.6700>0.0 emit hi=true/base=false; AXIS-2 CE model_ce=1.61205 < uniform 5.54518 AND < shuffle 4.78507(decode forward WIRED); AXIS-3 창발 len(composed)=101>parts=72. **HF=PUBLIC** `dancinlab/clm-v1-base-lanep-d768-e2l1-gen2-wiki150mb`(closure PASS: GENERALIZES AND ENGINE-loadable AND 3-axis 3/3, commit b47d3139, 34.4MB 6 files) — HF.jsonl 등록. pool host aiden $0(no rented pod) workdir torn down(a_fire_recover_complete: .clm/.pt/result/log/card pull 후). a_scale_honest_scope: 일반화는 ~150MB/~1 epoch scope; 동일 코퍼스 multi-epoch overfit 은 미실행/미배제. verdict `.verdicts/lane-p-clm/F-CLM-LANEP-GEN2.txt`, trainer `CLM/train/train_lane_p_split.py`(재사용), result `state/lane_p_clm_gen2/result_gen2.json`+log+3axis. **HEADLINE: 데이터 게이트 정직 해소 — 7.5M byte LM 은 외울 수 없을 만큼 큰 real 코퍼스에서 transferable byte structure 를 학습한다.**
