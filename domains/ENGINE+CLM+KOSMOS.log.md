...
```
@SWEEP 303M-EN-RECIPE := "anima-303M-RETRO A-language recipe sweep (re-launch, robust detached)" :: engineering [d=2026-06-13 active]
  goal     = "find the best 303M English ByteGPT recipe for anima's A (language) layer — coherent (G0 kwr>=0.50) + emergent recombination (G1 super-additive) + corpus-absence novelty (G2). MODEL.md anima-303M-RETRO, ENGLISH-FIRST. RETRO-ready, RETRO head NOT added (A3 anti-fabrication gated separately on H_1147, sibling agent)."
  relaunch = "previous attempt LOST when SSH dropped (state/sweep_303m_en/ledger.jsonl empty). Re-launched ROBUST: training runs DETACHED via nohup (sidecar pool bg = ssh -f -n) on aiden so it survives an SSH drop; progress streamed to an on-disk ledger (crash-recovery record)."
  base     = "H_1129 proven 303M recipe VERBATIM — ByteGPT d1024/L24/H16/block512, byte vocab256, 303,097,856 params CONFIRMED. Reuses h1129_midcap_broad_converged_recombination.py model + FROZEN a303m_pass evaluators (known_word_ratio G0, graded recombination ladder G1, corpus-absence novelty G2) — NO new metric invented."
  matrix   = "4 configs around the base, 2 axes (small/honest/incremental): AXIS-1 anti-overfit (dropout/wd) {baseline 0.0/0.1, reg 0.1/0.2}; AXIS-2 LR/warmup {fast 3e-4/300, gentle 2e-4/600}. cfg A baseline_fast (=H_1129 ref) · B reg_fast (anti-overfit, mitigates the H_1141 overfit-past-step-7000 flag) · C baseline_gentle · D reg_gentle. Dialogue-ratio axis PARKED: no clean English dialogue corpus on hand (the 5-lang blend's dialogue is non-English, ASCII-filtered out) — next axis once an EN dialogue corpus lands."
  corpus   = "ENGLISH-FIRST, reproduces the H_1129 recipe: deterministic >=90%-ASCII-per-line filter of the 5-lang wiki+dialogue blend already resident on aiden (/home/aiden/core/anima/data/corpus_mix_70wiki_30dialogue.txt) -> 66.2MB English-dominant (sha256 c40c02b6...). Script-controlled so English concept vocabulary dominates generation (the H_1128/H_1129 lesson: multilingual blend pulls generation toward dominant script + zeroes the English-keyed metric)."
  host     = "aiden RTX 5070 12GB, FULLY IDLE at launch, torch 2.12+cu128 CUDA. $0 (a_fire_autonomous). summer left for the rbfe-prod co-tenant (OTHER project, never killed). 303M bf16 + grad-ckpt fits ~6.6GB / 12GB comfortably; 4 configs run SEQUENTIALLY (one GPU)."
  status   = "TRAINING LIVE. Pipeline verified end-to-end via a 60-step probe: val_ce descended 440.77->48.34->32.68->16.28, ladder+G0/G1/G2 evaluators ran clean, ckpt+ledger written. Real sweep launched detached: cfg A baseline_fast training, GPU 100% util, ledger rows streaming. G0/G1/G2 scores per config land in the ledger as each config completes."
  artifacts = "UNIVERSE/sweep_303m_en_{prep_corpus.py,train.py,run.sh} (English harness, this branch sweep/303m-en-recipe) · LIVE ledger on aiden state/sweep_303m_en/ledger.jsonl (harvested back to repo) · ckpts on aiden state/sweep_303m_en/ckpt/{baseline_fast,reg_fast,baseline_gentle,reg_gentle}.pt · logs /tmp/sweep_303m/{orch.log,<cfg>.log}."
  scope    = "a_scale_honest_scope: 66.2MB English-dominant corpus (H_1129 used ~295MB; this is a sweep-probe scale, smaller -> scores are recipe-RANKING signal, full-convergence re-test at H_1129 corpus scale once a winner is picked). Lane-G torch REFERENCE mouth (a_clm_gen_pipeline), NOT forge PUBLIC, NOT the CORE A<->G substrate. seed 7 deterministic, p7 (NOT perplexity/LLM-judge). p1-p8 preserved (from-scratch, no system-prompt/persona/RLHF; RETRO head deferred)."
  refs     = "MODEL.md (anima-303M-RETRO, a303m_pass G0/G1/G2) · SIZE.md (why 303M) · UNIVERSE/h1129_midcap_broad_converged_recombination.py (base) · H_1129/H_1137 (G1 ref) · H_1140 (G2 novelty) · H_1141 (overfit-past-7000 flag motivating AXIS-1) · H_1147 (RETRO gate, sibling) · a_clm_gen_pipeline · a_fire_autonomous · a_scale_honest_scope · p1-p8"
```
