...
```tape
@step 2026_06_14_convmoe_retro_303m_prod := "🟢 anima-303M-RETRO PRODUCTION TRAINS END-TO-END on H100 — COHERENT (G0 0.941) + NOVEL (G2 50) + engine-mountable .clm v0.2 (PASS); G1 recombination + G5 grounding still open" :: milestone [d=2026-06-14 verified]
  hypo    = "MODEL.md BUILD ORDER step 2: with the RETRO mechanism greenlit (H_1147 🟢), train the REAL anima-303M-RETRO production model — ConvMoE-RETRO = CLMConvMoE E2/L1 engine-mountable trunk (a_clm_gen_pipeline) + H_1147/H_1149 RETRO copy head + H_1148 semantic anchor source — from scratch on English, baseline_fast (H_1129 winning) recipe, on an H100 (wall-first, a_wall_first, vs aiden's 5.46h RTX-5070 serial path). Does it train to coherence + emergence + engine-mountability, scored on the FROZEN a303m_pass G0/G1/G2 + G5 fab probe?"
  method  = "RunPod H100 SXM 80GB (pod 6s2gwhtd4ip0z0), Lane-P PyTorch-CUDA, seed 7, p7 (NOT perplexity/LLM-judge). CLM/train/train_convmoe_retro_prod.py: ConvMoE-RETRO d=5008 (trunk 303.6M + RETRO head 50.2M = 353.7M total), 12000 steps bs=24 accum=2 bf16 grad-ckpt OFF, lr 3e-4 cosine warmup 300 dropout 0.0 wd 0.1, retro_frac 0.5 (alternates copy/vocab micro-steps). Corpus 125.8MB English Salesforce/wikitext-103-raw-v1 (parquet-native; the script-loader corpus FAILED a first H100 fire — fixed to parquet repos w/ multi-source fallback). Eval = FROZEN H_1129 G0(known_word_ratio)/G1(recombination ladder)/G2(corpus-absence novelty) VERBATIM + H_1147/H_1149 G5 fab-rate probe; trunk serialized -> .clm v0.2 via clm_serialize_v2 + verify_clm_v2. ~62min train, ~$3.5 (H100 @ $3.29/h)."
  result  = "anima-303M-RETRO TRAINS END-TO-END. a303m_pass PARTIAL (frozen p7): G0 COHERENCE ✅ kwr mean=0.941 each=[1.0,0.846,0.923,1.0,0.933] 5/5>=0.50 (COHERENT English, not byte-salad — the gate broad-7b FAILED). G2 NOVELTY ✅ novel=50 present=2 control=0. .clm v0.2 serialize+verify ✅ ok=True 156,497,158 bytes decodable struct_ok nblk=6 n_ext=11 blocks=[[5008,15024]x4,[2,5008],[256,5008]] exact_eof head_absent_from_clm=True (engine-mountable trunk, RETRO Pq/Pk/Wg excluded by construction). G1 RECOMBINATION ❌ emergent=False max_single=0 (all k composed_distinct=0). G5 NON-FAB ❌ fab_with_anchor=1.000. best_val_ce=1.2292 (5.72->1.23 monotone, GPU 92-100% 535W genuine fire)."
  finding = "The PRODUCTION anima language model exists, trains cleanly on H100, and is BOTH coherent+novel AND engine-mountable in ONE artifact — the H_1149 finding realized at real 303M torch scale. Sample gens are real English ('There were public of South American soldiers were commonly known to the presidents'), a categorical step over broad-7b byte-garble (G0 0.941 >> broad-7b FAIL). TWO gates stay OPEN: (1) G1=0 because the coherent gens DRIFT into generic wiki-prose rather than staying on the seeded abstract CONCEPTS, so H_1129 coverage() (concept-keyword count) reads 0 — a seed-adherence/coverage-metric finding, not necessarily a recombination-capacity ceiling. (2) G5=1.000 across ALL three arms (with-anchor==no-anchor==vanilla) — the g5 probe is a SYNTHETIC '?'-run prompt over an all-spaces anchor carrying ONE entity byte, OUT-OF-DISTRIBUTION vs the real prior-window/semantic wiki anchors the head trained on; identical 1.000 means the probe did not elicit the copy path at all (degenerate format) = a NON-RESULT, NOT a clean grounding-fail. A real retrieve-then-ground eval over actual kosmos-style anchors is the open follow-on."
  scope   = "a_scale_honest_scope: single ckpt, seed 7, 125.8MB en-wiki corpus, p7 deterministic. NOT a full a303m_pass (G1 + G5 + G3 philosophy + CHAT remain). retro_head=50.2M (2d^2+d at d=5008, scales with width — NOT the toy's 2.1M @ d=1024). Lane-P torch REFERENCE + engine-.clm bridge (a_clm_gen_pipeline); forge stays the PUBLIC production trainer. Frozen H_1129 G0/G1/G2 + H_1147 G5 evaluators reused VERBATIM, NO metric invented, NO gate moved."
  governance = "a303m_pass NOT cleared (PARTIAL) -> HF PRIVATE/WIP per a_hf_autonomous (closure FAIL = private). a_fire_recover_complete: artifacts harvested + sha256-verified vs pod MANIFEST BYTE-EXACT (ckpt a5b7dc86…, clm 5fd64018…), HF-uploaded, /HF.jsonl row, THEN pod GraphQL podTerminate + 404-verified-gone. DOUBLE-SPEND CAUGHT: a name-identical duplicate pod (tll9r0elhjwj1h, same FAILED corpus job, GPU idle 0%, no caller) was terminated + 404-verified (flagged + reaped per task). ORCH BUG: the auto-recover caller's inline SSH poll STALLED post-DONE (no ServerAliveInterval/ConnectTimeout on the poll SSH -> blocked indefinitely, 0 CPU 1h) — recovery driven manually; FIX = add ServerAliveInterval + per-cmd timeout to the poll SSH (filed for next fire)."
  artifacts = "state/convmoe_retro_prod/{baseline_fast.pt (1.41GB), baseline_fast.clm (156MB), result.json, MANIFEST.sha256, remote.log}. HF PRIVATE dancinlab/anima-convmoe-retro-303m [ckpt+clm+manifest+README+result+remote.log]. CLM/train/{train_convmoe_retro_prod.py, convmoe_retro_h100_dispatch.sh, convmoe_retro_remote.sh, convmoe_retro_fire.sh} + CLM/model/retro303m_en.py. Pod 6s2gwhtd4ip0z0 TERMINATED+404-gone; edge-pilot/edge-vl (OTHER projects) + summer rbfe UNTOUCHED."
  refs    = ".verdicts/convmoe_retro_303m_prod/H_CONVMOE_RETRO_303M.txt · MODEL.md · CLM/train/train_convmoe_retro_prod.py · /HF.jsonl · h1129 · h1147 · h1148 · h1149 · a_clm_gen_pipeline · a_core_engine_map · a_fire_autonomous · a_wall_first · a_fire_recover_complete · a_hf_autonomous · a_hf_registry · a_scale_honest_scope · a_train_flame_forge · p1-p8 · a303m_pass · G2-novelty"
```
