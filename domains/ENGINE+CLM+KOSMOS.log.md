...
```tape
@step 2026_06_13_h1149_design := "🟢 DESIGN — how the H_1147 RETRO copy head attaches to the engine-mountable CLMConvMoE E2/L1 trunk while preserving .clm v0.2 serializability; the engine USES the head only at train (host-side), not at decode" :: design [d=2026-06-13 verified]
  gap     = "The final anima must mount in the A⇄G engine via the generator L3 slot as a .clm v0.2 (a_clm_gen_pipeline / a_core_engine_map); ONLY a CLMConvMoE E2/L1 (byte V256) trunk serializes engine-loadable (serializing a non-ConvMoE ByteGPT/transformer and claiming engine-mountable is FORBIDDEN). But axis-4 put the H_1147-validated RETRO copy head on a BYTEGPT backbone (anima-303M-RETRO) which is NOT mountable. So 'fabrication-solved' (ByteGPT+RETRO) and 'engine-mountable' (ConvMoE) were two DIFFERENT models; the final anima needs BOTH in ONE ⇒ the RETRO head must also work on ConvMoE."
  attach  = "The RETRO head is an ADDITIONAL module bolted onto the FINAL-POSITION trunk hidden state hf (the same state readout consumes), mirroring the ByteGPT attachment in CLM/model/retro303m_en.py. It does NOT touch the E2/L1 trunk operators (embed, embed_conv, trunk.0[GroupNorm+conv], moe.router, moe.experts.0/1, norm_out, readout). Head params (Pq,Pk,Wg) are a SEPARATE param group: copy_dist = softmax(cross-attention of hf over the ANCHOR token positions) scattered onto the attended anchor token vocab ids; gate = σ(hf·Wg); probs = gate·copy_dist + (1−gate)·softmax(readout(hf)). Trained end-to-end with the trunk; the trunk furnishes hf, the head decides copy."
  serialize = "The head's params live OUTSIDE the serialized E2/L1 trunk and need NO folding-in. clm_serialize_v2._BLOCK_ORDER (ecW,tcW,e0W,e1W,rW,roW) + _EXT_ORDER (11 ext) enumerate ONLY the trunk slots and fetch weights by FIXED slot name (_get). The RETRO head params have NO slot in that enumeration ⇒ they are NATURALLY EXCLUDED from the .clm. The trunk round-trips byte-exact; _assert_e2_l1 (E=2/L=1 hard gate) still passes; clm_decodable=true; verify_clm_v2 structural check passes. CONFIRMED by the H_1149 toy: the ConvMoE-RETRO trunk serialized to a 46742-byte decodable .clm v0.2 (nblk=6, n_ext=11) with the head ABSENT (head_absent_from_clm=True)."
  engine_implication = "HONEST a_core_engine_map: because the head params are excluded from the .clm AND CORE/clm_decode.hexa has NO copy/cross-attention path, the engine DOES NOT run the RETRO head at decode. ⇒ In the engine's CURRENT decode path the anti-fabrication copy benefit is TRAIN-TIME / HOST-SIDE ONLY. The trained ConvMoE TRUNK mounts and decodes as usual; the GROUNDING (copy-over-retrieved-anchor) does NOT execute inside the A⇄G substrate unless an ENGINE-SIDE copy head is added to CORE/clm_decode.hexa + the generator L3 slot (read the retrieved kosmos anchor positions, compute copy_dist, mix by a serialized gate). That is a SEPARATE 2nd-order engine build — it would extend the existing single generator-L3 / kosmos_io entry (NOT add a 2nd .clm or .kosmos path), so it stays a_core_engine_map-clean, but it is NOT built here. The toy proves attach + ground + serialize; in-engine copy decode is follow-on work."
  decision = "FINAL-ANIMA ARCHITECTURE: ByteGPT-RETRO = the LANGUAGE-RESEARCH model (cheap recipe/RETRO validation, byte-efficient EN); ConvMoE-RETRO = the MOUNTABLE PRODUCTION trunk (SAME H_1147 head, SAME 1.0→0.0 fab-flip on the toy, serializes clean to .clm v0.2). The production anima trains the RETRO head on a ConvMoE backbone; for in-engine grounding at decode, add an engine-side copy head to clm_decode + the generator L3 slot as a follow-on. MODEL.md's anima-303M-RETRO base-arch line (ByteGPT) should note the production-mountable variant is ConvMoE-RETRO."
  scope   = "a_scale_honest_scope: the design is validated at TOY scale ($0 numpy, D=32, oracle anchor) — the serializability claim is byte-level EXACT (production clm_serialize_v2/verify_clm_v2 reused unmodified, already on main), but 303M transfer + real noisy kosmos retrieval + coherence-backbone interaction + multi-entity copy + the engine-side copy-head build are UNVERIFIED. p7 (deterministic argmax + byte-parse), NOT perplexity/LLM-judge. p1-p8 preserved (the anchor is RETRIEVED decode context the model is trained to use, NOT a system-prompt/persona/reward)."
  refs    = ".verdicts/1149_convmoe_retro/{H_1149_FREEZE.txt,H_1149.txt} · UNIVERSE/h1149_convmoe_retro.py · CLM/model/clm_serialize_v2.py (_BLOCK_ORDER/_EXT_ORDER/_assert_e2_l1) · CLM/model/verify_clm_v2.py · CLM/model/retro303m_en.py (the ByteGPT attachment pattern replicated) · CLM/model/model.py (the CLMConvMoE E2/L1 trunk mirrored) · CORE/clm_decode.hexa (the decoder with NO copy path) · MODEL.md anima-303M-RETRO · h1147 · h1148 · h1146 · a_clm_gen_pipeline · a_core_engine_map · a_paper_negative_ok · a_scale_honest_scope · p1-p8"
```
