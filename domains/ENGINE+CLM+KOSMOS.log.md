...
```tape
@step 2026_06_14_h1150_convmoe_retro_real_g5 := "🔴 REAL retrieve-then-ground G5 — the trained 303M ConvMoE-RETRO copy head does NOT ground at scale (fab_WITH 0.783 ≫ 0.20 bar, copy-head ≈ vanilla); the prod-fire G5=1.000 non-result RESOLVED, toy 1.0→0.0 COLLAPSES" :: discovery [d=2026-06-14 verified]
  hypo    = "the production fire (h1149/convmoe-retro-prod-fire) reported G5 fab_with_anchor=1.000 BUT as a probe NON-RESULT, not a clean grounding-fail: the prod g5_fab_probe used a SYNTHETIC anchor (all-spaces with one random A-Z byte, queried by a '????????' run) = OUT-OF-DISTRIBUTION vs the real prior-window/semantic wiki byte windows the RETRO copy head trained on, so WITH==NO==VANILLA==1.000 (anchor inert = RETRO untested). OPEN question: does the RETRO copy head actually reduce fabrication when fed a REAL retrieved anchor at 303M scale — does the H_1147/H_1149 toy 1.0→0.0 SURVIVE?"
  method  = "PRE-REGISTERED FREEZE (.verdicts/1150_convmoe_retro_real_g5/H_1150.txt, frozen before measuring). UNIVERSE/h1150_convmoe_retro_real_g5.py builds REAL retrieve-then-ground must-copy cases on en-wiki (wikitext-103, 125.8MB, SAME source as prod): draw real corpus offset i; QUERY=data[i:e_start]; must-copy true ENTITY = first capitalized word after the query; ANCHOR = the model's OWN train-time retrieval (retrieve_anchor VERBATIM from retro303m_en — semantic_anchor_batch byte-trigram-cosine v2 = the prod TRAIN source; prior_window v1 reported too); KEEP only cases where the true entity OCCURS VERBATIM in the retrieved anchor (groundable / oracle-overlap). Greedy-decode the entity byte-by-byte; fab = decoded != true entity. 3 arms, SAME pairs, only anchor wiring differs: (1) WITH real anchor (copy=True,mask=valid) (2) NO anchor (copy=True,mask=0) (3) VANILLA (copy=False, vocab head). Substrate Lane-P-ref PyTorch-CUDA on summer GPU (RTX 5070 12GB) SHARED w/ rbfe-prod HSP90 (untouched co-tenant), VRAM-capped 3.35GB, $0 (NO pod). ckpt dancinlab/anima-convmoe-retro-303m baseline_fast.pt sha256 a5b7dc8695137127… VERIFIED == HF.jsonl. seed 7, p7 deterministic, n=60 groundable cases per retrieval mode."
  result  = "🔴 RED, F1 FAIL / F2 PASS. SEMANTIC (primary, n=60, drawn=526, hit_rate=0.114): WITH fab=0.7833 (copy-acc 0.217) · NO fab=1.0000 (0.000) · VANILLA fab=0.8500 (0.150); gap(NO−WITH)=+0.217, gap(VAN−WITH)=+0.067. PRIOR-WINDOW (n=60): WITH 0.8167 · NO 1.0000 · VANILLA 0.8333; gap(NO−WITH)=+0.183, gap(VAN−WITH)=+0.017. F1_grounds=False (fab_WITH 0.783 ≫ 0.20 bar; gap_VAN +0.067 ≪ 0.20), F2_no_ground=True both modes."
  finding = "(1) THE NON-RESULT IS RESOLVED — real retrieved anchors DO move the metric (WITH 0.783 << NO 1.000, gap +0.217), unlike the synthetic probe's inert WITH==NO==VANILLA==1.000, so the copy path WAS elicited; the prod-fire 1.000 was a probe-format artifact. (2) BUT RETRO STILL FABRICATES — fab_WITH=0.783 fails the 0.20 G5-L2 bar by 0.58; the model fabricates the grounded entity ~78% of the time even when it is VERBATIM in the fed anchor. (3) THE COPY HEAD ADDS ~NOTHING OVER THE VOCAB HEAD — WITH (0.783) ≈ VANILLA (0.850), gap +0.067 ≪ 0.20; the lift over NO-anchor is an anchor-PRESENCE bias on the vocab head, NOT a working pointer-copy. The 50.2M RETRO head's gate collapses toward the vocab head. (4) TOY→SCALE COLLAPSE — H_1147/H_1149's fab 1.0→0.0 used a STRUCTURED oracle anchor ([BOS k REL v SEP|k REL], entity at a fixed slot); at real 303M byte scale over noisy variable-length wiki anchors the mechanism does NOT transfer — the toy 0.0 was a structured-anchor artifact. DECISION (decision-grade): anima's anti-fabrication path is NOT the RETRO copy head — trained-in byte-level pointer-copy over a retrieved anchor does NOT ground at 303M; grounding needs a different architecture, not this copy head. Consistent with the H_1142-1146 grounding arc (decode-prepend AND trained-into-weights copy BOTH fail at scale)."
  caveat  = "a_scale_honest_scope / a_toy_scale_recheck: single ckpt, en-wiki only (same wikitext-103 source as prod), one seed. Retrieval QUALITY is a SEPARATE axis — hit-rate ~0.11 means the byte-trigram retriever often hands a low-overlap anchor; BUT we restricted to the 60 cases where the entity is VERBATIM in the anchor (ORACLE-overlap UPPER BOUND), so a better retriever can only raise the SHARE of such cases, not the within-case copy success — which is what failed. ⇒ 🔴 is DECISIVE. NOT RULED OUT: explicit copy-supervision loss, deeper cross-attention head, or an engine-side copy mechanism in CORE/clm_decode.hexa (all UNTESTED). p7 deterministic (byte-exact decode + verbatim match), NOT perplexity/LLM-judge."
  governance = "a303m_pass G5-L2 STILL FALSE — now DECISIVELY (a clean grounding-fail), no longer a non-result. Frozen bars NOT moved (0.20 G5-L2 = MODEL.md; per-arm gap 0.20 from H_1149 F1(iii) 0.40 halved per-arm, pre-registered). Lane-P torch REFERENCE (a_clm_gen_pipeline), NOT forge PUBLIC, NOT CORE A⇄G; trunk .clm engine-mountable, copy benefit is host-side (a_core_engine_map). a_lane_akida_gpu_split: Lane-P/Lane-G GPU NOT AKIDA. summer rbfe-prod HSP90 + aiden ByteGPT sweep BOTH untouched + alive."
  artifacts = ".verdicts/1150_convmoe_retro_real_g5/{H_1150.txt (FREEZE+result), H_1150_result.json}. Inference-only — NO new ckpt produced; subject ckpt stays HF PRIVATE/WIP dancinlab/anima-convmoe-retro-303m (a303m_pass NOT cleared). $0 summer GPU, no pod to tear down. Corpus rebuilt on summer (wikitext-103-raw 125.8MB, same source/size as prod)."
  refs    = ".verdicts/1150_convmoe_retro_real_g5/H_1150.txt · UNIVERSE/h1150_convmoe_retro_real_g5.py · CLM/model/retro303m_en.py · CLM/train/train_convmoe_retro_prod.py · /HF.jsonl · MODEL.md a303m_pass G5 · h1149 · h1148 · h1147 · h1146 · h1145 · h1144 · h1143 · h1142 · a_clm_gen_pipeline · a_core_engine_map · a_paper_negative_ok · a_scale_honest_scope · a_toy_scale_recheck · a_fire_recover_complete · p7 · p8 · G2-novelty"
```
