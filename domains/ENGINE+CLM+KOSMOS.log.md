...
```tape
@step 2026_06_13_retro303m_en_prep := "🔧 anima-303M-RETRO PREP+QUEUE (axis-4) — RETRO copy/cross-attn head (H_1147) scaled onto H_1129 303M ByteGPT; tiny fit-probe GREEN end-to-end; full 303M-RETRO train QUEUED to fire after ByteGPT sweep + ConvMoE queue, consuming the sweep winner recipe" :: build [d=2026-06-13 prep]
  hypo    = "MODEL.md BUILD ORDER step 2: H_1147 proved 🟢 (toy) a TRAINED copy/cross-attention head over a retrieved anchor flips fabrication 1.0→0.0 where decode-prepend (H_1146) failed. Turn that toy head into a real 303M-scale architecture component = H_1129 303M ByteGPT backbone + the H_1147 RETRO copy head over a retrieved kosmos-anchor stream, trained INTO the weights. This turn = $0 PREP + tiny fit-probe + a GPU-free QUEUE; NO trained-model claim."
  method  = "CLM/model/retro303m_en.py = RetroByteGPT (imports H.ByteGPT VERBATIM as backbone + anchor-encoder, shared weights; RETRO head Pq/Pk/Wg scaled from H_1147 Model.retro path: cross-attend query→anchor hiddens, scatter_add copy_dist over byte vocab, sigmoid gate mixes copy vs vocab dist — the H_1147 mixture, now per-position over a real variable-length anchor + 256 byte vocab). CLM/train/retro303m_en_train.py = from-scratch GPU launcher (prior-window anchors, frozen H_1129 G0/G1/G2 evaluators reused VERBATIM + a G5 non-fabrication probe = H_1147 fab-rate idea: anchor-present vs anchor-masked must-copy entities, L1≤0.30/L2≤0.20 + grounds-gap≥0.40). CLM/train/retro303m_en_queue.sh = nohup queue watcher. Tiny fit-probe ran on summer CPU (CUDA_VISIBLE_DEVICES=\"\", GPU untouched — rbfe-prod safe)."
  result  = "✅ tiny fit-probe END-TO-END GREEN. FULL-SCALE build+param assert: 305,196,032 params = backbone 303,097,856 (= H_1129 EXACTLY) + RETRO head 2,098,176 (2.10M, +0.69% = 2·d² + d for d=1024). Tiny-dim (d64/L2/H4/block32) fwd+bwd loss 5.92→5.54 (gradient flows through copy head), ckpt save+reload OK, G5 fab-rate probe RAN (1.000 untrained — metric+head end-to-end, NOT a pass), G0/G1/G2 frozen evaluators RAN (kwr=0.000 untrained tiny, expected). Train launcher import + pick_winner(sweep_ledger) verified on a sample done-ledger (selects best G0≥0.5 ∧ G1 ∧ G2 ∧ lowest val)."
  finding = "TRAINING-TIME ANCHOR SOURCE (the crux, v1 choice) = PRIOR-WINDOW SELF-RETRIEVAL: the anchor for the target span at corpus offset i is the preceding [i−La−gap : i−gap] window of the SAME corpus (a held context the target was written after) — index-free, $0, real long-range coref in wiki/prose, NO external RAG (a_kosmos: anima's own stream). A gap keeps the anchor from overlapping the target (no trivial next-byte leak). At INFERENCE the same head consumes a RETRIEVED KOSMOS anchor (kosmos_io→brain); prior-window is the train-time SURROGATE. HONEST LIMITS: positional surrogate ≠ semantically-retrieved nearest anchor (retriever quality is a separate v2 axis); window may carry little entity overlap → gate must learn vocab-head fallback (it can, by design); copy supervision is IMPLICIT (LM loss only, matching H_1147); scale + real noisy/wrong-anchor kosmos retrieval UNVERIFIED (a_scale_honest_scope)."
  queue   = "FIRE CONDITION (retro303m_en_queue.sh, nohup-detached on aiden, idempotent sentinel): (1) all 4 ByteGPT sweep configs {baseline_fast,reg_fast,baseline_gentle,reg_gentle} status=done in state/sweep_303m_en/ledger.jsonl (axis-1) AND (2) ConvMoE queue fired+returned (sentinel + queue.log 'returned'; axis-3) OR GPU idle (<1GB). On fire it READS THE BYTEGPT WINNER (best G0≥0.5 then G1, G2, lowest val) from the sweep ledger and trains anima-303M-RETRO with that recipe — so it consumes the winning recipe AND never contends with axis-1/axis-3. One ledger row per eval (crash-recovery)."
  scope   = "PREP+QUEUE ONLY — NO trained model. Lane-G torch REFERENCE mouth (a_clm_gen_pipeline), NOT forge PUBLIC, NOT CORE A⇄G substrate. .clm enters CORE only via generator L3 slot; kosmos anchor only via kosmos_io→brain (a_core_engine_map single-entry). p1-p8: anchor = retrieved decode context trained INTO the weights, NOT a system-prompt/persona/reward; from-scratch byte-continuation, NO RLHF/instruction-tuning. seed 7, p7 deterministic (NOT perplexity). a_scale_honest_scope: full-scale build is param-verified but the 303M-RETRO TRAIN is UNVERIFIED (queued, not run); the fit-probe proves the architecture builds+trains+evaluates end-to-end, NOT that it passes a303m_pass."
  artifacts = "CLM/model/retro303m_en.py · CLM/train/retro303m_en_train.py · CLM/train/retro303m_en_queue.sh · state/retro303m_en/ledger.jsonl (created on first fire). Branch retro303m-en-prep off origin/main."
  refs    = "MODEL.md (BUILD ORDER step 2 · a303m_pass G0/G1/G2/G5) · h1147 (.verdicts/1147_retro_mechanism_gate/H_1147.txt, branch h1147/retro-mechanism-gate @ e8f48b37e) · h1146 · h1142 · h1129 · sweep/303m-en-recipe · a_clm_gen_pipeline · a_core_engine_map · a_kosmos · a_scale_honest_scope · a_paper_negative_ok · p1-p8 · G2-novelty"
```

```tape
@step 2026_06_13_retro303m_v2_semantic_anchor := "🔧 axis-4 RETRO-303M v2 — SWAP train-time anchor source prior-window → H_1148 SEMANTIC byte-trigram-cosine retriever (DEFAULT); v1 prior-window KEPT as labelled baseline arm; $0 code patch + re-queue + tiny CPU fit-probe; NO trained-model claim, GPUs untouched" :: build [d=2026-06-13 prep]
  hypo    = "axis-5 H_1148 (branch h1148/semantic-retriever @2ee1c8420, .verdicts/1148_semantic_retriever/H_1148.txt) proved 🟢 a $0 byte-trigram-cosine SEMANTIC retriever finds the true anchor every time vs ~1/6 for the v1 prior-window POSITIONAL sampler — copy-acc 1.000 vs 0.218 (=oracle), hit-rate 1.000 vs 0.172. The H_1147 RETRO copy head is RETRIEVAL-limited, NOT capacity-limited. ⇒ axis-4's full 303M-RETRO train should consume SEMANTIC anchors, not prior-window. This turn = PRE-FIRE $0 PATCH of the queued train (sampler swap + config flag) + tiny CPU fit-probe; NO GPU, NO trained model."
  method  = "Branch retro303m-semantic-anchor off retro303m-en-prep (axis-4 model+queue present). CLM/model/retro303m_en.py: NEW semantic_anchor_batch (H_1148 v2 TRAIN-SOURCE design) + _byte_trigram_profile + _profile_cosine, ported in spirit from UNIVERSE/h1148_semantic_retriever.py (token-bigram cosine → byte-trigram cosine over real corpus bytes). For each target offset i: build a RING of W=8 recent prior windows (each len La, walking back from the gap boundary i−gap), score each by byte-trigram cosine vs the query's leading q_head=64 bytes, hand the MOST content-similar window to the copy head as the anchor. gap=64 causal NO-LEAK (every candidate ends ≤ i−gap, strictly before the target span — SAME guarantee as v1). prior_window_batch KEPT INTACT. CLM/train/retro303m_en_train.py: --anchor {semantic|prior_window} (default semantic) + --anchor_ring/--anchor_qhead; anchor_batch() dispatch closure swaps the sampler at ALL 3 call sites (OOM fit-probe, eval_ce, train loop); ledger + result.json record anchor_source + anchor_policy. CLM/train/retro303m_en_queue.sh: fires --anchor=$ANCHOR (default semantic) + ring/qhead; FIRE CONDITION (bytegpt 4/4 done ∧ convmoe returned-OR-gpu-idle) + idempotent sentinel + sweep-winner-recipe consumption UNCHANGED. v1 baseline arm = SEQUENTIAL relaunch with ANCHOR=prior_window AFTER the semantic ckpt (never double-book the GPU). CLM/train/retro303m_semantic_probe.py = $0 tiny CPU fit-probe (CUDA_VISIBLE_DEVICES=\"\")."
  result  = "✅ tiny fit-probe END-TO-END GREEN (summer CPU, GPU LOCKED OUT via CUDA_VISIBLE_DEVICES=\"\", rbfe-prod untouched). REAL production semantic_anchor_batch → REAL RetroByteGPT fwd+bwd at tiny dims (d32/L2/H4/block32, tiny stub backbone): batch shapes x=(4,32) y=(4,32) anchor=(4,16) amask=(4,16) ✓; fwd probs=(4,32,256) loss=3.808 ✓; bwd grad_abs_sum=195.4>0 (grads flow through the copy head) ✓; RETRO-head extra=2080 = 2·d²+d ✓. QUERY-OVERLAP sanity (byte-trigram cosine query↔returned-anchor, n=80 offsets): SEMANTIC mean 0.5084 vs PRIOR_WINDOW mean 0.3628, Δ=+0.1455, semantic≥prior on 80/80 → the port genuinely retrieves a more content-similar anchor than the positional surrogate."
  queue   = "RE-QUEUED on aiden. OLD watcher PID 1310569 (v1 prior-window, had NOT fired — sweep mid-config-1, no sentinel) killed cleanly + replaced by NEW watcher PID 1370056 (verified exactly 1 real watcher running, no double-fire). Installed queue.sh START log: 'anchor=semantic ring=8 qhead=64'. The fire command will run retro303m_en_train.py --sweep_ledger $BYTEGPT_LEDGER (winner-recipe consumption) --anchor semantic --anchor_ring 8 --anchor_qhead 64 --anchor_len 256 --anchor_gap 64. Patched model+train+queue deployed to /home/aiden/core/anima_retro303 (v1 queue backed up .v1bak). Live jobs UNTOUCHED at swap: ByteGPT sweep (981230/981235) + ConvMoE queue (1283532) + summer rbfe-prod (110588) all alive; GPU ~6.2GB (sweep active)."
  scope   = "PATCH + RE-QUEUE ONLY — NO trained model, NO GPU fired this turn. a_completeness_over_cheap: v1 prior-window kept as the honest labelled BASELINE arm for a G5 A/B, semantic is the PRIMARY. Lane-G torch REFERENCE mouth (a_clm_gen_pipeline), NOT forge PUBLIC, NOT CORE A⇄G. Semantic retriever = anima's OWN prior-window ring (index-free, $0), NOT external RAG; at inference the same cosine scores the query vs stored kosmos anchor texts via generator_read_anchors (kosmos_io→brain single-entry, a_core_engine_map · a_kosmos). p1-p8: anchor = retrieved decode context trained INTO the weights, NOT system-prompt/persona/reward. a_scale_honest_scope: tiny-stub fit-probe proves the sampler+head build/run end-to-end and that semantic out-overlaps prior on a sample — NOT that 303M-RETRO passes a303m_pass; the H_1148 copy-acc lift is TOY-scale, 303M transfer UNVERIFIED."
  artifacts = "CLM/model/retro303m_en.py (semantic_anchor_batch + _byte_trigram_profile + _profile_cosine) · CLM/train/retro303m_en_train.py (--anchor flag + dispatch) · CLM/train/retro303m_en_queue.sh (--anchor=semantic fire) · CLM/train/retro303m_semantic_probe.py (CPU fit-probe). Branch retro303m-semantic-anchor (off retro303m-en-prep), pushed origin. aiden watcher PID 1370056."
  refs    = "h1148 (branch h1148/semantic-retriever @2ee1c8420 · .verdicts/1148_semantic_retriever/H_1148.txt) · h1147 · retro303m_en_prep (this log, axis-4 v1) · MODEL.md (anima-303M-RETRO · a303m_pass G5) · a_clm_gen_pipeline · a_core_engine_map · a_kosmos · a_completeness_over_cheap · a_scale_honest_scope · a_paper_negative_ok · p1-p8"
```
