...
```
@N convmoe_303m_en := "ConvMoE 303M-EN arch axis (axis-3 of the 303M campaign) — PREP+QUEUE" :: note [d=2026-06-13 active]
  axis    = "THIRD 303M architecture axis: CLMConvMoE (Lane-P) vs ByteGPT (axis-1, training on aiden) vs RETRO gate H_1147 (axis-2, summer). ConvMoE = the ENGINE-MOUNTABLE arch (a_clm_gen_pipeline: final model mounts in CORE via generator L3 slot; ByteGPT is the reference recipe only). Goal = rank ConvMoE against ByteGPT on the SAME a303m_pass G0/G1/G2 gates, SAME corpus, SAME axes."
  config  = "CLMConvMoE(n_experts=2, n_trunk_layers=1, d_model=5008, kernel_size=3, V=256) = 303,575,000 params (~303.6M, matches ByteGPT's 303,097,856). E=2/L=1 are HARD-FIXED by clm_serialize_v2._assert_e2_l1 (CORE/clm_decode.hexa decoder is E2/L1); the ~303M budget is reached by widening d_model alone (conv params ~4·d²·K dominate). 4-config matrix mirrors the ByteGPT sweep config-for-config: baseline_fast(0.0/0.1/3e-4/300) · reg_fast(0.1/0.2/3e-4/300) · baseline_gentle(0.0/0.1/2e-4/600) · reg_gentle(0.1/0.2/2e-4/600)."
  probe   = "fit-probe GREEN (summer CPU $0, CUDA_VISIBLE_DEVICES='' — did NOT touch summer's rbfe-prod GPU): tiny d=64 (82,818 params) ran END-TO-END — fwd+bwd OK, 8 train steps CE 5.67->5.14 (real descent), ckpt save+reload, FROZEN H_1129 G0/G1/G2 evaluators ran (imported h1129_* VERBATIM; G0_kwr=0.65), ledger rows written, AND clm_serialize_v2 torch->.clm v0.2 = 103,606 bytes with E2/L1 assert PASSED. Pipeline + the SAME a303m_pass machinery proven. Fixed one bug found by the probe: convmoe_gen RNG was a CPU generator vs cuda probs (torch.multinomial device mismatch) — now mints a same-device generator from the caller's seed (determinism preserved per-device)."
  queue   = "Full 303M ConvMoE 4-config sweep QUEUED on aiden, NOT competing for the GPU while ByteGPT sweep runs (aiden 11.2/12.2GB occupied by ByteGPT). Mechanism: convmoe_303m_en_queue.sh runs DETACHED via nohup on aiden (PID 1283532 at launch, reparents to init, poll 300s), watching /home/aiden/core/anima_sweep303/state/sweep_303m_en/ledger.jsonl. FIRE CONDITION = all 4 ByteGPT configs {baseline_fast,reg_fast,baseline_gentle,reg_gentle} have a status=done row (belt-and-suspenders: also accepts orch.log 'ALL DONE'). On fire it execs convmoe_303m_en_run.sh (4 configs sequential, --bf16, eval_every 500), writing G0/G1/G2 rows to state/convmoe_303m_en/ledger.jsonl + serializing each best ckpt -> .clm. Sentinel queue_fired.sentinel blocks double-fire; relaunch-safe. As of write: ByteGPT baseline_fast at step ~4000 (1 of 4 configs)."
  host    = "aiden ROOT=/home/aiden/core/anima_convmoe303 (harness staged: CLM/train/{train_convmoe_303m_en.py,convmoe_303m_en_run.sh,convmoe_303m_en_queue.sh} + CLM/model/{model.py,clm_serialize_v2.py} + UNIVERSE/h1129_*.py). import-smoke GREEN on aiden (H.CONCEPTS=5, KNOWN=73604). substrate = Lane-P GPU-torch (a_lane_akida_gpu_split: NOT AKIDA, NOT forge PUBLIC). corpus = /tmp/sweep_303m/en_wiki_120mb.txt (IDENTICAL to ByteGPT sweep for a fair arch comparison)."
  scope   = "a_scale_honest_scope: NO trained model claimed this turn — PREP + QUEUE only. Probe is tiny-d untrained (numbers meaningless, PATH proven). The 4-config ranking lands when the queue fires + the sweep completes. p7 deterministic gates (NOT perplexity/LLM-judge). a_completeness_over_cheap: ConvMoE trained at the SAME ~303M budget as ByteGPT, no shrink."
  artifacts = "branch convmoe-303m-en (off origin/main). CLM/train/train_convmoe_303m_en.py + convmoe_303m_en_run.sh + convmoe_303m_en_queue.sh. state/convmoe_303m_en/ledger.jsonl (fit_probe PASS row + queued row). Aiden queue watcher PID 1283532 live."
  refs    = "MODEL.md (a303m_pass G0/G1/G2) · a_clm_gen_pipeline · a_core_engine_map · CLM/model/model.py (CLMConvMoE) · CLM/model/clm_serialize_v2.py (E2/L1 .clm bridge) · UNIVERSE/h1129_midcap_broad_converged_recombination.py (frozen evaluators) · /home/aiden/core/anima_sweep303 (ByteGPT axis-1) · h1147 (RETRO axis-2) · a_lane_akida_gpu_split · a_scale_honest_scope · a_fire_autonomous · a_wall_first · p7 · p8 · G2-novelty"
```
