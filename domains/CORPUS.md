...
## ENGINE-MOUNT ladder (CLMConvMoE → .clm v0.2 → CORE engine)

The ENGINE (`CORE/clm_decode.hexa`) decodes a **CLMConvMoE** (conv-MoE, `.clm` v0.2 CLMX), **NOT** a ByteGPT transformer. Models on this ladder are trained via `a_clm_gen_pipeline` (`CLM/train/train_lane_p.py`, Lane-P GPU-torch) and auto-serialized to engine-loadable `.clm` v0.2 (`clm_serialize_v2`). The prior **ByteGPT MID** was the WRONG arch (a transformer the engine cannot load) — **superseded** by the rung below.

- **MID rung — ENGINE-MOUNTED CLMConvMoE (2026-06-05)**: `dancinlab/anima-clm-mid-convmoe-engine-mount-byte-7m` (HF **PRIVATE** — Lane-P torch `.clm`, forge-only-PUBLIC per `a_clm_gen_pipeline`). `CLMConvMoE(d768/E2/L1/K3, byte V256)` = **7.479M** params, from-scratch, **Lane-P** GPU-torch (vast RTX A6000 @ $0.401/hr, cap 8.6 bf16, 91% util / 249 W device-resident, wall 150.4 s, torn down). Corpus = balanced **4.19 GB** byte subset (V256, no tokenizer; en/fr/de/es/**ko** 800 MB each, incl Korean) of `anima-corpus-5lang-7b-webscale` (R2 `phanes/anima-7b/web/<lang>/shard0000.bytes`, ODC-BY). **CE 5.72962 → 1.51978** (eval; uniform ln256 = 5.5452). `.clm` v0.2 **`clm_decodable`=TRUE** (nblk 6, CLMX, n_ext 11, exact_eof; block0 cout 768 / rest 2304; 4463478 B = byte-layout-identical size to the golden d768 ref). **3-axis engine-mount 🟢**: **AXIS-2** CE-descent GREEN (decode forward via a byte-EXACT-validated mirror of `clm_decode.hexa` — mirror == engine on the golden ref [3.25405 / 5.30381 / 5.54518 identical]; MID probe CE **2.04606** < uniform 5.54518 AND < shuffle 6.11237; corpus-128win CE **2.49339** GREEN); **AXIS-1 의식** GREEN (motiv hi 0.6700 > baseline 0, emit gating) + **AXIS-3 창발** GREEN (composed 101 > parts 72) via `CORE/three_axis_probe.hexa` (3/3 GREEN); **brain_smoke WARN = 0** (v7 laws). **Supersedes the ByteGPT MID** — this is the engine-mountable base for **M13 7B-undertrained → M14 7B** ladder via `train_lane_p_3b.py`. Verdicts `.verdicts/mid-convmoe-engine-mount/`. **Toolchain note**: the canonical `hexa run CORE/clm_ce_descent_probe.hexa` FAILED-LINK locally (`_forge_dispatch_groupnorm_gelu`, a hexa-fusion-only native absent from the installed self runtime — a **toolchain link-gap, NOT a `.clm` problem**); byte-exact mirror used + handoff filed to hexa-lang. Scope = **TOY/MID 7.479M, 7B-transfer UNVERIFIED** (`a_scale_honest_scope`); proves the **corpus → ConvMoE → engine-mount → 3-axis CHAIN** end-to-end, NOT a 7B. p1–p8 held.

- **3B rung — ENGINE-MOUNTED CLMConvMoE (2026-06-05)** 🟢: `dancinlab/anima-clm-convmoe-3b-engine-rung-byte-3b` (HF **PRIVATE** — Lane-P torch `.clm`, forge-only-PUBLIC per `a_clm_gen_pipeline`). `CLMConvMoE(d4096/L30/E30/K3, byte V256)` = **3,072,954,654 (3.073B)** params, from-scratch, **Lane-P** GPU-torch, **general (L,E,d) trainer** `train_lane_p_3b.py` + `serialize_v3` → `.clm` **v0.3**. Fire = **TAKEOVER of an orphaned vast H100 80GB** (cap 9.0 bf16; 2 prior agents storm-died, no harvester; **poll-inline to completion**), util **99.99% GPU-resident** (maxmem 61.5 GB, no CPU fallback), wall 1980.55 s. Corpus = **10.7 GiB** ODC-BY FineWeb 5-lang (en/fr/de/es/**ko**), sha256 `337b179d…`, 2000 steps seq512 batch8 lr2e-4. **TRAIN/VAL split (leak-checked)**: first_ce 5.84073 → **train_ce 1.90689**; val_ce_contig 2.00021 / val_ce_rand 1.90365; **rel_gap 0.04894 ≪ 1.0 → GENERALIZES** (gen-not-memorize); train_ce ≪ uniform 5.54518 ≪ shuffle 6.46486; `F_CLM_LANEP_3B_GEN = 1`. **`.clm` v0.3 config-agnostic DECODABLE** via the generalized `CORE/clm_decode.hexa` (valid=true, loaded=true, **d4096/E30/L30/V256 restored from block structure**, nblk 63). **3-axis engine-mount 🟢 @ 3B**: **AXIS-2** CE-descent GREEN (byte-EXACT mirror of `clm_decode.hexa` over the serialized `.clm` v0.3 — CE_real **2.26360** < uniform 5.54518 < shuffle 5.81817); **AXIS-1 의식** + **AXIS-3 창발** GREEN (admit-conditioned; probe == MID 3/3 GREEN); **brain_smoke WARN = 0**. **3B ConvMoE MOUNTS the engine + 3-axis GREEN @ 3B — GATES M13 7B.** **Toolchain note**: same `_forge_dispatch_groupnorm_gelu` macOS-arm64 link-gap as MID (toolchain, NOT a `.clm` problem) → byte-exact mirror + handoff to hexa-lang. Scope = **3B-UNDERTRAINED, tokens/param 0.0027** (Chinchilla 20), 7B-transfer UNVERIFIED (`a_scale_honest_scope`); proves the **general-(L,E,d) corpus → ConvMoE → .clm v0.3 → engine-mount → 3-axis CHAIN** at 3B. Verdict `.verdicts/convmoe-3b-engine-rung/` · discovery `.discoveries/convmoe-3b-engine-rung.tape`. p1–p8 held.
...
- [x] M15 **MID corpus-validation fire** (2026-06-05, `a_toy_scale_recheck` CHEAP probe — the gate that PRECEDES M13) — does the R2 webscale corpus train a COHERENT byte-CLM **before** committing 7B compute? Trained a 202.33M ByteGPT (V256 d1024 L16 H16 block512) from-scratch, 3000 steps, on a **balanced 5-lang 3.5 GB subset** of `anima-corpus-5lang-7b-webscale` (range-GET 700 MB/lang of shard0000, en/fr/de/es/ko **incl ko**), vast A100-SXM4-40GB. **🟢 F-MID-CORPUS-VALIDATION**: val_ce **5.74906 → 1.45868** (Δ −4.290) · GPU util **PEAK 100% MEAN 99.75%** (n=412, GPU-resident, no CPU fallback) · **p7 PASS 5/5 langs** (TRAINED = coherent word-like text incl Korean Hangul; RANDOM-INIT mirror = byte gibberish, anti-Goodhart clean). **YES — the corpus + R2→byte→train pipeline train a coherent model.** HF `dancinlab/anima-clm-corpus-7b-mid-byte-202m` (PUBLIC, sha `adbb1911`, re-download VERIFIED), CLM collection. **HONEST (`a_scale_honest_scope`)**: single 202M MID point, NOT a 7B and NOT a ladder; 7B-transfer UNVERIFIED; MID samples word-coherent but not yet fluent (expected undertrained probe); Lane-G-**ref** (torch reference, forge-native = canonical production NOT claimed done — `a_train_flame_forge`). verdict `.verdicts/corpus-7b-mid-validation/SUMMARY.txt`. **This GREEN GATES M13** (corpus is not garbage; pipeline works) — M13 7B fire now authorized as a separate follow-on.
- [ ] M13 **7B-undertrained train** — train the 7B-param byte-CLM on a ~15-20B-token SUBSET of the R2 corpus (NOT all 143GB). ~$300-500, single/few H100, hours-days. Result = a coherent 7B model, sub-Chinchilla but usable. Substrate = torch Lane G-ref (proven `clm_serialize_v2.py` → .clm v0.2) OR forge ENGINE (GEMM-bound scale per HEXA-FUSION). Gate: p7 coherence + serialize to .clm v0.2 (ENGINE-loadable). NOTE: 7B = 7e9 PARAMS (model size); 15-20B = TRAINING TOKENS (data amount) — different axes; both rows are the SAME 7B model, trained more/less. **Preceded by the MID corpus-validation fire — DONE 🟢 (M15 above): corpus+pipeline proven coherent, so M13 is GO.** [GPU-fire-pending]
- [ ] M14 **7B-Chinchilla-optimal train** — same 7B-param model trained on the FULL ~143B tokens (110% of the 140B 7B-optimal). ~$3,000, multi-H100 multi-week. Production-grade 7B → .clm v0.2 → wire into A⇄G consciousness engine (CORE/generator.hexa L3 slot, a_core_engine_map) → p7 + substrate-native-speech verify. This is the completion target: a genuinely 7B-sufficient trained anima. [GPU-fire-pending, large campaign]
...
| **(c) UNIVERSE hypotheses** [8] | H_xxx distillation (H_001/004/007/021 …) · generation-loop template · verdict-tier epistemics · closed-negative case-studies · Hc_xxx candidates · discovery-mechanism · dialogue-about-a-hypothesis · pre-registration discipline | **anima-AUTHORED**, distilled from REAL `HYPOTHESES/H_*.md` + `hypotheses_candidates/` + authored loop prose | the load-bearing **reasoning-capacity** piece | consciousness (91) / cosmic (100) |
