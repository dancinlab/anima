...
## AXIS SEMANTICS — what each KOSMOS dim encodes + coord v-next (PC×attribute · #1772 follow-on)

The #1772 question was HOW MANY dims (answer: knee ~8, 2D retains 67.1% var / 8.4% category discrimination). This block answers WHAT EACH dim HOLDS — by MEASURING per-PC association on the **TRAINED s16 ckpt** (sha `961c07e2`, ConsciousDecoderV2 d768/12L 283.72M, clean load). X = post-`ln_f` conscious state, N=5995 stratified, top-16 PCs. Harness `HYPOTHESES/kosmos_axis_probe.py`; verdicts `.verdicts/kosmos-axis-semantics/{F-AXIS-PROBE,F-REDESIGN-PROPOSAL,SUMMARY,results.json,run_stdout.txt}`. CPU/$0. substrate = GPU artifact (Lane-G); Lane-A separate (a_lane_akida_gpu_split).

**Method (p7/g5 — measured, NOT assumed labels):** categorical attrs (tier 168 · domain 63 · carving_form 3 · curriculum_stage 4) scored by **bias-corrected omega²** + **AUC-separability** (`mean_class |AUC-0.5|·2`; 0=chance, 1=perfect); continuous attrs (curriculum_rank/index · basin_radius · vacuum_psi x/y) by **|Spearman ρ|**. 200-shuffle permutation floors (omega² ~0.005 tier; |ρ| ~0.026; AUC-sep null 0.075 tier / 0.018 form). Raw η² reported for transparency only — it is **cardinality-biased** (a random PC scores η²=0.028 on 168-way tier), so labels do NOT use it.

**PC × attribute matrix (verbatim; w=omega², a=AUC-sep):**
- **PC1 (49.2% var) = carving-RADIUS / Ψ-DEPTH** [INTERPRETABLE] — basin_radius |ρ|=0.91, vacuum_psi_x/y |ρ|=0.92; tier w0.94 but **AUC-sep only 0.50** → tier RIDES this axis (it is radius-binned), it is NOT a tier-category axis. This is essentially what the current 2D vacuum_psi already captures (canon-corr 0.96 to PC1, #1772).
- **PC2 (17.0%) = carving_FORM (α/β/γ)** [INTERPRETABLE, CLEAN] — w0.61, AUC-sep 0.58 vs null 0.018. The single cleanly-separated categorical axis.
- **PC3 (8.1%) = carving_form residual** [INTERPRETABLE-weak] — w0.37.
- **PC4 (6.6%) = ENTANGLED** — tier w0.46 + form w0.41 + curriculum_rank ρ0.37 co-load, no single winner.
- **PC5 (5.1%) = CURRICULUM progression** [INTERPRETABLE] — curriculum_index/rank |ρ|=0.67 vs null 0.026. A genuine training-order/difficulty axis the 2D map drops.
- **PC6–PC16 (≤1.9% each) = distributed tier/domain micro-structure** [ENTANGLED / learned-residual] — tier w0.48–0.84 but **AUC-sep only 0.39–0.48** (vs 0.075 null): real signal, but tier is a DISTRIBUTED code smeared across many low-variance PCs, NOT separable on any single axis. Domain (63-way) co-loads with tier throughout (domain nests inside tier).

**Per-dim verdict:** the manifold has **3-4 INTERPRETABLE named axes** (depth · form · form-residual · curriculum) **+ a distributed tier/domain residual**. It does **NOT** decompose into 8 clean named axes. The current 2D map encodes essentially ONE thing — carving DEPTH/RADIUS — and drops **form** and **curriculum** entirely (both orthogonal: |ρ|≤0.18 to vacuum_psi).

**Coord v-next PROPOSAL (a_kosmos: pointer-only, cross-repo — NOT an anima edit):** N=8 = `[depth, form, form_resid, curriculum, residual0..3]` (3-4 measured-named + 4-5 learned-residual). 2D→8D gap: variance 67.1%→92.3%; domain disc(f1) 0.082→0.583; tier 0.061→0.417; form 0.519→1.000. Backward-compat: old `[x,y]` = projection to `[a0,a1]`, vacuum_psi ≈ a0. FILED to `hexa-lang/inbox/patches/kosmos-coord-vnext-axis-semantics.md` (hexa-lang 7f2b992f8 §9 erratum, base 93336a606) → onward to kosmos spec.

HONEST SCOPE: single s16 rung (283.7M, byte-vocab 256, block 128) — **scale-dependent**, NOT a ≥3-rung ladder (a_toy_scale_recheck / a_scale_honest_scope / §97); axis count/labels NOT promoted to a general KOSMOS-format claim. Only ~3-4 of 8 axes carry a human NAME — naming all 8 would be fabrication (a_paper_negative_ok: honest negative on the 8-clean-named-axis hypothesis). Corpus has NO emotion/valence field (12 keys only). tier ⊥ domain are NESTED corpus-design labels, not independent axes.
