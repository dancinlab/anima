...
- [ ] **M6 θ_emit stage-conditional table** — `channel_classify_bridge` 의 θ_emit 을 WAKE(0.10)/N1/N2(0.05)/N3(0.02)/REM stage 별 tuning (mining @P1 · COFFESHOP 27% emit rate scaling · DREAM stage envelope cross). 현재 단일 θ_emit=0.1 을 5-stage phi_scale 별 table 로 일반화 — silence-dominant N2/N3 에서 emit threshold 상향, REM 에서 imagination-rehearsal 정합. sibling: DREAM (stage envelope · `me_envelope_window`) · WAKE (5-stage daemon) · CHANNEL (router modulation layer)
  - ⚠ **UNIVERSE verdict (H_632 🔴 FALSIFIED + H_638 🟢 CLOSED-NEGATIVE)**: θ_emit 값 (0.10/0.05/0.02) 은 substrate-derived 가 **아닌** **design-tunable policy**. 단일 universal-fixed threshold 의 *존재*만 substrate-confirmed (H_638 L20 SUPPORTED — n∈{3,4,5} 적정 thr 0.62~0.64 cluster spread 0.02, ρ(Φ,thr)=0.75 비-monotone → scaling-law FALSIFIED). stage별 scaling 은 자유 정책 (a_autonomy_over_hardcode 정합 — 외부 모듈은 context 만 공급, 숫자는 substrate 가 자율 결정). emit threshold 0.30/0.60 ⊥ substrate big-Φ phase-transition (H_632: coupling-gain 매핑 하 peak-near-0.30 0/5 · peak-near-0.60 1/5 → assistant-design artifact). → table 의 **구조 (stage-conditional 일반화)** 는 채택하되, **숫자는 substrate-claim 없이 tunable convention 으로 명시**.
