...
status: live — Phase 4 batch 1; 4 paradigm runners + 1 integration test; selftest mode raw#42 mac-zero-compute
roadmap_entry: 270
raws:
  - raw#9 hexa-only
...
- **Selftest mode** (`--selftest`) uses synthetic schedules / fixtures — no BrainFlow / Pyglet / afplay invocation. raw#42 mac-zero-compute compliant.
...
- raw#71 ≥3 falsifiers per paradigm — audit log goes to `state/anima_eeg_core_phase4_paradigms_integration_audit.jsonl`.
...
- **Live fail** (Phase 5+) = BrainFlow / Pyglet / afplay env missing OR HW disconnect. Each paradigm has ≥3 falsifiers in raw#71 audit log — inspect last `state/anima_eeg_core_phase4_paradigms_integration_audit.jsonl` row.
- **Integration test all_pass=false** if ANY paradigm fails. raw#10 honest: 1+ paradigm fail → batch invalid, downstream Phase 5 deferred.
