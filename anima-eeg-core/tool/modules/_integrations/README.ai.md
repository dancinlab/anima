...
raws:
  ...
  - raw#65 idempotent
  - raw#68 fixpoint
  - raw#71 falsifier ≥3 (5 per module)
...
- `synthetic_fixture.hexa` (551 LOC) is **the** byte-identical regression baseline. Run it first — drift here means downstream selftests are invalid (raw#65 idempotent + raw#68 fixpoint).
...
- raw#71 ≥3 falsifiers per module (5 per module in practice).
...
3. **CLM-EEG P3 currently 380→300 LoC follow-up** (commit cfb771f2a, F3 raw#91 c2-floor revision). Three candidates A/B/C; **candidate B (PLI-wPLI volume-conduction immune) recommended**. Verify which version is wired before relying on metric semantics.
