...
status: live — Phase 1 mini-pipeline; 9 modules; transient writes under /tmp; raw#37 transient + raw#65 idempotent
roadmap_entry: 270
---
...
- All transient writes go to `/tmp/anima_eeg_core_phase1_integration/` — raw#37 transient compliant.
...
chflags_lock.fail (non-macOS host)
  → audit ledger writable but unlocked; raw#10 honest: chflags is macOS-only,
    Linux hosts get noop with WARN log
```

## raw#10 caveats

1. **macOS-bound chflags lock.** `chflags_lock.hexa` is darwin-native (raw#42 mac-local-USB-only adjacent). On Linux this is a noop.
