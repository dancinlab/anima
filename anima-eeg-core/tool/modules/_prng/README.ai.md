...
raws:
  - raw#9 hexa-only
  - raw#10 honest C3 (PORT decision; hexa-internal reference, NOT vs upstream C)
  - raw#65 idempotent
  - raw#71 falsifier ≥3 per module (F_SPLITMIX64_*, F_PCG32_*)
  - raw#82 darwin
  - raw#91 honest triad
---
...
│   └── inlines splitmix64 (no hexa import system; raw#9 hexa-only)
│
...
## raw#10 BYTE-IDENTITY CONTRACT (critical)
...
- True FFI-bridge to upstream Vigna/O'Neill C (raw#10 L1 honest): when a downstream needs cross-validation against the reference C statistical test suite (BigCrush etc.).
...
raw#10 honest: hexa runtime selftest live execution requires `hexa.real run <module> --selftest` on a host with the hexa runtime. On hosts where that's unavailable, the python reference + frozen-vector contract is the ground truth (this README + the `.hexa` file's pinned constants). Any hexa runtime drift will fail F_*_01 immediately.
