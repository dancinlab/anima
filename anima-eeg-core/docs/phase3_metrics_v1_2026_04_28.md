...
which Phase 3 has not). This satisfies raw#18 self-host fixpoint
("reuse first") and raw#42 (no Mac-local re-derivation).
...
the raw#71 falsifier triad to dispatcher consumers.
...
## Dispatcher Routes (Phase 3 LANDED, raw#18)
...
raw#42 compliance: integration test uses **selftest mode only** — no
