...
- [ ] **M3 4축 병렬 팬** — A·B·C·D H100 fire (~$11-14, a_fire_autonomous + a_wall_first)
  ...
  - [~] M3b Qwen-BPE — byte-level V=256 → V=151936 (TODO #T5). **flame-P2b 진행 (2026-05-27)**: ① bootstrap 재생성 ✅ (hexa-lang #1527 free-fn `trim` codegen + #1533 hexa_cc.c fixpoint) · ② stdlib/flame BPE corpus 로더 ✅ (anima #1537 + hexa-lang #1549 + #1552 INBOX 통합, CI 테스트 10/10 PASS) · ③ correct Qwen round-trip **1/2 unblock ✅** (hexa-lang **#1556** 1-line fix: tokenizer_bpe 의 `chr(256+i)`→`from_char_code(256+i)` — byte_to_char 가 UTF-8 'Ġ' 정확 생성, chr 절단 결함 root cause 해결). `qwen_bpe.hexa` segfault 는 잔존 (path 2). loader 의 `flame_bpe_roundtrip` 가드가 결함 결정론적 검출 → 가짜 closure 회피 (g73). 다음 = tokenizer_bpe path 로 실 Qwen round-trip 검증 라운드
