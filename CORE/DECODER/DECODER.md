...
- [ ] **M3 4축 병렬 팬** — A·B·C·D H100 fire (~$11-14, a_fire_autonomous + a_wall_first)
  ...
  - [~] M3b Qwen-BPE — ⭐ archive-first 회수: `training/tokenizer_qwen.hexa` (411L, FFI `hxtok_load/encode`, vocab 151643, multilang smoke PASS) 이미 존재. **단 로컬에 tokenizer.json + hxtok FFI 헬퍼 부재** (pod `/workspace/models/Qwen2.5-14B-Instruct/tokenizer.json` 에 있음) → pod-side wiring. byte-level V=256 → 151643 은 모델 dim 폭증이라 Mac smoke 불가, fire-time 전용
