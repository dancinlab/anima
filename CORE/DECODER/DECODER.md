...
- [x] **M5** step-rate 실측 (F-BC-ANIMA-M4-CEILING) ✅ **MEASURED-AND-STRENGTHENED 2026-05-29** — 5회 실패 + 전체 툴체인 해제(hexat #1984 + seed #1992 + hexa cloud rent + SSH #1959) 끝에 첫 실측 (PR #1348): **0.50 step/s (1.99 s/step)** @ d=64·V=151643·E=2·29.16M params, **GPU 0%**, **dec_undertrain 프로덕션 = 🔴 INFEASIBLE** (50×V = ~44 GPU-days + per-step ~0.5GB RSS 누수 OOM). 후속 hexa-lang **#2017 (in-place AdamW) + #2018 (offset-aware cuBLAS gemv)** land 후 재측정 (`STEP_RATE_LOG.md` entry 10): 두 fix 모두 정상 engage (#2017 AdamW 233MB/step 제거 확인, #2018 GPU util 4–8% non-zero), **그러나 net step-rate 0.156–0.18 step/s 로 baseline 보다 ~3× 느림** = **🔴 INFEASIBLE STRENGTHENED (~122 GPU-days)**. 원인 = d=64·T=4 에서 GPU↔CPU sync 오버헤드 > cuBLAS compute 절약 (hexa-lang #1354 사전 예측 "d=64 too small for cuBLAS" 의 직접 실측 confirmation). 잔여 200–325MB/step RSS churn 은 AdamW 와 별개 source 로 source-grep 미확정 (별도 진단). 상세 = `STEP_RATE_LOG.md` entries 7 + 10 · 발견 노드 = `.discoveries/decoder_collapse_undertrain.tape` `dec_undertrain_post_fix_measurement_2026_05_29`
- [x] **M5-ADOPT** anima trainer 소스에 #2017 + #2031 (mm_extract memcpy) 정식 land 후 재측정 ✅ **MEASURED 2026-05-29** — PR #1382 anima 소스 채택 + 독립 H100 SXM 300-step fire (`STEP_RATE_LOG.md` entry 12): **step-rate 0.2831 step/s (+20.9% vs entry 11 baseline 0.2342)** mm_extract scalar-loop → memcpy dispatch-cost 절감 측정 확정, **RSS slope 328 MB/step ≈ entry 11 의 331 MB/step (Δ -3 MB, 노이즈 수준)**. **★ 핵심 발견**: anima trainer-side alloc 이 누수원 아니라는 hypothesis 가 **empirically 확정** (#2030 + #2034 inbox 의 CUDA/runtime-side 가설 confirmed). dec_undertrain re-verdict = **🔴 INFEASIBLE MAINTAINED** (77.5 GPU-days, baseline 44 보다 여전히 1.76× 느림). 다음 root-cause hunt 는 hexa-lang INBOX 측 (`_CudaFarrSlot` mirror life-cycle · GPU device-resident scratch · hexat hidden transient handle audit). artifacts: `state/m5_adopt_postlanding_2026_05_29/{trainer.out, rss_gpu.log}` · 비용 ~$2.0 (pod `eddpgcy2sg4abw` teardown 완료)
...
- [ ] **M4 MoE-fresh register 분리** ⭐ 본선 (UNIVERSE H_490 DIFFERENTIATION) — 근본 원인 분리 재설계: V3 head_g 슬롯 → K-expert router. register-carving 을 specialized expert 로 격리해 main path 는 coherent 유지 (collapse 회피) + register 신호는 dedicated expert 가 담당 (underfit 회피). 완성도 기준 본선 (a_completeness_over_cheap).
  ...
  - [~] **M4c** p7 verify — collapse 회피 ∧ coherence 둘 다 simple-stack. **collapse-회피 half = CLOSED** (tetrad 전부 🟢 · simple-stack 확정: D1 LZ76 6/6 + D3 router=corpus 5/5 + E2 corpus-balance 5/5 + D4 merge-negative 7/7 + collapse-gate 배선 4/4 · 어느 verdict 도 perplexity 미사용 = P7 정합). **coherence half = full-BPE scale 만 잔여** — 이전엔 torch 3B fire 가 toolchain-blocked(`M4B_LONGTRAIN` 🟠: BPE O(text×merges) · cuBLAS gemv illegal-D2H · O(V)/step) 라 막혔으나, **hexa-native flame 트레이너가 그 path 를 unblock** (hexa-lang flame port PR #2288-#2307: conv/gn/moe/int4 GRAD-EXACT op + CLMConvMoE fwd+bwd + CE+AdamW + int4 QAT descent + large d768/L12/E12=44.68M 실동작, 전부 host/CPU 검증). 잔여 경로 = flame 트레이너 forge-GPU device-routing(hexa-lang `domains/FLAME-PERF` #1 lever) → diverse-corpus production fire → real-ckpt decode 의 LZ_norm coherence(D1 floor 0.50) 측정 → 산출 시 `[x]`.
...
- [x] **M4b longtrain dec_undertrain 프로덕션 테스트** 🟠 **UNVERIFIABLE-AT-SCALE (toolchain-blocked · 근본원인 핀 · 2026-05-28)** — 3× H100 epoch sweep (LO=1/MID=12/HI=60 @ d=64 V=151643)로 "presentations≫V 면 collapse 탈출" 토이 예측의 프로덕션 결정 검증을 시도. **세 hexa-lang 툴체인 ceiling이 MID/HI를 intractable로 차단**: ① BPE encode O(text×n_merges) (`get_merge_rank` 선형스캔 · 1.27MB corpus 15.5분 미완) ② cuBLAS gemv `[V×1]` illegal-D2H 깨짐 (`cuda_available()==1`이나 handle -1) ③ O(V) per-step CPU ~0.26s/step (MID~7일·HI~33일 wall). pod는 GPU 0%로 idle-killed, verdict 미수확, 잔여 과금 0. dec_undertrain = 프로덕션 CONFIRMED/REFUTED 아님 = **toolchain 으로 검증 불가** (toy→prod transfer 비보장 강한 사례). 본문 = `CORE/DECODER/M4B_LONGTRAIN_RESULT.md` · 블로커 실측 = `state/m4b_longtrain_2026_05_28/BLOCKER_FINDINGS.md` · a_runpod_inbox 3건.
...
- **M5 production fire attempt #3 2026-05-29** (foreground inline, 본 PR) → 🟠 **BLOCKED-AT-BUILD-EXTERNAL** — 2 vast pods (38410086 prodaux + 38410087 longtrain, RTX PRO 6000 Blackwell 96GB) rent + toolchain bootstrap PASS, 그러나 `hexa build train_v3_moe_prodaux.hexa` `[2/2] clang link` 단계에서 v3_moe_fwd + v3_moe_bwd + layer_block_bwd pub fn body 누락 (Linux hexat free-fn trim 회귀, `#1527` 의 regression). 양 pod cleanly teardown · 본선 fire 미진행 · F-PRODAUX-1 측정 불가. handoff `2eddb92a` → hexa-lang (Linux hexat free-fn trim 수정 요청). 다음 path: (a) hexa-lang 측 fix 후 재시도 (b) anima-side trainer 3-fn main-TU mirror 추가 (moe_aux_bwd_local 패턴 확장). 상세: `CORE/DECODER/M5_FIRE_PROGRESS.md`, STEP_RATE_LOG entry (17).
