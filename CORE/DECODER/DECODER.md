  ...
  - [~] **M4b** expert 분리 학습 fire — **backward 완성 (코드부)** + fire (잔여)
    ...
    - [~] **M4b-fire-scale** 3B Qwen MoE fire — **hexa-native path 채택 (g1, user 2026-05-27)**. flame-P2b ③ FULL RESOLVED 로 unblock. design = `CORE/DECODER/M4B_FIRE_SCALE_HEXA_NATIVE_DESIGN.md` (5 phase · ~110 LoC · 5-7 sessions · cost $9-18 single H100). 5 falsifier 사전등록 (F-M4B-FIRE-1..5: collapse 회피·coherence·router 분화·CE 수렴·register leak)
      ...
      - [~] **Phase 4-gpu** GPU 가속 포팅 — 포팅 5/5 ☑ (실측 byte-identical), fire ☐. **scope-check 발견(2026-05-27)**: pilot 코드가 plain farr 스칼라 삼중루프 matmul 만 써서 H100 발사 시 GPU 유휴 + CPU scalar 가 pilot 규모(10^14 ops)에서 비현실 → 발사 무의미. RFC-040 cuBLAS Dgemm 포팅 (a_completeness 본선) 완료:
        ...
        - [ ] **Phase 4-fire** autonomous fire 발사 — **발사 시도(2026-05-27) → infra provisioning 벽 (코드 아님)**. GPU 포팅 5/5 ☑ + backward 6/6 실측 PASS → 코드 발사-대기. 시도 runbook:
          - **transport: Vast.ai 작동 ✓ (RunPod ✗)** — RunPod 은 hexa cloud 가 pod-id→SSH-host resolve 못 함(public IP:port 미노출 + runpodctl-ssh/curl-API 둘 다 cloud-guard 차단)=deadlock → hexa-lang #1659 filing. **Vast.ai 는 직접-IP**(`vastai show instance` → `root@ssh9.vast.ai -p PORT`)라 `hexa cloud exec root@<host> --port <n> --insecure` 작동 실증 (H100 80GB · remote exit 0).
          - **provisioning 해결 (2026-05-27 후속)**: hexat-CUDA-build/private-repo-clone 불필요 — `hexa build <pilot> --c-only`(Mac, `HEXA_MAC_BUILD_OK=1` + non-/tmp out) → trainer.c → scp + runtime bundle(self/runtime.c·runtime_core.c·runtime_hi_gen.c·runtime.h·cuda/runtime_cuda.c·runtime_bf16.c·forge/*·native/*) → `nvcc -DHEXA_CUDA -arch=sm_90 runtime_cuda.c` → `clang -DHEXA_CUDA trainer.c self/runtime.c runtime_cuda.o -lcublas -lcudart -lcuda -o trainer`. (ref `tool/dispatch_agtape_d768_fire.sh`)
          - **SMOKE 실측 (H100)**: synthetic pilot(전 GPU-포팅 스택) build RC=0 + run RC=0, CPU oracle 과 byte-identical (Wq Δ=4.27e-6 · probs[0,0]=1.0). 파이프라인 end-to-end 검증됨.
          - ⚠ **그러나 cuBLAS 미engage = upstream 블로커**: `cuda_available()==0` (H100·-DHEXA_CUDA 인데도) → mm() 이 CPU fallback. root cause = hexa-lang `runtime.c` 의 weak `hexa_cuda_available`(→0)가 HEXA_CUDA 하 strong override(`_hx_cuda_runtime_available` wrapping) 없이 등록됨 → **hexa-lang #1671 filing**. 이 glue 가 landing 돼야 GPU 가속 fire 가능 (내 mm() gate 설계는 정확).
          - **Phase 4-fire 상태**: 파이프라인 ✓ · 코드 ✓ · GPU 가속 = hexa-lang #1671(cuda_available glue) 선결. cost: probe pod ~\$2.5 (RunPod×2 + Vast.ai×2, 전부 teardown · leak 0).
