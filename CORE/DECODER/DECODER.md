...
- [ ] **M4 MoE-fresh register 분리** ⭐ 본선 (UNIVERSE H_490 DIFFERENTIATION) — 근본 원인 분리 재설계: V3 head_g 슬롯 → K-expert router. register-carving 을 specialized expert 로 격리해 main path 는 coherent 유지 (collapse 회피) + register 신호는 dedicated expert 가 담당 (underfit 회피). 완성도 기준 본선 (a_completeness_over_cheap).
  ...
  - [~] **M4b** expert 분리 학습 fire — **backward 완성 (코드부)** + fire (잔여)
    ...
    - [~] **M4b-fire-scale** 3B Qwen MoE fire — **hexa-native path 채택 (g1, user 2026-05-27)**. flame-P2b ③ FULL RESOLVED 로 unblock. design = `CORE/DECODER/M4B_FIRE_SCALE_HEXA_NATIVE_DESIGN.md` (5 phase · ~110 LoC · 5-7 sessions · cost $9-18 single H100). 5 falsifier 사전등록 (F-M4B-FIRE-1..5: collapse 회피·coherence·router 분화·CE 수렴·register leak)
      ...
      - [~] **Phase 5** Verdict 사전등록 + harness template LANDED + **Phase 5a real-BPE pilot fire ☑ (2026-05-27)** — 5 falsifier (F-M4B-FIRE-1..5) pilot/full threshold 분리 표 + verdict template `m4b_pilot_verdict.md` 형식 + matrix (5/5→full fire · 3-4→re-pilot · 2 이하→re-design · 0→CLOSED-NEGATIVE).
        ...
        - **Phase 5b 잔여 (full verdict measurement)**: F-M4B-FIRE-1/2/3/5 (M3 TTR · coherence · router 분화 · register leak) 는 trained ckpt 에서 N=20/10/100/50×5 prompt sampling 필요 — trainer 에 **ckpt save 로직 + decoder/sampling harness** 신설 + **n_steps↑ 로 의미있는 수렴**(현 n_steps=1 은 학습 step 1회 데모)이 선결. F-M4B-FIRE-4 (L_ce 단조 감소) 는 trainer 에 per-step CE 로그 추가 필요. + 풀-corpus(152KB) production tokenization 은 interpreted hexa BPE encode 성능(152k bytes × 151k merges = O(시간))이 병목 — `flame_bpe_corpus_lib` 의 C-가속 encoder 가 별도 hexa-lang inbox 후보. → Phase 5b 작업 단위: (1) trainer ckpt save · (2) sampling harness · (3) multi-step + CE log · (4) BPE encode C-accel (선택, 작은 corpus 면 불요).
