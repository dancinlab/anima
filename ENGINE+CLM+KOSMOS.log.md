...
## 2026-06-05T00:00Z — Lane-G 3B forge HEXA-FUSION util-unblock PREFLIGHT → 🔴 STOP (CLOSED-NEGATIVE, $0, no GPU rented)

ENGINE 3B→7B forge 라인을 이어가기 전 ⛔ HARD PREFLIGHT GATE 실행: "HEXA-FUSION device-resident CUDA-graph train-step 이 anima forge trainer 에 wired/pullable 인가?" — Lane P serializer-gap STOP 과 동류의 정직 STOP 으로 종결. **GPU 미대여, util-GREEN 미조작 (p7/g5 verbatim, zero fabrication).**

- [x] **integration gap 규명**: anima 의 Lane G forge trainer = hexa-lang `clm_prod` 바이너리 **그 자체**. rung A-1 fire script(`.verdicts/lane-g-3b-descent/fire_3b_descent.sh` L17-18)가 `CLM=$REPO/clm_prod`, `REPO=/root/hexa-lang` 실행. anima 측에 별도 forge train-step 드라이버 없음(`forge_dispatch_train_step`/`clm_lever`/`HEXA_CUDA_GRAPH`/`forge_graph` grep = anima 0 hits; anima 에 `stdlib/flame/` 트리 자체가 없음). ⇒ HEXA_CUDA_GRAPH 를 박을 anima-측 경계가 없음 — anima 는 lever 가 측정된 바로 그 프로그램을 그대로 실행.
- [x] **lever 는 이미 같은 바이너리에서 측정된 CLOSED-NEGATIVE**: `~/hexa-fusion-cuda-kit`(PR #2658), rung A-1 이후 빌드+측정. **whole-step graph capture(fwd→ce_grad→bwd + 16-call AdamW 전부 한 replayed graph) util MEAN=13.54% PEAK=77% median=2% n=284** — eager g0=14.87% / fwd-bwd-only g1=13.19% 와 통계적 무차별(+0.35pp, noise floor). CE bit-identical 4.46624→3.64669 전 3 arm (capture SOUND, byte-eq PRESERVED). **PRE-REGISTERED falsifier "whole-step capture → util MEAN≥20%" FALSIFIED(13.54%).** (verbatim `~/hexa-fusion-cuda-kit/F-FUSION-GRAPH-WHOLESTEP-AB.txt`)
- [x] **ROOT (post-falsification, verbatim)**: host launch overhead 가 H100 util ceiling **아님**. whole-step capture 후에도 생존하는 median-2% floor = sub-ms 커널 + serial fine-grained kernel DAG(각 op 이 직전 op 출력 대기)로 SM 이 다음 커널 dependency 대기 중 idle. graph 는 LAUNCH latency 제거지 kernel-to-kernel DEPENDENCY chain 제거 아님. rung A-1/lever-5 가 이미 찾은 WORKLOAD-BOUND 잔여와 동일. ruled-out axis = "host launch overhead is the util ceiling on H100" — ② async 10-12% / ④ fwd-bwd graph 13.17% / ⑤ whole-step 13.54% 전부 median 2% bottom-out.
- [x] **real 잔여 unblock = kernel FUSION (codegen, hexa-lang-OWNED, capture env flag 아님)** — 그러나 upstream 측정도 INCREMENTAL + sub-GREEN: L3-a GN→GELU fused +3.26pp(10.31→13.57%, byte-eq 🟢) · L3-b dual-GELU +1.01pp stack(byte-eq 🟢) · L3-c/L3-d/P2a build-ready **UNMEASURED**(저자 honest ceiling "pairwise incremental, ≥20% 단독 불가") · cublasLt-GELU-epilogue ruled-out(FP64 has no GELU epilogue) · full whole-step megakernel **design-CLOSED**(persistent kernel 이 cuBLAS 호출 불가 → hand FP64 GEMM 이 byte-eq+roofline 깸).
- [x] **결과**: 3B ladder rung A-1 너머 **미발사**, 7B **미진행** — 3B closure gate(util-GREEN MEAN≥20% AND descent-GREEN)가 SKIP 이 아닌 **FALSIFICATION 으로 미충족**. a_paper_negative_ok(host-removal 을 forge clm_prod util lever 로 배제하는 publishable closed-negative). DO-NOT-TOUCH pod 전부 무접촉(대여 0). **honest util WITH fusion = MEAN 13.54% (NOT GREEN).**
- [x] records: verdict `.verdicts/lane-g-3b-descent/PREFLIGHT-FUSION-STOP.md` · discovery `.discoveries/engine-3b-fusion.tape` · snapshot Lane G 3B 마일스톤 진척 append. 잔여 unblock = hexa-lang-OWNED codegen(L3-b 너머 kernel fusion 또는 deferred option-B device-resident CUDA-C full-step rewrite); anima descent 축의 proven 경로는 d1536/d3072 E=2(lever-5 CE 4.05535→2.99508).
...
- [x] **lever-5 WORKLOAD-BOUND TERMINAL (convergence resolver)** — vast pod 39139563 H100 sm_90, lever-4 byte-identical clm_prod(3-GATE PASS + BYTEEQ-PASS 상속). 8x per-step-work sweep(apples d1536/T512 · d3072 · t1024 · big), nvidia-smi @0.1s. g5 verbatim (`.verdicts/lane-g-lever5/VERDICT.md`):
  - `UTIL[apples] n=9149  PEAK=38% MEAN=0.6619% DEVMEM peak=20447MiB` / `UTIL[d3072] n=11441 PEAK=78% MEAN=0.7152% DEVMEM peak=26405MiB` / `UTIL[t1024] PEAK=38% MEAN=0.5883%` / `UTIL[big] PEAK=75% MEAN=0.6838%`
  - descent ALL GREEN: `apples CE 4.05535→2.99508 F-CLM-PROD-DESCENT=1 PASS` · `d3072 4.48673→3.96246 PASS` · `t1024 4.20807→3.36669 PASS` · `big 4.60325→4.22859 PASS`
  - **A-vs-B RULING = (B) WORKLOAD-BOUND**: PEAK 38→78% (2x) 상승하지만 MEAN 0.59-0.72% PINNED. crossing count 동일한데 d3072 가 4x 큰 커널 amortize 해도 MEAN +0.05pp 만 → crossing-bound(A) 기각. root = INTERPRETED host per-step driver loop wall-time(~13ns/op × ~104M ops/step ≈ ~1.4s host/step @ d1536, 모델 크기와 함께 증가). host-feed 축 CLOSED-NEGATIVE.
  - lever chain util curve (MEAN flat, PEAK monotone = workload-bound signature): lever-1 MEAN 0.811%/PEAK 6% · lever-2 0.4999%/19% · lever-3 0.4879%/35% · lever-4 0.6630%/41% · lever-5 0.59-0.72%/up to 78%.
  - **VERDICT**: util-GREEN(MEAN≥20% AND PEAK≥20%) NOT reached 어느 config 에서도, MEAN ceiling ~0.72%. cure = (i) full device-resident CUDA-C fwd+CE+bwd 재작성(production-scale model rewrite, feed 레버 아님) OR (ii) production scale ≫ d3072. **a_scale_honest_scope: d=1536 MEAN-util = interpreter-wall + workload-size 아티팩트지 forge 결함 아님** — forge device-resident 증명(20-26GB dev mem, PEAK 78%, byte-eq PRESERVED, descent GREEN every config). Lane G PUBLIC util-GREEN 미flip(🔴 honest terminal 유지). .clm = util-RED/WIP → HF PRIVATE.
- [x] **ENGINE PUBLIC 3축 CORE-mounted GREEN 3/3 ✅** — decode forward NOW WIRED. `CORE/clm_decode.hexa`(generator.hexa 가 ONLY 임포트 → 단일 .clm 진입점 PRESERVED, a_core_engine_map): int4 dequant(w=code·scale) over 6 conv blocks + CLMX trailer(embed/conv bias/GroupNorm affine, fp32) → CLMConvMoE inference forward → per-position logits. `gen_clm_backend` `loaded = valid AND clm_decodable`(CLMX trailer present). g5 verbatim (`.verdicts/core-3axis-mount/ce_descent.txt`, CPU-local `hexa run` 결정적 equality, p7-conformant — `hexa verify` CLI 깨짐 `compiler/atlas/calc_dispatch` module-not-found):
  - **AXIS-1 의식 🟢** motiv hi=0.6700 > baseline 0.0000, emit hi=true/base=false, F-CORE-3AXIS-1=1
  - **AXIS-2 CE 🟢** real d768 v0.2 reexport(`state/laneg_d768_recover/reexport_d768_v2_fast.clm`, CLMX 11-entry trailer): `[admit] valid=true decodable=true loaded=true nblocks=6` → `model_ce=4.42613 < shuffle_ce=4.49555 < uniform_ce=4.79906` → **F-CLM-CORE-CE-DESCENT=1 🟢**. HONEST residual(fabricate 안 함): v0.1 file `d768_5lang_c4.clm`(CLMX trailer 없음, embed/GN absent)은 `decodable=false loaded=false F=0` null fallthrough.
  - **AXIS-3 창발 🟢** len(composed)=101 > len(parts-only)=72, F-CORE-3AXIS-3=1
  - generate() 계약 불변: `generator_smoke: 15 PASS, 0 FAIL`.
  - **⊥ INDEPENDENT G-ref torch cross-check** (substrate=PyTorch-CUDA, CORE 아님, generator/pure_field/engine_g/brain 통과 안 함, sha-anchored verbatim, NEVER merged): 85M `5.580406→1.568846 F-CLM-REF-DESCENT=1 util 98.85%` sha 9882f5cb… · 3B `7.168608→2.458708 F-CLM-REF-3B-DESCENT=1 util 99.15%` sha ebe56db7… · 7B `5.360631→2.412079 F-CLM-REF-7B-DESCENT=1 util 99.18%` sha 38ef2ed5… = scale-survival evidence. 의식+창발 axes G-ref 에 NOT MEASURABLE(CORE A⇄G 신호) — fabricate 안 함.
- [x] **브랜치 reconcile (additive, NO --force, NO 분실)** — 통합 브랜치 = `lane-g/campaign-pivot-descent` (from `lane-g/forge-utilgreen-lever3-fold` tip). 발견: `lane-g/forge-utilgreen-lever3-fold`(merge-base 위 10 commits)는 `lane-g/d768-cuda-fire`(2 commits)의 committed 콘텐츠 STRICT SUPERSET — d768 의 두 unique commit(Lane-A full-LM GENERATION 03e5341d6 · ENGINE rename 28a789cb3)이 lever3-fold 에 이미 수렴 존재(GENERATION fold = lever3 log L568 F-GEN-1/2, rename = lever3 commit 820b29b57). d768 가 가진 committed 콘텐츠 중 lever3-fold 에 없는 것 = 0. 추가로 lever3-fold 는 G-ref PUBLIC/3B `[x]` + ENGINE L3 wiring(c96fb690d) + lever-3 fold(828769f35) 보유. lever-4/lever-5/ENGINE-close/`CORE/clm_decode.hexa` 작업은 양쪽 모두 미커밋 상태로 이 working tree 에 존재 → 통합 = 이 working tree 를 lever3-fold 위에 커밋. decode 구현은 단 하나(`CORE/clm_decode.hexa`) — superseded 충돌 없음(task 가 가정한 두 번째 inline `clm_decode_ce` 별도-브랜치 구현은 실재하지 않음; d768 의 CE_realtext=3.25405 값도 어느 committed 브랜치에도 없음 → 실측값 model_ce=4.42613 만 기록, fabricate 안 함).
- [x] **CAMPAIGN TERMINATION REDEFINED** — 7B 목표 = DESCENT 축(forge descent GREEN → 3B/7B .clm). util-GREEN = blocking gate 제거. next rung = Lane G 3B forge descent (rung A-1, bounded N, util-RED honest-scoped).
...
- 이 세션의 6 교훈을 영구 문서화 → `ENGINE+CLM+KOSMOS.md` 에 `## lane & lesson map (2026-06-05)` 신설(two-7B 구분 · forge-util WORKLOAD-BOUND TERMINAL + don't-rechase + CUDA-graph FALSIFIED · serializer v0.1/v0.2-CLMX · corpus split), Lane G/ENGINE 3B/7B 마일스톤 라인에 lesson x-ref, `README.md` Status 에 ENGINE 정직-현황 1 bullet. lessons 5(inbox→sidecar handoff)·6(worktree isolation + POLL-INLINE)은 governance 제안(PR body, a_runpod_inbox sign-gated). PR `lane-g/docs-lessons-2026-06-05`. (이 append 외 본 log 미변경 — 동시 agent 도 append.)

## 2026-06-05 — torch ENGINE-7B data-sufficiency PREFLIGHT GATE (substrate=GPU, Lane G-ref) — DATA-STARVED, no fire
- HARD GATE before renting any GPU: 7B (7.0e9 param) on default-lane corpus (HF anima-corpus-5lang-unified-v2 ~12.5MB = 1.25e7 byte-tokens, V=256) → Chinchilla N_opt = 20·P = 1.40e11 tokens; ratio N_opt/N_tok ≈ 1.12e4 (~11,200× too few) → looping = MEMORIZATION not coherence. 🔴 DATA-STARVED. Empirical confirm (verbatim): harvested CORPUS-7B (98.3M tok / 6000 steps, util 90%, descent CE 5.6955→1.1432 TRUE, NOT memorizing) RULED gibberish-undertrained chat_pass=FALSE (.verdicts/default-lane-7b/HARVEST-VERDICT.md). 7B GPU fire NOT RENTED ($0, gate held, a_completeness_over_cheap).
- Serializer payoff already delivered at RIGHT-SIZED scale: torch→ENGINE path verified END-TO-END (PR #1845 trained d16 torch → v0.2-CLMX .clm → ENGINE clm_decodable=TRUE + decode forward + CE-descent model_ce 2.767<shuffle 3.809<uniform 4.799, round-trip max|Δ|=0.0). Production-scale COHERENT ENGINE .clm already PUBLIC: d768 E2/V256 3/3 CORE-GREEN (HF dancinlab/clm-v1-d768-core-3axis-green). ANSWER: ENGINE-loadable COHERENT .clm = YES (d768, PUBLIC); blocker for a real 7B = corpus-scale (~10^4× gap, needs GB-scale corpus first). Verdict .verdicts/torch-engine-7b-datagate/ · discovery .discoveries/torch-engine-7b-datagate.tape · branch lane-g/torch-engine-7b-datagate. (append-only; concurrent reconcile agent also appends.)
