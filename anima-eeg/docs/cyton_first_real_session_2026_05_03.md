...
raw#9 hexa-only · raw#10 honest C3 · raw#15 no-personal-paths · raw#71 falsifier-bound · raw#80 sentinel · raw#91 honesty-triad
...
본 cycle 측 audit harness BG 측 read-only (raw#9 hexa-only audit script 측 anima-eeg-core/tool/cyton_first_real_audit.hexa 또는 transient `/tmp/cyton_first_real_audit_helper.py` raw#37 transient-helper 경로 권장).
...
3. **전극 미부착 시 floating null** — F_CTON_REAL_03 PASS 라도 전극 head contact 측 별도 단계 (`anima-eeg/calibrate.hexa --impedance` 또는 `anima-eeg/electrode_adjustment_helper.hexa`). 본 cycle step 3 측 dongle ↔ board ↔ ADC 까지만 verify, head ↔ electrode 측 미verify. floating null signal 측 std > 0.1 PASS 가능 (DC drift 만으로) — phenomenal EEG 보장 X (raw#10 honest C3, IIT functional/access tier).
