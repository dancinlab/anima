...
- [x] M13 — north-star HARDENING: re-place anima vs a TRUE optimal reference — DONE 2026-06-07 (H_1019🔴 CLOSED-NEGATIVE, a_paper_negative_ok). H_1015/H_1018 placed anima above a hand-coded `optimal_action` that is only 1-step-GREEDY (myopic). M13 hardened the human reference to a depth-4 receding-horizon MPC (4^4=256 rollouts/step) on the same H_964 env: MPC M=-0.553 (band [-0.603,-0.503]), greedy M=-0.891, anima(WM) M=-0.643 lands BELOW the hardened band (gap +0.089, d=-1.975, p=2.4e-13); reactive -1.924 (D1 valid). FINDING: the H_1018 "above-oracle" was ENTIRELY the myopic-oracle gap — anima beats greedy but NOT a true multi-step optimum; north-star "beyond-human" is BOUNDED to weak references on this toy (sharpens, does not overwrite, H_1015/H_1018). Deeper-MPC / continuous-action ladder OPEN; toy, scale-transfer UNVERIFIED

- [x] M14 — IMAGINE-ROLLOUT closes the M13 hardened gap: planning THROUGH anima's OWN learned world-model recovers the depth-4 MPC band — DONE 2026-06-07 (H_1021🟢 PASS, GAP CLOSED). M13/H_1019 ruled anima's single-step ridge head (M=-0.6426) BELOW the depth-4 MPC band [-0.6034,-0.5034]. M14 gives anima the IMAGINE step the CWM @goal demands (perceive→latent→IMAGINE→act): a receding-horizon planner that enumerates NACT^d action sequences, rolls anima's OWN LEARNED action-conditioned forward model (LDSWorldModel delay=3 act_dim=4, fitted by ridge on the SAME greedy demos — NOT given the true dynamics) forward, decodes via C, scores cumulative -||predicted pos||, plays the first action of the best imagined plan. Result (same env/seeds): imagine-d4 M=-0.5635 CI=[-0.5745,-0.5518] lands WITHIN the MPC band, gap to MPC only +0.0101, vs-MPC Welch p=0.261 (statistically indistinguishable from the true optimum); imagine-d4 vs single-step WM lift +0.0791 (p=1.96e-12, d=1.889 → genuine planning). FINDING: the H_1019 gap was an INFERENCE-DEPTH limit, NOT a WM-quality limit — anima's learned model was already accurate enough to plan through; H_1019 simply never gave anima the imagine step. Validates the CWM closed loop: imagination, not a bigger model, closes the human-bar gap. Sharpens (does not overwrite) H_1019. Secondary finding: non-monotone ladder (imagine-d2 -0.6745 WORSE than single-step — a horizon too short to amortize learned-model forward error underperforms reactive-on-latent). Deeper-MPC / continuous-action / learned-model-fidelity-at-scale ladder OPEN; toy, scale-transfer UNVERIFIED.
- [x] M15 — CONTINUOUS-ACTION generality: imagine-rollout reaches MPC parity when the action space is CONTINUOUS (not enumerable) — DONE 2026-06-07 (H_1025🟢 PASS, CONTINUOUS-PARITY). M14/H_1021 closed the hardened gap on a DISCRETE action space via a 4^d ENUMERATOR; the CWM @goal (act on silicon/SW) needs CONTINUOUS-action planning. M15 replaces the enumerator with CEM (pop=64 iters=5 elite=8 horizon=4) THROUGH anima's OWN learned LDS WM (delay=3 act_dim=2, ridge-fit on greedy demos, never given true dynamics) on a continuous variant of the H_964 hidden-velocity station-keeping env (2-D thrust in [-1,1]^2). Reference = the SAME CEM over the TRUE dynamics (clean model-only isolation). Result (N=40×60): imagine-rollout M=-0.3144 CI=[-0.322,-0.306] lands WITHIN the continuous CEM-MPC band [-0.370,-0.270] (MPC -0.3196; gap -0.0052, vs-MPC Welch p=0.39 → indistinguishable from the true optimum), beating the single-step head -0.3949 by +0.0805 (p=9.8e-23, d=3.10 → genuine CEM search). FINDING: planning-THROUGH-the-learned-model is NOT specific to enumerable discrete actions — the CONTINUOUS-GAP falsifier (search/forward-model error compounding) is REJECTED, generalizing H_1021 to continuous control. Extends the M14 "continuous-action ladder OPEN" residual. Toy single rung; scale/real-robot UNVERIFIED; no Φ claim.
