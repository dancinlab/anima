...
| RTSC_29 | QE 실DFT RbOs2O6(β-pyrochlore) flat-band ΔE + 자성 정직검사 | 🟠/🔴 | aiden(np=3→4, RTSC_27 DFPT 공존·무손상) 실측. SCF(nspin=1) 24iter, **E_F=−0.4290eV**, 77전자. **Os-5d flat band ΔE_flat=+0.377eV(band43, E_F 위, disp 0.048eV, projwfc Os-5d 0.44)** + **E_F에 평탄 O-p/Os-d 혼성 manifold(band38-40, |ΔE|<0.07eV, band39는 E_F 가로지름)** → **CoSn(−0.44)·CsV3Sb5(+0.92)보다 얕음 = ΔE축 승**(셋 중 E_F에 최근접). **그러나 nspin=2 정직검사가 "비자성" 반증**: Os 0.3μB 시드가 0으로 **붕괴 안 함**(total 1.7→5.6μB 성장·진동, abs ~9.4μB), 자성解 E=−644.96Ry로 비자성(−644.80)보다 **0.16Ry(≈2.2eV) 낮음** → **이상 Fd-3m PBE 자성이 기저**(CsV3Sb5 2.8→0.01 붕괴와 정반대). MP "비자성" proxy 반증(RTSC_21 CoSn 재현). **정직 caveat**: 실물은 비자성 SC(Tc≈6.3K) → ~5μB는 frustrated 5d 이상격자 PBE 과대자화(rattling/SOC 미반영) 가능성 큼 = 함수/구조 artifact 플래그, 실물 자성 증명 아님. **종합**: ΔE lever 해결하나 자성 결함 도입 — 이상격자 PBE상 kagome보다 나은 base 아님; artifact면(유력) 최고 base 후보(비자성∧flat@E_F). 다음 = non-PBE/SOC/왜곡셀 모멘트 quench 재검사. RTSC_27·테넌트 무손상 |
