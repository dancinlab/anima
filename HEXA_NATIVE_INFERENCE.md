...
| 5 | hexa native forward smoke + PyTorch parity (1-layer subset) | ✅ tool/hexa_native/phase5_forward_smoke.hexa + phase5_pytorch_reference.py + phase5_parity_run.sh — **HIDDEN-STATE PARITY ACHIEVED** on real Phase 1A.1 ckpt 1-layer subset (f32, layers.0 + tok_emb + norm_f, 167 MB). hexa h_x[0..4] vs PyTorch h_x[0..4]: **max \|Δ\| = 6.25 × 10⁻⁷** (4000× tighter than 1e-3 tolerance). 5 / 5 F-P5-* falsifiers PASS (LOAD-1, HEADER-2, TOK-EMB-FINITE-3, FORWARD-SHAPE-4, FORWARD-FINITE-5). Wall: ~2.5 min on M1 Mac (RESOURCE_LOCAL_HEXA=1 interp). Peak RSS: 9.13 GB (overshoots the original F-P5-MEM-CAP-7 4 GB target — F-MEM-CAP-7 FAIL, documented in §honest concerns 2). |
| 5∥ | full 24-layer + LM-head logits parity | 🚨 BLOCKED on 2 runtime issues: (a) `safetensors_mmap_read_f32_farr` does not support BF16 (raw ckpt is BF16; F32 subset 209 MB workaround used for Phase 5.1); (b) hexa-interp matvec inner loop arena pressure ≈ 30 MB/s — extrapolating, full 24-layer + LM-head would peak ≥ 50 GB RSS and ~70 min wall. Path forward: hexa-lang RFC 031 (bf16 mmap reader) + RFC 032 (matmul builtin) — out of Phase 5 scope. |

---

## 📜 Honest concerns

1. **hexa runtime async dispatch**: `hexa run` 이 aiden 으로 offload + timeout 발생. **MITIGATED**: `RESOURCE_LOCAL_HEXA=1` env var forces local interp (`build/hexa_interp` wrapper inspects this flag and exec's `~/.hx/packages/hexa/build/hexa_interp.real` directly).
2. **interp matvec arena pressure** (discovered Phase 5.1): self-host interpreter allocates ~12-88 B of HexaVal/VALSTRUCT scratch per inner-loop scalar arithmetic. A `(1024×1024)` matvec → ~30 MB arena before fn-return rewind; a `(2752×1024)` FFN matvec → ~240 MB. Per-layer peak ≈ 700 MB; per-LM-head matvec (32000×1024) ≈ 7.5 GB. Full 24-layer forward extrapolates to ≥ 50 GB RSS — infeasible without runtime change. **Path forward**: hexa-lang RFC 032 (native matmul builtin: `farr_matmul(w, x, out_dim, in_dim) → farr`, avoids per-scalar Val boxing). See runtime.c §7544 comment for the parallel qmirror situation. Phase 5.1 1-layer hidden-state parity validates the math; full 24-layer parity gated by RFC 032.
3. **bf16 mmap reader missing** (discovered Phase 5.1): real ckpt is BF16, but `safetensors_mmap_read_f32_farr` only reads F32. Phase 5.1 workaround: emit a one-time F32 subset (`/tmp/anima_phase5_subset_f32.safetensors`, 167 MB, layers.0 + tok_emb + norm_f) via the PyTorch reference (`--emit-subset`). **Path forward**: hexa-lang RFC 031 (`safetensors_mmap_read_bf16_farr` — straight bf16→f32 widen, 4 ops per element).
