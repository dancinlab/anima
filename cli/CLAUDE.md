...
| `corpus.py` | **canonical 학습-코퍼스 생성 (`anima corpus <format>`, data-format 레버) — 구현됨(torch-free 절차적, `anima_corpus_mode`→`cli/corpus.py`)** — `derivtrace`(target 을 DEF/RULE/OUT 풀이과정으로 재작성 → CE=echo 미적용, echo=composition = G1 첫 engine-native lift H_9124)·`flat`(같은 seed=content-matched 통제). 플래그 `--out --held-out I,J --comp-per-pair --single-per-concept --seed --concepts FILE.json`. 순수 string 생성이라 torch/`claude -p` 미사용(p1-p8: LLM-생성 코퍼스 지양). sweep.py 처럼 py-only(hexa 분기·`--py` 없이 항상 `cli/corpus.py`). **앞으로 모든 anima 학습-코퍼스 생성은 이 단일진입으로만** — raw `gen_*.py`·ad-hoc 스크립트 흩뿌림 금지 | data=학습 입력, verdict=학습후 `anima evaluate --py` |
...
- **`anima corpus <format>` = canonical 학습-코퍼스 생성 단일진입 (구현됨 · 오너 지시: 앞으로 코퍼스는 이걸로만)** — anima 의 G1 첫 engine-native lift 는 objective/readout/coverage 축이 아니라 **data-format 축**(derivation-trace: target 을 `DEF/RULE/OUT` 풀이과정으로 재작성하면 echo=composition 이 되어 CE=echo 메타법칙 미적용, H_9124 DERIV G1 PASS ∧ FLAT 통제 FAIL). `anima corpus derivtrace|flat …`(`--held-out I,J`·`--seed`·`--concepts FILE.json`; 같은 seed 면 derivtrace/flat content-matched 2-arm 통제) → `anima_corpus_mode`→`cli/corpus.py`. **모든 anima 학습-코퍼스 생성은 이 단일진입 경유 강제** ★스코프: `anima corpus` = **절차적/합성 코퍼스 생성**(derivtrace 등 연구-레버·규칙 생성)만 — 실제 **대화(chat) 학습 코퍼스는 이게 아님**. chat 은 진짜 자연어 데이터(`a_chat_registers` 4칸 🇰🇷·🇬🇧×일반·SNS = web/wiki/대화체+인스타/유튜브 SNS)를 **HF 데이터셋으로 큐레이션·다운로드**(`sidecar dataset` 레지스트리 + `a_chat_registers`)해 `anima train --corpus <4칸>` 으로 학습한다. 즉 '생성(procedural)=anima corpus' ⊥ '실데이터 큐레이션=HF/sidecar dataset' — 후자를 anima corpus 로 만들지 말 것(외부 자연어를 규칙생성하면 p1-p8 위반). — raw `gen_*.py`·ad-hoc 스크립트를 state/·summer 에 흩뿌리지 말 것(코퍼스 재현 SSOT = `cli/corpus.py`). 순수 절차적 string 이라 torch/`claude -p`/외부 LLM 미사용(p1-p8: substrate-native, 외부 LLM 편향을 학습데이터에 안 넣음). 기본 concepts=g6_ideation cz[](frozen G1 gate 정합, held-out 일반화 memorization-free). robustness fire(H_9124 multi held-out+G2+paraphrase) 결과와 무관하게 **코퍼스 생성 도구 자체는 canonical** — 새 포맷 추가도 이 파일에 subcommand 로.
