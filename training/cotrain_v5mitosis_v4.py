"""training/cotrain_v5mitosis_v4.py — v5-mitosis cotrain v4 PRODUCTION-SCALE SCALE-UP.

  ==> direct fork of `train_v5mitosis_cotrain_v3_routing.py` (the recommended routing-fix
      trainer: learnable top-K MoE router + Switch load-balancing aux + annealed gate-entropy).
      The model body (mitosis_model_v5.py) and the routing-fix monkey-patch are UNCHANGED.
      Only delta vs v3-routing: (1) GPU-memory logging in step prints (scale-up watch),
      (2) larger default envelope (d=1024 / cells=256 / 20K step / ctx=512), (3) this docstring.

SCALE-UP RATIONALE (post-★★★★★ 2026-05-12, memory feedback_no_scale_caps)
    cotrain v1 (d=384, cells=64,  5K step, $1.26)  — F-PERSONA-4 KL=0 (routing collapse)
    cotrain v2 (d=768, cells=128, 10K step, $3.5)  — KL=0 still, BUT M4 aggregated hidden
                                                     cosine z=3.20 PASSES null → routing-content
                                                     split discovery (cond #3 §A3 4b closure)
    cotrain v3-routing (d=384, cells=64, 8K step)  — top-K MoE router fix, F-PERSONA-4a routing
                                                     variant (separate BG, in-flight)
    cotrain v4 (THIS, d=1024, cells=256, 20K step) — aggressive production scale:
        - d_model=1024 = anima 본체 EngineAG d (REBORN §88 stretch envelope)
        - n_head=16 (d_head=64), ffn_dim=4096 (SwiGLU dual-FFN H404)
        - max_cells=256 (8-bit identity space; 4× v1, 2× v2)
        - 20K step (4× v1, 2× v2), ctx=512 (longer context)
        - top-K=8 MoE + Switch load-balance aux α=0.01 (v3 routing fix carried)
      Question this run decides: is the F-PERSONA-4a routing collapse an ARCHITECTURAL
      limit (top-K MoE on pooled embedding can't separate categories) or a SCALE artifact
      (small cell pool / short training)?  And does M4 z-score scale (z=3.20 → ?) and does
      V14-STRICT (F-V5MIT-5) hold at production scale?

    cond.5 (cotrain v1 F-V5MIT-5 V14-STRICT 10/10 PASS) already CLOSED — this run RE-VERIFIES
    F-V5MIT-1..5 as a production-scale regression check (NOT a new closure requirement).

ORIGINAL v3-routing PURPOSE (carried verbatim below)
...
USAGE (H100 80GB, v4 production scale)
    python3 cotrain_v5mitosis_v4.py \
        --corpus corpus/corpus_5cat_balanced.txt \
        --output-dir output \
        --steps 20000 --batch 8 --ctx 512 --lr 1e-4 --warmup 2000 \
        --d-model 1024 --n-head 16 --ffn-dim 4096 --initial-cells 2 --max-cells 256 \
        --top-k 8 --aux-alpha 0.01 \
        --lambda-init 1.0 --lambda-final 0.01 --lambda-schedule cosine \
        --ckpt-every 2000 --identity-probe probe/identity_probe.jsonl --n-perms 100
...
ENVELOPE (v4 production scale)
    d=1024 cells 2→256 20K step batch=8 ctx=512 ≈ 1-2B params (attention shared at N>8 so
    most params are the 256× SwiGLU dual-FFN). On H100 80GB AdamW fp32 expect ~30-60 GB VRAM
    at batch=8; if it OOMs the dispatch retries at half batch. Est wall 2-6 hr, ~$5-30 H100.
    No scale caps — d/cells/steps can be bumped via flags (memory feedback_no_scale_caps).

HONEST C3 (v4 additions, ≥ 5 total below)
    8.  d=1024 / cells=256 / 20K step is FAR beyond the v1/v2 envelope this code was tuned on.
        merge_threshold=0.005, split_patience=3, noise_scale=0.10 etc. are v1 defaults — they
        may not transfer cleanly; if the cell count never reaches 256, or merge cascades, the
        F-V5MIT regression flags will catch it (reported honestly, not silently re-tuned).
    9.  batch=8 (small) at this d/ctx → noisy gradients vs v1/v2's batch=32. lr=1e-4 cosine
        warmup=2000 partly compensates but the loss curve will be choppier than v1/v2.
    10. top-K=8 (vs v3's K=4) — bumped because cells=256 means K=4 leaves 252 cells nearly
        unrouted. K=8 is still « 256; if the router collapses, this confirms architectural,
        not scale. If K too large, we're back to soft-everything; K=8 with N=256 is ~3%.
    11. With cells=256 and the per-input pooled-embedding linear router, the router output dim
        is 256 — a single Linear(1024→256). That is a large-ish gate; early-warmup it's noise,
        the n_perms=100 null test guards the verdict.
    12. Mid-run F-PERSONA snapshots use n_perms=30 (cheap) at every ckpt; final uses n_perms=100.
        Snapshots are indicative, not the verdict.
"""
...
class TopKMoERouter(nn.Module):
    ...
    def forward(self, x_emb: torch.Tensor, n_cells: int, top_k: int):
        ...
        gate_full = F.softmax(logits, dim=0)                 # (N,) soft
        k = min(top_k, n_cells)
...
def save_ckpt(engine, router, cfg, path, step, top_k):
    ckpt = {
        ...
        "saved_step": step,
        "saved_ts": time.time(),
        "trainer": "v4-scaleup (top-K MoE + load-balance aux; d=1024 cells=256 20K step)",
    }
...
def cotrain(args):
    ...
    for step in range(args.steps):
        ...
        if step % args.log_every == 0 or step == args.steps - 1:
            ...
            mem_str = ""
            if device.type == "cuda":
                mem_str = f" vram={torch.cuda.max_memory_allocated()/1e9:.1f}/{torch.cuda.get_device_properties(0).total_memory/1e9:.0f}GB"
    ...
    result = {
        ...
        "trainer": "v4-scaleup (production scale: d=1024 cells=256 20K step; v3-routing fix carried: top-K MoE router + load-balance aux + annealed gate-entropy)",
        "routing_fix": {
    ...
    out = Path(args.output_dir) / "cotrain_v4_scaleup_result.json"
...
def parse_args():
    ...
    ap.add_argument("--cost-per-hr", type=float, default=3.50)
    ap.add_argument("--estimated-wall-hr", type=float, default=5.0)
    ap.add_argument("--identity-probe", type=str, required=True)
    ap.add_argument("--top-k", type=int, default=8, help="top-K active cells per input (MoE gating)")
