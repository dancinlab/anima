...
class ByteCorpus:
    def __init__(self, path: str, block: int):
        self.data = torch.tensor(list(Path(path).read_bytes()), dtype=torch.uint8)
        self.block = block
...
def main():
    ...
    summary = {
        ...
        "scope": "7B byte rung on GB-scale default corpus — a_scale_honest_scope; honest 🟢/🔴 per descent+strict-p7+anti-goodhart",
        "params": nparams,
        "tok_seen": tok_seen,
        ...
        "chat_pass": (descent and trained_eval["verdict"] == "PASS" and mirror_eval["verdict"] == "FAIL"),
    }
