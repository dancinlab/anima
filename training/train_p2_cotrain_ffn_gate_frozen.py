...
def main():
    ...
    # §84 inverse: FFN.gate-FROZEN cotrain (cycle 2026-05-11 next-cycle ★★★★★ test)
    print("[freeze] FFN.gate-FROZEN cotrain mode — freezing only FFN.gate × 24 layers, rest unfrozen", flush=True)
    n_trainable_before = sum(p.numel() for p in model.parameters() if p.requires_grad)
    for name, p in model.named_parameters():
        if ".ffn.gate" in name:
            p.requires_grad = False
    n_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[freeze] trainable: {n_trainable_before:,} → {n_trainable:,} ({n_trainable/n_trainable_before*100:.1f}%, FFN.gate frozen)", flush=True)
