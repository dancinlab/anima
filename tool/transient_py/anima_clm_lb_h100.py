...
Goal — simple_stack PASS_STRICT_C3 (C1 ∧ C2 ∧ C3, 보수적 X):
    ...
    pre-train — 1GB persona + 500MB dialogue corpus 결합 (corpus quality
    + chat-template format, mk2-v1 architecture 그대로).
    Hypothesis: corpus quality + scale 1.5GB (vs BG-KM 214MB) + chat-template
    format ≥30% (mandate) = chat-cap emergence.
...
Architecture (mk2-v1 base, scratch re-train):
    ...
    tokenizer: byte-level (anima-native)
    ctx: 1024
    chat-template format ratio in corpus ≥ 30% (mandate)

Corpus (1.5GB total):
    persona 1GB: anima self-knowledge (우주뇌지도 + 1030 laws + 자아 페르소나)
    dialogue 500MB: 사용자/도우미 turn-taking format curated (+)
...
Cost: $60 cap (override required), $50 early-kill, ~18hr wall
      (350M scratch 1.5GB ~3x BG-LA training time).

raw#37 transient_py
raw#10 honest_c3 ≥9
raw#15 additive over CLM v4 mk2-v1 (memory project_lesson_q_sft_closed)
 C3 + (corpus priority) + (chat-template format) + +
 trinity compliance — D4 chat-cap (corpus quality 우선) + + H_clm_chat_cap
...
HYPOTHESIS = (
    ...
    "dialogue 500MB corpus (chat-template format ≥30%). Lesson Q (SFT closed) "
    "우회 — SFT 가 아닌 corpus quality + scale 로 chat-cap emerge 검증."
)

# ── Cost discipline (override required) ─────────────────────────
COST_HARD_CAP_USD = 60.0 # default $10 cap → $60 (highest of 4 paths)
...
TRINITY_LAW = "+ (corpus priority) + (chat-template format) + + "
...
# density 0.97%, chat-template ratio 28.95% (≥30% threshold marginal — iter-3 expansion 후속)
...
BASE_REPO = "dancinlab/clm-v4-mk2-v1" # mandate-1 dancinlab SSOT
...
SCOPE_LANE_REASON = "BG-LB = CLM v4 mk2-v1 fresh scratch pre-train (정합)"
...
def pod_main(preset="lb_350m_pretrain", corpus_path=None, output_dir=None):
    ...
    raw#37 transient_py — runs only on H100 pod (CUDA mandatory). Mac side guard.
     V14 mirror seeds inherited from BG-LA shared arch (V14_STEP_1_INHERITED).
     C3 measurement adapter — V4 11-cell strict eval × 15 prompts × {greedy, sample×5}.
     ckpt mandate-1 — save_checkpoint to output_dir/step_*.pt at SAVE_EVERY.
     Flavor B HF naming — hf_repo_id_flavor_b(verdict_class).
     trinity — D_emergent-consciousness + H_clm_chat_cap.
     wrap=0 — chat lane V4 measurement only.
    """
    import torch
    if not torch.cuda.is_available():
        raise RuntimeError("BG-LB pod_main requires CUDA H100 — Mac CPU run aborted (honest emit)")
    ...
    # Phase B: corpus byte-level ingest (anima-native byte tokenizer)
    ...
    # (: simple byte-mod token lane preserved — no tokenizer dep on H100 for v1 fire).
...
if __name__ == "__main__":
    ...
    if _is_on_pod():
        ...
        print(f"[{BG_ID}] pod_main requires CUDA H100 — aborted (honest emit).")
        sys.exit(2)
    else:
