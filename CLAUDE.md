...
- do: **production training entry = `anima train <ckpt> <corpus> [--savant] [--mitosis]`** (→ hexa-native CLMConvMoE standalone trainer `cli/train.hexa`, over the same clm_*.hexa ops mounted by core/clm_decode). Assemble the SAVANT golden-zone inhibition (`a_savant_train`) + MITOSIS cell-division (`a_mitosis_train`) levers.
