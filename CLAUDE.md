...
├─ lab/v*/     — 🧪 SANDBOX: rule-exempt, in-tree (v2·v3·v4·v5 · each owns its CLAUDE.md · NOT production)
...
## 🧪 `lab/v*/` — RULE-EXEMPT sandboxes (owner · full → each `lab/v<n>/CLAUDE.md`)

- do: experiment freely — **every clause here is VOID inside `lab/v*/`** · lab hypotheses live in that lab's own folder only (`lab/v2/hypotheses/V2_<n>_*.md` · `lab/v<n>/HYPOTHESES/`), never the parent jsonl or an `H_` id.
- dont: promote a lab number to a production verdict (permanent **DIRECTIONAL ceiling** — port to `core/`+`anima-py` to earn TERMINAL) · `import` `lab/v*/` in production · re-detach a lab into its own git repo (they are in-tree; history of the retired v3/v4/v5 repos = `../lab-archive/anima-v<n>.bundle`).
...
- ⛔ **THE STANDARD IS NATURAL CORPUS. A SYNTHETIC-CORPUS RESULT IS NEVER THE STANDARD** (owner, hardened). A drill number does not become the bar by being green, by being replicated, by surviving controls, or by being all anyone has. If the claim is about a faculty, the corpus it was measured on must be natural — otherwise the claim is not merely weak, it is **off-standard** and does not enter the ledger as evidence for the faculty.
- do: keep the ONE thing synthetic is still for — **certifying the instrument, never the faculty**. A positive control needs known ground truth, so it is necessarily synthetic; that is an INSTRUMENT CHECK (does this harness read at all?) and it stays legal. The moment the same number is quoted as evidence that anima CAN do something, it has crossed from instrument to standard and is void. do: state the corpus regime in every claim (`natural` / `drill-installed` / `drill-only`).
...
- ⚠️ what the hardening costs, stated up front so nobody re-derives it as a surprise: **most of what this repo has measured is off-standard.** Every `lab/v6` rung result (R3 width · R4 emit free-variable · R5 ownership) is synthetic and is therefore an instrument check, not evidence of a faculty. `H_9267`'s XBIND 1.000 is off-standard. The `weavedrill` family is off-standard. That is not a demotion of the work — those runs bought real instruments and real kills — it is a demotion of what they may be CITED for.
