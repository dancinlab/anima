...
- 🔒 **권위 평가 = 실제 `anima` CLI 바이너리 ONLY + 2층 코드강제(2026-06-26 clm303 side-harness precedent):** terminal G-게이트 verdict 의 canonical 측정 = **`hx install anima` 로 깔린 실제 `anima eval <ckpt>` 바이너리**(배포 산출물 그 자체 = pod 가 돌리는 바로 그 CLI). 소스-run(`hexa run cli/anima.hexa`)은 차선, **decode 만 떼낸 py side-harness(`g_gates.py`→`clm_decode_*` 직접)는 verdict 소스 아님** — py 2-production 엔진은 verdict 가 아니라 **CLI 와 byte-parity 보장되는 미러**. 2층 기계강제: **(verdict)** `tool/enforce_anima_gates.py` = py-엔진 terminal G-verdict 은 **parity-record(py 채점==wired CLI on golden) 必, 없으면 PR 차단**(harness verify.checks) · **(code)** `tool/parity_gate.py`(CI hexa-engine 잡) = 매 PR golden d768(303M OOM 없음)을 `anima eval` ⇄ `core/g_gates.py` 돌려 G0-G6 byte-동일 강제(drift=CI red=머지 차단, hexa⇄py LOCKSTEP 기계화). **precedent:** clm303_clean 을 py side-harness gen=40 으로 a7b_pass=FAIL 박을 뻔 — decode 만 byte-parity 였고 G1/G6 채점은 wired CLI 미대조 + gen=40 이 h1129 native 80/120 보다 짧은 budget artifact → DIRECTIONAL 정정, 위 2 게이트 신설.
