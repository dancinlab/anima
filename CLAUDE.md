...
- ✅ **production 학습 진입 = `cli/train.hexa`** (hexa-native CLMConvMoE 단독 trainer, `anima train <ckpt> <corpus> [--savant] [--mitosis]` CLI 로 dispatch) — `.py` 트레이너 금지. SAVANT golden-zone inhibition(`a_savant_train`) + MITOSIS cell-division(`a_mitosis_train`) 레버를 같은 clm_*.hexa ops(core/clm_decode 가 mount) 위에 조립.
