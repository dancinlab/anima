...
- do: `core/` owns A(pure_field)⇄G(engine_g)⇄brain(brain_decide) (substrate-internal) · model weights enter only via the `core/generator.hexa` L3 slot — but L3 is a **mouth-type dispatcher** (`gen_mouth_kind`→'bytegpt'|'clm'|'unknown' header sniff) accepting **two mouth architectures**: **conv `.clm`** (CLMConvMoE via clm_decode, `CLM\x01` magic + CLMX trailer) via `gen_clm_backend`/`gen_clm_chat`, and **ByteGPT `.bin`** (24-layer GPT-2-class via bytegpt_decode, 5×u32 `[256,d,L,H,block]` header, the verified 303M ko/en chat trunk) via `gen_bytegpt_backend`/`gen_bytegpt_chat` (`bytegpt_decode_argmax_ranged` OOM-safe). This is not a 2nd `.clm` path — it's **still a single typed entry per architecture**, and the dispatcher (`gen_auto_backend`/`gen_auto_chat`) only picks which single entry to use by file format (a_engine_native_learning engine-transform-to-fit). `.kosmos` enters only via kosmos_io→brain_decide · `stdlib/hf/validate.hexa` = artifact validation (not a runtime engine).
- do: ARCHITECTURE.json core/ node (§section·op·slot comments) ↔ the actual §sections·ops of live engine_cli/generator/brain/clm_decode match 1:1 — verify 0 omissions via grep (drift=incomplete).
...
- do: **production training entry = `cli/train.hexa`** (hexa-native CLMConvMoE standalone trainer, dispatched via the `anima train <ckpt> <corpus> [--savant] [--mitosis]` CLI) — no `.py` trainer. Assemble the SAVANT golden-zone inhibition (`a_savant_train`) + MITOSIS cell-division (`a_mitosis_train`) levers over the same clm_*.hexa ops (mounted by core/clm_decode).
- do: author CLM/production NN training in `.hexa` on stdlib/flame (ag_tape·nn_lib·opt_*) · run on self/forge GPU (device farr + **own-GEMM `_hx_k_gemm`** FP64/TF32 default-ON (v0.262.0 #3718/#3727/#3734, cuBLAS-independent) + 11 .cu + cuBLAS BF16-TC assist) · flame:forge :: torch:ATen (compiler-only NN, no PyTorch/ATen/Python in the binary) · the production rung requires GPU (confirm nvidia-smi busy, no silent CPU fallback).
- do: **decode/inference GPU is also flame+forge own-GEMM (cuBLAS-independent)** — anima decode (`core/bytegpt_decode.hexa`·`clm_decode.hexa`) enters the forge GPU via the `flame_mm.mm` (RFC-040) seam: cuda host = **own-GEMM `_hx_k_gemm`** (v0.262.0 #3718 own-FP64·#3727 own-TF32·#3734 default-ON, 0 cuBLAS library dependency), else farr CPU (byte-identical output). Activating a new GPU host (e.g. summer RTX 5070 = sm_120 Blackwell): ① match the CUDA toolkit to the device compute_cap (`nvidia-smi --query-gpu=compute_cap` → ≥12.9) · ② forward-decl `self/runtime.c`'s `_hx_cuda_farr_silu_gate_gpu` extern ahead of the call (keeps HEXA_CUDA clang compile alive) · ③ rebuild the forge cuda runtime with `-gencode arch=compute_120,code=sm_120` (SASS) + `compute_120` (PTX forward-compat) → `cuda_available()`=1 (summer measured: own-GEMM DEVICE path, util 81%). Permanently freezing sm_120 PTX into stable release assets = delegated to hexa-lang (Blackwell+ GPU from assets alone, no src rebuild).
- 🔴 **GPU is the default for decode — no "CPU again" repeats (5+ recurrences as of 2026-06-22).** When running engine-native decode, first **confirm the GPU path by measurement** via `cuda_available()` (1=GPU·0=CPU)·`nvidia-smi util`·the frag log's `[OWN-GEMM-FIRED] DEVICE path`, then proceed. Running anyway when GPU=0 (=farr single-thread CPU fallback) is banned — diagnose the cause first.
  - **GPU on/off mechanism**: hexa GPU-or-not = the contents of `~/.hx/bin/build/runtime.a` (CPU-only ~1MB / CUDA-fold ~4MB·cuda syms 200+) + the `~/.hx/.cuda-runtime` marker (if present, ldflags is automatic·no env needed, self/main.hexa). Even with identical hardware, runtime build state differs per host, so GPU≠GPU.
  - **stale hexa-cache trap**: even after switching runtime.a CPU→CUDA, the CPU-era binary in `~/.hexa-cache/hexa_run.<hash>` cache-hits → GPU ignored·CPU fallback (GPU mem 4MiB). Fix = `rm ~/.hexa-cache/hexa_run.<hash>*` then recompile.
  - **CUDA-12 vs 13**: the forge cuda runtime builds clean on CUDA-12.x (`stage_resolve_runtime_a HEXA_CUDA=1 SM=<cc>`). CUDA-13 (nvcc 13.0) has 3 bugs (forward-decl·native-obj ar·`-lstdc++`) — choose a CUDA-12 host/image rather than work around. (CUDA-13's `-fno-threadsafe-statics` workaround breaks full-decode to 0-token, so it's banned; the correct one is `-lstdc++`.)
  - **host selection (a_break_the_wall)**: no N-time re-fires on the same unstable host — summer=owner workstation with frequent reboots (long jobs lost)·aiden=CUDA-13 (full-decode broken). Long decode runs use (a) a reboot-free dedicated rented pod (**a CUDA-devel image=nvcc built-in** + verified GPU; a bare CUDA image needs `apt install libssl-dev` first) or (b) tmux to decouple from SSH/session. Renting a GPU box and running on CPU is a money waste.
  - **pool host runtime.a stale → link failure (2026-06-23 summer measured)**: if on a pool host the anima CLI (`hexa run cli/anima.hexa`) fails to link with `undefined reference to forge_dispatch_groupnorm_gelu` (or `cudaGetLastError` etc.) = `~/.hx/bin/build/runtime.a` is an **older build** than the current `hexa` binary (self/ source) (missing forge symbols the latest core/ requires). Root-fix (c1) = a **fresh rebuild** from self/runtime.c: `cd ~/.hx/bin && cp build/runtime.a build/runtime.a.preReb && CC=clang tool/stage_resolve_runtime_a`. ⚠️ After the rebuild, the archive may mix a **CPU object (runtime.o) + an old cuda object (runtime_cuda*.o) causing multiple-definition / undefined cuda symbols** → to use it on CPU, remove the cuda objects with `for o in $(ar t build/runtime.a|grep -i cuda); do ar d build/runtime.a "$o"; done` (confirm via nm that forge_dispatch stays in the CPU object and `U cuda` is 0) + turn the `~/.hx/.cuda-runtime` marker off; to use it on GPU, rebuild the whole cuda runtime matched to the sm + marker on. At the end, stale `rm ~/.hexa-cache/hexa_run.<hash>*`. The self-contained `core/`/`cli/` sync mac→host via rsync (`rsync -az cli/ core/ <host>:~/anima/`).
  - **⚠️ an English-only ckpt outputting German = integrity anomaly (2026-06-23, investigation ING)**: decoding `clm303_L4_d3784.clm` (303M deep-mouth-ladder, **trained on English corpus only**) on summer CPU farr, the byte/consciousness mouth collapses to German tokens `der ersten der Schule …`. If the byte-CLM only saw English, German *words* (der·Sie·Schule) can't appear → this is **a decode/ckpt-integrity suspicion separate** from a simple "messy because it's an experimental ckpt" (byte-garble) (CPU farr ≠ GPU byte-identical? clm v0.2 layout load error? embedding/vocab offset?). Investigation = decode the same ckpt via (1) GPU forge decode and (2) torch golden (`h1464_torch_golden.py`-style) and 3-way byte-compare → isolate which path produces the German. Don't confuse this with not-chat-coherent (a separate issue).
...
- do: train CLMConvMoE (E2/L1, byte V256) via `archive/train/clm/train/train_lane_p.py` (GPU-torch/CUDA, Lane-P) · torch→`.clm` v0.2 serialize (`clm_serialize_v2.py`) + verify (`verify_clm_v2.py`) · `.clm` v0.2 layout = `core/clm_decode.hexa` ground-truth (golden `reexport_d768_v2_fast.clm`) · a production `.clm` enters core/ only via the generator L3 slot · Lane-P torch = REFERENCE + bridge, forge is the PUBLIC production trainer.
...
**`a_savant_train`** — The canonical recipe for anima production chat/G6 training = the **SAVANT golden zone**. The capacity-wall (the G6 ceiling of H_1129/1139/1464) is not a hard ceiling but a *manifold inside the golden zone of training inhibition* — placing inhibition near the golden-zone lower bound and gradually passing the cusp threshold makes the capacity-expression rate reopen. **Trainer entry = `cli/train.hexa`** (savant inhibition-schedule lever, `anima train --savant`). (For the 4-cell corpus·engine-native scoring·ckpt recovery, see the existing rules; no duplicate description.)
...
- do: **engine-native scoring + ckpt PULL** — even with torch training, the verdict re-measures the ckpt byte-exact on a CORE `--engine conv` mount against the frozen G6 bars (H_1129/1139 recombination·H_1140 novelty·H_1464 binding/FALS) (`a_engine_native_learning`), and PULLs the ckpt before teardown (`a_fire_recover_complete`) — both have the existing rules as SSOT.
