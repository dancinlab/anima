...
# known-word lexicon for coherence (English signature words + Korean chat markers + common words)
KNOWN = set()
for _c, _kw in CONCEPTS:
    KNOWN |= {w for w in words(_c)}
    KNOWN |= _kw
KNOWN |= {"사용자","도우미","the","a","is","of","and","to","in","that","it","i","you",
          "think","question","more","than","just","information","processing","profound",
          "we","are","at","step","loss","decreasing","steadily","this","when","from",
          "what","how","why","not","but","with","for","be","can","will","they","its"}
...
def run_ladder(m, dev, block):
    ...
    for i, (c, _) in enumerate(CONCEPTS):
        o = gen(m, f"사용자: {c}? | 도우미: ", 80, dev, block)
    ...
    for k in (2, 3, 4, 5):
        ...
        comp_seed = "사용자: " + " ".join(concepts_k) + " — combine these. | 도우미: "
