...
## Per-H index — brain-structure ladder + session facets (H_1280–1304, a_hypothesis_register)
...
| H_1304 | G6 IDEATION ★ depth-floor dig — deterministic FALSIFIABILITY detector (comparator+measurable+negatable, NOT LLM-judge, p7) + composition-routed (G1 recombination) ideation vs flat sampling | 🟠 HONEST-CONFIRMED-THIN (G6 stays THIN, bar UNMOVED c9; detector 10/10 calibrated; 3 seeds; live verify303m_g6 decode path. FALS A_flat=0.00 → B_composed=0.667 (one falsifiable idea EARNED via recombination, NOVEL 6.3→19) but M2 FALS≥1 FALSE + M1 DIST≥5 FALSE; controls decisive B_shuffle FALS=0, B_ablate FALS=0+coherence-collapse → the nudge tracks the EARNED composed pairing not the conditional shell. a_break_the_wall: genuine new angle, frozen-first, wall HELD. DIRECTIONAL torch-mouth, no R2/no wiring (THIN); toy 303M) | [H_1304_g6_ideation_falsifiability.md](H_1304_g6_ideation_falsifiability.md) |
