...
since: 2026-05-29
sister: SPATIAL/scan/spatial_coupling_phi.hexa, XENO/detector/invariant_detector.hexa, SPATIAL/state/spatial_s1_2026_05_29/, UNIVERSE/H_829, UNIVERSE/H_838, UNIVERSE/H_839, UNIVERSE/H_841, UNIVERSE/H_842, .verdicts/844_spatial_coupling_phi/S1_run.txt
...
# H_844 — SPATIAL S1 spatial-coupling-scale detector
...
6. verdict → .verdicts/844_spatial_coupling_phi/S1_run.txt (g73 per-H gate)
```
...
$ env hexa run SPATIAL/scan/spatial_coupling_phi.hexa
  ...
  → .verdicts/844_spatial_coupling_phi/S1_run.txt verbatim copy
```
...
- **feedback-universe-h-slug-stale-verify 정합**: 3-신호 검증 (`git ls-tree origin/main UNIVERSE/ | grep H_844` zero hit + `git log --all` 의 H_844 collision zero + `gh pr list` 의 open PR 0건) 후 H_844 사용.
...
- **artifacts**:
  ...
  - .verdicts/844_spatial_coupling_phi/S1_run.txt
