...
- hexa_only: true — `UNIVERSE/state/h286_split_brain_dual_phi_2026_05_26/run_h286.hexa`
- LLM: none (raw#12 strict)
- Φ kernel: `import` 으로 `UNIVERSE/lib/phi_native.hexa` 의
...
- run:
  ```
  /Users/ghost/.hx/bin/hexa parse UNIVERSE/state/h286_split_brain_dual_phi_2026_05_26/run_h286.hexa
  HEXA_MEM_UNLIMITED=1 /Users/ghost/.hx/bin/hexa run UNIVERSE/state/h286_split_brain_dual_phi_2026_05_26/run_h286.hexa
  ```
...
- **UNIVERSE/H_234 cross-substrate-phi-coupling-density** (PARTIAL): proxy
  의 축별 비단조성(entropy r=0) 기록 — 본 H 는 connectivity-manipulation
  방향성에서의 proxy pathology 로 확장.
- **UNIVERSE/H_239 alternative-phi-metric-cross-validation** (CONSISTENT):
  동일-substrate 3-metric 순위 일치; 본 H 는 *manipulation 방향*에서 proxy 가
  faithful-Φ 와 갈라질 수 있는 후속 lane 을 연다.
- **UNIVERSE/H_204 weak-panpsy threshold + H_207 Kuramoto**: criticality /
  synchronization 위 Φ — CML 의 edge-of-chaos regime 과 sister.
- **UNIVERSE/H_201 asymm-div + H_203 asymm-merge + REBORN §88 mitosis**:
  anima cell-pool 의 SPLIT/MERGE event — 본 H 의 경고("split 측정에 spatial-MI
  proxy 부적합")가 직접 적용되는 anima-native 사례.
- **UNIVERSE/H_004 Hard Problem**: phenomenal split ≠ measured split (L7).
- **UNIVERSE/lib/phi_native.hexa**: RFC 036 phi_spatial byte-equal port —
...
`UNIVERSE/state/h286_split_brain_dual_phi_2026_05_26/run_h286.hexa`
...
VERDICT: CLOSED-NEGATIVE  (4/6 falsifiers PASS)
  ...
  result.json -> UNIVERSE/state/h286_split_brain_dual_phi_2026_05_26/result.json
================================================================
```
