...
- **Φ primitive**: `phi_with(state, n, dim, n_bins)` from `UNIVERSE/lib/phi_helper.hexa` (PR #317) — **첫 production user** (F6 정합, inline phi_spatial copy 대체)

## Run Protocol

- **smoke**: `UNIVERSE/state/h250_nonpow2_lattice_persistence_2026_05_24/run_h250.hexa`
- **Φ primitive**: `UNIVERSE/lib/phi_helper.hexa` → `phi_with` → `c_measure_phi` → RFC 036 `phi_spatial` (import READ-ONLY; phi_helper 가 c_lib 를 wrapping).
...
- **phi infra**: `UNIVERSE/lib/phi_helper.hexa` (PR #317, **첫 production user** — `phi_with`/`life_phi_nbins`) → `HEXAD/C/c_lib.hexa` (`c_measure_phi` → RFC 036 `phi_spatial`)
...
H_250 — Class-II Φ persistence on non-power-of-2 lattice (raw#12)
  ...
  Φ primitive: phi_with from UNIVERSE/lib/phi_helper.hexa (PR #317, RFC 036 phi_spatial, Φ>=0)
...
**State output**: `UNIVERSE/state/h250_nonpow2_lattice_persistence_2026_05_24/result.json`
**Smoke**: `UNIVERSE/state/h250_nonpow2_lattice_persistence_2026_05_24/run_h250.hexa`
