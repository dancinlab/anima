...
title: OMEGA 최소게이트 gB·base + gA·A (G + w2..w6 전부 drop)가 honest closure form 인가 — frozen leak-free CDV2 d512 위 2-param 게이트가 a_only 와 base 를 동시에 이기는가 (F-OH1-MINGATE · #1801)
domain: omega · clm · substrate-decode-closure · minimal-gate · a-wire · leak-free · frozen-ckpt · lane-g-gpu · falsifier
source: UNIVERSE/H_913 (#1800 multi-wire GATE 🔴 — closure 가 한 wire 에만 산다는 RULING) · #1801 OH1 follow-up · UNIVERSE/omega_gate_form_sweep.py · .verdicts/omega-engine/F-OH1-MINGATE.txt
...
since: 2026-06-04
sister: UNIVERSE/H_913_omega_multiwire_gate_closed_negative.md, UNIVERSE/H_915_omega_replacement_rigor.md, UNIVERSE/H_916_omega_scale_ladder.md, .verdicts/omega-engine/F-OH1-MINGATE.txt, .verdicts/914_omega_minimal_gate_a_wire/
verdict: 🟢 SUPPORTED-NUMERICAL (F-OH1-MINGATE — frozen leak-free CDV2 d512 위 최소게이트 min_learned(gB·base + gA·A, G+w2..w6 drop)이 a_only(1.144181)·base(3.097779)를 동시에 이김: min_learned 0.883525 ≤ a_only AND < base → OH1_HOLDS=True · #1800 baseline 6-decimal CROSS_CHECK_OK)
---

# H_914 — OMEGA OH1: 최소게이트 gB·base + gA·A 가 honest closure form (F-OH1-MINGATE)

## 1. 가설

H_913(#1800)이 competent leak-free CDV2 d512 위 full multi-wire 게이트의 FAIL(GATED 3.6435 >
base)을 보였고, A-head logit-bias 단독(a_only 1.1446 ≪ base)이 막대하게 유용함을 보였다. 그렇다면
**최소게이트 gB·base + gA·A** (G 와 w2..w6 전부 drop, gB·gA 2-param 만 학습)가 honest closure
form 인가 — a_only 를 MATCH 가 아니라 BEAT 하면서 base 도 이기는가?

## 2. 동기

- full multi-wire 게이트의 gA=3.369 overshoot + gG=−0.999 가 signal 인가 무관 wire 의 variance
  인가 — 후자라면 그 wire 들을 drop 한 2-param fit 이 clean operating point 를 회수해야 한다.
- frozen ckpt(re-train 0) 위 순수 gate-form sweep: substrate 는 고정, 게이트 form 만 비교 =
  apples-to-apples.
...
verdict 영속: `.verdicts/omega-engine/F-OH1-MINGATE.txt` (사본 `.verdicts/914_omega_minimal_gate_a_wire/`).
...
## 5. 결과 (verbatim · held-out TEST CE nats/byte · F-OH1-MINGATE)
...
- 최소게이트(gB·base + gA·A, G+w2..w6 drop)가 a_only 를 MATCH 가 아니라 BEAT(0.8835 < 1.1446,
  Δ+0.261 nats/byte) + base 도 BEAT(Δ+2.214).
- 2-param free fit 이 gB=0.040, gA=0.901, gG=0.000 착지: full-bus 의 gA=3.369 overshoot +
  gG=−0.999 가 무관 wire 의 variance 였음을 확정. drop 하니 honest 2-param fit 이 a_only 보다
  나은 clean operating point 를 회수.
- **데이터가, assertion 이 아니라, gate point 를 골랐다.** H_913 RULING(closure 는 A-head
  logit-bias wire 에 완전히 산다) 확인. (단 OΩ-RIGOR H_915 가 보이듯 이는 coupling 이 아니라
  REPLACEMENT 임 — A-head 가 base mouth 를 대체.)

## 7. scope (정직 · a_scale_honest_scope · p7)

single d512 rung; observation-only frozen forward; CE 는 held-out PREDICTION number, NOT
verdict-of-truth(p7). Cross-check 가 #1800 을 6-decimal 재현(byte-faithful harness). substrate
re-train 0. Lane-G/GPU (a_lane_akida_gpu_split).

## 8. 산출물

- verdict: `.verdicts/omega-engine/F-OH1-MINGATE.txt` (verbatim · 사본 `.verdicts/914_omega_minimal_gate_a_wire/`)
