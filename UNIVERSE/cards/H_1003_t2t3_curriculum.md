...
## 5. Scale follow-up (status note)

- **2026-06-07 · H_1005 (🔴 CURRICULUM-HORIZON-CAPPED, SPLIT):** the scale ladder (target length T2 ∈ {20,40,80}, T3 ∈ {18,36,72}, SAME curriculum-GRU / capacity / fixed 40-epoch budget) shows this crack is **horizon-bounded, not unbounded, at fixed compute** — and task-structure-dependent. **T2 cumulative XOR-parity SCALES** (solves to len 80 = 4×, gap vs LM ~0.42–0.49, d≥2.7 at every length). **T3 modular path-integration is HORIZON-CAPPED** — solves at 18 here too but BREAKS at 36 (2×, → chance, sep lost) and collapses to the LM at 72. mem-aug=1.0 at every length ⇒ the T3 cap is trainability-at-horizon, not representability. So H_1003's 🟢 crack is **horizon-robust for T2 but horizon-local for T3** (caps near its original length). See [H_1005](./H_1005_curriculum_scaleup.md). (Original 🟢 verdict preserved — the crack is real at the H_1003 length; H_1005 bounds its reach by horizon.)
