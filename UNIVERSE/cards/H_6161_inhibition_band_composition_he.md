...
savant 골든존 inhibition(dp≈0.25)이 결합에 필요한 합성표현을 오히려 degrade(An&Du: noise injection→합성표현 체계적 저하)하나. GZ_LOWER 아래로 낮춘 arm 에서 G1/HE 회복 측정.
...
- **kill / 🧱 조건:** dp<GZ_LOWER arm G1==default(LIFT 0) ∧ HE 불변이면 🧱 NOT-SUPPORTED(inhibition-noise 가설 REFUTED).
- 측정 = engine-native (`core/` `.hexa` decode) TERMINAL; numpy/torch mirror = DIRECTIONAL (a_engine_native_learning). frozen bar 불변.

## 관련

H_1821(HE metric) · H_1561 savant golden-zone(다른 축) · [[lit-binding-objective-external-arxiv]] NEW레버A · H_1840 γ
