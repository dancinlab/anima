...
**메커니즘:** 조합을 컴파일 단계로 사전 융합(~dup of #40 kosmos Merge).
...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1887_gen_compile_composition/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1887_gen_compile_composition.md` (this card)
- `state/1887_gen_compile_composition/` (probe 발사 시 생성)
