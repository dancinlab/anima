# H_1862 — gradient → 진화적 crossover(GA)
...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1862_gen_grad_to_evolutionary_crossover/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1862_gen_grad_to_evolutionary_crossover.md` (this card)
- `state/1862_gen_grad_to_evolutionary_crossover/` (probe 발사 시 생성)
