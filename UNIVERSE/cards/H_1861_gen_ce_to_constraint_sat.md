# H_1861 — CE → constraint-satisfaction objective
...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1861_gen_ce_to_constraint_sat/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1861_gen_ce_to_constraint_sat.md` (this card)
- `state/1861_gen_ce_to_constraint_sat/` (probe 발사 시 생성)
