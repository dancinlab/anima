...
**id:** H_1883
**slug:** gen_antimatter_combination
...
**메커니즘:** 개념/반개념(negation) 소멸·생성 대칭, 조합=부분소멸 후 잔여.
...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1883_gen_antimatter_combination/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1883_gen_antimatter_combination.md` (this card)
- `state/1883_gen_antimatter_combination/` (probe 발사 시 생성)
