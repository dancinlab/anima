...
- **wired:** `WIRED-live` — `core/engine_cli.hexa` §Field (`field_apply` + `field_apply_mfield` + `field_signal_entropy` + `drug_lsd_mfield` + `pci_perturb` + `pci_complexity` + `field_lane_mean`) · `engine_cli_smoke.hexa` cases 303-308 (6 frozen bars) · FULL smoke **308 pass / 0 fail RC=0** deterministic ×3 · ARCHITECTURE.json §Field lockstep ✓
...
- **DIRECTIONAL → R2 ENGINE-NATIVE WIRED:** R1 = numpy mirror(`grep -lE 'import torch|gauge_lib|numpy' state/1503_field/*.py`
  ...
  cases 303-308 byte-exact 재측정 + ARCHITECTURE.json §Field lockstep, FULL 308/0 RC=0 deterministic ×3
...
1. **R2 ENGINE-NATIVE WIRED ✅** — §Field 7 op + smoke 303-308 + ARCHITECTURE lockstep, FULL 308/0 RC=0 (완료).
