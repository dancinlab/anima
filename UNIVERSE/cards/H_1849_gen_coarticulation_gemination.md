# H_1849 — 음운론 coarticulation/gemination

**id:** H_1849
**slug:** gen_coarticulation_gemination
...
**메커니즘:** 인접 심볼이 곱셈적으로 상호변형 → additive 아님.
...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1849_gen_coarticulation_gemination/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1849_gen_coarticulation_gemination.md` (this card)
- `state/1849_gen_coarticulation_gemination/` (probe 발사 시 생성)
