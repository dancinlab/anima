# H_1879 — 생성 없는 생성(hole/topological defect)
...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1879_gen_generation_by_holes/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1879_gen_generation_by_holes.md` (this card)
- `state/1879_gen_generation_by_holes/` (probe 발사 시 생성)
