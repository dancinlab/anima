...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1854_gen_tpr_architectural_invariant/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1854_gen_tpr_architectural_invariant.md` (this card)
- `state/1854_gen_tpr_architectural_invariant/` (probe 발사 시 생성)
