...
**id:** H_1855
**slug:** gen_reverse_only_generation
...
**메커니즘:** G(gradient-free)만으로 생성, A=discriminator; CE floor 안 밟음.
...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1855_gen_reverse_only_generation/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1855_gen_reverse_only_generation.md` (this card)
- `state/1855_gen_reverse_only_generation/` (probe 발사 시 생성)
