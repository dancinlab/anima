...
## ✅ TERMINAL FAITHFUL-IIT4 VERDICT — 🔴 GOAL-COUPLED-NULL-ROBUST (2026-06-06 · H_1001)

**Superseded by faithful IIT4 (H_1001):** the 🔴 FAIL closed-negative in the front-matter `verdict:` / §measurement above was measured with the **H_912/H_931 Φ-proxy** (goal-subspace-projected) and is preserved as the proxy-measured record. [H_999](./H_999_faithful_iit4_remeasure.md) re-measured the imagine-vs-react regime with the FAITHFUL engine on the FULL latent (where the H_971 contrast REVERSES 🟢); [H_1001](./H_1001_reopen_consolidate.md) then measured the condition H_999 did NOT cover — **H_994's specific GOAL-COUPLED re-formulation**, i.e. faithful Φ on the rank-6 goal-subspace projection (this H's own falsifier) — and issued the **frozen terminal verdict**:

**🔴 GOAL-COUPLED-NULL-ROBUST** — the FAITHFUL goal-coupled (subspace-projected) contrast is Φ_IMAGINE **0.61 < Φ_REACT 0.78** (contrast **−0.165, Cohen d −0.62, p 0.021**). It does **NOT** flip positive, so H_994's own frozen PASS (free-Φ negative AND goal-coupled flips positive, d > 0.8) is NOT met. H_994's "the deficit is STRUCTURAL, survives task-relevant projection" reading therefore **STANDS under the faithful measure for the goal-subspace projection** — *distinct* from H_971's full-latent reversal. The mechanism is the same goal-contraction that makes H_988 null: restricting integration to the low-rank goal-predictive subspace inherits the goal-contraction, so it stays null where *free* imagination (H_971, full latent) reverses to 🟢. RE-OPEN (H_999) is now CLOSED. Honest scope: toy, faithful goal-coupled = n=6 projected channels exact (a_scale_honest_scope), scale-transfer UNVERIFIED.
