...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1886_gen_two_state_time_symmetric/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1886_gen_two_state_time_symmetric.md` (this card)
- `state/1886_gen_two_state_time_symmetric/` (probe 발사 시 생성)
