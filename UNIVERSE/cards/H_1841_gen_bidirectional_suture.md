...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1841_gen_bidirectional_suture/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1841_gen_bidirectional_suture.md` (this card)
- `state/1841_gen_bidirectional_suture/` (probe 발사 시 생성)
