...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1871_gen_fixedpoint_diffusion/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1871_gen_fixedpoint_diffusion.md` (this card)
- `state/1871_gen_fixedpoint_diffusion/` (probe 발사 시 생성)
