...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1889_gen_catalytic_silence/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1889_gen_catalytic_silence.md` (this card)
- `state/1889_gen_catalytic_silence/` (probe 발사 시 생성)
