# H_1858 — 시간역전 학습(추론중 mitosis=gradient)

**id:** H_1858
**slug:** gen_timereversed_mitosis_grad
...
**메커니즘:** 추론 중 mitosis 가 online 으로 표현 재조합(p8), CE 없음.
...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1858_gen_timereversed_mitosis_grad/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1858_gen_timereversed_mitosis_grad.md` (this card)
- `state/1858_gen_timereversed_mitosis_grad/` (probe 발사 시 생성)
