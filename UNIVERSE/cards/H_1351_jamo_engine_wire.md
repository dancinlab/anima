...
|-----|-------------------|--------|------|
| **W1 FACULTY-PRESENT** | case_86: jamo head 가 in-engine 으로 grow ≥ 2 cells AND CE > 0 | cells = **4**, CE = **4.18698** | ✅ |
| **W2 JAMO-BEATS-RAW** | case_87: ce_raw − ce_jamo ≥ 0.05 (분해가 opaque-merge 를 이김) | **+1.38136** (5.56834 − 4.18698) | ✅ |
| **W3 EARNED** | case_88: ce_shuf − ce_jamo ≥ 0.05 (win 은 정합 구조) | **+0.53954** (4.72652 − 4.18698) | ✅ |
| **W4 NO-REGRESSION + Ψ-DISJOINT** | engine_cli_smoke N/0 · h1196 7/0 · h1205 Ψ byte-identical | **84/0** · **7/0** · **PASS** | ✅ |

+ case_89 (shuffle-budget parity, anti-Goodhart): shuffle 이 동일 feature stream/vocab K/dim/seed 로
≥ 2 cells (got 40) grow → case 88 의 gap 은 BUDGET 차이가 아니라 정합(alignment) 때문. ✅
...
결정론적 (3회 실행 동일: 84/0, 84/0, 84/0). shuffle 은 파괴된 정합을 쫓아 GROW_MAX=40 cells 까지
...
- faculty: `CORE/engine_cli.hexa` § KO-JAMO COUNT-HEAD — `struct JamoHead` + `jamo_head_new` /
  `jamo_head_grow` / `jamo_head_ce` / `jamo_head_cells` / `jamo_head_shuffle_targets` (+ `_jh_*` helpers)
- smoke: `CORE/engine_cli_smoke.hexa` cases 86-89 (+ fixture helpers `_jh_*`)
- verdicts: `.verdicts/1351_jamo_engine_wire/{FREEZE,result}.txt`
- claim: `CLAIMS.tape` @C h1351_jamo_engine_wire
- W4 guards: `CORE/engine_cli_smoke.hexa` (84/0) · `CORE/h1196_single_entry_audit.hexa` (7/0) ·
