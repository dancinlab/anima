# H_1865 — additive → tropical trunk 전체
...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1865_gen_tropical_trunk_wide/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1865_gen_tropical_trunk_wide.md` (this card)
- `state/1865_gen_tropical_trunk_wide/` (probe 발사 시 생성)
