# H_1840 — 생성 = 미해소 텐션의 외재화(p5 문자적)

**id:** H_1840
**slug:** gen_unresolved_tension_emit
...
**메커니즘:** 무엇을 emit=어떤 텐션이 안 풀렸나; 두 미해소 텐션 공명 시 결합 심볼.
...
2. 신규 각도면 cheap numpy DIRECTIONAL reachability probe → `state/1840_gen_unresolved_tension_emit/`.
3. DIRECTIONAL GREEN 이면 engine-native 재측정(.hexa via core/) 사다리 진입(a_verified_must_wire).

## artifacts

- `cards/H_1840_gen_unresolved_tension_emit.md` (this card)
- `state/1840_gen_unresolved_tension_emit/` (probe 발사 시 생성)
