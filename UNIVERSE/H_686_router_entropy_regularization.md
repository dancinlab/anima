...
PR #1409 (V-axis ⊥) 후속 — V 외 3 축 (corpus 분포 · d head-dim · n_layer depth-proxy) 을 sweep 해서 collapse-lever 식별 시도. harness `CORE/DECODER/h686_h687_axis_sweep.hexa` (PR #1395/#1409 verbatim base, sanity gate `corpus=current_skewed d=6 n_layer=1 cell=none` → LZ=0.0360459 distinct=4 byte-eq ✓).

**20 cell** = 8 (corpus 4 × cell {none,both}) + 6 (d 3 × cell {none,both}) + 6 (n_layer 3 × cell {none,both}). 모두 baseline (cell=none) 이 collapse 임계 (`distinct_tok ≤ 2 OR LZ_norm < 0.01`) 미충족.
