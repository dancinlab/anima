...
def main():
    ...
    # ── STEP 0b: discretization-read determinism guard (pure fn of bits, all 6 binnings) ──
    print("STEP 0b — discretization-read determinism guard (each (nb,scheme) read = pure fn of bits):")
    Hg0, Hp0 = planning_trajectories(0, PLAN_DEPTH)
    det_ok = True
    for nb in NB_GRID:
        for scheme in SCHEME_GRID:
            b1 = both_phi_disc(Hp0, args.n, nb, scheme)
            b2 = both_phi_disc(Hp0, args.n, nb, scheme)
            this = bool(abs(b1[0] - b2[0]) < 1e-12 and abs(b1[1] - b2[1]) < 1e-12)
            det_ok = det_ok and this
    print(f"  all {len(NB_GRID)*len(SCHEME_GRID)} binnings deterministic at n={args.n}: {det_ok}")
    if not det_ok:
