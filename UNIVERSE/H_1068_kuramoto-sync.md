...
verdict: 🟢 SUPPORTED — K=0.5 r=0.20 → K=4.0 r=0.97. 임계결합 위에서 자발적 위상 동기 (anima 진동자 격자 정합).
---

# H_1068 — 쿠라모토 동기화 — 임계결합 위 자발 동기

> **가설.** 임계결합 위 자발 동기

## 1. FROZEN FALSIFIER (pre-registered 2026-06-14)
- **BLADE.** 결합을 올려도 위상 동기 질서변수 r가 유의하게 오르지 않는다.

## 2. 측정 결과 (REAL sim · discovery_batch_01.py::D05)
K=0.5 r=0.20 → K=4.0 r=0.97. 임계결합 위에서 자발적 위상 동기 (anima 진동자 격자 정합).
