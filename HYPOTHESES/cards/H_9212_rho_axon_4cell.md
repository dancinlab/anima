...
## ⚠️ INFRA-QUARANTINE — v6 reach (pip channel) `import anima` packaging bug (2026-07-08)
seam round-3 op-grip pod(runpod 251GB CPU · hexa v0.716.0)에서 `[2] v6 reach: anima-python evaluate --rho-axon`를 같이 태웠으나 **infra-fail**: 설치된 `anima-python 0.13.2`가 `anima_py/__init__.py::main`서 `import anima`(canonical py 디스패처) → `ModuleNotFoundError: No module named 'anima'` → `anima-python: command not found`. **packaging 버그**(pip dist `anima-python` wheel이 `anima` 모듈을 미포함/미노출). **infra이지 seam 결과 아님** — 별도 detached retry 진행 중(대기 금지), 이 라운드 seam 판정(E/B/F)과 무관. reach NUMBER는 여전히 ⑦ pool 대기. follow-up = pip 채널 `anima-python` 패키징 fix(격리, 별건). raw: pod_r3_full.log L685-698.
