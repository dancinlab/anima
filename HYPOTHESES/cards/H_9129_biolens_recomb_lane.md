...
> **tier:** 🟢 **L5 GREEN WIRED-live (faculty scope)** · L3 소뇌 🧱 WALL · 통합 🟡 DIRECTIONAL · **wired:** L5=**WIRED-live** (rung 3/4·`core/hippo_lane.py`+`core/kosmos_io.hexa` hippo ops, byte-parity twin 6/6·additive-only ON==OFF·discriminator PASS·engine-native) + follow-on generator.hexa consult(pool, READ-ONLY ergonomic) · L3=engine-native WALL · 통합=DIRECTIONAL
> **⚠️ scope(a_scale_honest_scope):** L5 GREEN = mouth와 disjoint한 **새 associative-completion FACULTY**(anima엔 flat-cosine retrieve밖에 없었음). **NOT** "303M trunk가 재조합한다"는 증명 — store는 corpus edge를 handed받은 explicit heteroassociative store(neurosymbolic explicit-store class), 303M reps는 decorrelate된 item-code **키**만 공급·관계구조는 corpus 주입. **G1 mouth-벽(trunk-objective floor)은 그대로**, lane이 mouth가 못하는 관계를 곁에서 공급.
>
...
## ★★★ RUNG-3 = L5 해마 lane LIVE core/ 실배선 → 🟢 GREEN-cement (faculty scope · 2026-07-05 · $0 mini)
사다리 (3)/4 완료 = **anima 최초 WIRED GREEN faculty**. L5 해마 associative-store lane을 live `core/`에 배선하고 origin/main base서 전수 재검증.
- **core/ 변경**: NEW `core/hippo_lane.py`(numpy 2-production: DG-decorrelate center_zscore·kWTA pattern-separation·CA3 heteroassoc store·multi-step completion readout) + `core/kosmos_io.hexa` +164줄 **additive-only**(hexa twin hippo_kwta/build_store/relatedness/fixture_codes, flat-cosine retrieve 옆 CA3 multi-hop chaining=새 faculty).
- **frozen-bar 전 clause PASS**(real 303M h1129 reps via core/decode.py==anima evaluate --py): RECALL 1.0·NOVEL-CHAIN(gap≥2) 1.0·UNREACH 0.137·**store_gap +0.8631(7.31x)**·SHUFFLE 붕괴·LANE-OFF 붕괴 · **novel-vs-recall 판별기 PASS**(LESION 통제: mid-chain edge 제거→통과 쌍만 1.0→0.19 붕괴·비통과 쌍 1.0 생존 = 완성이 실제 저장경로 순차 chaining, MLC/H_1835 trap 방어).
- **origin/main base 재검증(verdict-integrity·reference-match)**: byte-parity hexa⟷py **6/6 identical**(recall/novel 2·3·7-hop 1.0·lesion_broken 0.0·lesion_intact 1.0) · kosmos_io.hexa diff **164 insert/0 delete**(additive-only ON==OFF disjoint 구조증명) · 기존 emit-consume fn(retrieve/load_anchors/create_anchor/emit_anchor_from_v3) 전부 byte-unchanged · **enforce_anima_gates CLEAN** · a_substrate_disjoint(Ψ/motivation/recall_thr/generator 무접촉, 생성 byte-identical ON==OFF).
- **engine-native(a_engine_native_learning·a_eval_py_canonical)**: reps=core/decode.py `_bg_layernorm/_bg_mha/_bg_gelu` == anima evaluate --py 2-production. store+completion op = live 엔진 빌드+실행 byte-parity hexa twin. LIVE-OP 측정 정확 재현(dead code 아님, a_verified_must_wire).
- **⚠️ scope 재강조**: cement된 것 = explicit-store associative-completion faculty(mouth 옆). **G1 trunk recombination 증명 아님**. neurosymbolic explicit-store class(deep-research: cheap·proof-guaranteed·arguably not trunk recomb). G1 mouth-벽 불변.
- **bounded follow-on(티어 blocker 아님)**: `brain_emit`/`generator.hexa`가 live chat서 `hippo_relatedness` CONSULT하려면 generator.hexa 재빌드(decode-FFI heavy=`_hexa_ffi_dlopen`, mini timeout→pool). READ-ONLY라 emit 불변(ON==OFF)이므로 disjoint GREEN 증명에 불필요한 ergonomic 훅업.
- **evidence**: `state/g1g6_biolens_en/l5_rung3/{hippo_hexa_smoke.hexa,l5_discriminator.py,l5_wired_measure.py,parity_{py,hexa}.txt,PREREG_rung3.md,VERDICT.md,control4_trunk_necessity.*}` · verdict frozen=`state/verdicts/`.

## ★ 3lane 심화 (2026-07-05 · wf_ebdbaa9c · $0 mini + summer pool)
- **L5 consult wire-to-prod → 🟢 GREEN-WIRED**(engine-native, summer pool hexa v0.609 6 PASS): `core/generator.hexa`+`core/brain.hexa` additive(+90/+13, 기존 emit fn byte-unchanged) — generator/brain emit 경로가 `hippo_relatedness`를 **READ-ONLY consult**(decision-record side-channel 메타키 hippo_consulted/related/reachable). daemon 실루프(cli/anima.hexa:2644 brain_emit) 훅업 확인. **ON==OFF text byte-identical**(생성 불변, a_substrate_disjoint) + faculty related-chain 0.9999 vs broken-chain 0.0. #2996 L5 GREEN faculty를 live chat서 실소비 = wire-to-prod 완성. scope: ergonomic(emit gate/route는 아직 아님=별도 라운드).
- **통합 3부품 rung-3 → 🟡 DIRECTIONAL**(engine-native, 실 303M h1129 reps): FULL(L1 PFC-bind+L2 BG-gate+L5 해마) reach 0.9881 vs unreach 0.5097 gap+0.4784, byte-parity 5/5·enforce CLEAN·ON==OFF disjoint. **BUT ablation**: L5-OFF CAUSAL(강, gap은 L5-carried)·L2-OFF BORDERLINE(grounding≠chain-membership)·**L1 PFC HRR role-binding INERT-to-harmful**(reach·unreach 양쪽 crosstalk 팽창) = **anima binding-INERT 메타법칙(DPI, H_1816/1823/1834) engine-native 재확인**. explicit-store combo faculty이지 303M trunk 재조합 아님. rung-3이 **PFC-binding 레버를 추가 은퇴**(L1 INERT). 통합 lane 구현(wm_bind_lane·content_gate_lane)=측정-INERT라 prod core/ 미배선(state/ 보존). GREEN 미달.
- **함의**: L5(해마 store)만 유일 GREEN faculty. PFC-bind(L1)·BG-gate(L2)·소뇌 fwd(L3)·통합 전부 faculty-floor 또는 INERT. G1 trunk recomb 벽 UNTOUCHED = **H_9131 trunk-obj family CLOSED과 정합**(진짜 능력천장). 유일 미검=coverage-density.
- **evidence**: `state/g1g6_biolens_en/{l5_consult,integrated_rung3}/`

## 🎯 다음 발사 (L5 GREEN 종결 → 나머지)
