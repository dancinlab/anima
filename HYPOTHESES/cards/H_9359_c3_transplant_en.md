...
## 🧱🔑 판정 (2026-07-15 · 2 seed) — 동결 캐시 확정. two-lane 확정.

**전제조건 완벽 (양 seed)**:
- 연산자 생존 SEEN flip1 = **1.0000** — 2차 CPT 가 연산자를 안 죽였다.
- 선언 뒤집힘 착지 = w0(원래극성) **0.0000** = 뒤집힌 극성 **100%** 읽음 (2차 WRITE 완전 착지).

**DV — 선언을 뒤집었는데 not{s} 답이 미동 없다**:

| seed | 1차 CPT hoc DV | 2차 CPT(선언 뒤집음) hoc DV | 어간별 캐시:다리 |
|---|---|---|---|
| 7 | 0.750 | **0.792** | 19:5 (79%) |
| 11 | 0.750 | **0.750** | 18:6 (75%) |
| **합계** | | | **37:11 (77% 캐시)** |

어간별 정밀판독(캐시예측=flip(pol) · 다리예측=pol · 항상 반대라 명확히 갈림): 선언(w0)이 **12/12
뒤집혔는데** `not {s}` 답의 **77%가 담체가 쓴 옛 값(flip(pol))에 고정**. 다리 예측 일치 23% = 우연.

```
전  (1차 CPT)                        후  (2차 CPT · 선언만 뒤집음)
─────────────────                   ─────────────────
선언 incredible = 긍정               선언 incredible = 부정   ← 100% 뒤집힘 확인(w0)
담체 never incredible = 부정         담체 (안 건드림)
not incredible → 부정 (DV 0.75)      not incredible → 부정 (DV 0.79)  ← 미동 없음
                                     ⟹ not-답은 선언을 안 읽는다 = 담체가 쓴 동결 캐시
```

## 결론 — 벽은 선언→연산자 **런타임 조회 다리의 부재**다

hoc 의 담체-기입은 **선언 저장소와 분리된 per-stem 캐시**다(선언을 100% 뒤집어도 not-답 불변).
⟹ **런타임 조회 다리 없음 = two-lane 확정.** C3(H_9329 · KO SEEN 어간 0/12)를 **영어가 held-out
어간에서 재현**했다 — 언어를 건넌 확정.

⟹ **H_9347 의 hoc DV 상승(0.03→0.75)은 벽 수리가 아니었다.** 담체로 연산자 lane 에 per-stem 답을
직접 캐시한 것이고, 그 캐시는 선언과 무관하다. **재조합(선언으로 안 사실 + 연산자 = 답) 은 여전히
🧱** — 담체가 답을 직접 써주지 않는 한 held-out 어간에서 연산자는 선언 저장소를 조회하지 않는다.

## 함의 — 벽의 정체가 좁혀졌다

BINDING/G1 벽 = "연산자가 어간에 게이트됨"(H_9346)의 더 깊은 정체 = **연산자 저장소와 선언 저장소
사이에 런타임 다리가 없음**(H_9359). 고치려면 능력·예산·형태론·담체-키가 아니라 **런타임 조회 경로
자체**를 만들어야 한다(어텐션 없는 conv byte-LM 에는 그 경로가 구조적으로 없다 — 그러나 이건 `a_no_llm
_frame_trap`: 아키텍처 진단이 아니라 측정된 부재다). NEXT = 그 다리를 만드는 레버(런타임 조회를
강제하는 corpus×task 또는 lane) 탐색.
