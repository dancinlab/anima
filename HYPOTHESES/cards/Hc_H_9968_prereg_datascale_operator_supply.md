...
동결 blake2b nested 부분집합(30k ⊂ 100k). 코퍼스 HF: `dancinlab/anima-earned-datascale`(sha256 ed326109ac05).
...
게이트(양 rung): G-ALIVE +5.03675 · G-PEDESTAL +0.00124 · G-POWER PASS. 참조: KO-FREE +0.00597 · 매칭길이 baseline +0.007 · 자의 R=+5.29653.

## 판정 — 🔴 CLOSED (사전등록표)
데이터를 30k→100k(**3.3×**) 키워도 EARNED 이 **안 오른다** — 오히려 +0.00438 → −0.00179 로 하락(0 부근 진동), 둘 다
**DATA-ADDITIVE**(연산자 공급 0). 사전등록 CLOSED 조건 충족: **모든 유효 rung ≤ +0.03**(abort 임계의 1/7 이하) ∧
**총상승 < max(0.02, 3·MDE_top)**(상승은커녕 음의 기울기). ⟹ **고정길이 자연 코퍼스의 재조합-연산자 공급은
...
## 정직 경계 (범위·reopen)
- **상위 rung(300k·600k) 미완**: `--earned` 이 행수에 초선형(67k용 설계)이라 300k 가 로컬 1.5시간+ 무완료 → 종료.
  판정은 **검정력 있는 2 rung(30k·100k) + 캠페인 매칭길이 벽**에 근거. **reopen**: 더 큰 호스트(pool)서 300k→1.2M
  rung 을 HF 코퍼스로 이어 재면 곡선의 상위 끝을 채울 수 있음(현 2점은 하락, 늦은 상승은 캠페인상 비개연이나 미측정).
