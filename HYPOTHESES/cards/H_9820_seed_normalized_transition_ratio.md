...
## 실행 결과 (동결 커밋 이후 실행 · 로컬 맥 CPU · 2026-07-20)

### ⛔ 선결 무효 조건 ② (재현 검사) 불합격 — 팔을 읽지 않는다

| seed · 팔 | [[H_9819]] (aiden · GPU) | 이번 (로컬 · CPU) | |
|---|---|---|---|
| 7 · baseline | 10800 | 10800 | ✅ |
| 7 · `duel` | **7200** | **10800** | ❌ |
| 7 · `rank1` | **14400** | **10800** | ❌ |
| 11 · baseline | 14400 | 14400 | ✅ |
| 11 · `duel` | 7200 | 7200 | ✅ |
| 11 · `rank1` | 10800 | 10800 | ✅ |

**4/6 일치 · 2/6 불일치** ⟹ 동결 규칙대로 **⛔ NON-DETERMINISTIC**. 팔 판독 금지.
(선결 ①(baseline 전이점 존재)은 통과했으나 ②가 깨지면 ①은 무의미하다.)

### 왜 어긋났나 — 잡음이 아니라 **지표의 구조적 결함**

진원지는 seed 7 `duel` 의 7200 셀 하나다:

| | d_acc | 32항목 중 | 바(0.75 = 24/32) |
|---|---|---|---|
| aiden | 0.7812 | **25** 개 | 1개 위 → 전이점 **7200** |
| 로컬 | 0.7188 | **23** 개 | 1개 아래 → 전이점 **10800** |

**항목 2개 차이가 전이점을 한 rung 통째로 옮긴다.** 두 값 모두 참값 0.75 에서 **0.41σ**
안이라 통계적으로 **구별 불가능**하다(n=32 · sd 0.0765).

이건 우연이 아니라 **정의에서 나오는 필연**이다. 전이점은 "**처음** 바를 넘은 rung" 이므로
전이점을 결정하는 셀은 **정의상 언제나 바에 가장 가까운 셀**이다. [[H_9819]] 의 전이점을
결정한 6개 셀 실측:

| 결정 셀 | d_acc | 바에서 |
|---|---|---|
| s7 base 10800 | 0.7500 | **0.00σ** (정확히 24/32) |
| s7 duel 7200 | 0.7812 | 0.43σ |
| s7 rank1 14400 | 0.8750 | 2.14σ (유일하게 안전) |
| s11 base 14400 | 0.8125 | 0.91σ |
| s11 duel 7200 | 0.7500 | **0.00σ** |
| s11 rank1 10800 | 0.7500 | **0.00σ** |

**6개 중 5개가 바에서 1σ 미만, 3개는 정확히 바 위** — 항목 **하나**만 뒤집혀도 전이점이 한
rung 움직인다. ⟹ 전이점 DV 는 잡음을 평균 내지 않고 **계기가 가장 눈먼 지점에 모든 무게를
싣는다**. [[H_9819]] 의 "분모 교체" 로는 안 고쳐진다 — 분자·분모가 **둘 다** 임계 통계다.

### [[H_9819]] 의 독해에 하는 일

- seed 7 세 팔의 전이점이 aiden 은 **3 rung 에 분산**(10800/7200/14400), 로컬은 **전부
  10800 동일** ⟹ 같은 세 팔이 호스트에 따라 "구조 있음" 과 "차이 없음" 으로 갈린다.
- 전이비도 뒤집힌다: s7 `duel` aiden r=0.667(바 통과) vs 로컬 r=1.000(불통과).
- ⟹ [[H_9819]] 의 "표 밖" 은 **유지**되나 이유가 바뀐다: 결과가 애매해서가 아니라
  **지표가 그 차이를 잴 능력이 없어서**다. 거기 적은 표 밖 관측 ①(duel 방향 일관)·
  ③(rank1 미재현)은 **근거를 잃는다**(둘 다 전이점 비교였다).

### 살아남는 것 / 무너지는 것 (경계)

- **무너짐**: 전이점·전이비에 기댄 모든 독해 — [[H_9819]] 전체, 이 H 의 팔 판정.
- **살아남음**: [[H_9817]] GROKKING BREAK — 임계 넘김이 아니라 **0.5000 → 1.0000**(≈5.7σ)
  이고 `.clm` 1.0000 · 경로 일치 1.0000 · 누출 Δ 0.0072 로 **독립 경로 확증**. 임계 근처
  셀에 의존하지 않으므로 사정권 밖이다.
- **미측정**: seed 4302·4303·13 은 **의도적으로 돌리지 않았다**. 동결 규칙이 팔 판독을
  금지한 이상 새 seed 의 팔 데이터는 **쓸 수 없고**, 재현 검사는 seed 7·11 로 완결된다
  (계산을 아낀 게 아니라 **쓸 수 없는 데이터를 만들지 않은 것**).

## 다음 (별도 사전등록 필수)

전이점·전이비 계열은 **폐기**한다. 임계를 통과시키지 않는 DV 로 가야 한다:

- **연속 DV**: 상전이 구간(3600~14400)의 rung 별 d_acc **평균**(곡선 아래 면적) — 모든
  셀이 기여하므로 잡음이 평균되고, 구간 한정으로 천장도 피한다.
- **검정력 선결**: 어떤 DV 든 **n=32 는 너무 작다**. 패널 확장(K↑) 또는 반복으로 **셀당
  sd 를 0.03 이하**로 먼저 떨어뜨린 뒤에만 팔 비교를 사전등록한다.

## Cross-links

[[H_9819]] 이 H 가 무너뜨리는 대상(동시에 동기) · [[H_9817]] 사정권 밖 · [[H_9816]] 판독불가 유지
