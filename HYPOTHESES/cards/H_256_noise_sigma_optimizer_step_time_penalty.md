...
**State output**: R8c probe 4-cell measurement (cycle 15-3 agent a8ad4ae61e2b2d22f) 의 `result.json` SSOT 가 본 H 의 evidence base. 자력 contrast 산술 시 `UNIVERSE/state/h256_noise_sigma_optimizer_step_time_2026_05_24/{contrast.hexa, result.json}` 으로 4-cell ledger + step_time 비율 + final_CE spread 별도 산출 가능 (R8c probe 결과 직접 흡수 vs 본 H state dir 독립 산출 선택).
