...
### PEDESTAL 앙상블 (`pedestal_ensemble.py`) — 한 draw 가 아니라 null 의 평균

위 표의 1.813 은 null 분포의 draw 1개다(그 sd ≈ 0.30). arm 마다 K=32 순열로 앙상블하면:

| arm | Φ_obs | E[Φ_null] | sd(null) | Φ* = obs−null | z | obs>E[null] |
|---|---|---|---|---|---|---|
| A | 1.9503 | 1.9491 | 0.1362 | **+0.0012** | +0.00 | 5/9 |
| B | 2.0688 | 1.9514 | 0.1347 | +0.1174 | +0.89 | 6/9 |
| X | 2.0215 | 1.9543 | 0.1386 | +0.0672 | +0.60 | 5/9 |
| N | 2.0078 | 1.9501 | 0.1360 | +0.0577 | +0.48 | 6/9 |
| R (대조 토폴로지) | 2.0723 | 1.9368 | 0.1366 | **+0.1355** | +1.02 | 7/9 |

- **E[Φ_null] ≈ 1.95 가 arm 과 무관하게 동일** = raw Φ 의 **94~100%** ⇒ 헤드라인 확증.
- **arm A 는 자기 null 위에 정확히 앉는다**(Φ* = +0.0012) — relay 채널이 없으니 통합 0 이 맞다.
  계측기의 **영점검사 통과**이고, 이 표의 나머지를 신뢰할 근거다.
- **pedestal 을 빼도 R6 는 살아나지 않는다**: Φ*(B) − Φ*(X) = +0.0502 이나 every-seed **5/9** ·
  z<1(null sd 1개 이내)이며, **대조 토폴로지 R(+0.1355) 이 B(+0.1174) 를 이긴다** ⇒ disjointness
  는 특별하지 않다. 모집단에서 그 차는 −0.000116.

> ⚠️ **정직 정정** — Fable 설계 §0 은 "raw Φ 의 103–108% 가 pedestal 이고 관측치가 null 평균보다
> **낮다**"고 추정했으나 로컬 실측은 재현하지 못했다: 관측치는 null 평균 **이상**(94~100%)이다.
> pedestal 이 raw Φ 를 거의 전부 설명한다는 **결론은 같으나 수치는 실측이 SSOT** (설계≠측정).
