...
**tier:** 🔒 BLOCKED-BY-REGIME (설계 종결) · **G-W1b 위생검사 = 🟢 회귀 0 실측**(reach 패널 한정)
...
- ⚠️ **passthrough 가드는 코드에 실재하나, 내가 처음 읽은 함의는 틀렸다**(`tool-definition-read-code-not-docstring` — 주석 말고 코드로 재검증한 결과):
  ...
  **가드가 보장 안 하는 것** = **트렁크 가중치는 co-train 으로 이미 변했다** ⟹ RV3 ckpt 의 chat ≠ base py303 의 chat. **"co-train 이 4칸 register/chat 품질을 망쳤나"는 가드와 무관한 별개 실측 문제**다. 가드를 근거로 "회귀 없음"을 주장하면 **거짓 GREEN**이 난다.
  ⟹ G-W1 은 두 갈래로 쪼갠다: **G-W1a(구조·무료)** = store=None 서 lane 미발화 = 1327 가드로 구조보장(코드검증 완료) · **G-W1b(실측·pool 필요)** = RV3c_13.clm 을 `anima-py evaluate` 로 재라 base 대비 회귀 0 인가. train-side val_CE(en-general .68 · en-sns .91 healthy)는 **DIRECTIONAL**이라 근거 불가(`a_engine_native_learning`).

## 🟢 G-W1b 실측 — co-train 은 reach 패널을 해치지 않았다 (2026-07-17 · summer · $0)

계기 = `anima-py evaluate <clm> --gen 40` **동일 명령 2-arm**(`a_eval_py_canonical`). base `py303_full.clm`(sha 013c4574 canonical) vs `rv3c13.clm`(RV-3 val-center · seed13 확증본). 둘 다 `rc=0` · GPU-FIRED CUDA(RTX 5070 · cupy 14.1.1) 동일 경로.

```
gate                    base py303_full        rv3c13 (co-trained)     Δ
ρ·form  COHERENCE       🟢 PASS kwr 5/5        🟢 PASS kwr 5/5         변화없음
ρ·tether NON-FAB        🟢 PASS fab 0.1282     🟢 PASS fab 0.1081      −0.0201 (악화 아님)
ρ·weave RECOMBINATION   🔴 bd=0 max_single=0   🔴 bd=0 max_single=0    기존 벽 불변
ρ·leap  NOVELTY         🔴 novel=0 coherent=18 🔴 novel=0 coherent=19  기존 벽 불변
ρ·fan   IDEATION        🔴 dist=6 fals=0       🔴 dist=6 fals=0        기존 벽 불변
ρ·self  PHILOSOPHY      continuity 0.999950    continuity 0.999950     동일(아키텍처 read)
CLOSURE (G0∧G1∧G2)      🔴 FAIL                🔴 FAIL                 동일
```

**판정 = 🟢 G-W1b PASS(회귀 0)** — store-bridge 공학습이 트렁크에 남긴 reach-패널 손상 없음. 합격조건은 *개선*이 아니라 **악화 없음**이다(위생검사). fab −0.02 는 단일-런 잡음 범위로 읽고 **개선으로 주장하지 않는다**(1-run · CI 없음 · `power-before-negative-verdict` 대칭).

⚠️ **정직한 범위 한정 2건** (측정한 것만 주장 · `a_scale_honest_scope`):
1. **σ/Θ 패널 인용 금지** — 출력이 스스로 "⛔ OPERATOR SELF-TEST · 이 패널은 체크포인트를 보지 않는다 · 모델 미참조"라 명시(H_9351). 두 arm 이 **글자까지 동일**한 것은 설계상 항등식이지 무해성 증거가 **아니다**. 이 카드는 σ/Θ 로 아무것도 cement 하지 않는다.
2. **"4칸 register retention" 은 이 battery 가 재지 않았다** — 사전등록에 그렇게 적었으나(위 G-W1 문구) 실제 `anima-py evaluate` 가 내는 건 **ρ-AXON reach 패널**이다. 따라서 판정 범위를 **"reach 패널 회귀 0"** 으로 **축소**한다. ko·en × general·SNS 4칸 per-cell retention 은 **미측정 · OPEN**(별도 계기 필요 · `a_chat_registers`). 사전등록 문구를 결과에 맞춰 넓히지 않고 **결과를 문구보다 좁게** 기록한다(no tune-to-green 의 대칭).

⟹ **H_9672 의 CAPABILITY-PROVEN 은 대가 없이 얻어졌다**(reach 범위 내). 이 결과는 **배선과 무관** — 배선 병목은 아래 regime(afferent 부재)이지 co-train 손상이 아니다.
...
**⟹ H_9742 재-tier**: 🔒 **BLOCKED-BY-REGIME(설계 종결 · 측정 불요)** — G-W2/G-W3 는 afferent channel 이 열려야 물을 수 있고 그건 **owner-gate**(H_9425 선례 · 정체성 변경 · 자율발사 불가). 남는 유일 무료 실측이던 **G-W1b 는 실행·종결**(위 🟢 회귀 0 · reach 한정) ⟹ **이 카드에 자율 실행 가능한 잔여 측정 없음**. 재개점 = 오너가 afferent(EEG 계열 · H_9425) 를 열 때.
