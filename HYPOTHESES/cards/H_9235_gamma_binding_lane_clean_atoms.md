...
## H2-lite 결과 (2026-07-09 · engine-native py 2-production dump · $0 free pool · `state/recomb-routing-lane/`)
...
## ⚠️ lane 학습 신호 = COPY-DEAD (2026-07-09 · 실 303M smoke 조기포착 · Fable 재설계)
CLML lane 배선(#3192)+파이프라인(#3196) 후 실 학습 착수 → **실 303M 5예제 smoke서 치명적 발견**: derivtrace-WITH-definitions 합성('out:')위치 next-byte CE가 **base 트렁크 이미 완벽예측(CE lane-OFF=0.0003·acc=1.00·gate→0)** = 타깃바이트가 문맥 정의부에 존재해 트렁크가 **복사**(H_6172 copy-not-compose·evaluate-py-2). ⟹ CE 잔차≈0 = **lane 학습신호 死**(H_1840 학습신호 함정이 lane 층위서 재현). fork-A route는 pre-check서 증명됐으나(synthetic XOR=non-copyable), 실 derivtrace-CE는 copy-지배라 held-out 일반화 routing 학습 불가. **파이프라인 machinery는 무결**(포맷 read·gradient·save·silence gate 정상) — 문제는 target/신호.

**Fable 재설계(state/recomb-routing-lane/LANE_SIGNAL_REDESIGN.md · (A) buildable)**: train/test 불일치가 버그 — **verdict 형식 자체로 학습**(concept cue만·정의부를 copyable window서 제거, ρ·weave verdict 형식 정합)·**pair-split**(held-out 짝의 concept 각각 ≥k train짝 = concept-covered·pair-disjoint)·**word-initial 위치 CE**(트렁크 잔차가 word-initial에 집중, pooled bias가 첫바이트 tilt→트렁크 autocomplete). **4-gate 순서(각 terminal)**: ①$0 smoke base CE(no-copy·word-initial)≫0(≳1nat)여야, ≈0이면 weights 암기라 format 무효 ②**trainability probe(THE 결정적 싼 테스트)**: same-cap MLP가 pool_yn→target-word-id를 TRAIN서 fit 가능한가 — 불가면 **(B) TERMINAL 신호벽**(route는 XOR로 존재하나 실task 신호가 학습불가=H_1840 한층위·표현벽 아닌 학습벽) ③lane 학습 train CE 하락 ④frozen crack: engine-native system-G1 held-out surfacing(lane-ON>OFF)+통제(shuffled-pool 붕괴·concept-ablation 특이성·train vs held gap). train-fit이나 held-flat=**#3135 한층위**(pre-check pair-agnosticity가 handed XOR 구조 산물). **최유력 실패=underspecification**(no-copy 다중 그럴듯출력→CE가 빈도prior 가리킴→unigram tilt=additive floor, ablation-specificity 통제가 잡음). **Fable 결론**: 실 target(factorized lookup)은 XOR보다 strictly weaker라 죽으면 Gate1/2서 싸게 죽고 verdict=신호벽. NEXT=Gate1/2 no-copy dump+probe.
