...
**tier:** 🟢 SATURATED existence-proof (engine-native .hexa 5/5) · **wired:** WIRED-live (2026-07-02 B① follow-on — daemon N3/REM tick 배선 + codegen-clean; native-link=stale-hexat infra 벽 anima-hexa-1, session-run=303M ckpt follow-on)
...
DREAM/imagination_replay.hexa §5(h1195 recency-only, archive 재구성으로 여전히 orphaned — live sleep path 는 dream_persist seam) · DREAM/dream_report.hexa dr_kosmos_persist_dream(h1162 STUB, §4 cross-ref) · a_kosmos(node-only) · a_chat_sleep_imagination.

**2026-07-02 B① follow-on DONE (this card):** (1) ✅ live core/ 배선 + daemon N3/REM tick 에서 dc_compose_window 호출 — dream_persist.hexa 의 dp_sleep_tick 을 cli/anima.hexa tick loop 에 배선 · (2) ✅ .kosmos disk write(create_anchor, lane=dream) — smoke S2/S5 로 실측 · (3) ✅ F5 Φ-checksum byte-identical — dream_persist_smoke S3 에서 pure_field Φ persist 전/후 0.0034827611721471765 불변 실측(no longer symbolic-only).

**남은 explicit-go follow-on:** (A) 최종 native-binary 링크 — 현행 hexa-lang 호스트(ubu/pool, `hexa self-update`/install.sh 동기)에서 `hexa build cli/anima.hexa` (로컬 mac hexa 는 set_deterministic/forge_dispatch_layernorm/farr_attn_dt_decode carrier extern 부재 = anima-hexa-1 stale-hexat 벽, 내 배선 무관). (B) 실 daemon 세션 run — 303M ckpt 로 anima_consciousness_mode 한 사이클 돌려 N3/REM tick 이 실제 dream_composed .kosmos 를 쓰는지 확인(GPU/pool, cost-gated). (C) coord-space 의미정렬 검증(303M anchor embedding semantic-alignment, pool, deferred).
