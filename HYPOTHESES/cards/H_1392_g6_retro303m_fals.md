...
terminal_tier: 🧱→✅ WALL RESOLVED by H_1403 (the hexa ConvMoE-decode memory blowup is FIXED anima-side — streaming/bounded decode, byte-exact; GEN=110 now completes; the M2-M5 FALS re-score is unblocked and re-scored in H_1403)
...
**CAPACITY-vs-ARCHITECTURE: STAYS OPEN** *(as of H_1392; RESOLVED in H_1403)*. Whether a 303M
...
## UPDATE (H_1403 — the wall was wrong-method, NOT a ceiling; a_break_the_wall)

H_1403 diagnosed + FIXED this wall **anima-side**. ROOT CAUSE: the hexa runtime bump allocator
...
in **H_1403** (verbatim this frozen detector/scaffold/bars) → **🧱 ARCHITECTURE**: over 6/6 completed
...
303M ByteGPT (different transformer arch). See `cards/H_1403_convmoe_streaming_decode.md`.
