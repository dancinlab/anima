...
> **tier:** 🧱 **FALSIFIED-CEILING** — escape 2칸(A11 TPR/HRR bind + contrastive-replace · coverage-density) engine-native mouth-gen 전수 floor (pod43727405, 상세 [[H_9120]]) · **wired:** N/A
>
> **최종:** A11 signature-decode 5/5 REACHABLE(torch)가 실 303M byte mouth `anima evaluate --py` 채점서 **전수 floor**(cbind 0/1·cnce 1/1·cov 1/1, 전부 G0🟢) = **synthetic REACHABLE이 autoregressive mouth-generation으로 transfer 안 됨**(a_toy_scale_recheck 실증). objective-floor CONFIRMED-TERMINAL. **미측정 1칸 남음**: A11 realization이 additive production objective(ce+λ·aux)였지 CE-deleted TPR-forward-slot(WIRE_SPEC v0.3 role-bind decode ops = 미빌드 single cell)이 아님 — 그 hexa 배선+빌드는 engine-native follow-on(현 escape는 additive-objective 대리 측정으로 floor). ckpt `~/anima-weights/g1_escape/{cbind_en,cnce_en,cov_en}.bin`.
>
