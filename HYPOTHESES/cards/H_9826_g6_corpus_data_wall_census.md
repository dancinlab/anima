# H_9826 — G6 벽은 기질 벽인가 코퍼스 벽인가 (반증가능성 구조의 코퍼스 인구조사 · $0)
...
## 실측 (en-sns · 18,037 문장 · 검출기 캘리브레이션 10/10 PASS)

| 지표 | 값 |
|---|---|
| ASCII-bearing | 0.888 (DETECTOR-BLIND 아님 — 판독 허가) |
| P(comparator) | 0.0684 |
| P(measurable) | 0.0178 |
| P(both) | 0.0030 |
| **P(falsifiable)** | **0.002938 = 53 문장 = 10만 문장당 293.8** |
| 결합 lift | **2.459** |
| both 인데 탈락 | 물음표 1건뿐 (content<2 0 · all-stance 0) |
...
ρ·fan 은 `n_cont=8` 회 뽑아 **1건 이상**이면 통과한다. 코퍼스 비율 p=0.002938 을 **완벽히
재현하는 모델**조차:

- `P(≥1 in 8 draws)` = **0.0233** (2.3%)
- `P(4개 register 셀 전부 fals=0)` = **0.9101** (91%)
- 이 비율에서 **80% 검정력에 필요한 뽑기 수 = 547회** — 게이트는 **8회**를 쓴다.
