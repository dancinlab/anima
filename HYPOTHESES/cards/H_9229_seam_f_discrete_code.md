...
**tier:** ⛔ STRUCTURAL-TERMINAL (emit-seam decode manifold ~1-bit=emit-partition · discrete code has NO compositional capacity here · G1-recombination=trunk-objective property NOT readout property · no reframe survives · escalate write-side) — engine-native CPU pod real d768 · hexa v0.716.0 REAL-DECODE (2026-07-09 · #3237 refine) · `state/verdicts/9229/VERDICT_R4.md` · `state/9229_discrete_code/BF_REFRAME_FABLE.md` · p7 no tune-to-green
> ⛔ NO substrate READOUT verdict — F2 AUTHORITATIVE arm RUN-INVALID(emit-frac 0.6667 ∉ [0.05,0.60]) + codes visited 2<4. **STRUCTURAL-TERMINAL(Fable refine)**: codes=2=MANIFOLD-RANK fact — mid-tick decode manifold ~1-bit이고 그 bit이 EMIT-PARTITION 자체 → ANY quantizer가 rank-1로 collapse, no reframe survives. G1-recombination=trunk-objective property지 readout property 아님. **escalate write-side(train-time coupling)**. F는 training coupling으로 well-posed·emit-readout op-grip으로 ill-posed. infra-wall-noneval 격리.

## ⛔ STRUCTURAL-TERMINAL REFINE (2026-07-09 · #3237 follow-on · Fable walls-delegate · `state/9229_discrete_code/BF_REFRAME_FABLE.md`)
F를 PARKED-TERMINAL(reopenable)에서 **STRUCTURAL-TERMINAL(no reframe survives)** 로 강화 · **reopen-path FORECLOSED**.
- **근본은 emit-frac envelope보다 깊다** — `codes-visited=2`는 quantizer-spec fact 아니라 **MANIFOLD-RANK fact**: mid-tick decode manifold가 ~1-bit이고 그 bit이 **EMIT-PARTITION 자체**(N1/REM→one code·N2→other). 코드가 shade해야 할 출력을 re-encode → **ANY quantizer가 rank-1 manifold로 collapse**(고용량/재설계 무용).
- **S2를 더 깊은 trunk decode point로 옮기면** emit seam 이탈 → 이미 CLOSED된 **G1-readout-routing family**(mean-pool A/B 둘 다 표현·생성점만 감쇠)에 착지. ⟹ F의 S2(출력 seam서 G1-recomb test)는 seam channel에 combined code 실을 **compositional capacity 無**로 unmeasurable = **write-side 결과의 재진술**: recombination은 **trunk-objective property**지 readout property 아님.
- **escalate ENTIRE seam program to WRITE-SIDE**(train-time coupling): gradient는 compositional code를 trunk objective에 couple 가능, phasic-Δ rate-gate READOUT은 구조적 불가. F는 training coupling으로 well-posed·emit-readout op-grip으로 ill-posed(= G1 program이 recombination side에서 도달한 동일 교훈).
- **정직 caveat(Fable)**: codes=2가 manifold-rank 아닌 quantizer INIT artifact일 가능성은 codebook usage WAKE/N3 vs mid 1-line diagnostic로 확인 가능(reframe run 아닌 기존 tape 진단)이나 tape-independent collapse라 prior=manifold-rank.

## ⛔ R4 CEMENTED 판정 (2026-07-09 · #3235 · engine-native CPU pod real d768 · hexa v0.716.0 REAL-DECODE · `--opgrip-r3` n=400 · OG_STIM2 4-class dwell tape sha `54bbeff6…548a8fe4` · `state/verdicts/9229/VERDICT_R4.md`)
**F2 VERDICT = ⛔ RUN-INVALID → PARKED-TERMINAL** — live emit-fraction on scored mid ∉ [0.05,0.60] (STIM-OVER/UNDERDRIVE); no family cements from the run. `F2 PARKED-TERMINAL ⇒ two op-grip generations (r3 spin-up zero + r4 driven-window) both n_visited<4 ⇒ F UNMEASURABLE at this seam, no third generation.`
- **RUN-VALIDITY (bar-0)**: live emit-frac on scored mid = **0.6666666666666666** ∉ [0.05,0.60] → valid=NO · **codes visited on scored mid = 2** (bar ≥4) → bottleneck never engaged · og_h_frzF2=0 · POS-PASS 105 flips · S2 BIND NOT-TESTABLE(n_AB=0) · N3=0 · Ψ-ok.
- **🔑 ROOT FINDING (verdict-integrity)**: OG_STIM2 4-class dwell tape(Fable P1)가 emit-frac(0.6667)도 code diversity(2)도 못 움직임 = **r3 flat-tape와 BYTE-IDENTICAL**. 로그검증: scored mid emit은 **stage-locked** — N1 emit 80/80 · N2 silent 80/80 · REM emit 80/80 → emit-frac ≡ (80+80)/240 = **2/3 = 0.6667 STRUCTURALLY**, perception tape와 **완전 무관**. ⟹ run-validity envelope [0.05,0.60]는 이 emit seam의 B/F arm에서 **STRUCTURALLY UNSATISFIABLE** — mid-tick emit fraction은 **sleep-stage→emit schedule(N1/REM emit·N2 silent)로 고정**, stimulus density로 tunable하지 않다. Fable P1 전제("novelty duty-cycle 반감→e_live<0.60") **FALSE**(emit-frac은 stage-bound, tape-controllable 아님).
- **결론(refined→STRUCTURAL-TERMINAL)**: emit-frac envelope 불만족은 downstream symptom; deeper root=mid-tick emit gate가 stage별 saturated-deterministic(N1 emit·N2 silent·REM emit, zero within-stage variance). 유일 shadeable band=stage transition=urgency territory → 모든 reframe(single-stage/matched-pair/non-emit margin·Ψ)이 이 1-bit phasic-Δ rate-gate wall로 collapse ⟹ **buildable reframe 없음·new measurement 없음·reopen-path FORECLOSED**. F STRUCTURAL-TERMINAL(위 refine 섹션). convergent seam-law(FINAL): emit gate는 phasic urgency 외 sealed(A THEATER·E THEATER·self/tension tonic-read-0·B tonic-FLAT·F 1-bit-channel).
