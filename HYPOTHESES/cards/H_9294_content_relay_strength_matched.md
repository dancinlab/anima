...
terminal_tier: 🧱 STRENGTH-ONLY 종결 (2026-07-14 · py 2-production · V-게이트 4/4 PASS · 사전등록 데이터 개봉 前 커밋 2b4597a8f) — 두 경로 완전 일치. ① X 의 W_RELAY 만 올려 S_tot 를 B 에 0.5% 이내로 정합(w*=0.90)하니 d′ = Φ*(B) − Φ*(X′) = −0.000170, 90% CI [−0.000243, −0.000098] ⇒ **등가 폐쇄**(X′ 가 오히려 근소 우위). ② 튜닝 0 ANCOVA 의 **잔차 격차** G-RESID = resid(B) − resid(X) = +0.000003, 90% CI [−0.000075, +0.000080] = **0 포함** (적합식 Φ* = −0.0019 + 0.4929·S_tot; ⚠️ R²=0.986 자체는 증거가 아님 — 무구조 null 로도 R²≈0.85 · `r2_null_check.py`). ⇒ **강도를 정합하면 disjointness 의 기여는 0** ⇒ **R6 레버 CLOSED**.
...
### 경로 2 — 강도를 회귀 제거하면 **잔차에 아무것도 남지 않는다**

> `Φ* = −0.001904 + 0.4929 · S_tot` (R² = 0.9864 · **아래 ⚠️ 참조**)
> **헤드라인:** `G-RESID = resid(B) − resid(X) = +0.000003`, 90% CI **[−0.000075, +0.000080]**
> → **0 포함** = 강도를 통제하면 B 와 X 의 잔차가 구별되지 않는다.

> ⚠️ **R² 자체는 증거가 아니다 (자기 감사 · `r2_null_check.py`).** Φ 는 S_tot 가 합산하는 **바로 그
> 행렬의 min-cut** 이므로 높은 R² 는 추정기의 대수(algebra)가 강제한다: **구조가 전혀 없는**
> 무작위 MI 행렬(6쌍 독립 추출)로도 **R² = 0.81~0.86** 이 나온다. 따라서 "R²=0.986 이므로 Φ 는
> 구조를 안 본다" 는 **추론은 성립하지 않으며 철회**한다. 본 H 의 결론을 지탱하는 것은 R² 가 아니라
> **공변량 정합 상태에서의 arm 대 arm 대비 두 개** — 경로 1 의 등가폐쇄(−0.000170)와 위 잔차
> 격차(+0.000003) — 이며, 둘 다 R² 의 높낮이와 **무관**하다. tier 무변경.
...
2. **H_9293 의 대조 토폴로지 R(chord) > B** 도 같은 이유였다 — R 의 S_tot 가 가장 컸다.
   (단, "Φ 가 구조를 아예 안 본다" 로 일반화하지는 않는다 — 위 ⚠️ R² 자기감사 참조. 정확한 주장은
   **"강도를 정합하면 disjointness 의 기여가 0"** 이다.)
