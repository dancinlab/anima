...
- **`구현됨·미배선`**: torch 모듈+codec+numpy 미러+decode tap 완료 — 아직 train.py `--store-query-src`(fresh_k 설정·tap detach·model.clms(yn_fresh=)) 없음·evaluate 진단 없음. train flag 없이는 lane-5 .clm 생성 불가(full-chain test=증분6).
