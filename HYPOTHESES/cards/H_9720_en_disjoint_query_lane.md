...
1. **[$0·최우선] lookup≠addressing 판별 — ✅ store-side 통제로 대부분 해소 (2026-07-18 · summer 303M $0)**:
   ...
   - 🔧 격리 `--store-addr-census` 계기공백(별건 follow-on): census 는 **penultimate __last + random W_q** 를 재므로 fresh 의 실제 주소경로(L3-tap+학습 W_fresh/W_q_fresh) 판별 불가 — `--dump-hidden` 을 fresh tap 으로 확장하거나 census 를 fresh-query-path 로 확장 필요(위 store-side 통제가 실제 lookup 경로를 이미 검증하므로 census 는 보완재).
