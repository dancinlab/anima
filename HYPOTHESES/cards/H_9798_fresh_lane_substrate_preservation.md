...
1. **검정력 부족 — 대역 논쟁이 아니라 seed 수가 병목**(아래 §정밀분석). 형식 TOST 미실시.
...
### 📐 정밀분석 — 병목은 대역이 아니라 검정력 (2026-07-20 · lab full 불일치를 자체계산으로 판정)

Δ = fresh − C-noscore = **3.12201**(s7) · **4.12414**(s4302). n=2 ⟹ mean **3.6231** · sd **0.7086** ·
se **0.5011** · **df=1** ⟹ t(0.95,df=1)=6.314 ⟹ **95% 단측 하한 = 0.4594** (90% CI [0.4594, 6.7868]).

**두 모델이 갈렸고 둘 다 부분적으로 틀렸다** (`a_lab_full_diverge` · 자체계산으로 판정):
- **fable** "KILL TERMINAL · 대역-강건(δ≤6.8 전 대역서 등가 불성립)" — 산술은 재현되나 **논리 오류**:
  "등가를 *보이지 못함*"과 "비등가를 *보임*"은 다른 주장이다. 후자는 **하한 > δ** 를 요구한다.
- **sol** "사전등록 결함 inconclusive" — 신중하나 점추정이 통제산포의 **7.51배** · 부호 2/2 일치인
  사실을 과소평가.

**자체계산 판정**: 자연스러운 대역 후보 = C-noscore 자신의 seed 산포 **0.4824**.
하한 **0.4594 < 0.4824** ⟹ **형식 비등가 주장은 간발의 차로 불성립**. 원인은 효과크기가 아니라
**df=1 의 t배수 6.314** — 즉 **검정력**이다.

```
   0        0.4594   0.4824                              6.7868
   |──────────┤ 하한  ├ 대역                                  |
              └─ 0.023 차이로 형식주장 실패 (효과 아닌 검정력 탓)
```

**감도분석** — 3번째 seed 를 넣으면(df=2, t=2.920) 3rd 값이 3.0/3.6/4.3 어느 쪽이어도
하한 **2.38~2.78 ≫ 0.4824** ⟹ 형식 비등가 성립. ⟹ **s4303 3-arm 발사**(2026-07-20).

⟹ 현재 정직한 최대 주장: **서술적으로 강한 반증(2/2 부호일치·7.5× 산포), 형식 판정은 검정력 미달**.
KILL 도 inconclusive 도 아닌 **under-powered directional falsification**.

### 🏁 3-seed 완결 — 조건① 형식 성립 · 조건② 부호만 (2026-07-20)

s4303 3-arm 발사·회수·측정 완료(pod 3기 전량 teardown · 9 ckpt HF 보존 · sha256 대장 9줄).
계산식은 **s4303 측정 전에 고정**했다(t(0.95,df=2)=2.920 · 대역=C-noscore seed range) — 사후 재조정 없음.

| arm | s7 | s4302 | s4303 |
|---|---|---|---|
| C-noscore | 6.65809 | 6.17565 | 6.23232 |
| fresh | 9.78010 | 10.29979 | 10.46047 |
| legacy | 9.94953 | 10.83380 | 11.47854 |

**조건①(fresh ≢ C-noscore) — ✅ 형식 비등가 성립**
Δ = {3.1220, 4.1241, 4.2282} · mean **3.8248** · sd 0.6108 · se 0.3527 · df=2
⟹ **95% 단측 하한 2.7950** vs 대역 **0.4824** ⟹ 하한이 대역의 **5.8배**, 여유 **+2.3126**.
(사전 감도예측 2.38~2.78 과 일치 — 예측이 맞았다.) ⟹ **"분리 lane = 무간섭 보존" 반증 확정.**

**조건②(fresh < legacy) — ⚠️ 부호 3/3 일치이나 형식 미달**
Δ = {−0.1694, −0.5340, −1.0181} · mean −0.5738 · se 0.2458 ⟹ 95% 단측 **상한 +0.1439 > 0**
⟹ 0 을 배제 못 함. seed 간 분산이 커서(격차가 −0.17→−1.02 로 3배 벌어짐) n=3 로는 부족.
**서열 noscore < fresh < legacy 는 3/3 seed 단조** — 방향은 견고하나 크기 주장은 DIRECTIONAL.

**⟹ 최종 판정**: **🔴 이중구조 KILL — TERMINAL** (조건① 형식 성립 · 사전등록 falsify 는 OR 규칙이라
①만으로 충분). 부차 관측 "detach 가 공유보다 덜 해친다"는 **DIRECTIONAL**(부호 3/3, 크기 미확정).

**남은 표기 의무 2종**(TERMINAL 을 막지는 않음 — 판정 방향과 무관):
- main-only warm-start · CLMS 미복원 · **lane-off CE**(gen.txt 에 store 질의 0 ⟹ lane 미발화).
- CE₀ 잔여 0.078 미규명(아래 절) — 4 arm 공통모드라 ΔCE 무해.
