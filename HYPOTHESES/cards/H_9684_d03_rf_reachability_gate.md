...
**status:** 🟢 게이트 EXECUTED ($0 · D≈4B≪RF35B ⟹ D0-3 발사 가능 · 내 도달성 우려는 반증 · 이 lane 벽=최적화 교착 확인) — source=교차-lane(H_9560/H_9611 census → H_9672 D0-3)
...
## 🟢 거리 census 실행 = **게이트 통과** · 내 우려는 반증됐다 (2026-07-17 · $0 · 코드 실독)
게이트를 **실행**했다(모델 불요·순수 산술). 계기 = `cli/corpus.py::_sb_emit_block`(storebind 블록의 실 query 구성).

**구조 실독**: `prompt = "%s %s => " % (op_s, entity)` · `op_s ∈ {is, not}` · 질의점(주소 계산 지점) = prompt 마지막 byte(우측정렬 `T−1`).
⟹ **개체가 query 프롬프트 *안*에 있다.**

| prompt | 총 | 개체 끝 | 질의점 | **D(개체끝→질의)** |
|---|---|---|---|---|
| `"is demar => "` | 12B | 8B | 12B | **4B** |
| `"not demar => "` | 13B | 9B | 13B | **4B** |
| `"not merad9 => "` | 14B | 10B | 14B | **4B** |

**⟹ D ≈ 4 byte ≪ RF = 35 byte ⟹ 게이트 판정 = "D≤35 대다수" = D0-3 유의미·발사 가능.** 양성통제(D≤5) **자동 충족**.

### ⚠️ 자기정정(11번째) — 내가 우려한 함정은 이 lane 에 없다
이 카드를 쓸 때 나는 "개체가 질의에서 멀면 D0-3 은 BY CONSTRUCTION 실패한다"를 우려했다. **실측: 아니다.** `_sb_emit_block` 이 개체를 질의 프롬프트 안에 둔다 ⟹ **RF 도달성은 이 lane 의 문제가 아니다.**

### 🔑 그런데 게이트는 반대 방향으로 값졌다 (해석을 **무효화**가 아니라 **정당화**)
D≈4 = 개체가 **질의점 RF 한복판** ⟹
- **D0-3 이 FAIL 하면 그건 진짜 "(3) 트렁크 미인코딩"이다** — "못 닿아서"라는 변명이 **배제**된다.
- ⟹ [[H_9672]] 판정표의 **"D0-3 FAIL ∧ T3 성공 = addr gradient 가 트렁크를 구부려 인코딩 생성"** 해석이 **이제 정당하게 성립**한다(도달성 교란 없음).
- 사전등록의 값: 결과가 어느 쪽이든 **읽는 법이 미리 정해져 있었다**.

### 📌 파생 함의 — 내 RF/GN 축은 이 lane 의 벽 설명에서 **빠진다**(정직한 배제)
개체가 RF 안(D=4)인데도 Stage1.5 에서 주소가 **chance(P1 0.586)** 였다 ⟹ 이 벽은 **도달성이 아니라 순수 최적화 교착**(H_9672 의 (2) 닭-달걀 진단)임이 **내 census 로도 독립 지지**된다. 내 GN/RF 축(beyond-RF 는 GN bus 뿐)은 **이 lane 에 적용되지 않는다** — 그건 *원거리 문맥* 벽이고, 이 lane 은 *근거리인데 못 배우는* 벽이다. **두 벽은 별개다.**
⟹ [[H_9560]]/[[H_9611]] 의 scope 를 이만큼 좁힌다: 그 census 는 **beyond-RF 문맥 다리**에 대한 것이지, **within-RF 주소학습 교착**과는 무관.

## 상태
🟢 게이트 EXECUTED ($0 · DIRECTIONAL) — **D≈4B ≪ RF 35B ⟹ D0-3 발사 가능·양성통제 자동충족·"구부림" 해석 정당화**. 내 도달성 우려는 **반증**(11번째 자기정정) · 파생: 이 lane 의 벽 = 도달성 아닌 **최적화 교착**(H_9672 진단 독립지지) · 내 RF/GN 축은 이 lane 서 **정직히 배제**. **distinct-from-kills:** RF 측정 카드들과 달리 그 측정을 타 lane 진단에 게이트 적용 — 결과는 게이트 통과(발사 허가).
