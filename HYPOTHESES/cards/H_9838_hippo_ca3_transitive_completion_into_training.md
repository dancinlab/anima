...
## 🔻 심화 + 골화 (2026-07-21) — 실제 표현으로 올리자 주장이 깨졌다

### 무엇을 심화했나

첫 판은 `hippo_lane.fixture_codes` 가 **심어준** 정수 코드 위에서만 측정했다. 그건 CA3 산술의 상한만
재는 것이지 "이 모델이 실제로 들고 있는 표현 위에서 되는가" 를 재지 않는다. `--hippo-reps <ckpt>` 를
추가해 **생산 trunk 의 진짜 penultimate**(`core/decode.clm_penult_pooled_W` → `dg_decorrelate` →
`dg_codes`)로 같은 배터리를 돌렸다. 새 계기가 아니라 **코드 출처만 교체** — 팔·통제·바 전부 불변.

### 실측 (`py303_full.clm` · 3 seed × 3 기하 · 로컬 CPU 12초 · $0)

```
부하    심어둔 코드                   실제 303M 표현
 8항목  CERTIFIED                    CERTIFIED  (단 pool=5·우연 0.2 → 절제가 바닥바 위, 판별력 없음)
16항목  CERTIFIED ← 판정부하          INVALID   값-셔플 받침대 0.3750 > 바 0.3077
32항목  INSTRUMENT-DEAD              INVALID   값-셔플 받침대 0.1562 > 바 0.1500
```

**판별력과 통제 타당성을 동시에 갖춘 부하가 실제 표현에는 하나도 없다.** 16·32 항목에서 터진 것은
**참값 0 받침대**다 — 구조를 없앤(값-셔플) 저장소가 바를 넘겼다는 뜻이고, 그건 정답이 연쇄가 아니라
**코드 기하 자체**에서 나온다는 신호다. 이 받침대가 존재하는 이유가 정확히 이 경우다
(`phi-estimator-needs-zero-truth-pedestal`).

### 왜 이렇게 됐나 — 진단이 이미 코드에 있었다

`core/hippo_lane.py` 헤더가 미리 경고했다: **"Raw single-token 303M reps are near-collinear
(a dominant shared direction)"**. 새로 넣은 분리 진단이 그걸 수치로 확인한다:

```
심어둔 코드 16항목 : within=0.0469  across=0.0117  margin=+0.0352  ← 사실상 직교
실제 표현 16항목   : within=0.0625  across=0.0260  margin=+0.0365  ← 겹침이 2.2배
```

심어둔 코드는 **손으로 만든 유리한 기하**였다. DG 탈상관 + kWTA 는 실제 표현을 받침대가 거부할
만큼 분리시키지 못한다.

### 이것이 함께 푸는 수수께끼 — kWTA 비인과의 정체

첫 판에서 "패턴분리를 끄면 붕괴한다"는 예측이 어긋난 것(Δ +0.0625 / −0.1250)을 그때는 설명하지
못했다. 분리 진단이 답을 준다: **심어둔 코드는 이미 직교라 kWTA 가 제거할 분리가 없었다.**
비인과는 "분리가 불필요하다"가 아니라 **"이 세계에선 분리가 이미 공짜였다"** 였다.
실제 표현에서는 그 공짜가 사라지고, 사라지자 받침대가 터졌다 — 두 관측이 한 원인이다.

### 굳히는 것 (골화) — 범위를 좁혀서

- 🟢 **CA3 다단계 완성 산술 자체는 실재**한다 — 심어둔 직교 코드 위, 16항목/12간선, 우연 0.0769
  대비 0.9375, 1단계 절제 시 0.1538(Δ +0.7837). **DIRECTIONAL, 토이 한정.**
- 🔴 **in-vivo 사용 가능성은 부정**된다 — 생산 trunk 의 실제 표현 위에서는 판별 가능한 모든 부하가
  INVALID. 현재 형태의 이 레인은 실제 표현에 대해 **읽을 수 없다**.
- ⛔ 따라서 **쓰기 경로(`train --hippo-aux`) 설계는 보류**한다. 읽기가 실제 표현에서 무효인 레인에
  분리-보존 법칙(`a_substrate_disjoint`)을 깨는 쓰기 경로를 뚫는 것은 순서가 뒤집힌 지출이다.
- 🚫 **구조 조정으로 초록불 만들기 금지.** DG 차원·active·탈상관 방식을 바꿔 받침대를 통과시키는
  탐색은 정의상 tune-to-green 이다. 되살리려면 **설정을 사전등록한 별도 H** 로만.

### 재현

```
anima-py evaluate --hippo-transitive-selftest                                   # 심어둔 코드(토이)
anima-py evaluate --hippo-transitive-selftest --hippo-reps <ckpt.clm>           # 실제 표현
```
