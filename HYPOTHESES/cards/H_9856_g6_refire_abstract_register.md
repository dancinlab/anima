# H_9856 — G6 재발사: 추상 register 확장 + 교정된 FORGET 게이트 (사전등록 · 발사 전 동결)

**status:** ⛔ **INVALID (선결 ②③ 실패 · 손상 지속)** — 그러나 선결이 이번엔 **제 일을 했다**.
비율은 계속 올랐다(BASE 0.00415 → cycle1 0.0162 → **cycle2 0.0294**, 7.09×).
**wired:** yes — `anima-py corpus falsidrill [--falsi-ablate]` (담체 12 · 2-register 어휘 · #4284 착륙)
**source:** [[H_9837]] REAL 팔 실측 2건 — ① 밀도 전이 8.2% ② `ρ·form` 통제 붕괴

## 실측이 지목한 두 병목 (추측 아님)

**① 형식이 주제를 못 건넜다.** [[H_9837]]: 코퍼스 밀도 **0.197 → 모델 0.0162 = 8.2%** 만 전이.
드릴은 `rainfall·salt·voltage`(물리량)인데 평가 프롬프트는 `consciousness arises from cells: `
(추상·정신). **한 번도 보여준 적 없는 register 간극을 건너라고 요구**한 셈이다.
→ 교정: 어휘의 절반을 **정신·추상**(attention·belief·thought·insight·judgement…)으로.
평가 개념 **내용어는 여전히 전부 held out** — 일반화 축은 그대로 두고 register 간극만 닫는다.

**② 경직 담체가 모델을 망가뜨렸다.** `ρ·form` 통제 `self-shuffle 0.0 → 0.4`.
→ 교정은 [[H_9855]] 에서 착륙(담체 3 → 12 · 어순 2방향 · 길이 가변).

## 🔒 교정된 선결 무효조건 (BLOCKING · **값이 아니라 축과 통제로** 건다)

[[H_9837]] 의 무효 원인을 그대로 고친다. convergence
`forget-gate-must-read-the-axis-verdict-not-the-reinforced-value`:

1. `HILLOCK` = **LIVE**
2. **`ρ·form` 축 판정 == PASS** — val 이 아니라 **축**(통제 포함). val 은 드릴이 강화하는 지층이라
   항상 통과하므로 게이트로 쓰면 위조다.
3. `ρ·form` 통제 **`self-shuffle ≤ 0.05`**(그 축의 동결 상한) — 지난번 죽은 바로 그 지층
4. `dist ≥ 5` (TEMPLATE-COLLAPSE 차단)

하나라도 어기면 ⛔ **INVALID** — 팔을 **읽지 않는다**(글자대로 통과 = tune-to-green).

## 🔒 판정표 (동결 · 발사 전 · BASE = fals-rate 0.004149 · sd 0.00414)

| 조건 (선결 4/4 통과 전제) | 판정 |
|---|---|
| `ρ·fan` **PASS** ∧ fals-rate > **0.01657**(=BASE+3sd) | 🔑 **G6 BREAK** — 단, ABL 팔이 특이성을 확인해야 완결 |
| `ρ·fan` PASS ∧ fals-rate ≤ 0.01657 | 🟡 **GATE-PASS-UNDERPOWERED** — 통과가 뽑기 운(지난번 12% 사건 재현) |
| `ρ·fan` FAIL ∧ fals-rate > 0.01657 | 🟡 **RATE-UP / GATE-MISS** — 비율은 올랐으나 8뽑기서 안 나옴 |
| `ρ·fan` FAIL ∧ fals-rate ≤ 0.01657 | 🔴 **NEGATIVE** — 이 용량에서 밀도+register 는 레버가 아니다 |
| fals-rate < BASE − 2sd | ⚠️ **ANTI-LEARN** — 그 팔 INVALID, 별도 조사 |
| 선결 1~4 중 하나라도 실패 | ⛔ **INVALID** (손상) — 과학 판정 없음 |

**추가 스윕 금지**: 이 표 밖 하이퍼 불변. dose 조정은 🔴/🟡 칸에서만, 별도 H 로.

## 코퍼스 검증 (발사 전)

| 팔 | 줄 | 반증가능 | 담체 | held-out 누수 |
|---|---|---|---|---|
| real | 600 | **600 (1.0000)** | 12 | 0 |
| ablation | 600 | **0 (0.0000)** | 12 | 0 |

⚠️ **정직 고지**: 이 드릴 문장들은 **형식 드릴**이지 사실 드릴이 아니다 —
`humidity causes a quantity of recollection that is higher than …` 는 의미상 참이 아니다.
이 개입이 가르치는 것은 **반증가능한 주장의 형태**이며, 내용의 진리성은 이 실험의 대상이 아니다.
그 한계는 판정에 그대로 남는다(**형식 통과 ≠ 좋은 과학적 주장**).

## 재생성 커맨드

```
anima-py corpus falsidrill --out fd_real.txt --n-blocks 24000 --seed 7
anima-py corpus falsidrill --out fd_abl.txt  --n-blocks 24000 --seed 7 --falsi-ablate
anima-py train --init py303_full.clm --d 3784 --L 4 --e0 3 --emax 3 --no-mitosis \
  --corpus mix_real.txt --steps 2000 --ckpt-every 500 --seq-len 512 --batch-size 4 --seed 7
anima-py evaluate <ckpt> --rho-axon --rho-axes fan,form --rho-no-cells --fan-draws 250
```

## Cross-links

[[H_9837]] 두 병목을 실측한 팔 · [[H_9855]] 담체 교정 · [[H_9828]] 밀도 목표수치 · [[H_9829]] 연속 rate


---

## 실측 (cycle2 · CPT 2000 step · val_CE → 1.776 DESCENT · 혼합 밀도 0.197)

```
HILLOCK LIVE    rep 0.00 · distinct2 1.00
CARRY   ρ·form  FAIL  val=1.0 Δ=0.6 · self-shuffle=0.4      ← 선결 ②③ 실패
BRANCH  ρ·fan   FAIL  val=8 Δ=7 · greedy 1 · falsifiable=0 · fals-rate=7/238=0.02941±0.0110
```

### ⛔ 판정: INVALID — 선결 ②(`ρ·form` 축 == PASS) · ③(`self-shuffle ≤ 0.05`) 실패

동결 규칙대로 **팔을 읽지 않는다.** [[H_9837]] 때와 다른 점은, 이번 선결은 **올바르게 명세돼 있었고
그래서 손상을 실제로 잡았다** — 위조 게이트가 아니라 작동하는 게이트다.

### 🔑 담체 다양화는 손상을 못 고쳤다 = 원인 오진이 판명됐다

[[H_9855]]는 `self-shuffle 0.0 → 0.4` 의 원인을 **담체 경직**(3종 반복)으로 지목했다.
담체를 **3 → 12** 로 늘렸는데 **`self-shuffle` 은 0.4 그대로**다. ⟹ **담체는 원인이 아니다.**

남은 후보(다음 H 가 가를 것):
- **드릴 어휘가 좁다** — 명사 약 40개(`rate·size·level·mood`…)가 24,000줄에 반복된다.
  출력이 짧고 흔한 사전 단어로 쏠리면, 바이트를 섞어도 **다른 사전 단어**가 될 확률이 커진다
  (사전 235,976 단어). 담체보다 이쪽이 `self-shuffle` 을 직접 올린다.
- 드릴 비중 15% · 2000 step (용량 축)

### 표 밖 관측 — 비율은 단조 상승했고 동결 하한을 넘었다

| 팔 | fals-rate | lift | 8뽑기 통과확률 |
|---|---|---|---|
| BASE | 1/241 = 0.00415 | 1.00× | 0.033 |
| cycle1 (담체 3 · 물리량만) | 4/247 = 0.01619 | 3.90× | 0.122 |
| **cycle2 (담체 12 · 2-register)** | **7/238 = 0.02941** | **7.09×** | 0.212 |

- **동결 하한 `BASE+3sd = 0.01657` 을 cycle2 가 처음 넘었다**(0.02941). 선결 실패로 판정에는
  못 쓰지만, **register 확장이 전이를 실제로 올렸다**는 방향증거다(cycle1 3.90× → cycle2 7.09×).
- **게이트가 안 열린 이유는 능력이 아니라 추첨**이다: 0.0294 에서 8뽑기 중 1건 이상일 확률은
  **0.212** ⟹ **79% 확률로 놓친다**. cycle1 의 통과(0.122 확률)와 cycle2 의 미통과는
  **같은 비율 궤적 위의 서로 다른 뽑기 결과**이지 역행이 아니다.
- ⟹ 이 게이트로 "통과" 를 얻는 것은 비율을 **0.083** 까지 올리거나(현재의 2.8배 더),
  게이트 자체를 다-뽑기 연속 판정으로 바꾸는 것([[H_9829]]) 둘 중 하나다.

### 다음

1. **손상 원인을 어휘 폭으로 가르기** — 드릴 명사를 40 → 수백으로 늘리고 `self-shuffle` 재측정.
   담체(기각됨)·어휘·용량 3후보 중 하나를 남긴다.
2. 그 뒤에야 dose(비중·step) 를 움직인다 — 손상 게이트를 통과한 뒤에만 유효(`corpus-py-1` (D)).


---

## cycle3 실측 (어휘 400 채굴 · [[H_9857]] · CPT 2000 step · val_CE → 1.647 DESCENT)

```
HILLOCK LIVE    rep 0.00 · distinct2 1.00
CARRY   ρ·form  FAIL  val=0.8 Δ=0.6 · self-shuffle=0.2     ← 선결 ②③ 여전히 실패
BRANCH  ρ·fan   PASS  val=8 Δ=7 · greedy 1 · falsifiable=1 · fals-rate=26/243=0.1070±0.0198
```

### ⛔ 판정: 여전히 INVALID (선결 ②③) — 그러나 두 축 모두 크게 움직였다

| 팔 | fals-rate | lift | 8뽑기 통과확률 | `ρ·form` val | self-shuffle |
|---|---|---|---|---|---|
| BASE | 0.0041 | 1.0× | 0.033 | 1.0 (PASS) | 0.0 |
| cycle1 (담체3·물리량) | 0.0162 | 3.9× | 0.122 | 1.0 | 0.4 |
| cycle2 (담체12·2-register) | 0.0294 | 7.1× | 0.212 | 1.0 | 0.4 |
| **cycle3 (어휘400)** | **0.1070** | **25.8×** | **0.596** | **0.8** | **0.2** |

**① 밀도 레버는 작동한다 — 게이트선을 넘었다.** 게이트를 절반 이상 통과하려면 `p ≥ 0.083` 인데
cycle3 은 **0.1070 (1.29배)**. 8뽑기 통과확률이 **0.596** 으로 **추첨이 아니라 과반**이 됐고,
`ρ·fan` 이 실제로 **PASS**(`falsifiable=1`)했다. [[H_9828]] 이 산술로 예측한 목표를 실측이 따라잡았다.

**② 어휘가 손상의 부분 원인임이 확증됐다.** `self-shuffle 0.4 → 0.2` — 정확히 **절반**.
[[H_9855]] 의 담체 가설은 [[H_9856]] cycle2 가 반증했고(3→12 인데 0.4 불변), [[H_9857]] 의 어휘
가설은 **부분 확증**(400 으로 늘리자 절반으로 감소). 다만 바 `0.05` 에는 여전히 미달.

**③ 그리고 이번엔 val 도 떨어졌다** — `ρ·form val 1.0 → 0.8`. 앞선 두 사이클은 val 이 미동이라
통제만 죽었는데, 이번엔 형식-정확도 자체가 내려갔다. 손상이 다른 지층으로 번진 것이므로
선결을 완화할 이유가 **더 줄었다**.

⟹ 동결 규칙대로 **팔을 읽지 않는다.** 게이트 통과를 G6 돌파로 박제하면 그것이 곧
tune-to-green 이다 — 선결은 정확히 이런 경우를 위해 있다.

### 남은 후보 = 용량 (dose)

담체 ✗ 반증 · 어휘 △ 부분확증(절반) ⟹ 남은 것은 **드릴 비중 15% · 2000 step**.
그리고 이제 그 조정에는 **여유가 생겼다**: 발화율이 게이트선의 1.29배이므로,
비중/스텝을 낮춰 손상을 줄이면서도 `0.083` 위에 머무를 여지가 있다.
다음 H 는 그 dose 축을 사전등록해 `self-shuffle ≤ 0.05` 와 `fals-rate ≥ 0.083` 을
**동시에** 만족하는 지점을 찾는다.


---

## 🚨 verdict-integrity 강등 (2026-07-21 · 자가적발)

이 카드의 **CPT 학습 팔 전부가 금지된 toolchain 에서 돌았다.**

`pyproject.toml` 의 `[train]` extra 는 `torch>=2.1,<2.13` 이고 그 상한에 이유가 박혀 있다 —
**torch 2.13+cu130 은 Blackwell sm_120 학습에서 일반화를 조용히 퇴행시킨다**
(H_9734: 같은 코드·코퍼스·base·seed·recipe 인데 2.11.0 은 addr **0.95**, 2.13.0 은 **0.0078**).

그런데 나는 원격 호스트에 `pip install "torch"` 를 **맨손으로** 쳐서 extra 를 우회했고,
딸려온 것이 **2.13.0+cu130**, 호스트는 **RTX 5070 = Blackwell sm_120** — H_9734 가 퇴행을 측정한
**바로 그 조합**이다.

⟹ 이 카드의 학습 팔 수치는 **verdict-integrity 의심**으로 강등한다.
측정(eval)은 numpy 경로라 무관하지만, **무엇을 측정했는지**(=그 ckpt)가 오염 가능하다.
특히 관측된 `ρ·form` 손상과 그 **비단조성**이 금지 버전의 산물일 가능성을 배제할 수 없다.

교정: 핀 안쪽(`<2.13`) 버전으로 재학습해 대조한다 — 버전이 다르면 재현이 아니라 다른 실험이다.
재발방지는 convergence `bare-pip-install-torch-bypasses-the-load-bearing-pin` 에 등록.


---

## ✅ clean 팔 (torch 2.12.1 · 핀 안쪽) — **선결 4/4 통과 · 동결표 최초 적용**

`#4310` 이 강등한 toolchain 오염을 제거하고 **동일 인자**로 재학습(val_CE → 1.647 DESCENT).

```
step1500   CARRY  ρ·form  PASS  val=1.0 Δ=1.0 · self-shuffle=0.0
           BRANCH ρ·fan   FAIL  val=8 Δ=7 · greedy 1 · falsifiable=0
                                · fals-rate = 7/118 = 0.0593 ± 0.0217
```

| 선결 (H_9856 동결) | 결과 |
|---|---|
| ① `HILLOCK` LIVE | **PASS** (REACH GRADE=CARRY) |
| ② `ρ·form` 축 == PASS | **PASS** ← 세 사이클 만에 처음 |
| ③ `self-shuffle ≤ 0.05` | **0.0** PASS |
| ④ `dist ≥ 5` | **8** PASS |

⟹ **4/4 통과 = 동결표를 적용할 자격이 처음으로 성립.**

### 🟡 판정: RATE-UP / GATE-MISS

동결표의 해당 칸: *`ρ·fan` FAIL ∧ fals-rate > 0.01657* →

- **fals-rate 0.0593** = BASE(0.00415) 대비 **14.3배**, 동결 하한 `BASE+3sd = 0.01657` **초과**
- 그러나 8뽑기 게이트는 **미발화**(`falsifiable=0`)

**게이트 미발화는 능력이 아니라 추첨이다.** 0.0593 에서 `P(≥1 in 8) = 0.387` ⟹
**61% 확률로 놓친다.** 같은 비율에서 발화/미발화가 갈리는 것은 게이트가 8뽑기 이진이기 때문이며,
[[H_9829]] 가 만든 연속 rate 판독이 정확히 이 구멍을 메우려는 것이다.

### 🔑 toolchain 강등이 옳았음이 실측으로 확인됐다

| | step2000 fals-rate |
|---|---|
| 오염 팔 (torch 2.13+cu130) | **0.1070** |
| clean 팔 (torch 2.12.1) | (측정중) — step1500 은 0.0593 |

같은 코퍼스·같은 인자·같은 seed 인데 **수치가 다르다.** `#4310` 의 강등은 과잉이 아니었고,
버전이 다르면 재현이 아니라 다른 실험이라는 판단이 실측으로 지지된다.

### 남은 것

- clean step2000 판독(진행중) — 오염 팔 0.1070 과 직접 대조
- 2 seed 복제 — 게이트 발화 여부가 추첨이므로 단일 실행으로는 어느 쪽도 주장 불가
- 게이트를 **연속 rate 판정**으로 바꾸는 것([[H_9829]]) — 이진 8뽑기로는
  `p=0.0593` 모델과 `p=0.10` 모델을 실용적으로 구분할 수 없다


---

## 🔻 정정 — 내가 #4314 에서 한 toolchain 비교는 틀렸다

#4314 에서 *"오염 0.1070 vs clean 0.0593 ⟹ toolchain 이 수치를 바꿨다"* 라고 적었다.
**그 둘은 step 이 다르다** — 0.1070 은 step**2000**(오염), 0.0593 은 step**1500**(clean).
사과-오렌지 비교였다.

**같은 step 으로 다시 비교하면:**

| step2000 | fals-rate |
|---|---|
| 오염 (torch 2.13+cu130) | 0.1070 |
| clean (torch 2.12.1) | **0.1034** |

**차이가 미미하다.** ⟹ toolchain 강등은 **절차적으로는 옳았지만**(핀 위반은 위반이다),
**이 지표에 대한 실측 효과는 같은 step 에서 무시할 만하다.** #4314 의 "실측으로 확인됐다" 는
주장을 **철회**한다.

## ✅ 그 대신 확인된 것 — 비단조 손상은 실재하고 재현된다

| toolchain | step1500 | step2000 |
|---|---|---|
| 오염 (2.13) | `self-shuffle 0.0` PASS | `0.2` FAIL |
| clean (2.12.1) | `self-shuffle 0.0` PASS | `0.2` FAIL |

**두 toolchain 에서 패턴이 동일하다.** ⟹ [[H_9859]] 가 관측한 비단조성은
**torch 아티팩트가 아니라 실재하며 재현**된다. (강등된 H_9859 의 관측이 이로써 복권된다.)

## 🔑 이 캠페인의 핵심 긴장 — 게이트는 손상된 지점에서만 열린다

clean 팔 두 rung 에 동결표를 적용한 결과:

| rung | 선결 4/4 | `ρ·fan` | fals-rate | 판정 |
|---|---|---|---|---|
| **step1500** | **PASS** | FAIL (미발화) | 0.0593 | 🟡 **RATE-UP / GATE-MISS** |
| **step2000** | **FAIL** (`self-shuffle 0.2`) | **PASS** (`falsifiable=1`) | **0.1034** | ⛔ **INVALID** |

**게이트가 발화하는 지점은 모델이 이미 손상된 지점이다.**
손상 없는 지점(1500)은 비율이 0.0593 이라 8뽑기서 61% 확률로 놓치고,
비율이 충분한 지점(2000)은 `ρ·form` 이 무너져 판독 자격을 잃는다.

⟹ 현재 개입에서 **"깨끗하게 통과" 하는 지점은 존재하지 않는다.** 이것은 실패가 아니라
**제약의 정확한 형태**이고, 다음 설계가 풀어야 할 문제를 정의한다:
비율을 올리는 것과 형식을 보존하는 것이 **이 용량에서는 함께 가지 않는다.**
