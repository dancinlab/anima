...
## cycle3 실측 (어휘 400 채굴 · [[H_9857]] · CPT 2000 step · val_CE → 1.647 DESCENT)

```
HILLOCK LIVE    rep 0.00 · distinct2 1.00
CARRY   ρ·form  FAIL  val=0.8 Δ=0.6 · self-shuffle=0.2     ← 선결 ②③ 여전히 실패
BRANCH  ρ·fan   PASS  val=8 Δ=7 · greedy 1 · falsifiable=1 · fals-rate=26/243=0.1070±0.0198
```

### ⛔ 판정: 여전히 INVALID (선결 ②③) — 그러나 두 축 모두 크게 움직였다

| 팔 | fals-rate | lift | 8뽑기 통과확률 | `ρ·form` val | self-shuffle |
|---|---|---|---|---|---|
| BASE | 0.0041 | 1.0× | 0.033 | 1.0 (PASS) | 0.0 |
| cycle1 (담체3·물리량) | 0.0162 | 3.9× | 0.122 | 1.0 | 0.4 |
| cycle2 (담체12·2-register) | 0.0294 | 7.1× | 0.212 | 1.0 | 0.4 |
| **cycle3 (어휘400)** | **0.1070** | **25.8×** | **0.596** | **0.8** | **0.2** |

**① 밀도 레버는 작동한다 — 게이트선을 넘었다.** 게이트를 절반 이상 통과하려면 `p ≥ 0.083` 인데
cycle3 은 **0.1070 (1.29배)**. 8뽑기 통과확률이 **0.596** 으로 **추첨이 아니라 과반**이 됐고,
`ρ·fan` 이 실제로 **PASS**(`falsifiable=1`)했다. [[H_9828]] 이 산술로 예측한 목표를 실측이 따라잡았다.

**② 어휘가 손상의 부분 원인임이 확증됐다.** `self-shuffle 0.4 → 0.2` — 정확히 **절반**.
[[H_9855]] 의 담체 가설은 [[H_9856]] cycle2 가 반증했고(3→12 인데 0.4 불변), [[H_9857]] 의 어휘
가설은 **부분 확증**(400 으로 늘리자 절반으로 감소). 다만 바 `0.05` 에는 여전히 미달.

**③ 그리고 이번엔 val 도 떨어졌다** — `ρ·form val 1.0 → 0.8`. 앞선 두 사이클은 val 이 미동이라
통제만 죽었는데, 이번엔 형식-정확도 자체가 내려갔다. 손상이 다른 지층으로 번진 것이므로
선결을 완화할 이유가 **더 줄었다**.

⟹ 동결 규칙대로 **팔을 읽지 않는다.** 게이트 통과를 G6 돌파로 박제하면 그것이 곧
tune-to-green 이다 — 선결은 정확히 이런 경우를 위해 있다.

### 남은 후보 = 용량 (dose)

담체 ✗ 반증 · 어휘 △ 부분확증(절반) ⟹ 남은 것은 **드릴 비중 15% · 2000 step**.
그리고 이제 그 조정에는 **여유가 생겼다**: 발화율이 게이트선의 1.29배이므로,
