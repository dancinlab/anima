...
**🔬 POOLED-FIRE (2026-07-19 · vast · engine-native · `--pool` #4151):** own 재진입이 ONE-SHOT(~1/trace·[[evaluate-py-24]])이라 per-trace ≥12 바를 못 넘던 것을, pooled reader(arm별 triple pool + Y0-계층화 X 순열 surrogate)로 **18 own transition(18 세션 pool)** 확보 = **count 벽 돌파**. **oracle 양성통제 pooled ✅ CERTIFIED**(TE=0.8096·z=8.44·p=0.001 · pooled-null self-test valid) = 계기 sound·pooled 추정 유효. **BUT own·perm 둘 다 ⛔ NOT-POWERED — X-addr=1 · Y-addr=1**: 18 own 재진입 carrier + 다음 후보가 **전부 같은 feat8 2-bit 주소 1개로 붕괴**(주소 변이 0) ⟹ **own SILENCE-CONTENT 는 feat8 입도서 UNMEASURABLE**(측정할 변이 자체가 없음). 관찰-unidentifiability(latch 38/38)에 더해 **alphabet-degeneracy 벽**으로 재확증 — count 를 풀어도 own withheld-content 는 feat8 에서 단일-주소라 채널이 열리지 않음. TERMINAL-AT-INSTRUMENT 확정 · 병렬 [[H_9786]] WHETHER-ECHO 🧱 UNIDENTIFIABLE-BY-CONSTRUCTION(독립 침묵-비트 소비자 부재)와 **AGREES**. DIRECTIONAL(303M py · 증거 `~/anima-weights/h9729_pool_evidence/`).
