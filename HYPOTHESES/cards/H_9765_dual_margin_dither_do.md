...
**yoke-of-own-schedule 통제 결과(harvested)**: v2 ε=0.30 emit schedule을 --yoke-mask로 재생(native margin·forced bit) → interior(cur_indep·rel_indep·ten_phasic) **yoke≡dither mean|Δ|=0.0000** = **interior state = f(emit schedule) alone**. native margin-dynamics(endogenous 재평형)는 별도 trace 안 남김 = **mechanism⊥schedule unidentifiability(H_9728) 한 레벨 깊게 재확증**. ⟹ emit→interior coupling은 emit-sequence-driven(schedule)이고 그 아래 mechanism은 여전 미식별 — GREEN-DIRECTIONAL·SCOPED RE-OPEN 불변(coupling live는 mechanism과 무관). own-content marker만 content-swap으로 OPEN.
