...
## 🔎 R3 — 내가 붙인 "천장은 낙관적 상한" 단서를 공격했더니, **단서가 틀렸다**

R2 에서 나는 이렇게 적었다: *"in-vivo 슬롯은 동시출현 블록 엔티티라 균등 추출 8-way 보다 더
헷갈릴 수 있어 천장은 상한"*. 이 문장은 `charge_store`(창의 바이트에서 known 단어를 순서대로 8개
뽑는 지각-충전 · H_9696)를 전제했다. **코드를 읽으니 전제가 성립하지 않는다.**

### ⛔ 단서 철회 — 배선된 store 는 균등 추출이다

| 확인 | 결과 |
|---|---|
| `charge_store` 호출부 | **0개** (repo 전수 · 정의만 존재 = 미배선 설계) |
| 배선된 lane 의 store 출처 | 매니페스트 (`evaluate --store` → `it["store"]`) |
| 매니페스트 슬롯 추출 법칙 | `cli/corpus.py:1828 _sb_emit_block` → **`rng.sample(range(len(entities)), store_slots)`** = **비복원 균등 추출** |
| held-out 평가 블록 | `_sb_emit_block(ev_rng, ev, store_slots)` — 같은 법칙, held-out 한정 |

⟹ probe 의 방해자 법칙(테스트 절반에서 비복원 균등 추출)은 **프로덕션 평가 매니페스트와 일치**한다.
동시출현 걱정은 **미배선 경로에만** 해당하며, 그 경로가 배선되면 그때 다시 물어야 한다.

**부수 확증**: 내가 동결한 프롬프트 템플릿 `"is {e} => "` 가 생성기의
`"%s %s => " % (op_s, entity)` 와 **바이트 단위로 동일**하다. 독립 출처에서 온 템플릿이 맞았다.

### 📏 최악 경계 — 공짜로 얻어지는 하한 (ADVERSARIAL)

각 정답에게 **점수가 가장 높은 방해자 7개**를 붙이면(어떤 블록 생성기도 이보다 나쁘게 만들 수 없다)
정답이 이기려면 모든 경쟁자를 이겨야 하므로 **전체 순위 1등과 정확히 동치**다 —
따라서 추가 적합 없이 full-pool top-1 이 그대로 최악 경계다 (수치로 항등 확인).

```
             적대(최악 블록)  ≤   균등(in-vivo 충실)      FLOOR=0.25
  stage 1        0.2031      ≤       0.6496
  stage 2        0.2656      ≤       0.6774
```

### 읽는 법

- **판정은 균등 팔이 맞다** — 그것이 배선된 매니페스트가 실제로 뽑는 방식이다. R2 의 REACHABLE 유지.
- 최악 블록에서는 stage1 0.2031 이 FLOOR 0.25 **아래**, stage2 0.2656 이 **간신히 위**다 ⟹
  적대적으로 고른 블록에서는 이 lane 의 주소지정이 **바닥에 붙는다**. 여유가 없다.
- 즉 R2 의 0.6774 는 낙관 편향이 아니라 **설계 일치**이고, 취약성은 다른 데 있다 —
  방해자가 우연이 아니라 **적대적으로** 배치되는 상황.
