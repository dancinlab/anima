...
GRU 스크린 전에 "big_phi 가 통합을 읽나, 크기/분산을 읽나"를 $0 손제작 TPM 으로 먼저 물었고 **통과**했다:
크기·활동·주변분포를 맞춘 non-integrated 계(SHUFFLE 2-cycle+copy · SINK)가 cross-edge 가 있는데도
**정확히 Φ=0**, 통합계만 Φ>0(ROT 3.0 · XOR 2.25 · 분리폭 2.25 · 노이즈 다리 통과). ⟹ 이 카드의 GRU
4-arm 스크린이 이제 의미를 가진다. edge-cut 레시피도 확정(H_9959): j→i 절단=소스비트 인과 주변화,
ROT c2→c0 절단 시 collapse-Δ=2.0. (단 학습된 순환이 실제 통합을 만드는지는 여전히 GRU 스크린이 결정.)
