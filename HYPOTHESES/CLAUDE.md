...
3. **frozen-first · tune-to-green 금지 (p7·c9).** bar 는 측정 전 사전등록(`archive/state/verdicts/<slug>/*FREEZE*`), 사후이동 금지. FALSIFIED/negative/DISPUTED 도 결과(은폐 금지). LLM 자가판정 금지 — 캡처된 출력이 증거.
...
## 등록 흐름
research → `hexa verify` → `archive/state/verdicts/<slug>/<id>.txt`(verbatim) → `cards/H_<id>.md` + `HYPOTHESES.jsonl` 1줄. tier 무관(🟢·🧱·🔴/🟠·🔵 전부 남김 — 벽도). 다음 H 연속 제안(`a_h_continuous_no_branch`). 교훈/재발학습은 ARCHITECTURE.json `convergence.records[]`(root-cause SSOT).
