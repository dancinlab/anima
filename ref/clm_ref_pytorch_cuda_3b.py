#!/usr/bin/env python
"""Lane-G-ref PyTorch+CUDA 3B-scale REFERENCE trainer (anima CLM+KOSMOS domain).

substrate = PyTorch-CUDA   lane = Lane-G-ref   rung = 3B reference

THIS IS A BOUNDED-BUDGET 3B-SCALE REFERENCE RUNG, NOT A CONVERGED PRODUCTION
...
at ~3B params on a bounded N steps, that (a) the same ByteGPT/Transformer arch
trains (CE descends) and (b) it saturates the GPU (util >> 20%) at 3B scale —
a throughput-justified 3B REFERENCE. It is an a_completeness_over_cheap optional
...
Scale honesty (a_scale_honest_scope): the 3B rung of the ref ladder (85M -> 3B).
We do NOT claim convergence — bounded N steps, descent + util demonstrated only.

Scales the same byte-level (V=256) decoder-only GPT to ~3B params, trained with
AMP/bf16 + gradient checkpointing on the same 5-lang c4 backbone corpus the
85.6M PUBLIC ref used (dancinlab/clm-backbone-5lang-sample).
"""
...
class ByteGPT(nn.Module):
    def __init__(self, vocab=256, d=2560, n_layer=40, n_head=20, block=512, p_drop=0.0,
                 grad_ckpt=True):
        super().__init__()
        self.block = block
...
def main():
    ...
    ap.add_argument("--n_layer", type=int, default=40)
    ...
    ap.add_argument("--out", default="/root/laneg_ref_3b/clm_ref_pytorch_cuda_3b.pt")
    ap.add_argument("--log", default="/root/laneg_ref_3b/clm_ref_3b_train.log.json")
    args = ap.parse_args()
    ...
    result = {
        ...
        "rung": "3B-scale reference",
        "note": ("3B-scale reference rung, bounded N steps, descent + util demonstrated, "
        ...
        "descent": {"first_val_ce": first_ce, "last_val_ce": last_ce,
                    "F_CLM_REF_3B_DESCENT": 1 if descent else 0, "verdict": "PASS" if descent else "FAIL"},
        "util": usum,
