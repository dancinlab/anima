<!--
COMPENDIUM 출처: anima 과거 commit `c7d0548f7` (parent of `f8fc39aff` migration deletion)
원본 commit: `63ca36abe` (2026-05-13) — 539-line 초기 compendium
누적 부록 commits (§10/§11/§12 통합본):
  - c05c397bd §12 봉쇄심화 부록
  - 1221ac546 §12.5 path 1 base-rate audit
  - ab97b3066 §10.1 F-PERSONA-4 §44→§52 silent-drop ledger
  - 0a6077c67 §12.5 paths 3+4 LANDED
  - 2f2f98404 §10.1+§11 v6 cell-parallel FAIL
  - 7386c8a96 §10.1 closure ii v6 F-V5MIT-4 FAIL root cause
  - fa902716a anima 4 방향 design tier
  - c7d0548f7 anima-agent absorb (현재 = 783 lines 누적 cumulative)

restoration commit: PR #85 (2026-05-16) — user 게이트 통과 후 복구.
HEXAD/SAVANT/README.md `누락 priority ★★★ critical` 직접 해결.

직접 편집 금지 — historical evidence anchor. 후속 update 가 필요하면
HEXAD/SAVANT/SAVANT.tape (current architecture SSOT) 에서 진행.
-->

# SAVANT.md — Golden Zone & Savant 전수조사 compendium

> History → [./COMPENDIUM.log.md](./COMPENDIUM.log.md).
...
> §10.1 F-PERSONA-4 KL>0 saga (§44 → §52 silent-drop 차단 ledger) → [./COMPENDIUM.log.md](./COMPENDIUM.log.md).
...
> §12.5 봉쇄심화 후속 path (LANDED ledger) → [./COMPENDIUM.log.md](./COMPENDIUM.log.md).
