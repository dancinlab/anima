...
- **2026-05-19** — Folder renamed `HEXAD/LOIHI` → `HEXAD/NEUROMORPHIC`
  (English, broader substrate scope). Canonical path is now
  `HEXAD/NEUROMORPHIC/`; a thin `HEXAD/LOIHI/README.md` forwarding stub
  is retained so previously-shared links still resolve. PII artifacts
  moved local-only (still gitignored under the new path). The earlier
  Log line above (`LOIHI.md` → `HEXAD/LOIHI/README.md`) is left intact
  as accurate-for-its-date history (g6 append-only / g3 drift-avoidance).
