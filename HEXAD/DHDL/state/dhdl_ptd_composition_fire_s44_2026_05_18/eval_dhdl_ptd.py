#!/usr/bin/env python3
"""eval_dhdl_ptd.py — RESEARCH.md §44 DH-DL + PTD-aux composition eval.

Reuses §27 evaluator pattern. Measures (DESIGN_S38.md §5):
  (a) 3-class accuracy + confusion matrix on held-out traces.
  (b) THRESHOLD-DISTILLATION GAP — same metric as §27 (§27 gap=0.00063).
  (c) Per-class next-physics-state MSE (CONTINUE_THINK-corner localization).
  (d) Representation-shaping probe: gap(λ=0) vs gap(λ=0.3) when eval'd on
      both heads — emitted by a higher-level driver script (eval_compare.py).

$0 Mac CPU.
Output: eval_result_lam{N}.json
"""
...
IM_THRESHOLD = 0.3  # §24 byte-exact (spontaneous_lib.hexa SSOT)
...
def _build_next_record_map(recs):
    """Build (trace_idx, step) -> idx within `recs` list for PTD pairing.
    Mirror trainer logic but on `recs`'s own indexing."""
    keymap = {}
...
def evaluate(corpus_path: Path, head_path: Path, out_path: Path) -> dict:
    ...
    for line in corpus_path.read_text(encoding="utf-8").splitlines():
        ...
        recs.append(r)
    # we need the FULL corpus for PTD pairing (next-record may be in train too)
    next_idx, has_next = _build_next_record_map(recs)

    holdout = _stratified_split(recs, 0.2)

    # accuracy + confusion + threshold gap
    confusion = [[0, 0, 0] for _ in range(3)]
    correct = 0
    gap_count = 0
    gap_records = []
    class_total = [0, 0, 0]
    # PTD aux per-class MSE accumulators
    ptd_mse_per_class = {0: [0.0, 0], 1: [0.0, 0], 2: [0.0, 0]}  # [sum, count]
    ptd_mse_all = [0.0, 0]

    # build full-corpus standardised features for PTD pairs
    def _norm_x(r):
        return _normalize(r["_x"], mean, std)

    for ridx, r in enumerate(holdout):
        x_norm = _norm_x(r)
        y = r["_y"]
        safety_ok = r["_safety"]
    ...
    # PTD MSE on the FULL corpus's held-out pairs (recs idx -> next_idx[i])
    # We use the holdout subset; PTD target is the next record's normalised features.
    # First build idx-in-recs for holdout to map
    ...
    out = {
        ...
        "honest_verdict": (
            f"§44 composition eval @ lambda_ptd={lambda_ptd}. Gap "
            f"{gap:.5f} vs §27 0.00063. PTD aux MSE mean {ptd_mse_mean}. "
            "Per-class MSE localises which corner the next-physics-state is "
            "hardest to predict. PTD aux is REGULARISER not new supervision — "
            "DISTILLATION still the verdict (DESIGN_S38.md §7)."
        ),
    }
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="§44 DH-DL + PTD eval")
