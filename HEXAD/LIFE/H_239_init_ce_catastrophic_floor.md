...
# H_239 — init_CE Catastrophic Floor (substrate 초기화 불일치)
...
- **H239.1 (above-uniform)**: 6 axes 전부 init_CE > ln(151936)=11.93 — 어느 축도 균등 floor 아래로 시작하지 않음.
- **H239.2 (catastrophic gap)**: mean init_CE − ln(V) ∈ [+2.3, +2.9] nats — 약 +2.5 nats 의 일관된 초과.
- **H239.3 (range bound)**: 측정된 init_CE ∈ [14.18, 14.79] (모든 6-axis), 폭 ≤ 0.7 nats — 축 간 분산이 gap 자체보다 작음.
- **H239.4 (mis-calibration sign)**: init_CE − ln(V) > 0 의 부호가 양 — net 이 정답에 균등보다 *낮은* 확률을 부여 (자신감 방향이 틀림).
- **H239.5 (noise_sigma sensitivity)**: layer-0 noise_sigma injection (audit PR #255) 이 init_CE 를 ln(V) 쪽으로 끌어내림 — mismatch 가 초기화 분산에 인과적으로 의존.
...
- **C1 (above-uniform)**: H239.1 6/6 axes init_CE > ln(V)
- **C2 (catastrophic gap)**: H239.2 mean gap ∈ [+2.3, +2.9]
- **C3 (range)**: H239.3 init_CE 폭 ≤ 0.7 nats
- **C4 (baseline closed-form)**: ln(151936) = 11.931… `hexa verify` 🔵 SUPPORTED-FORMAL
- **C5 (noise causal)**: H239.5 noise_sigma↑ → gap↓ (audit PR #255 방향성)
...
- **sister H (substrate/life)**: H_132 (frozen-cells — init = 분열 이전 동결 상태의 부담), H_018 (genesis — self-reference 없는 진공 출발 vs init mismatch 출발), H_157 (Law 76 panpsychism — init_CE 가 Ψ=1/2 fixed point 에서 얼마나 먼지), H_240 (substrate autonomy emit ratio — *학습 후* substrate 의 자율 vs *학습 전* init 부담), H_241 (cluster init_CE byte-equal — 본 가설의 6-axis 측정을 3-cluster 로 분해).
...
**State output**: (design + 흡수 cycle — 자력 fire 시 `HEXAD/LIFE/state/h239_init_ce_floor_2026_05_24/{run_baseline.hexa, result.json}`)
