...
def main():
    ...
    if all(data.get(c) for c in CELLS) and all(eval3.get(c) for c in CELLS):
        ...
        lines.append(f"1. **All 5 ckpts loaded + ran 4 evals on CPU bf16** (mmap+meta+assign zero-copy stack), no quantization, RoPE base 50000 patched per training spec.")
        lines.append(f"2. **Eval 1 verbalization**: at L_ce ~3.85 (200-step trainer) all 5 ckpts emit mostly byte-level pseudo-text — coherent language has NOT emerged in this attempt10 grid (training scope was 2000 steps not 8000).")
        lines.append(f"3. **Eval 2 leak rate** across 25 probes/cell: A={all_leaks['vA']}/25, A_s42={all_leaks['vA_s42']}/25, B_s42={all_leaks['vB_s42']}/25, C={all_leaks['vC']}/25, D_s42={all_leaks['vD_s42']}/25. λψ↑ pattern: see Eval 2 cross-cell table.")
        lines.append(f"4. **Eval 3 mitosis splits** (40-step decode on '안녕? 너는 누구야?'): A={n_splits['vA']} A_s42={n_splits['vA_s42']} B_s42={n_splits['vB_s42']} C={n_splits['vC']} D_s42={n_splits['vD_s42']}. Adaptive split threshold tracked per-layer tensions; all cells saturated split events (substrate tension > moving-window threshold persistently).")
        lines.append(f"5. **Honest C3**: 200-step convergence floor means cross-cell text-level signal is weak; deltas mostly seed-noise. Hexa-native `mitosis_hook.hexa` selftest evidence + this Python port × real 8.92B substrate = D4 live evidence extended from synthetic-d8 to real-d3072.")
    else:
