...
P21H_HEAD_G_WEIGHT=${P21H_HEAD_G_WEIGHT:-0.1}
...
for GPU in "${GPU_CASCADE[@]}"; do
  ...
  for CLOUD in SECURE COMMUNITY; do
    Q=$(cat <<JSON
{"query":"mutation { podFindAndDeployOnDemand(input:{cloudType: $CLOUD, gpuCount:1, volumeInGb:0, containerDiskInGb:120, minVcpuCount:8, minMemoryInGb:64, gpuTypeId:\"$GPU\", name:\"p21h-v3-$INIT_VARIANT\", imageName:\"$IMAGE\", dockerArgs:\"\", ports:\"22/tcp\", volumeMountPath:\"/workspace\", env:[{key:\"PUBLIC_KEY\", value:\"$PUBKEY\"}]}) { id machineId } }"}
JSON
)
    R=$(gql "$Q"); echo "[create] cloud=$CLOUD gpu=$GPU resp: $(echo "$R" | head -c 250)"
...
CMD="bash $P21HR/launch_trainer_p21h.sh $P21HR/train_p21h_v3.py \
  ...
  --head-g-weight $P21H_HEAD_G_WEIGHT \
