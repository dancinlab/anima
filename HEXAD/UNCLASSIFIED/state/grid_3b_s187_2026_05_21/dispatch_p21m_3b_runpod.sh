#!/usr/bin/env bash
# P21M-3B dispatch — fresh r32 LoRA on Qwen2.5-3B-Instruct + 5-lang wiki + anima mix.
#
# Forked from dispatch_p21m_runpod.sh. Differences:
#   - Base = Qwen/Qwen2.5-3B-Instruct (instruction-tuned 3B)
#   - No vP21 adapter init — fresh LoRA via get_peft_model() in train script
#   - bsz=1 (3B + LoRA peaks ~30GB activations on H100 80GB at block=512)
#   - 1500 steps (~50 min H100 SXM estimate)
#   - Cost cap $20 ($15 train + $5 buffer)
set -uo pipefail
...
WATCHDOG_SEC=${WATCHDOG_SEC:-6000}   # 100 min cap (3B = ~3× wall vs 1.5B)
SSH_TRIES=60
...
P21M_BSZ=${P21M_BSZ:-1}                # 3B → bsz=1 default
...
exec > >(tee -a "$LOG") 2>&1
echo "=== P21M-3B dispatch start $(date -u +%FT%TZ) ==="
echo "    base=$BASE_MODEL (FRESH LoRA — no vP21 init)"
...
echo "    cost_cap=\$20 (\$15 train + \$5 buffer)"
...
teardown() {
  ...
  echo "=== P21M-3B dispatch end $(date -u +%FT%TZ) rc=$rc ==="
}
trap teardown EXIT
...
for GPU in "${GPU_CASCADE[@]}"; do
  ...
  Q=$(cat <<JSON
{"query":"mutation { podFindAndDeployOnDemand(input:{cloudType: ALL, gpuCount:1, volumeInGb:0, containerDiskInGb:120, minVcpuCount:8, minMemoryInGb:64, gpuTypeId:\"$GPU\", name:\"p21m-3b-multilingual\", imageName:\"$IMAGE\", dockerArgs:\"\", ports:\"22/tcp\", volumeMountPath:\"/workspace\", env:[{key:\"PUBLIC_KEY\", value:\"$PUBKEY\"}]}) { id machineId } }"}
JSON
)
...
$SSH "mkdir -p $P21MR/$BUILD_REL $P21MR/$S16_REL $P21MR/$ANCHOR_REL $P21MR/out_main $P21MR/wiki_parts"
...
echo "[3b] fresh LoRA path — skipping vP21 adapter upload"
...
# 3B fresh-LoRA path: pass empty string for --vp21-adapter-dir → train script does get_peft_model()
...
for i in $(seq 1 $MAX_ITERS); do
  ...
  if [ $((i % 2)) -eq 0 ]; then
    $SSH "tail -2 $P21MR/train.log" 2>/dev/null | sed "s/^/[train-P21M-3B] /"
  fi
  sleep 60
done
...
echo "=== P21M-3B pipeline complete $(date -u +%FT%TZ) ==="
