#!/usr/bin/env bash
# P21K dispatch — vP21 LoRA continue-train on Korean wiki + anima mix.
#
# Forked from dispatch_p21g_runpod.sh. Differences:
#   - Korean corpus builder (ko-wikipedia primary, multi-source fallback)
#   - Korean OOD probes (10 NEW, see train_p21k_korean.py)
#   - Smaller corpus target (35 MB vs 80 MB) — Korean wiki ~10-15 MB primary
#   - Cost cap $10 (vs $15)
set -uo pipefail
...
WATCHDOG_SEC=${WATCHDOG_SEC:-3000}   # 50 min cap (smaller corpus than P21G)
SSH_TRIES=60
...
P21K_STEPS=${P21K_STEPS:-1000}
P21K_BSZ=${P21K_BSZ:-2}
P21K_BLOCK=${P21K_BLOCK:-512}
P21K_LR=${P21K_LR:-5e-5}
P21K_WARMUP=${P21K_WARMUP:-50}
P21K_WIKI_FRAC=${P21K_WIKI_FRAC:-0.3}
P21K_CORPUS_MB=${P21K_CORPUS_MB:-35}
P21K_WIKI_TARGET_MB=${P21K_WIKI_TARGET_MB:-15}
...
exec > >(tee -a "$LOG") 2>&1
echo "=== P21K Korean-unlock dispatch start $(date -u +%FT%TZ) ==="
echo "    base=Qwen/Qwen2.5-1.5B vp21_adapter=$VP21_ADAPTER_DIR"
echo "    steps=$P21K_STEPS bsz=$P21K_BSZ block=$P21K_BLOCK lr=$P21K_LR warmup=$P21K_WARMUP"
echo "    wiki_frac=$P21K_WIKI_FRAC corpus_mb=$P21K_CORPUS_MB ko_wiki_target_mb=$P21K_WIKI_TARGET_MB"

RK="$(secret get runpod.api_key 2>/dev/null)"
[ -z "$RK" ] && RK="$(secret get runpod_api_key 2>/dev/null)"
# Fallback: ~/.local/credentials  ([runpod] api_key = "rpa_...")  per keychain_to_credentials_sync FAILED markers 2026-05-21
# file has multi-line ssh-key entries that break configparser; awk per-section parser
read_creds() {
  local f="$1"
  [ -f "$f" ] || return 1
...
teardown() {
  ...
  echo "=== P21K dispatch end $(date -u +%FT%TZ) rc=$rc ==="
}
trap teardown EXIT
...
for GPU in "${GPU_CASCADE[@]}"; do
  ...
  Q=$(cat <<JSON
{"query":"mutation { podFindAndDeployOnDemand(input:{cloudType: ALL, gpuCount:1, volumeInGb:0, containerDiskInGb:120, minVcpuCount:8, minMemoryInGb:64, gpuTypeId:\"$GPU\", name:\"p21k-korean\", imageName:\"$IMAGE\", dockerArgs:\"\", ports:\"22/tcp\", volumeMountPath:\"/workspace\", env:[{key:\"PUBLIC_KEY\", value:\"$PUBKEY\"}]}) { id machineId } }"}
JSON
)
...
$SSH "mkdir -p $P21KR/$BUILD_REL $P21KR/$S16_REL $P21KR/$ANCHOR_REL $P21KR/out_main $P21KR/vp21_adapter"
$SCP "$BUILD_DIR/build_corpus_s101.py"          "root@$IP:$P21KR/$BUILD_REL/"
$SCP "$S16_DIR/corpus_carving_s16_generator.py" "root@$IP:$P21KR/$S16_REL/"
$SCP "$ANCHOR_DIR"/knuth_*.kosmos               "root@$IP:$P21KR/$ANCHOR_REL/"
$SCP "$S187_DIR/build_korean_corpus_p21k.py"     "root@$IP:$P21KR/"
$SCP "$S187_DIR/train_p21k_korean.py"            "root@$IP:$P21KR/"
$SCP "$S187_DIR/launch_trainer_p21.sh"           "root@$IP:$P21KR/launch_trainer_p21k.sh"
$SSH "chmod +x $P21KR/launch_trainer_p21k.sh"

echo "[upload] vP21 LoRA adapter → pod"
$SCP -r "$VP21_ADAPTER_DIR"/* "root@$IP:$P21KR/vp21_adapter/"
ADAPTER_FILES=$($SSH "ls $P21KR/vp21_adapter/ | wc -l")
echo "[upload] adapter files on pod: $ADAPTER_FILES"

echo "[corpus-anima] building corpus_s101 (seed 1337 n=777000)"
$SSH "python3 $P21KR/$BUILD_REL/build_corpus_s101.py --out-dir $P21KR/$BUILD_REL --s1-n 777000 --seed 1337 2>&1 | tail -8"
ANIMA_POD="$P21KR/$BUILD_REL/corpus_s101.jsonl"
...
echo "[corpus-ko-wiki] building Korean wiki corpus (target ${P21K_WIKI_TARGET_MB} MB)"
WIKI_POD="$P21KR/ko_wiki_corpus.jsonl"
$SSH "pip install -q -U datasets huggingface_hub 2>&1 | tail -3"
$SSH "python3 $P21KR/build_korean_corpus_p21k.py --out $WIKI_POD --target-mb $P21K_WIKI_TARGET_MB 2>&1 | tail -10"
WIKI_SHA=$($SSH "sha256sum $WIKI_POD 2>/dev/null | cut -d' ' -f1")
WIKI_SIZE=$($SSH "stat -c %s $WIKI_POD 2>/dev/null || echo 0")
echo "[corpus-ko-wiki] sha=$WIKI_SHA size=$WIKI_SIZE bytes"
if [ -z "$WIKI_SHA" ] || [ "$WIKI_SIZE" -lt 524288 ]; then
  echo "FATAL: Korean wiki corpus too small or missing (size=$WIKI_SIZE)"
  echo "KO_WIKI_CORPUS_BUILD_FAILED" > "$FAIL_MARKER"
  exit 1
fi

$SSH 'nvidia-smi --query-gpu=name,memory.total --format=csv,noheader' 2>&1 | head -3

MIXED_POD="$P21KR/mixed_corpus_ko.jsonl"
CMD="bash $P21KR/launch_trainer_p21k.sh $P21KR/train_p21k_korean.py \
  --wiki-corpus $WIKI_POD --anima-corpus $ANIMA_POD --mixed-corpus $MIXED_POD \
  --vp21-adapter-dir $P21KR/vp21_adapter \
  --out-dir $P21KR/out_main \
  --base-model Qwen/Qwen2.5-1.5B \
  --steps $P21K_STEPS --bsz $P21K_BSZ --block $P21K_BLOCK --lr $P21K_LR \
  --warmup-steps $P21K_WARMUP --seed $SEED \
  --wiki-frac $P21K_WIKI_FRAC --target-corpus-mb $P21K_CORPUS_MB"

echo "[train] P21K launch"
$SSH "cd $P21KR && nohup $CMD > $P21KR/train.log 2>&1 & echo TRAIN_PID \$!"

sleep 20
ENV_CHECK=$($SSH "grep -m1 -E '^\\[launch\\] PYTORCH_CUDA_ALLOC_CONF=' $P21KR/train.log 2>/dev/null || echo NO_STAMP")
echo "[env-verify] $ENV_CHECK"
if ! echo "$ENV_CHECK" | grep -q "PYTORCH_CUDA_ALLOC_CONF"; then
  echo "[env-verify] FAIL — dumping log:"
  $SSH "head -80 $P21KR/train.log 2>/dev/null"
  echo "ENV_PASSTHROUGH_FAILED" > "$FAIL_MARKER"
  exit 1
fi

RESULT_POD="$P21KR/out_main/result.json"
TRAIN_PROC_FRAGMENT="train_p21k_korean.py"
...
for i in $(seq 1 $MAX_ITERS); do
  ...
  if [ "$ALIVE" = "DEAD" ]; then
    ...
    $SSH "tail -100 $P21KR/train.log" 2>/dev/null
    echo "TRAIN_CRASHED" > "$FAIL_MARKER"; exit 1
  fi
  if [ $((i % 2)) -eq 0 ]; then
    $SSH "tail -2 $P21KR/train.log" 2>/dev/null | sed "s/^/[train-P21K] /"
  fi
  sleep 60
done

$SSH "test -s $RESULT_POD && echo RESULT_DONE" 2>/dev/null | grep -q RESULT_DONE || {
  echo "FATAL: result.json never produced"; $SSH "tail -80 $P21KR/train.log" 2>/dev/null
  echo "TRAIN_NO_RESULT" > "$FAIL_MARKER"; exit 1; }

SAVE_POD=1
for k in 1 2 3 4 5; do
  $SCP "root@$IP:$RESULT_POD"                        "$VDIR/result.json" && \
  $SCP "root@$IP:$P21KR/out_main/heldout_vp21k.json" "$VDIR/heldout_vp21k.json" && \
  $SCP "root@$IP:$P21KR/out_main/vp21k_eval1.json"   "$VDIR/vp21k_eval1.json" && \
  $SCP "root@$IP:$P21KR/out_main/mix_info.json"      "$VDIR/mix_info.json" && \
  $SCP "root@$IP:$P21KR/train.log"                   "$VDIR/train.log" && \
  $SCP -r "root@$IP:$P21KR/out_main/lora_adapter"    "$VDIR/" && \
...
print('  Korean OOD generalize=', d.get('n_ood_generalize_total'), '/20  memorize=', d.get('n_ood_memorize_total'), '/20')
...
echo "=== P21K pipeline complete $(date -u +%FT%TZ) ==="
