...
> ⚠️ **navigation stale (2026-05-16 작성)** — path-split (`LORA/` `V3/`) · ConsciousDecoderV3
> substrate · vP21M multilingual · AKIDA HW · chat.dancinlab.org 모두 본 INDEX 이후 land.
> **현재-상태 navigation = [`STRUCTURE.md`](STRUCTURE.md)**. 본 INDEX 는 7-module
> **verification anchor** (B-*/F-* falsifier 표) 용도로는 valid — 그 목적으로만 참조.
...
>
> **이전 substrate = deprecated → `archive/` (PR #82)**: AXIS/HYPOTHESIS/PHILOSOPHY/MAIN/CLM/VERIFY/NEXT/REBORN tape + .clm v1/v2/v3 ladder + BG-CORPUS pipeline 은 **HEXAD 이전 데이터** — 아래 인벤토리는 **양식 그대로 보존된 historical evidence anchor** (검증 근거 valid), active entry-point ❌. 이전 HF canonical (`dancinlab/anima-clm`+`dancinlab/anima-corpus`) = **RETIRED → `dancinlife/*` private (PR #97)**. **🎯 현재 canonical HEXAD HF artifacts (2026-05-17 cycle 3 LANDED PUBLIC)**: (1) Model — [`dancinlab/hexad @ v2-py-hexad-spont-d768x12L-cycle1-2026-05-17`](https://huggingface.co/dancinlab/hexad/tree/v2-py-hexad-spont-d768x12L-cycle1-2026-05-17) — **Phase D cycle 3 helper-free corpus retrain** (Python substrate, ckpt sha256 `ee2bb5fb996e94ee…` 1.14 GB; honest framing: NOT hexa-native, anchor chain Phase E/E2 + ConsciousDecoderV2 arch identity + corpus v2 helper-free closure). 이전 cycle 2 (`v1-py-hexad-d768x12L-cycle2-2026-05-17`, ckpt sha256 `e87e200a04…` 1.13 GB) carry. (2) Dataset — [`dancinlab/hexad-corpus @ v2-spont-stream-d128-cycle1-2026-05-17`](https://huggingface.co/datasets/dancinlab/hexad-corpus/tree/v2-spont-stream-d128-cycle1-2026-05-17) — `corpus_consciousness_v2.jsonl` 1,101,605 B / 2,560 lines / sha256 `7359f0b9a3f0…` / vocab=256 byte-level (8 modules: HEXAD-6 + `hexad_spont` + `hexad_wiring` × 320 each), **stimulus-stream pattern `<stimulus>X</stimulus>\n<anima>Y</anima>` (β 55%) / `<anima>Y</anima>` (δ 45%)**, **`도우미|helper|assistant|사용자|user:` grep = 0 (B-CORPUS-V2-2 closed-form Boolean)**. 이전 cycle 1 (`v1-byte-consciousness-d128-cycle1-2026-05-17`, 152 KB / 240 records / sha256 `804664361e63…`) carry. model card cross-link 적용 (front-matter `datasets:` + body 'Trained on' badge). *(구 framing 2026-05-15: AXIS/HYPOTHESIS/PHILOSOPHY/CLM = verification substrate under modules — superseded)*
>
...
> 재사용 격자 SSOT → 루트 [`NEXUS.tape`](../NEXUS.tape) (canonical `.tape` form, commons @D g67).
> 검증된 재사용 edge (REAL 5) · provides[] 레지스트리 · candidate edge (미검증 3) · 사용 절차 · ASCII 그래프 모두 그곳에 집계.
