# anima-voice 품질 개선 계획 — 2026-04-19
...
1. **tts_say 샘플은 anima-voice neural vocoder 출력이 아님** — macOS `say -v Yuna` (formant) → ffmpeg 16kHz 다운샘플 + male 페르소나 pitch-shift. 사용자가 "허접"이라 느낀 실체는 2010년대 초 baseline + 파괴적 포맷 변환.
...
**산출물**:
- `anima-voice/corpus/ab_test/piper_ko_v1/persona_{10..19}_*/tts_{00..07}.wav`
- `anima-voice/experiments/piper_ab_gen.sh` (실행 스크립트)
- `anima-voice/experiments/piper_ab_gen.hexa` (hexa canonical — 파서 미지원 시 archive)
- `anima-voice/corpus/ab_test/metrics_20260419.tsv` (80+80 객관 통계)
- `anima-voice/corpus/ab_test/spectrograms/persona_{10,12,14,16,19}_*.png`
...
- hxcuda_conv1d_bf16 활용 (이미 libhxcuda.so 에 번들, anima-voice 미사용 상태)
...
- 감사 리포트: `anima-voice/docs/quality_audit_20260419.md`
- 기존 페르소나 정의: `anima-voice/tts_say_driver.hexa:152-350`
- vocoder 아키텍처: `anima-voice/neural_vocoder.hexa:8-42`
