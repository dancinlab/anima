# anima-voice 품질 감사 — 2026-04-19 (야간 자율 세션)
...
1. Agent A — 파이프라인 trace (anima_voice → audio_token_predictor → neural_vocoder 경로)
...
10. Agent J — bench_anima_voice + eval_likert_ab + synth_probe (기존 평가 infra)
...
### 핵심 진단: 현재 tts_say 샘플은 anima-voice neural pipeline 산출물이 아님
...
- **anima_voice / neural_vocoder / transformer / emotion_prosody / audio_token_predictor**: tts_say 경로에 전혀 참여하지 않음
...
1. **샘플 청취** — `anima-voice/corpus/ab_test/piper_ko_v1/persona_10_ice_queen/tts_00.wav` 등
...
- `/home/aiden/mac_home/dev/anima/anima-voice/experiments/circulus_tts.c` (220 LOC)
...
**3. 로드맵 P4 공식 등록 (shared/roadmaps/anima-voice.json)**
...
**로드맵 업데이트**:
- `shared/roadmaps/anima-voice.json` — P4 phase 추가 (Q2/β/γ/δ 4 트랙)
