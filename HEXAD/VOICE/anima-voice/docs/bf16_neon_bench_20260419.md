...
## 2. anima-voice compile results (hexa_v2 → C → clang)
...
| bench_anima_voice.hexa | OK | FAIL — `split` identifier unresolved (codegen bug) 4x |

**2/4 files compile cleanly**. transformer.hexa parse error is a genuine syntax issue
at line 1516 (unexpected `]` + `intent` keyword). bench_anima_voice fails because
hexa_v2 codegen emits bare `split` where it should be a string-method reference
(hexa_v2 codegen gap — not anima-voice bug).
...
Blockers:
1. **bench_anima_voice.hexa** — codegen bug (split). Can't link.
...
- anima-voice 4-file compile: 2 PASS, 2 FAIL (upstream hexa_v2 codegen)
