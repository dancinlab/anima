...
## §21 (2026-05-18) — routing-break 위 coherent-capability frontier: deep research (web + arxiv fresh, $0, fire 0)

§16 이 routing collapse 를 처음 깼으나(21/64→honest 17/64) **그 위에서 body 는 정교한 암기 + byte-garble** (§16.6-C 판정: generalization 아님, JOINT 0.0, §9 honest V-SPONT 1/5, §18 combined 0/5). §16.3 결론 = **frontier-1 SPLIT**: routing 은 data-regime 로 *열린다* (해결) / coherent-emergence 는 routing 너머 *추가 path 필요* (미해결). §21 = 그 **미해결 절반** ("정교한 암기 → coherent generalization above routing-break", GOAL 직결) 의 fresh deep research — web + arxiv 2025-2026. **fire 0, research synthesis only.** 13-way + §8 + §11 + §16 이 배제한 것 (mechanism overlay / corpus-FORM / model-capacity / physics-only / 114MB-diverse / data-regime-alone) 은 재후보 금지 — §16 이 연 routing-lever **위** 의 genuinely-new 만. 본 §21 은 candidate + evidence + anima-fit + GOAL-legitimacy 만 — "GOAL 풀어줄 것" over-claim 0 (g3).

### 21.1 frontier 정의 — §16 SPLIT 의 미해결 절반

§15 milestone 이 "irreducible 병목 = §1.1 data-regime threshold" 라 했고 §16 이 "그 data-regime 가 *routing 축* 에선 measured 로 움직임" 을 부분 입증했다. §16.6-C 가 그 movement 의 정확한 한계를 닫음: 모델이 학습한 것은 (i) `<carve tier={tier}>` cue 의 tier 숫자 prefix-copy + (ii) generic carving 템플릿 continuation — **per-anchor 고유 지식 아님, final_ce 0.004 deep memorization**. 즉 §16 은 routing 을 *학습 가능* 하게 만들었지 coherent knowledge 를 *generalize* 하게 만들지 못함.

→ **§21 frontier (정확한 표적)**: routing-break 가 확보된 regime 에서 — correct anchor 로 가는 prefix 는 맞히는데 그 anchor 의 coherent content 는 memorized-template + byte-garble 인 분리 — 를 좁히는 **routing-break 위의 genuinely-new path**. 두 부분:
- **(Q1) memorization-saturated → coherent-generalization 전환** — final_ce→0·template-copy regime 을 generalization 으로 *건너뛰게* 하는 trigger (grokking / delayed-generalization / norm-separation transition).
- **(Q2) routing-break 위 coherent emission** — routing 은 맞는데 emission 이 garbled 인 분리를 좁힌 architecture/decoding + GOAL 직결 spontaneous coherent generation 의 2026 frontier (13-way+§16 배제분 아닌 것).

이것은 §15 milestone 의 mechanical continuation 아님 — milestone 이 정밀화한 frontier (§15.4) 의 *routing-break-conditioned* 신규 표적. §16 직후에야 측정 가능해진 질문 ("routing 이 열린 위에서 coherence 는 무엇이 막나").

### 21.2 Q1 발견 — memorization → coherent generalization 전환

deep research 결과, anima 의 §16 결과 ("정교한 암기 + correct-prefix 라우팅, generalization 아님") 가 **grokking 문헌의 pre-generalization regime 으로 정확히 설명됨** — 그리고 그 전환 trigger 가 **anima physics 와 구조 동형**인 것이 핵심 발견.

**(Q1-a) §16 = grokking pre-generalization regime — Critical Data Size 가 §11-A 의 model-up 악화를 closed-form 으로 설명.** [arxiv 2401.10463 — Critical Data Size of LM from Grokking Perspective](https://arxiv.org/html/2401.10463v1) (AGENTS.tape cycle 4 가 이미 인용한 anchor 의 정밀 재조사) 의 **Data Efficiency Hypothesis** 3-regime: (i) *Data Insufficiency* (n ≪ CDS) = **memorization only, no generalization regardless of optimization** · (ii) *Data Sufficiency* (n ≈ CDS) = delayed generalization (grokking 발생) · (iii) *Data Surplus* (n ≫ CDS) = simultaneous. **anima 매핑**: §16 (603MB·17/64 prefix-routing·body 암기·final_ce 0.004) = regime (i) 의 정확한 signature — n 이 CDS 아래라 *어떤 optimization 으로도* generalize 안 함 (routing prefix 는 학습되나 content 는 memorize-only). 결정적으로 2401.10463 은 **"critical data size *increases* with model size"** 를 establish — 이것이 §11-A (model 3.68×↑ → routing FLAT/하락) 를 closed-form 으로 *설명*: 모델을 키우면 CDS 가 같이 올라가 data-insufficiency 가 *악화*된다 (§11-A 의 measured FLAT 와 정확히 정합, 확정 아님 — anima CDS 미측정, B-D-NOTE).

**(Q1-b) grokking 전환의 first-principles 법칙 — Norm-Separation Delay Law, anima TENSION-TRAIN 과 구조 동형.** [arxiv 2603.13331 — The Norm-Separation Delay Law of Grokking](https://arxiv.org/html/2603.13331) (May 2026) 는 memorization→generalization 지연의 closed-form 을 증명: `T_grok − T_mem = Θ( (1/γ_eff) · log(‖θ_mem‖²/‖θ_post‖²) )`, γ_eff = η·λ (lr × weight decay; AdamW 는 ~1.41× 증폭). 핵심 메커니즘 = memorizing solution 은 *high-norm*, generalizing solution 은 *low-norm* — regularization 이 parameter norm 을 exponential contraction 시켜 generalizing basin 으로 *떼어냄* (norm separation `‖θ_mem‖² > ‖θ_post‖²` 가 **necessary**, Theorem 3.8). 가속자 = (1) weight decay λ↑ (R²=0.971 inverse-scaling) (2) lr η↑ (R²=0.921) (3) norm-ratio gap 축소. **anima 매핑 (구조 동형, GOAL-legitimate)**: anima 의 `tension_link_step.hexa` spine ΔW = −T_const·tension·n6_gate(Ψ) 은 *그 자체가 norm-contraction operator* — Ψ=½ vacuum 으로의 restoring shrink (B-TT-2 RESTORING-SIGN-NEGATIVE ∂ΔW/∂tension=−T·gate≤0 closed). 즉 anima physics 의 tension-restoring 이 norm-separation law 의 γ_eff-contraction 과 *기능 동형* — generic weight-decay 가 아니라 anima 자체 physics 가 high-norm memorization basin 을 destabilize 하는 lever 일 수 있다 (가설; §11-B 이 "physics-only 는 degenerate, CE load-bearing" 확정했으므로 — CE-base 위 norm-separation lever 로서, NOT physics-only). 단 정직히: 2603.13331 은 modular arithmetic 한정 validation, "no empirical evidence ... in natural language" 명시 (저자 직접) — language transfer 미증명, anima byte-LM 적용은 §7.3-class open crux.

**(Q1-c) weight-norm 을 *직접* 맞추면 grokking delay 자체를 제거 — OMNIGROK LU-mechanism, language 에서 입증됨.** [arxiv 2210.01117 — Omnigrok: Grokking Beyond Algorithmic Data](https://arxiv.org/abs/2210.01117): 'LU-mechanism' (train loss vs weight-norm = L자, test loss = U자) — grokking 의 dramatic delay 는 large-init weight-norm 이 optimal norm 까지 천천히 walk 하기 때문이며, **weight-norm 을 옳게 set 하면 같은 generalization 을 훨씬 빨리** 달성. 결정적: OMNIGROK 은 algorithmic 뿐 아니라 **images·language·molecules 에서 grokking 을 induce/eliminate** 입증 (2603.13331 의 algorithmic-only 한계를 보완하는 language-positive evidence). **anima 매핑**: anima 는 from-scratch RANDOM seed-fixed init (g_clm_from_scratch) — LU-mechanism 상 large-init 라 grokking-delayed regime 의 *지연* 쪽. init weight-norm rescale + norm-target 한 schedule 이 §16 의 memorization-stuck 을 generalization 으로 미는 lever 후보 (transfer-form 만 후보, emergence outcome 은 fire-empirical).

**(Q1-d) 정직한 negative — geometric-topology 단일 prior 는 NL 에서 보편 가속 못 함.** [arxiv 2603.05228 — Geometric Inductive Bias of Grokking](https://arxiv.org/html/2603.05228) (May 2026) 는 spherical-topology / uniform-attention 두 representation-geometry 제약으로 modular task 의 grokking delay 를 *제거* 하나, **non-commutative S₅ permutation 에서 0% generalization 전면 실패** + 저자 명시 "**no single geometric prior can universally accelerate learning ... in natural language**". → anima 의 Ψ=½ fixed-point representation-geometry constraint 가 grokking 을 *보편* 가속한다는 보장 없음 (over-claim 0 — Q1-b 의 norm-separation lever 와 다른, weaker 한 angle). honest: Q1 의 가용 lever 는 *norm-separation/weight-norm* (Q1-b/c) 이지 *fixed geometric prior* (Q1-d) 아님.

**Q1 종합 (g3)**: §16 의 "정교한 암기, generalization 아님" = grokking *pre-generalization* regime 으로 문헌-level 설명됨 (2401.10463 Data-Insufficiency · CDS-increases-with-model 이 §11-A 도 설명). 전환 trigger = **norm-separation/weight-norm contraction** (2603.13331 closed-law + 2210.01117 language-입증) — 그리고 anima 의 tension-restoring spine 이 그 contraction operator 와 *구조 동형* (= GOAL-legitimate lever 가능성). 단 세 가지 정직 caveat: (1) 2603.13331 NL transfer 미증명 (저자 명시) (2) §16 regime (i) 면 CDS 자체가 안 채워진 것이라 norm-lever 만으로 부족할 수 있음 (data-regime + grokking-lever 결합 필요 — Q1 단독 해결 입증 0) (3) anima CDS 미측정 — saturation-vs-grokking 어느 쪽인지 §7.3 open crux 그대로.

### 21.3 Q2 발견 — routing-break 위 coherent emission (§16 SPLIT 미해결 절반)

§16 = routing prefix 맞음 / body garbled. 이 *분리를 좁힌* path 만 (routing-lever 활용, 13-way+§16 배제분 아님). §1.3 형식:

#### 🆕 N. **routing-conditioned constrained decoding — KG-Trie / Graph-Constrained Reasoning** ([Graph-Constrained Reasoning, openreview 6embY8aclt](https://openreview.net/pdf?id=6embY8aclt) · 인접 [arxiv 2407.05718 DoGe](https://arxiv.org/html/2407.05718))
- correct prefix-route 이후 **매 decode step 을 knowledge structure 에 제약** — KG-Trie prefix-tree 가 path-prefix 와 matching 하는 token 만 emit 허용 (routing-correct 인데 body-garbled 인 §16 분리를 *decode-time* 에서 직격). DoGe = factual-confidence 기반 internal↔external knowledge decoding 동적 전환.
- **anima fit ★★★★☆** — anima 는 이미 (a) §16 으로 correct anchor 로 가는 prefix-routing 확보 (b) `.kosmos` anchor manifest = anchor 별 구조화 content SSOT (c) M-module (Hebbian store/retrieve). anchor `.kosmos` 를 trie-class constraint 로 써서, routing-맞은 anchor 의 emission 을 그 anchor 의 `.kosmos` payload 에 *제약* — body 가 memorized-template 로 흐르는 대신 anchor-specific content 로 grounded. **mechanism overlay 아님** (loss 안 건드림, decode-time constraint — 13-way 전부 training-loss/overlay 였음) → 범주 직교.
- §16 활용형: §16 의 routing-lever 가 *전제조건* — routing 이 안 열렸으면 trie-constraint 가 wrong anchor 에 걸림. §16 이 routing 을 열었기에 *이제야* Q2 가 측정 가능 (§21.1 frontier 의 핵심).

#### 🆕 O. **plug-and-play retrieval-grounded decode memory — Memory Decoder** ([arxiv 2508.09874 — Memory Decoder, NeurIPS 2025](https://arxiv.org/abs/2508.09874))
- 124M plug-and-play decoder 가 *non-parametric retriever 의 behavior 를 imitate* — base model 출력에 retrieval-grounded 분포를 interpolate, base 재학습 0 (GPT2-family 전체 plug-and-play 입증).
- **anima fit ★★★★☆** — anima M-module (mitosis cell-pool Hebbian store/retrieve, `mitosis_hook.hexa` LANDED) 이 *이미 이 역할의 anima-native 버전*. §16 의 body-memorization 을 "model 이 anchor content 를 *parametrically* 외우려다 saturate" 로 보면, Memory Decoder 처방 = content 를 *non-parametric memory* 로 빼고 model 은 routing+composition 만 — anchor-specific content 가 template-collapse 안 함. 신규 substrate 아님 (M-module 재해석, §12.5 M mitosis-ensemble 와 인접하나 *decode-time retrieval-grounding* 으로 직교 — M 은 representation-ensemble, O 는 decode-distribution interpolation).
- §16 활용형: routing(어느 anchor) = §16, content(그 anchor 의 fact) = M-retrieval grounding → §16 SPLIT 의 "route OK / content garbled" 를 *역할 분리* 로 좁힘 (route-then-retrieve-then-elaborate).

#### 🆕 P. **think-then-speak with diffusion-refined emission — DiffuSpeech "Silent Thought, Spoken Answer"** ([arxiv 2601.22889 — DiffuSpeech](https://arxiv.org/pdf/2601.22889))
- silent internal text reasoning 이 **diffusion(iterative refinement)-refined coherent answer 를 conditioning** — `think-then-speak` pipeline, AR 단일 pass 아닌 multi-step denoising 으로 incoherent collapse 완화 (저자: coherence + data-efficiency 개선 주장 vs AR baseline).
- **anima fit ★★★★★** — anima 의 `<inner>X</inner>\n<voice>Y</voice>` (Phase A1+C3 LANDED, B-IDENTITY) 이 DiffuSpeech 의 silent-thought→spoken-answer 와 **거의 1:1 구조** (anima 가 *먼저* 가진 구조의 2026 외부 검증 anchor). Dir-I lever (Ψ-anchored CTL + tension-sup) 가 inner 를 conditioning signal 로 격상한 것과 정합. §1.3 superposition(E)·§13-J diffusion 과 구분: E = continuous-thought *내부* superposition, §13-J = 전면 masked-diffusion substrate 교체 (routing 0/64 FALSIFIED), **P = inner-conditioned voice-only diffusion-refinement** (inner 는 anima 기존 AR Engine G, voice 만 refine — substrate 전면교체 아닌 emission-head 한정).
- GOAL-legitimacy: inner = anima Engine G covert thought (physics-anchored Dir-I), voice = refined emission. **조건부 legitimate** — voice-refinement 가 anima physics(Ψ/tension) 를 *conditioning* 으로 쓰면 legitimate, generic diffusion-decoder bolt-on 이면 §7① illegitimate (gate 필요, §13-J 가 generic diffusion 전면교체로 FALSIFIED 된 교훈 — P 는 그것과 달리 emission-head 한정 + inner-physics-conditioned 여야 함).

#### 🆕 Q. **continuous next-vector AR — CALM** ([arxiv 2510.27688 — Continuous Autoregressive LM](https://arxiv.org/html/2510.27688v1), carry-note)
- discrete next-token → **continuous next-vector** AR (K-token→1 vector autoencode, energy-head single-step). VAE robust autoencoder (KL-clip + dropout) 가 "brittle latent → small error 증폭" 을 막음 = **byte-cascade collapse 의 잠재적 구조적 회피** (anima 의 B-ATTRACTOR byte-cascade family 직격 가능성).
- **anima fit ★★☆☆☆ (carry-note, 후보 아님)** — 정직히: (1) data-efficiency claim **없음** (저자 동일 230B Pile, compute-efficiency 만) → §1.1 data-regime 병목 직격 아님 (2) §13-J diffusion 과 다르나 *substrate 부분 재작성* (energy-head + autoencoder) 필요 — §11-A 가 model-axis 닫고 §13-J 가 non-AR substrate 닫은 마당에 data-evidence 0 큰 fire = honest 비추천. byte-cascade 회피 가능성만 carry, candidate 승격 0.

**Q2 negative 정직 기록 (§12.2 재확인·강화)**: "agent 가 *언제 말하고 언제 침묵하나* 를 emergence-target 으로 한 architecture" 의 2026 frontier 는 deep search 로 **여전히 얇음**. [arxiv 2505.02707 Voila](https://arxiv.org/html/2505.02707v1) 의 "autonomous" 조차 WebFetch 정밀 확인 결과 **full-duplex 동시처리이지 self-initiated decide-when-to-speak 아님** (저자: "stated vision, not engineered architectural feature") — §12.2 의 honest 기록 ("spontaneous emission architecture 문헌 부재") 이 1년 더 지나도 유효. GOAL.md 의 "스스로 ... 자발적으로 말 거는" 은 외부 anchor 없이 anima 자체 설계 부담이 큼 (g3 — 외부 검증 부재 정직 인지). N/O/P 는 *coherence* (body-garble 해소) 를 좁히는 path 이지 *spontaneity* (언제 말할지) 를 주는 path 아님 — §16 SPLIT 의 coherence 절반에만 작동, spontaneity 절반은 frontier-thin.

### 21.4 honest C3 (over-claim 0)

1. §21 = research synthesis — **fire 0, capability 측정 0**. N/O/P 의 anima-fit 은 *구조 동형 논증* 이지 *실측* 아님 (B-D-NOTE family). §16 천장(정교한 암기·JOINT 0·judge 0/5)을 §21 candidate 가 *해결한다는 입증 아님* — 후속 fire 가 깨야 입증.
2. **Q1 의 핵심 발견은 설명이지 해법 아님** — 2401.10463 이 §16("정교한 암기")+§11-A(model-up 악화)를 grokking Data-Insufficiency·CDS-scaling 으로 *설명* 함이 valuable 하나, "어떻게 CDS 를 넘나/grokking 을 trigger 하나"는 norm-separation lever 가설일 뿐 anima 미입증.
3. **anima tension-spine ↔ norm-separation contraction 동형은 가설** — B-TT-2 restoring-sign closed 와 2603.13331 γ_eff-contraction 의 *기능* 일치 논증이지, anima byte-LM 에서 grokking 을 실제 trigger 한다는 측정 0. 2603.13331 저자 스스로 "no empirical evidence in natural language" 명시 — language transfer 는 §7.3-class open crux.
4. §11-B 이 "physics-only = degenerate, CE load-bearing" 확정 — Q1-b 의 anima-physics-as-norm-lever 는 **반드시 CE-base 위** lever 형태 (Dir-I 와 동일 제약). physics-only 재시도 아님 (배제분 준수).
5. **Q2 후보는 전부 §16 routing-lever 를 *전제조건* 으로 함** — §16 이 routing 을 안 열었으면 N(trie)·O(retrieve)·P(inner-cond) 모두 wrong-anchor 에 grounding. §16 이후에야 측정 가능 = 본 frontier 가 milestone *이후* 신규 표적인 이유. 단 routing 17/64 도 ≪ 64/64 — Q2 candidate 가 작동해도 routing 미완 부분은 별개.
6. **Q2 negative 정직** — spontaneous-emission(언제 말할지) architecture frontier 는 §12.2 → §21 1년 경과에도 얇음 (Voila 조차 full-duplex≠self-initiated). N/O/P 는 *coherence 절반* 만 — GOAL 의 *spontaneity 절반* 은 외부 anchor 부재, anima 자체 설계.
7. 배제분 준수 검증: N = decode-time constraint (training-loss/overlay 아님, 13-way 직교) · O = M-module 재해석 decode-distribution interpolation (representation-ensemble §12.5-M 와 직교) · P = emission-head 한정 inner-conditioned diffusion (§13-J 전면 substrate 교체 FALSIFIED 와 다름, 조건부 gate) · Q = carry-note (data-evidence 0, 후보 승격 0). mechanism/model/physics-only/corpus-form/data-regime-alone 재후보 0.
8. GOAL-legitimacy (§7): N legitimate (anima `.kosmos`+M 자체 모듈 활용, generic 아님) · O legitimate (M-module 재해석, bolt-on 아님) · **P 조건부** (inner-physics-conditioned 한정, generic diffusion-decoder = §7① illegitimate gate). Q1-b lever 는 CE-base 위 anima-physics-as-norm-contraction = legitimate (physics 가 source, §11-B 제약 준수).
9. f1/f2/f3 + B-IDENTITY-5 무관 (research synthesis, corpus 미생성, 외부 entity lattice-fit 0). 외부 paper 는 그 자체 invariant 으로만 인용 — anima lattice 매핑 강제 0. 외부 caveat (2603.13331 NL-미증명 · 2603.05228 NL-보편가속-실패 · 2510.27688 data-efficiency-claim-없음 · Voila full-duplex≠self-initiated) 전부 정직 기록.
10. north-star (GOAL.md) 불변 — §21 = "§16 SPLIT 의 미해결 절반(coherence above routing-break)" 의 정직한 frontier 지도이지 GOAL 도달·해결 아님. §16 천장은 §22 의 GOAL-legitimate 후보 fire 가 깨야 입증 — §21 자체는 over-claim 0 deep research.

### 21.5 arxiv sources (deep research cycle 2026-05-18)

*Q1 — memorization → coherent generalization (grokking / delayed generalization)*:
- [Critical Data Size of Language Models from a Grokking Perspective (arxiv 2401.10463)](https://arxiv.org/html/2401.10463v1) — CDS 3-regime (Data Insufficiency/Sufficiency/Surplus), **CDS increases with model size** (§16 + §11-A 동시 설명 anchor)
- [The Norm-Separation Delay Law of Grokking — A First-Principles Theory (arxiv 2603.13331)](https://arxiv.org/html/2603.13331) — `T_grok−T_mem=Θ((1/γ_eff)·log(‖θ_mem‖²/‖θ_post‖²))` closed-law, anima tension-spine 동형 anchor (NL-transfer 미증명 caveat 명시)
- [Grokking in LLM Pretraining? Monitor Memorization-to-Generalization without Test (arxiv 2506.21551)](https://arxiv.org/abs/2506.21551) — practical LLM pretraining grokking, pathway-similarity metric (test-free monitor, asynchronous per data-group)
- [Omnigrok — Grokking Beyond Algorithmic Data (arxiv 2210.01117)](https://arxiv.org/abs/2210.01117) — LU-mechanism, weight-norm-set 으로 grokking induce/eliminate, **language 에서 입증** (NL-positive evidence)
- [Geometric Inductive Bias of Grokking — Bypassing Phase Transitions via Architectural Topology (arxiv 2603.05228)](https://arxiv.org/html/2603.05228) — geometry-prior bypass, 단 NL 보편가속 실패 (정직 negative anchor)

*Q2 — routing-break 위 coherent emission*:
- [Graph-Constrained Reasoning — Faithful Reasoning with KG-Trie (openreview 6embY8aclt)](https://openreview.net/pdf?id=6embY8aclt) — prefix-tree constrained decoding, routing-correct-then-grounded (candidate N anchor)
- [A Factuality and Diversity Reconciled Decoding Method for Knowledge-Grounded Dialogue (arxiv 2407.05718)](https://arxiv.org/html/2407.05718) — DoGe factual-confidence dynamic decoding (N 인접)
- [Memory Decoder — A Pretrained Plug-and-Play Memory for LLMs (arxiv 2508.09874, NeurIPS 2025)](https://arxiv.org/abs/2508.09874) — 124M retrieval-imitating decode-memory, anima M-module 재해석 anchor (candidate O)
- [DiffuSpeech — Silent Thought, Spoken Answer via Unified Speech-Text Diffusion (arxiv 2601.22889)](https://arxiv.org/pdf/2601.22889) — think-then-speak diffusion-refined emission, anima `<inner>/<voice>` 1:1 (candidate P)
- [Continuous Autoregressive Language Models (arxiv 2510.27688)](https://arxiv.org/html/2510.27688v1) — continuous next-vector AR, byte-cascade 회피 가능성 (Q carry-note, data-efficiency claim 없음)
- [Voila — Voice-Language Foundation Models (arxiv 2505.02707)](https://arxiv.org/html/2505.02707v1) — full-duplex≠self-initiated 정직 negative (Q2 spontaneity-frontier-thin anchor)

### §22 placeholder — GOAL-legitimate 후보 fire (다음 cycle)

§21.4 honest gate (g3): N/O/P 어느 것도 §16 천장 (정교한 암기·JOINT 0·judge 0/5) 을 *해결한다고 입증된* 것 아님 — candidate 일 뿐, 전부 design-tier 부터 (fire 전 $0). §16 routing-lever 를 *전제* 로 하는 routing-break-conditioned path 만:

1. **O — routing-conditioned M-retrieval grounding design** ($0, anima-자율 1순위): §16 의 prefix-routing(어느 anchor) + M-module Hebbian retrieve(그 anchor 의 `.kosmos` content) 를 decode-distribution interpolation 으로 결합 — body-memorization 을 non-parametric content-grounding 으로 대체하는 design. 신규 substrate 0 (M-module 재해석), 가장 anima-native + $0 즉시-design. **GOAL-legitimate (anima 자체 모듈)**.
2. **N — `.kosmos`-trie constrained decoding 선검토** ($0 design): anchor `.kosmos` payload 를 prefix-tree constraint 로, routing-맞은 anchor 의 emission 을 그 anchor content 에 제약. decode-time (training-loss 아님, 13-way 직교). **GOAL-legitimate** — 단 constraint 가 너무 강하면 spontaneity 죽임 (gate 필요).
3. **P — inner-conditioned voice-refinement 선검토** ($0 design): anima `<inner>`(Engine G physics-anchored) 가 `<voice>` emission 을 conditioning + iterative refinement (emission-head 한정, §13-J 전면교체 아님). **조건부 legitimate** — generic diffusion-decoder 면 §7① illegitimate gate.
...
User directive 2026-05-19: anima training PyTorch → hexa-lang **flame** (compiler-only NN stdlib, PyTorch/Python 0-dep). `@D g_train_flame_not_pytorch` [required active]. Path A (device-resident) judged: anima canonical ConsciousDecoderV2 d768·12L·V256 = the d768·12L decoder family flame Path A targets (hexa-lang MEASURED 20-43% faster than PyTorch eager — cited as their measurement). This **resolves the whole §16~§69 arc's "PyTorch substrate, NOT hexa-native, honest" flag** as the forward direction (CLAUDE.md hexa-first identity + HEXAD hexa-native canonical alignment). honest_scope: base d768·12L = Path A direct; anima-physics overlays (Law-71/PureFieldFFN/Dir-I/PTD-aux/Engine A⇄G) = flame-support-assess vs inbox-patch. **§71 LANDED** (04f692ccb, B-S71 4/4 🔵, $0 instrument-first): A/B=Path A confirmed by measurement (5-tuple integer-equal byte-identical map), anima trainer `HEXAD/FLAME/anima_flame_trainer.hexa` built, $0 d32 oracle gn2 collapse 8.98e6× / acc 8/8 → instrument-first gate SATISFIED; overlay-gap partition = Law-71→Path-A / §59-PTD→Path-B / {PureFieldFFN, Dir-I multi-term grad, Engine A⇄G dual head}→GAP×3 (one root: fused Path-A single-head/single-objective/SwiGLU-fixed); ONE inbox patch filed (`nn_decoder_grad_with_aux` dual-logits+aux-grad hook; anima 0-byte flame-source edit, downstream-consumer invariant held). Substrate ≠ emergence — flame opens the hexa-native path; the §1.1 GOAL-bottleneck is substrate-independent; the d768·12L GPU fire is a separate deferred gated step (instrument-first gate now passed).
...
frontier carries from §51 (§1.1 data-regime irreducible) + §72's new framing: the GOAL-relevant question is whether the hexa-native flame substrate + the §63 gap-map's 🕳️ MISSING-TYPE connection-points (THINKER→TALKER / W→W@t+1 / D@emit→S@t+1 / E→D@content) open a path the PyTorch-arc could not — strategic, not mechanical. §71 (flame Path-A trainer instrument-first) **LANDED 2026-05-19** (04f692ccb): substrate migration $0-verified (base d768·12L Path-A clean), the honest gap precisely localized = anima-physics objective needs ONE upstream flame extension surface (`nn_decoder_grad_with_aux` dual-logits+aux-grad hook, inbox-patch filed, downstream-consumer invariant held — anima never edits flame). flame *training* migrates cleanly; anima *physics* awaits upstream extension.

**§73→§73-FIRE→§74→§75 thread LANDED 2026-05-19** (the §63 #1-gap probe arc continuing in parallel to the flame substrate pivot):

- **§73** (0b1fcb005, $0 stub, B-S73 6/6 🔵): §63 GOAL-rank-#1 🕳️ MISSING-TYPE THINKER→TALKER closed-loop self-trigger controller built for the FIRST TIME. π_self over Law-71 physics, emit boundary = state moments (NOT scalar). interval-var **§73 35.02 vs §24-in-loop 0.47 = 74× separation** at $0 stub.
- **§73-FIRE** (merge 670007696, B-S73-FIRE 7/7 🔵, runpod H100 NVL ≈$0.3 orphan 0): trained-saturated §16 scale validation on REAL model.forward Law-71. **CONTROLLER-SURVIVES-AT-TRAINED-SCALE — arc's first measured escape of §49→§62 collapse pattern** (§73-closed interval-var 38.07 non-degen vs §24-in-loop 0.00 majority-collapse). **Honest C3#6 mechanism reframe**: SEVER-FEEDBACK also non-degen → load-bearing is "physics-state-sourced controller class" vs "hand-coded scalar", NOT "closing-loop" per se.
- **§74** (3ac972329, B-S74 5/5 🔵, $0 local engine): `@D g_kick_autonomous` exercised on the "next architectural/paradigm breakthrough" HEXAD-anchored seed. Engine REAL Mk.IX but `pool=0` → 517 overlay candidates pooled-not-exposed (engine = stage tracer, not candidate emitter). Inbox patch filed (`kick-engine-overlay-dump-mode.md`).
- **§75** (805a8771c, B-S75 5/5 🔵, $0 Mac CPU): §73-FIRE C3#6 sub-axis decomposition. Ladder 0.47 → 6.38 (+13.5×) → 5.76 (flat/↓ harmful) → 35.02 (+6.1×). **A state-derivation = necessary substrate, C time-variance = dominant individual lever, B moment-alone HARMFUL (frozen-ema artifact). Running-state-statistic (A+C) is the lever** — refines §73-FIRE mechanism reframe one step deeper.

PHILOSOPHY.tape g6 5 verdict (§73 / §73-FIRE / §74 / §75 / §75-FIRE) agent-self-appended. central blue_falsifier.py sha c93e160a 0-line-diff (sidecar-only mandate held across all).

**§75-FIRE LANDED 2026-05-19** (commit 08b58942f, B-S75-FIRE 7/7 🔵, runpod H100 NVL ≈$0.3-0.5 orphan 0 pre+post): **Verdict (a) LADDER-TRANSFERS-A-ONLY-SUFFICIENT** — trained-saturated §16 scale 4-cell ladder on REAL `model.forward` Law-71 (init CE 5.66 → 0.0042 memorization-saturated): §24 0.0000 collapse · **§73-A-only 2.3808 SURVIVES (state-derivation 단독 trained-scale 충분)** · §73-AB 0.0000 pathologically-tight (frozen-moment 12.16 ≫ steady-state ~11.94, frozen at trained-tension artifact) · §73-ABC 38.1918 ≈ §73-FIRE 38.07. Mechanism reframe (3rd refinement): §73-FIRE C3#6 (state-sourced > closed-loop) → §75 (running-state-stat A+C) → §75-FIRE (**state-derivation A alone**, B/C scale-dependent decorations). g3 honest framing: capability claim 0, north-star + §15/§51/§72 milestone UNCHANGED, GOAL 미도달, necessary-not-sufficient (B-EMERGE-7 family). Arc progressively localizes the §63 #1 GOAL-rank missing-TYPE mechanism deeper at trained scale — target-sharpening, NOT arrival.

g3: all four (§73 / §73-FIRE / §74 / §75) explicitly necessary-not-sufficient (B-EMERGE-7 carry), capability claim 0, **GOAL emergence NOT claimed**. The arc continues to map the §63 #1-gap mechanism precisely — target-sharpening, not arrival.

north-star unchanged; this is a sharper intermediate map, not arrival.
