...
### 2026-05-18 — §19 step 0 TRIBE pipe sanity LANDED ($0 inference-only, RESEARCH.md §19.4)
`state/eeg_anchor_s19_2026_05_18/step0_tribe_sanity.py` graded gates: **G0 IMPORT / G1 API / G2 HF_REACHABLE / G3 CONFIG_LOAD / G4 CKPT_META 全 PASS → `pipe_credible_through_ckpt_load=true`** (axis C TRIBE feasibility 확립 — best.ckpt 708.9MB mmap-load 108 tensors + model 구성, NO weight mutation, GPU fire 아님). py3.12 venv (PEP 668 carry). **ADDENDUM §8 C3 #3 verification gap = dependency+API level CLOSED** — `cortexlab-toolkit 0.1.0` modern pip 설치 성공 (구 system pip 21.2.4 false-negative), import name=`cortexlab`, `neuralset/neuraltrain 0.0.2` (ADDENDUM §2 blocker) pull, `TribeModel.from_pretrained("facebook/tribev2")` HF Hub weight 구성. honest residual: `lightning`(PyTorch Lightning) cortexlab base deps 부재 → `pip install lightning` 2.6.1 해소 (차후 재발 방지 기록). G5 full BOLD forward = heavy backbone DL boundary 정직 기록 (step 0 목적 = pipe-credibility, G0–G4 충족). **closed: B-CT3-1..5 5/5 🔵** sidecar (`F_CT_3_gate.py`, central blue_falsifier.py 변경 0) — F-CT-3 사전등록 gate (PASS r≥0.5 / DISCARD r<0.3 / INCONCLUSIVE gray) 가 closed-form (Pearson r∈[−1,1] bounded · 3-way exact Interval partition · monotone · deterministic · gray-zone non-empty). B-EEG-NOTE = 실제 r 값 EMPIRICAL (OpenBCI hardware 측정 OUTCOME, 미래 EEG fire — NOT counted 🔵). 정직: **§19 = 측정축, GOAL *생성* 아님** — step 0 ≠ F-CT-3 (EEG↔BOLD bridge = step 2, hardware-in-loop 차후). §20 의 'TRIBE actual impl 부재' = hexa-brain stub-flag 관찰, §19 step 0 의 cortexlab-toolkit→facebook/tribev2 실 weight load 와 상보적. GOAL 거리 §15 milestone 불변. SSOT = RESEARCH.md §19 + state/eeg_anchor_s19_2026_05_18/ + archive/PHILOSOPHY.tape §verdict_eeg_anchor_s19. $0 · branch 0 (anima main) · docs/* 신규 0.
