#!/bin/bash
# UBM-E6 single-path re-fire — picks a DISTINCT A100 offer by rank index to
# avoid the multi-contract-same-offer collision (4 paths all picked offer
# 31632904, only 1 provisioned). Usage: PATH_NAME=alpha OFFER_RANK=1 bash refire_path.sh
set -uo pipefail
...
STEPS="${STEPS:-2000}"
WATCHDOG_MIN=75
PATH_NAME="${PATH_NAME:?set PATH_NAME}"
OFFER_RANK="${OFFER_RANK:-0}"

PDIR="$LOCAL_DIR/out/$PATH_NAME"
mkdir -p "$PDIR"
LABEL="ubm-e6-carving-$PATH_NAME-r2"
cd "$LOCAL_DIR"
echo "=== UBM-E6 re-fire path=$PATH_NAME offer_rank=$OFFER_RANK ==="; date -u

echo "[$PATH_NAME 1/9] selecting A100 offer (rank $OFFER_RANK)..."
OFFER=$($VASTAI search offers 'gpu_name in [A100_SXM4,A100_PCIE,H100_SXM,H100_PCIE] num_gpus=1 reliability>0.98 dph_total<2.5 disk_space>40 inet_down>500 cuda_max_good>=12.0' -o dph_total --raw 2>&1 | python3 -c "
import json,sys
d=json.load(sys.stdin)
rank=$OFFER_RANK
if len(d)<=rank: sys.stderr.write('not enough offers\n'); sys.exit(1)
o=d[rank]; print(o['id']); sys.stderr.write(f\"{o['gpu_name']} \${o['dph_total']:.3f}/hr\n\")")
[ -z "$OFFER" ] && { echo "[$PATH_NAME] ERROR no offer"; exit 1; }
...
[ -z "$IID" ] && { echo "[$PATH_NAME] ERROR create: $CREATE"; exit 1; }
...
[ -z "$SH" ] && { echo "[$PATH_NAME] ERROR SSH not ready"; exit 1; }
...
echo "[$PATH_NAME 5/9] verify..."
$SSH 'cd /workspace/anima && python3 -c "import torch;print(torch.__version__, torch.cuda.is_available(), torch.cuda.get_device_name(0))" && sha256sum corpus_carving.jsonl'

echo "[$PATH_NAME 6/9] sanity..."
...
$SSH "bash /workspace/anima/run_sanity.sh" 2>&1 | tail -3 || true

echo "[$PATH_NAME 7/9] MAIN fire $STEPS steps..."
...
echo "[$PATH_NAME 8/9] verify + SAVE_POD + pull retry >=5..."
