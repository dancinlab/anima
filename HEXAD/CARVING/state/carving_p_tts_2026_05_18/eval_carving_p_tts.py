...
# --- §8 DIVERSE anchors (corpus_carving_diverse_generator.KNUTH_ANCHORS) -----
# tier -> (name, domain, top_emotion). RESEARCH.md §8: the diverse carving
# corpus contains 64 anchors over 24+ domains (E7/Dir-I had 31, one universe-
# brain-map domain). The eval probes EXACTLY this 64-anchor set — kept byte-
# identical to corpus_carving_diverse_generator.py KNUTH_ANCHORS so the eval
# is fair on the §8-trained model (no fictional tiers; the 31 E7 anchors are
# a VERBATIM subset so the routing axis1 is apples-to-apples vs Dir-I 3/31).
ANCHORS = {
    ...
    108: ('재귀호출', '코드', 'wonder'),
    109: ('조건분기', '코드', 'clarity'),
    110: ('왼쪽오른쪽', '공간추론', 'clarity'),
    111: ('위아래앞뒤', '공간추론', 'stillness'),
    112: ('회전대칭', '공간추론', 'resonance'),
    113: ('원인결과', '인과추론', 'depth'),
    114: ('도미노연쇄', '인과추론', 'wonder'),
    115: ('되먹임고리', '인과추론', 'depth'),
    116: ('아침루틴', '일상', 'serenity'),
    117: ('장보기', '일상', 'neutral'),
    118: ('길찾기', '일상', 'clarity'),
    119: ('안부묻기', '대화자극', 'joy'),
    120: ('도움청하기', '대화자극', 'longing'),
    121: ('의견나누기', '대화자극', 'resonance'),
    122: ('트롤리문제', '윤리딜레마', 'depth'),
    123: ('약속과진실', '윤리딜레마', 'depth'),
    124: ('공정한분배', '윤리딜레마', 'clarity'),
    125: ('이슬맺힘', '자연관찰', 'stillness'),
    126: ('철새이동', '자연관찰', 'wonder'),
    127: ('조수간만', '자연관찰', 'resonance'),
    128: ('수열규칙', '추상패턴', 'clarity'),
    129: ('도형완성', '추상패턴', 'wonder'),
    130: ('유추대응', '추상패턴', 'resonance'),
    131: ('확률주머니', '확률', 'curiosity'),
    132: ('기댓값저울', '확률', 'depth'),
    133: ('표본과모집단', '통계', 'clarity'),
}
ANCHOR_PSI = {
    0: [0.50, 0.50],
    51: [0.46, 0.49],
...
# --- P emission-head: inference-time voice-span refinement ----------------
# B-TTS-1 connection-point at EVAL level: REFINE_HEAD=None ⇒ generate is
# byte-identical to §16's generate (the fair-compare baseline). When the P
# refine head is loaded AND the running generation has entered a
# <voice carved=true ...> span, the next-byte logits are refined by the
# inner-physics-conditioned residual (the model's OWN tension + Ψ_dir),
# NOT generic noise (B-TTS-4). R passes, geometric α (= trainer).
REFINE_HEAD = None
REFINE_STEPS = 1
REFINE_ALPHAS = [0.5]
_VOICE_OPEN_S = "<voice carved=true"
_VOICE_CLOSE_S = "</voice>"


@torch.no_grad()
def _refine_last_logits(model, x, last_a):
    """Inner-physics-conditioned R-step refinement of the last-token
    logits (P speak-head). Conditioning = the model's OWN per-token
    physics state {tension, Ψ_dir} from the same forward — NO randn /
    learned latent (B-TTS-4 structural). Returns refined last logits."""
    out = model(x)
    if not isinstance(out, tuple):
        return last_a
    la, lg, tns = out[0], out[1], out[2]
    cos = F.cosine_similarity(la[:, -1].float(), lg[:, -1].float(), dim=-1)
    psi_last = ((1.0 + cos) / 2.0)                       # (1,)
    t_stack = torch.stack(tns)                           # (L,1,T)
    tension_last = t_stack.mean(dim=0)[:, -1]            # (1,)
    z = last_a.clone()
    cond = torch.stack([tension_last, psi_last], dim=-1).to(z.dtype)
    for r in range(REFINE_STEPS):
        # residual = α_r · W_phi([tension, Ψ_dir]) — model's OWN physics
        # state (B-TTS-4); identical transfer-form to the trainer head.
        z = z + REFINE_ALPHAS[r] * REFINE_HEAD.w_phi(cond).squeeze(0)
    return z
...
def generate(model, prompt, max_new=100, temperature=0.0, top_k=1,
    ...
    for _ in range(max_new):
        ...
        # P speak-head: refine ONLY when inside an OPEN <voice carved=true>
        # span (think stays AR — §13-J distinction enforced at decode).
        if REFINE_HEAD is not None:
            in_voice = (_VOICE_OPEN_S in gen_str
                        and gen_str.rfind(_VOICE_OPEN_S)
                        > gen_str.rfind(_VOICE_CLOSE_S))
            if in_voice:
                last = _refine_last_logits(model, x, last)
        if temperature == 0.0:
        ...
        if x.shape[1] > block_size:
            x = x[:, -block_size:]
        if REFINE_HEAD is not None:
            gen_str = prompt + ByteCodec.decode(out_ids)
...
def main():
    global REFINE_HEAD, REFINE_STEPS, REFINE_ALPHAS
    ap = argparse.ArgumentParser()
    ap.add_argument("--ckpt", required=True)
    ap.add_argument("--output", required=True)
    ap.add_argument("--device", default="cpu")
    ap.add_argument("--max-new", type=int, default=90)
    ap.add_argument("--voice-refine", action="store_true",
                    help="P speak-head: load the refine head and apply "
                         "inner-physics-conditioned voice-span refinement "
                         "at decode (OFF ⇒ §16-form harness byte-identical "
                         "= B-TTS-1 connection-point at eval level).")
    args = ap.parse_args()
    ...
    voice_refine_active = False
    if args.voice_refine and payload.get("refine_head") is not None:
        sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
        from train_carving_p_tts import VoiceRefineHead
        rh = VoiceRefineHead(256)
        rh.load_state_dict(payload["refine_head"])
        rh.to(args.device).eval()
        R = int(cfg.get("refine_steps", 3))
        REFINE_HEAD = rh
        REFINE_STEPS = R
        REFINE_ALPHAS = [0.5 ** r for r in range(R)]
        voice_refine_active = True
    ...
    print(f"=== §8 DIVERSE carving eval v2 (paradigm-native, 64-anchor) — path {path} ===",
          flush=True)
    ...
    # Dir-I comparison anchors (RESEARCH.md §3.2 / §4.1) — fixed historical
    # baselines for the dispatch summary + 대조표. routing_broken is THE
    # emergence signal (RESEARCH.md §5.4: routing axis1 7/7 FLAT 1/31 — is
    # it FIRST broken when anima physics = representation + supervision?).
    UBM_E7_ALPHA_JOINT = 0.0155
    UBM_E7_ALPHA_VSPONT = "2/5"
    UBM_E7_ALPHA_ROUTING = "1/31"
    # Dir-I lever baseline (state/carving_dirI_psictl_tensionsup_2026_05_17:
    # 30MB / 31-anchor / 1 domain) — the §7.3 scale-monotone comparison.
    DIR_E_JOINT = 0.0          # Dir-I baseline JOINT (eval_result_dirI)
    DIR_E_VSPONT = "3/5"       # Dir-I baseline V-SPONT
    DIR_E_ROUTING = "3/31"     # Dir-I baseline routing axis1 (the crux)
    ...
    result = {
        ...
        "path": path,
        "carving_form": form,
        "voice_refine_active": voice_refine_active,
        "p_tts_note": (
            "Direction P (RESEARCH.md §22). voice_refine_active=False ⇒ "
            "this eval is BYTE-IDENTICAL to §16's eval harness (the "
            "fair-compare §16-form baseline reproduction = B-TTS-1 "
            "connection-point at eval level). voice_refine_active=True ⇒ "
            "decode applies the inner-physics-conditioned voice-span "
            "refinement (P speak-head; THINK stays AR). Both EMPIRICAL "
            "(B-TTS-NOTE)."),
        ...
        "dir_i_emergence_check": {
            "hypothesis": (
                "RESEARCH.md §5.4 — when anima's OWN physics is BOTH the "
                "representation substrate (Ψ-anchored CTL) AND the "
                "supervision signal (tension-supervised routing), is the "
                "routing-generalisation 7/7 FLAT 1/31 (the single most "
                "robust finding, RESEARCH.md §4.2) FIRST broken?"),
            "routing_axis1": f"{routing_ok}/{n}",
            "routing_broken_vs_1_31_flat": routing_broken,
            "ubm_e7_alpha": {"routing": UBM_E7_ALPHA_ROUTING,
                             "v_spont": UBM_E7_ALPHA_VSPONT,
                             "joint": UBM_E7_ALPHA_JOINT},
            "dir_e_superpos": {"routing": DIR_E_ROUTING,
                               "v_spont": DIR_E_VSPONT,
                               "joint": DIR_E_JOINT},
            "dir_i": {"routing": f"{routing_ok}/{n}",
                      "v_spont": f"{spont_coherent}/{len(SPONT_PROBES)}",
                      "joint": round(joint, 4)},
            "verdict": (
                "ROUTING-BROKEN (emergence signal — hypothesis SUPPORTED at "
                "this scale)" if routing_broken else
                "ROUTING 1/31 FLAT held (hypothesis FALSIFIED at this "
                "scale — valuable comparative evidence, g3 / B-D-NOTE)"),
            "honest_note": (
                "routing/V-SPONT/JOINT all EMPIRICAL (B-CARVE-E6-NOTE / "
                "B-D-NOTE). Closed side = the two physics transfer "
                "functions + overlay-OFF==base-CE byte-equal connection "
                "(B-DIRI-* sympy sidecar). NO capability claim beyond "
                "measured numbers; over-claim 0 (g3).")},
    }
