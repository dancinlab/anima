...
| [`growth_lane.kosmos`](UNIVERSE-BRAIN-MAP/anchors/growth_lane.kosmos) | 56 | growth-register | anima **lane growth** (4th lane = lane default + growth-register) 의 4-pillar corpus — (a) cross-disciplinary science[21] REAL CC-BY-SA Wikipedia + PD Gutenberg · (b) self-knowledge[12] + (c) UNIVERSE hypotheses[8] + (d) dialogue[6] anima-AUTHORED — en/fr/de/es/ko byte256, 4.41MB; anti-register tags grep=0 의 representative anchor + manifest pointer |
...
| [`corpus_5lang_7b_webscale.kosmos`](UNIVERSE-BRAIN-MAP/anchors/corpus_5lang_7b_webscale.kosmos) | 57 | 다국어+web-scale+7B-optimal | anima **7B WEB-SCALE default-lane corpus** (ODC-BY web bulk FineWeb/-2 reaching Chinchilla-7B-optimal: **143.60 GiB = 22 tok/param = 110% of the 140B-token 7B-optimal**, where clean-only capped at 0.27%; **ko web bulk INCLUDED** fixing the Gutenberg-ko-zero gap; **R2-STAGED** bucket phanes — HF holds manifest+card+sample heads, NOT the 143 GB; PII-scrubbed, byte-V256, tags grep=0) 의 representative anchor + R2 manifest pointer |
| [`303m_kr_en_sns/`](../KOSMOS/303m_kr_en_sns/) (3 anchors: `kr_303m`/`en_303m`/`sns_303m`) | 58·59·52 | 한국어·영어·사회성 | anima **303M KOSMOS SET — 3 register components in 3 lanes** (🇰🇷 Korean `ko_303m_058` + 🇬🇧 English `en_303m_059` off FineWeb ODC-BY sample heads, ~120 KiB curated each · 📱 SNS persona-voice `sns_303m_052` 217 turns off anima authored persona×SNS register). A grounding/carving anchor SET (NOT a raw training corpus), byte V256, PII-clean, parser-witness 3/3 valid. SCOPE: curated **sample-scale** anchors (a_scale_honest_scope) — full webscale = `corpus_5lang_7b_webscale.kosmos`, full SNS = `persona_sns_corpus.kosmos`; SNS thin = anima's held authored material is the $0 ceiling. tension 5-ch = REPRESENTATIVE. HF PUBLIC `dancinlab/anima-kosmos-303m-kr-en-sns` (KOSMOS collection). |
...
- **lane growth (4th lane)** (tier 56): manifest ref → `serving/corpus/growth_lane.txt`, sha256 `34999434…`, 4,618,750 B, langs en/fr/de/es/ko, HF `dancinlab/anima-corpus-growth-lane`. The **4th anima lane** = `lane default + growth-register` (NOT agent/default/persona) — grows conceptual range + self-model + reasoning capacity. 4 pillars: **(a) cross-disciplinary science[21]** = REAL CC-BY-SA-4.0 Wikipedia `prop=extracts` by named title per-lang (4.27 MB) + PD Project Gutenberg primary texts (206 KB: Darwin Origin pg1228 + Descent pg2300, Maxwell pg15491, James pg57628); **(b) self-knowledge[12] + (c) UNIVERSE hypotheses[8] + (d) dialogue[6]** = anima-AUTHORED self-corpus (139 KB, deterministic seed 20260605) from the repo's own docs + real `HYPOTHESES/H_*.md`. per-license CC-BY-SA-4.0 4.27 MB / PD 206 KB / anima-authored 139 KB. **HONEST**: anti-register tags grep=0 (no `[role:|[persona:|[character:|[assistant:|[system:`), no 'you are anima', 0xFE/0xFF=0, UTF-8 round-trip OK (p2/p3/p4/p6 held) · science=REAL clean-licensed (cited), self/hyp/dialogue=anima-authored-labeled (teaches anima ABOUT ITSELF + how it reasons, NOT cooperation templates) · per-lang gap honest (PD en-only; Poincaré pg37157 + Boole pg15114 = no-plain-text-on-Gutenberg recorded gap, concepts via Wikipedia, NOT fabricated; ko/es lean Wikipedia, uneven). Scope (a_scale_honest_scope) = feeds the PROVEN ~18M chat rung, NO 7B claim; TRAIN = SEPARATE follow-on GPU fire. NO scraped non-licensed, NO PII.
...
- **7B web-scale corpus** (tier 57): manifest ref → `r2://phanes/anima-7b/MANIFEST.json` (**R2-STAGED**, bucket `phanes`, prefix `anima-7b/web/`), HF `dancinlab/anima-corpus-5lang-7b-webscale`. The **web-scale extension** that closes the gb-balanced 7B-gap with **ODC-BY web bulk** (Common-Crawl-derived FineWeb/FineWeb-2): **143.60 GiB** (154.19 GB decimal) = **22.0 tok/param for 7B** (Chinchilla-optimal 20 → EXCEEDS) = **110.1% of the 140B-token 7B-optimal** (clean-only capped at 0.27%). 5-lang balanced, 20 shards, per-lang (decimal GB): en 25.83 (`HuggingFaceFW/fineweb` sample/10BT) / fr 32.79 / de 31.00 / es 32.94 / ko 31.64 (`HuggingFaceFW/fineweb-2` fra_Latn/deu_Latn/spa_Latn/kor_Hang). **ko web bulk INCLUDED** (~31.64 GB) fixes the Gutenberg-ko-zero gap (tier 55 had ko art/consciousness=0). **PII-scrubbed** (email→[EMAIL], phone→[PHONE]), 0xFE/0xFF stripped → byte V=256, injection-tags grep=0. **R2-STAGED, NOT HF-mirrored, NOT local** — HF holds the pointer `MANIFEST.json` (per-shard R2 keys+bytes+sha256, the primary artifact) + `CORPUS_CARD.md` + `README.md` + `samples/{en,fr,de,es,ko}.head.txt` (200 KB/lang); the full corpus is fetched FROM R2 for training. predecessor=tier 55. **Numbers verified vs the R2 manifest byte counts (143 GB itself NOT re-measured). NO 7B-TRAINED claim — this is the CORPUS; the 7B TRAIN is a separate follow-on GPU fire. persona/identity stay AUTHORED (web bulk = factual/linguistic base only).** User-decision A: "NO scraped" relaxed → ODC-BY web sanctioned for 7B scale.
...
- 위치: [`EEG_CLM/`](../EEG_CLM/) · 캡처: `capture_native.py` (brainflow 우회 native serial 33-byte 패킷, Daisy 16ch 디인터리브) · 분석: `music_eeg_compare.py` + `HYPOTHESES/h1275_music_extraction.py` (verdict `.verdicts/1275_music_extraction/`).
