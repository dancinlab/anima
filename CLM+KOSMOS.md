...
└─ ◷ P4 full 3B/7B     DEFERRED — NOT throughput-justified (Lane G util host-feed-bound, scale-invariant; 2026-06-02 mid-d1536 fire below). Unblocks ONLY after the backward feed is moved ON-DEVICE (device im2col/col2im + device adam) and/or the per-step GEMMs are FUSED — raising scale alone idles the GPU MORE, not less.

PUBLIC-grade Lane-G gate: util-GREEN (≥20%) AND descent-GREEN. STATUS 2026-06-02 = NOT MET (descent 🟢, util 🔴). PUBLIC NOT reached on Lane G; the throughput path is proven (forge on the GPU) but the host-feed ceiling blocks the util gate at every scale tested. LEVER PROGRESS 2026-06-02: lever (b) FUSED per-step conv GEMMs LANDED — strided-batched `forge_dispatch_matmul_batched` builtin (cublasDgemmStridedBatched) + 2-expert conv2_*_batched, byte-eq CPU-local (F-FORGE-BATCHED-EQ / F-CLM-CONV2-BATCHED-FWD/BWD-EQ all max|Δ|=0.0; full-trainer CE trajectory IDENTICAL). NO GPU fire on (b) alone — the mid-d1536 finding holds that (a)+(b) TOGETHER are the unblock and (b) does not touch the dominant host im2col/col2im/adam peg. REMAINING GAP to util-GREEN = lever (a): device-side im2col/col2im (device-resident x_col fed to the batched GEMM with no H2D/D2H roundtrip) + device-AdamW wire (the `_hx_cuda_farr_adamw_step_gpu` kernel already exists). Until (a) lands the GPU stays starved regardless of (b); the single small util fire is deferred to post-(a).
