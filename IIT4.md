...
| [`HEXAD/IIT4/lib/`](HEXAD/IIT4/lib/) | 엔진 구현 (M2 distinction · M3 relation · M4 bigphi 예정) |
...
> **status 2026-05-25**: M0 + M1 LANDED. M1 = `iit4_tpm.hexa` (TPM state-by-node · cause/effect repertoire · intrinsic difference) — COPY n=2 손계산 검증 + rotate-3 구조 sanity **13/13 🟢 SUPPORTED-NUMERICAL** (`state/iit4_m1_smoke_2026_05_25/`). 다음 = M2 distinctions (`iit4_distinction.hexa` — small-φ via min-partition ID + MICE purview search). small-N(n≤8) exact 라 $0 mac-local·GPU 무관.
