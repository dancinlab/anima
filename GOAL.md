...
- 🟡 **Real-ckpt-load smoke PARTIAL** (2-layer progress 2026-05-13 PM): Phase 1A.4 .safetensors (663 MB) `--prompt "안녕? 너는 누구야?" --max-new 3 --mode greedy` → binary executes 12.9s wall + 3.22 GB peak RSS exit 0, mmap + safetensors header parse ✅, **kv_cache sentinel fix (`n_layers=0` initial)** resolved `n_layers` error chain ✅, weights load 218/218 ✅, forward emits 1 byte token ✅. **잔여 gap**: 생성 byte 가 invalid UTF-8 (LRM + replacement char), 한국어 텍스트 아님 → `gen_pick_next` / `tok_decode_str` AOT-vs-interpreter divergence (next-cycle argmax dump + byte comparison). Fix attempts: void+empty fallback `242d7ae76` (ineffective) → kv sentinel (effective at one layer).
