...
| 🆕 v5-mitosis cotrain v4-multi FRESH-INIT DDP (post-★★★★★) | D3/D4 cond #3+#5 multi-GPU evidence (v4 single A100 의 fresh-start multi-GPU 비교) | Vast.ai 4× RTX PRO 6000 S 96GB | `training/cotrain_v5mitosis_v4_multi.py` (v4 fork + DDP, fresh-init) + `state/anima_v5mitosis_cotrain_v4_multi_2026_05_13/dispatch_h100_v4_multi.sh` (torchrun --nproc_per_node=4) | $5.33/hr 4× RTX PRO 6000 S (est $26.67 / cap $80) | 🔄 **IN-FLIGHT 2026-05-13** — fresh-init DDP path (v5 DDP 는 v4 step-2000 resume continuation, 본 BG 는 step 0 from scratch direct v4 single 비교). 같은 envelope d=1024 cells=256 20K step + cells-static (initial_cells=max_cells=256 from step 0, DDP-safe by construction; v4 single 도 step ~150 에 cells=256 saturate). per_gpu_batch=2 × world_size=4 = effective_batch=8 (v4 single 의 original batch 일치). find_unused_parameters=True (top-K=8 over 256 cells = 248 unused). per-rank seed offset (args.seed + rank). H100_SXM 4× 시장 empty → RTX PRO 6000 S 96GB 대체 (v5 DDP H200 보다 저렴, anima H404 / H800 dual-FFN d=1024 cells=256 ctx=256 batch=2 fit). HONEST C3 (≥ 8): cells-static 의미 (F-V5MIT-1 verdict surface 변경, F-V5MIT-5 N/A inherit-from-v4-saga), find_unused 비용, aux gradient averaging, per-rank seed, wall target (4-5hr), no-resume → fresh-init dynamics, rank-0 only F-PERSONA. doc: `docs/anima_clm_v5_mitosis_cotrain_v4_multi_2026_05_13.md`. cond #3 ☑ already (PSCC §50) — multi-GPU comparison evidence reinforcement |
