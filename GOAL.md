...
- ☑ **Real-ckpt-load smoke FULL CLOSURE 2026-05-13 PM**: Phase 1A.4 .safetensors (663 MB) — binary executes, mmap + safetensors header parse ✅, **kv_cache sentinel fix** (`n_layers=0` initial) resolved noisy `n_layers` errors ✅, weights load 218/218 ✅, forward path runs ✅, **AOT vs interpreter byte-by-byte PARITY 확인** — both produce `gen_ids=[13,239,160,183,35]` for `--prompt "안녕? 너는 누구야?" --max-new 5 --mode greedy --seed 0` ✅. max_new=20 → valid Korean output "이 이 이 이 이" (model greedy collapse 자체 특성, infra 정상). Layer 3 결국 **NOT A BUG** — max_new 부족으로 multibyte UTF-8 sequence 미완성이었을 뿐. **TRUE upstream finding**: `hexa run <single-file no-use>` 가 ubu-1 자동 dispatch (resource-r router), `RESOURCE_LOCAL_HEXA=1` 로 local 강제 시 정상 (memory `feedback_hexa_resource_local_dispatch`).
...
cond #2 evidence-tier upgraded: interpreted SSOT (24L byte parity PSCC §43) → AOT native arm64 binary + AOT binary arg parser (this session) → real-ckpt-load smoke (PARTIAL, dims-bug). doc `docs/anima_chat_aot_native_2026_05_13.md` (7 §, 7 honest C3). $0 Mac local.
