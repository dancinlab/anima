...
| 🆕 v5-mitosis cotrain v4 SCALE-UP (post-★★★★★) | D3/D4 cond #3+#5 production-scale evidence | Vast.ai A100 SXM4 80GB pod 36624731 (retained on OOM, refire reuses) | `training/cotrain_v5mitosis_v4.py` (v3-routing fork) + `state/anima_v5mitosis_cotrain_v4_scaleup_2026_05_12/dispatch_h100_v4.sh` + `refire_existing_pod.sh` | ~$0.94/hr A100 (cap $40; no caps directive) | 🔄 **IN-FLIGHT 2026-05-13** — d=1024 (= EngineAG, REBORN §88 stretch) / n_head=16 / ffn=4096 / cells_max=256 / 20K step / top-K=8 MoE + Switch load-balance aux α=0.01 + annealed gate-entropy λ 1.0→0.01 (v3-routing fix carried). 1st dispatch (batch=8 ctx=512) OOM at cells=66 (`torch.stack(cell_outs)` 256-cell-class, 77.6GB) — dispatch OOM-retry was a no-op (remote `python\|tee` returns tee's exit 0 on crash → fixed: remote `set -o pipefail`, both dispatch + refire). refire reuses live pod batch 8→2 ctx 512→256 (d=1024/cells=256 headline preserved). H100 SXM marketplace empty within reliability+budget → A100 80GB fallback (same VRAM class, ~2× slower → wall ~10-16hr at the O(N)-over-cells loop). doc skeleton `docs/anima_clm_v5_mitosis_cotrain_v4_scaleup_2026_05_12.md` (11 §, 7+ honest C3, `<<<>>>` placeholders) + `hf_push.py` (gated on F-V5MIT-5 V14-STRICT). Decides: routing collapse architectural vs scale + M4 z-score scaling (v2 z=3.20→?) + V14-STRICT production robustness. cond #3 ☑ already (PSCC §50 §A3 4b z=3.20) — this is evidence reinforcement, not closure requirement |
...
| 🆕 v5-mitosis cotrain v4-multi FRESH-INIT DDP (post-★★★★★) | D3/D4 cond #3+#5 multi-GPU evidence (v4 single A100 의 fresh-start multi-GPU 비교) | Vast.ai 4× H200 143GB pod 36636926 (attempt 3) | `training/cotrain_v5mitosis_v4_multi.py` (v4 fork + DDP, fresh-init) + `state/anima_v5mitosis_cotrain_v4_multi_2026_05_13/dispatch_h100_v4_multi.sh` (torchrun --nproc_per_node=4) | $13.45/hr 4× H200 (est $67.23 / cap $80) | 🔄 **IN-FLIGHT 2026-05-13 attempt 3 (step 50 rolling)** — vram 107/150 GB rank0, loss step 50 = 694. 같은 v4 envelope d=1024 cells=256 20K step + cells-static (DDP-safe; v4 single 도 step ~150 에 cells=256 saturate). per_gpu_batch=2 × world_size=4 = effective_batch=8 (v4 single original batch 일치). find_unused_parameters=True. per-rank seed offset. **3-attempt saga**: ① 4× RTX PRO 6000 Blackwell sm_120 ($5.33/hr pod 36635742) — NCCL `Cuda failure 'invalid argument'` at DDP init (pytorch 2.5.1+cu121 NCCL kernels sm_50..sm_90, no PTX fall-through). ② 4× A100 SXM4 80GB ($6.71/hr pod 36636365) — OOM at batch=2 AND batch=1 (5.37B fp32 + AdamW fp64 m+v + grad = 85-95 GB/rank > 80 GB). ③ 4× H200 143GB ($13.45/hr pod 36636926, current) — sm_90 native + 141 GB fits comfortably. wasted attempts $1.06 total. Filter tightened to `[H100_SXM,H100_NVL,H100_PCIE,H200]` + MIN_GPU_RAM_MB=120000. doc: `docs/anima_clm_v5_mitosis_cotrain_v4_multi_2026_05_13.md` (10 §, 8 honest C3 + §5.5/§5.6 attempt post-mortem). cond #3 ☑ already (PSCC §50) — multi-GPU comparison evidence reinforcement |
