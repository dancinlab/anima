...
- ✅ 🆕 **AOT compile + CLI driver LANDED 2026-05-13 PM** — `anima_chat_aot.hexa` (stripped variant, mitosis/M3 stubs) → 440 KB arm64 Mach-O native binary via `hexa build` (commits `50056902d` + `16acce465`), CLI args `--prompt/--ckpt/--max-new/--mode/--temp/--seed/--help/--smoke`, F-AC-HEXA helpers 17/17 PASS native execution. 🟡 real-ckpt-load PARTIAL (dims dict-key AOT-vs-interpreter gap), ⛔ Linux build BLOCKED (ubu no hexa). doc `docs/anima_chat_aot_native_2026_05_13.md`. **D1 deployment tier upgrade** (interpreted SSOT → AOT native binary + CLI). 별개: D4c "anima CLI" (session-level cell-pool persistence) 는 PSCC §35 design LANDED 별도 lane.
