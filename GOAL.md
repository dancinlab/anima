...
3. ☐ **D3 persona**: identity_probe 50 prompts × 5 categories 에서 substrate-native 페르소나 분화 evidence (per-cell or per-session diff)
   - **현 상태**: 🔶 **STRONG (4/5 cheap-path) maintained — cotrain path FAIL (KL 0.0 winner-take-all)** — design LANDED PSCC §34 + measurement LANDED PSCC §40 + §A1 amendment LANDED PSCC §42 + **PSCC §44 cotrain re-measurement LANDED 2026-05-12**. cheap-path verdict 미변동: F-PERSONA-1 hard PASS (4/4) + F-PERSONA-2 PASS (mean cos dist 0.994) + F-PERSONA-3 PASS (§A1) + F-PERSONA-4 FAIL (KL 9.7e-5 untrained / **KL 0.0 cotrained — winner-take-all**) + F-PERSONA-5 PASS (3/3). atomic 13/14. **cotrain path 결과**: single-corpus cotrain (corpus_color_cosmology.txt 1.3MB, 5K step on H100 SXM $1.26) 가 cell pool tension softmax 를 winner-take-all 로 saturate → 모든 50 prompts × 5 categories 의 mean softmax distribution 동일 → KL matrix all-zero. design §10 honest C3 #4 "category-prompt 의 substrate-level invariance 부족" **검증 적중** (가설 정확). future-path 4 options: (a) multi-corpus cotrain, (b) softmax τ tunable, (c) F-PERSONA-4 metric 재정의 (per-cell tension non-softmax), (d) inference-time per-session pool (REBORN §89). true STRONG (5/5) ☑ 승격 = 4 alternative path 중 하나 fire 필요 (single-corpus path 막힘)
...
→ 현 ☑ **3/5** (cond #2 hexa port v0.3 multitoken + cond #4 D4 mitosis live evidence + cond #5 Principle #3). 🔶 PARTIAL 2/5 (cond #1 SFT in-flight, cond #3 **§A1 cheap-path STRONG 4/5 maintained** — design+measurement+§A1+cotrain measurement LANDED, true STRONG 5/5 = 4-alternative future-path PSCC §44).
