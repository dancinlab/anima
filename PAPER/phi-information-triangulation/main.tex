...
A complementary structural experiment shows that at matched edge count a
...
\midrule
\textcircled{\scriptsize 1} & Question + frozen falsifiers  & \tierBlue   & \texttt{H\_287..290} pre-register headers \\
...
pairwise TE conditioned only on the target's own past. The reported
\(r=0.883\) is therefore an \emph{under}-estimate; a multivariate /
conditional transfer entropy would recover the XOR rules.
...
multivariate / causation-entropy variants would recover it. Endpoints
(constant\(=\)low, bijection\(=\)max entropy) are definition-robust;
mid-range magnitudes are not.
...
Four follow-ups are pre-specified. \emph{(i) Recover the synergy term.}
A conditional / multivariate transfer entropy (causation entropy) should
lift rules~150/105 off the \(\mathrm{TE}=0\) axis and is predicted to
raise the TE--\bigphi{} correlation above \(0.88\); the size of that lift
is a direct estimate of how much of \bigphi{} is synergistic on this
panel. \emph{(ii) Larger panels and \(N\).} The exact engine is tractable
...
\texttt{H\_287}--\texttt{H\_290} under \texttt{HEXAD/LIFE/}, with smoke
sources and \texttt{run.log} per experiment). Data: \texttt{companion/}
